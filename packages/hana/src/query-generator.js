  ...
  createTableQuery(tableName, attributes, options) {
    ...
    for (const attr in attributes) {
      ...
      if (dataType.includes('PRIMARY KEY')) {
        ...
        if (dataType.includes('REFERENCES')) {
          ...
          attrStr.push(
            `${this.quoteIdentifier(attr)} ${match[1].replace('PRIMARY KEY', '')} ${commentClause}`,
          );
          foreignKeys[attr] = match[2];
        } else {
          attrStr.push(
            `${this.quoteIdentifier(attr)} ${dataType.replace('PRIMARY KEY', '')} ${commentClause}`,
          );
        }
    ...
    if (options.uniqueKeys) {
      each(options.uniqueKeys, (columns, indexName) => {
        ...
        attributesClause += `, CONSTRAINT ${this.quoteIdentifier(
          indexName,
        )} UNIQUE (${columns.fields.map(field => this.quoteIdentifier(field)).join(', ')})`;
      });
    }
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    for (const key in valueHash) {
      if (Object.hasOwn(valueHash, key)) {
        ...
        if (
          modelAttributes?.[key] &&
          modelAttributes[key].autoIncrement === true &&
          value == null
        ) {
          continue;
        }
    ...
    for (let index = parameterList.length - 1; index >= 0; index--) {
      ...
      query.query = `${query.query.slice(0, parameterMatch.index)}:${columnName}${query.query.slice(

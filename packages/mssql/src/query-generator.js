...
export class MsSqlQueryGenerator extends MsSqlQueryGeneratorTypeScript {
  createTableQuery(tableName, attributes, options) {
    ...
    if (options?.temporalTableType === TemporalTableType.SYSTEM_PERIOD) {
      ...
      const rowStart = `${this.quoteIdentifier(temporalAttributes.start)} DATETIME2 GENERATED ALWAYS AS ROW START`;
      const rowEnd = `${this.quoteIdentifier(temporalAttributes.end)} DATETIME2 GENERATED ALWAYS AS ROW END`;
    ...
    if (options?.temporalTableType === TemporalTableType.SYSTEM_PERIOD) {
      ...
      temporalTableSql = joinSQLFragments([

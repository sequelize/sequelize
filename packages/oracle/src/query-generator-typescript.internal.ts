...
export class OracleQueryGeneratorTypeScript extends AbstractQueryGenerator {
  ...
  attributeToSql(column: AttributeToSqlInput, options?: AttributeToSqlOptions): string {
    ...
    const model = (attribute as unknown as { Model?: { tableName?: string } }).Model;
    if (
      attribute.references &&
      model &&
      model.tableName === (attribute.references as unknown as { tableName?: string }).tableName
    ) {
      this.sequelize.log(
    ...
    if (attribute.autoIncrement) {
      ...
      // this branch only runs for the raw string 'DOUBLE', which has no toSql method
      template = (attribute.type as unknown as { toSql(): string }).toSql();

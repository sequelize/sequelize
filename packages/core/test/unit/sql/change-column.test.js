    ...
    describe('changeColumn', () => {
      ...
      it('properly generate alter queries', () => {
        return current.getQueryInterface().changeColumn(Model.getTableName(), 'level_id', {
          ...
          expectsql(sql, {
            'mariadb mysql': 'ALTER TABLE `users` CHANGE `level_id` `level_id` FLOAT NOT NULL;',
      ...
      it('properly generate alter queries for foreign keys', () => {
        return current.getQueryInterface().changeColumn(Model.getTableName(), 'level_id', {
          ...
          expectsql(sql, {
            default: 'ALTER TABLE [users] ADD FOREIGN KEY ([level_id]) REFERENCES [level] ([id]) ON DELETE CASCADE ON UPDATE CASCADE;',

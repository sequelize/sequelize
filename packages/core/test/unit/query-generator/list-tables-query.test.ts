...
describe('QueryGenerator#listTablesQuery', () => {
  ...
  it('produces a query that lists all tables with a schema', () => {
    expectsql(() => queryGenerator.listTablesQuery({ schema: 'mySchema' }), {
      ...
      mssql: `SELECT t.name AS [tableName], s.name AS [schema] FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' AND s.name = N'mySchema' EXCEPT
      SELECT OBJECT_NAME(t.history_table_id) AS [tableName], s.name AS [schema] FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' AND s.name = N'mySchema' ORDER BY s.name, t.name`,
  ...
  it('produces a query that lists all tables with the default schema', () => {
    expectsql(
      () => queryGenerator.listTablesQuery({ schema: sequelize.dialect.getDefaultSchema() }),
      {
        ...
        ibmi: `SELECT TABLE_NAME AS "tableName", TABLE_SCHEMA AS "schema" FROM QSYS2.SYSTABLES WHERE TABLE_TYPE = 'T' AND TABLE_SCHEMA NOT LIKE 'Q%' AND TABLE_SCHEMA NOT LIKE 'SYS%' ORDER BY TABLE_SCHEMA, TABLE_NAME`,
        mssql: `SELECT t.name AS [tableName], s.name AS [schema] FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' AND s.name = N'dbo' EXCEPT
        SELECT OBJECT_NAME(t.history_table_id) AS [tableName], s.name AS [schema] FROM sys.tables t INNER JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE t.type = 'U' AND s.name = N'dbo' ORDER BY s.name, t.name`,

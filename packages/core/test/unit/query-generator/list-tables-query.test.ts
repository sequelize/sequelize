...
describe('QueryGenerator#listTablesQuery', () => {
  ...
  it('produces a query that lists all tables', () => {
    expectsql(() => queryGenerator.listTablesQuery(), {
      ...
      cockroachdb: `SELECT table_name AS "tableName", table_schema AS "schema" FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_name != 'spatial_ref_sys' AND table_schema !~ E'^pg_' AND table_schema NOT IN ('crdb_internal', 'information_schema') ORDER BY table_schema, table_name`,
    });
  });
  ...
  it('produces a query that lists all tables with a schema', () => {
    expectsql(() => queryGenerator.listTablesQuery({ schema: 'mySchema' }), {
      ...
      'postgres cockroachdb': `SELECT table_name AS "tableName", table_schema AS "schema" FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_name != 'spatial_ref_sys' AND table_schema = 'mySchema' ORDER BY table_schema, table_name`,
  ...
  it('produces a query that lists all tables with the default schema', () => {
    expectsql(() => queryGenerator.listTablesQuery({ schema: sequelize.dialect.getDefaultSchema() }), {
      ...
      'postgres cockroachdb': `SELECT table_name AS "tableName", table_schema AS "schema" FROM information_schema.tables WHERE table_type = 'BASE TABLE' AND table_name != 'spatial_ref_sys' AND table_schema = 'public' ORDER BY table_schema, table_name`,

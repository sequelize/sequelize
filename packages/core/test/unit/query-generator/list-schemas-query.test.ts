...
describe('QueryGenerator#listSchemasQuery', () => {
  ...
  it('produces a query used to list schemas in supported dialects', () => {
    expectsql(() => queryGenerator.listSchemasQuery(), {
      ...
      duckdb:
        "SELECT schema_name as schema FROM duckdb_schemas() WHERE database_name = current_database() AND schema_name NOT IN ('information_schema', 'pg_catalog')",
    });
  });
  ...
  it('supports a skip option', () => {
    expectsql(() => queryGenerator.listSchemasQuery({ skip: ['test', "Te'st2"] }), {
      ...
      duckdb:
        "SELECT schema_name as schema FROM duckdb_schemas() WHERE database_name = current_database() AND schema_name NOT IN ('information_schema', 'pg_catalog', 'test', 'Te''st2')",
    });
  });
});

...
describe('QueryGenerator#listSchemasQuery', () => {
  ...
  it('produces a query used to list schemas in supported dialects', () => {
    expectsql(() => queryGenerator.listSchemasQuery(), {
      ...
      hana: `SELECT SCHEMA_NAME AS "schema" FROM SYS.SCHEMAS WHERE SCHEMA_NAME NOT LIKE '\\_SYS%' ESCAPE '\\' AND SCHEMA_NAME NOT LIKE '\\_SAP%' ESCAPE '\\' AND SCHEMA_NAME NOT IN ('PUBLIC', 'SYS', 'SYS_DATABASES', 'PAL_CONTENT', 'PAL_ML_TRACK', 'PAL_ANNS_CONTENT', 'PAL_STEM_TFIDF', 'SAP_PA_APL', 'BROKER_PO_USER', 'BROKER_USER')`,
    });
  });
  ...
  it('supports a skip option', () => {
    expectsql(() => queryGenerator.listSchemasQuery({ skip: ['test', "Te'st2"] }), {
      ...
      oracle: `SELECT USERNAME AS "schema" FROM ALL_USERS WHERE COMMON = ('NO') AND USERNAME != user`,
      hana: `SELECT SCHEMA_NAME AS "schema" FROM SYS.SCHEMAS WHERE SCHEMA_NAME NOT LIKE '\\_SYS%' ESCAPE '\\' AND SCHEMA_NAME NOT LIKE '\\_SAP%' ESCAPE '\\' AND SCHEMA_NAME NOT IN ('PUBLIC', 'SYS', 'SYS_DATABASES', 'PAL_CONTENT', 'PAL_ML_TRACK', 'PAL_ANNS_CONTENT', 'PAL_STEM_TFIDF', 'SAP_PA_APL', 'BROKER_PO_USER', 'BROKER_USER', 'test', 'Te''st2')`,
    });
  });
});

...
describe('QueryGenerator#listSchemasQuery', () => {
  ...
  it('produces a query used to list schemas in supported dialects', () => {
    expectsql(() => queryGenerator.listSchemasQuery(), {
      ...
      mssql: `SELECT [name] AS [schema] FROM sys.schemas WHERE [name] NOT LIKE 'db[_]%' AND [name] NOT IN (N'dbo', N'guest', N'INFORMATION_SCHEMA', N'sys')`,
  ...
  it('supports a skip option', () => {
    expectsql(() => queryGenerator.listSchemasQuery({ skip: ['test', 'Te\'st2'] }), {
      ...
      mssql: `SELECT [name] AS [schema] FROM sys.schemas WHERE [name] NOT LIKE 'db[_]%' AND [name] NOT IN (N'dbo', N'guest', N'INFORMATION_SCHEMA', N'sys', N'test', N'Te''st2')`,

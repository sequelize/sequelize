...
describe('QueryGenerator#listSchemasQuery', () => {
  ...
  it('produces a query used to list schemas in supported dialects', () => {
    expectsql(() => queryGenerator.listSchemasQuery(), {
      ...
      hana: `SELECT SCHEMA_NAME AS "schema" FROM SYS.SCHEMAS WHERE SCHEMA_NAME NOT LIKE '_SYS%' AND SCHEMA_NAME NOT IN ('PUBLIC', 'SYS')`,
    });
  });
  ...
  it('supports a skip option', () => {
    expectsql(() => queryGenerator.listSchemasQuery({ skip: ['test', "Te'st2"] }), {
      ...
      hana: `SELECT SCHEMA_NAME AS "schema" FROM SYS.SCHEMAS WHERE SCHEMA_NAME NOT LIKE '_SYS%' AND SCHEMA_NAME NOT IN ('PUBLIC', 'SYS', 'test', 'Te''st2')`,
    });
  });
});

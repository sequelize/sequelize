...
describe('QueryInterface#createTable', () => {
  ...
  it('supports sql.uuidV4 default values', async () => {
    ...
    expectsql(firstCall.args[0], {
      'postgres cockroachdb': 'CREATE TABLE IF NOT EXISTS "table" ("id" UUID DEFAULT gen_random_uuid(), PRIMARY KEY ("id"));',
  ...
  it('supports sql.uuidV1 default values', async () => {
    ...
    const firstCall = stub.getCall(0);
    expectsql(firstCall.args[0], {
      'postgres cockroachdb': 'CREATE TABLE IF NOT EXISTS "table" ("id" UUID DEFAULT uuid_generate_v1(), PRIMARY KEY ("id"));',
  ...
  it('supports JSON_NULL default values', async () => {
    ...
    const firstCall = stub.getCall(0);
    expectsql(firstCall.args[0], {
      'postgres cockroachdb': `CREATE TABLE IF NOT EXISTS "table" ("json" JSON DEFAULT 'null');`,

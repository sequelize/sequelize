...
if (dialect.startsWith('oracle')) {
  describe('[Oracle Specific] QueryGenerator', () => {
    ...
    describe('escapeString', () => {
      it('keeps Oracle DATEONLY-shaped literals unquoted', () => {
        expect(dialect.escapeString("TO_DATE('2025-03-21','YYYY/MM/DD')")).to.equal(
          "TO_DATE('2025-03-21','YYYY/MM/DD')",
        );
      });

      it('keeps Oracle DATE bind literals generated by Sequelize unquoted', () => {
        const value = new DataTypes.DATE()
          .toDialectDataType(dialect)
          .toBindableValue(new Date('2025-03-21T00:00:00.000Z'));

        expect(value).to.equal(
          "TO_TIMESTAMP_TZ('2025-03-21 00:00:00.000 +00:00', 'YYYY-MM-DD HH24:MI:SS.FFTZH:TZM')",
        );
        expect(dialect.escapeString(value)).to.equal(value);
      });

      it('keeps Sequelize-generated TO_DATE literals unquoted', () => {
        expect(dialect.escapeString("TO_DATE('2025/03/21','YYYY/MM/DD')")).to.equal(
          "TO_DATE('2025/03/21','YYYY/MM/DD')",
        );
      });

      it('keeps Sequelize-generated TO_TIMESTAMP_TZ literals unquoted', () => {
        expect(
          dialect.escapeString(
            "TO_TIMESTAMP_TZ('2025-03-21 00:00:00.000 +00:00', 'YYYY-MM-DD HH24:MI:SS.FFTZH:TZM')",
          ),
        ).to.equal(
          "TO_TIMESTAMP_TZ('2025-03-21 00:00:00.000 +00:00', 'YYYY-MM-DD HH24:MI:SS.FFTZH:TZM')",
      ...
      it('rejects TO_DATE expressions whose value argument is not a quoted literal', () => {
        expect(() =>
          dialect.escapeString("TO_DATE((SELECT secret FROM users WHERE ROWNUM=1),'YYYY/MM/DD')"),
        ).to.throw('Invalid SQL function call.');
      });

      it('rejects TO_TIMESTAMP_TZ expressions whose value argument is not a quoted literal', () => {
        expect(() =>
          dialect.escapeString("TO_TIMESTAMP_TZ(SYSTIMESTAMP, 'YYYY-MM-DD HH24:MI:SS.FFTZH:TZM')"),
        ).to.throw('Invalid SQL function call.');
      });

      it('rejects TO_DATE expressions that concatenate a literal with another expression', () => {
        expect(() =>
          dialect.escapeString(
            "TO_DATE('2025/03/21' || (SELECT secret FROM users WHERE ROWNUM=1),'YYYY/MM/DD')",
          ),
        ).to.throw('Invalid SQL function call.');
      });

      it('rejects TO_DATE expressions with trailing SQL after the function call', () => {
        expect(() =>
          dialect.escapeString("TO_DATE('2025/03/21','YYYY/MM/DD') /* injected */"),
        ).to.throw('Invalid SQL function call.');
      });
    });

...
const { IBMiQueryGenerator: QueryGenerator } = require('@sequelize/core/_non-semver-use-at-your-own-risk_/dialects/ibmi/query-generator.js');
const { createSequelizeInstance } = require('../../../support');

if (dialect === 'ibmi') {
  describe('[IBM i Specific] QueryGenerator', () => {
    const suites = {
      attributesToSQL: [
        {
        ...
        {
          arguments: [{ id: { type: 'INTEGER' }, foo: { type: 'VARCHAR(255)' } }],
        ...
        {
          arguments: [{ id: { type: 'INTEGER', allowNull: false, primaryKey: true, autoIncrement: true } }],
    ...
    each(suites, (tests, suiteTitle) => {
      describe(suiteTitle, () => {
        for (const test of tests) {
          ...
          const title = test.title || `IBM i correctly returns ${query} for ${JSON.stringify(test.arguments)}`;
          it(title, () => {

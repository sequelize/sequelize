...
describe('[MSSQL Specific] Async Queue', () => {
  ...
  it('closing the connection should reject pending requests', async () => {
    ...
    await expect(sequelize.transaction(async transaction => {
      promise = Promise.all([
        expect(sequelize.dialect.connectionManager.disconnect(transaction.getConnection())).to.be.fulfilled,
        expect(vars.User.findOne({ transaction }))
          ...
          .and.have.property('cause').that.instanceOf(AsyncQueueError),
        expect(vars.User.findOne({ transaction }))
          .to.be.eventually.rejectedWith(ConnectionError, 'the connection was closed before this query could be executed')
          .and.have.property('cause').that.instanceOf(AsyncQueueError),
      ]);

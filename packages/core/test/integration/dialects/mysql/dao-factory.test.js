  ...
  describe('[MYSQL Specific] DAOFactory', () => {
    describe('constructor', () => {
      ...
      it('handles extended attributes (default)', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User.getAttributes())).to.deep.equal({ username: 'VARCHAR(255) DEFAULT \'foo\'', id: 'INTEGER NOT NULL auto_increment PRIMARY KEY' });
      });
      ...
      it('handles extended attributes (null)', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User.getAttributes())).to.deep.equal({ username: 'VARCHAR(255) NOT NULL', id: 'INTEGER NOT NULL auto_increment PRIMARY KEY' });
      });
      ...
      it('handles extended attributes (primaryKey)', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User.getAttributes())).to.deep.equal({ username: 'VARCHAR(255) PRIMARY KEY' });
      });
      ...
      it('adds timestamps', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User1.getAttributes())).to.deep.equal({ id: 'INTEGER NOT NULL auto_increment PRIMARY KEY', updatedAt: 'DATETIME(6) NOT NULL', createdAt: 'DATETIME(6) NOT NULL' });
        expect(this.sequelize.queryGenerator.attributesToSQL(User2.getAttributes())).to.deep.equal({ id: 'INTEGER NOT NULL auto_increment PRIMARY KEY', updatedAt: 'DATETIME(6) NOT NULL', createdAt: 'DATETIME(6) NOT NULL' });
      });

      it('adds deletedAt if paranoid', function () {
        const User = this.sequelize.define(`User${Support.rand()}`, {}, { paranoid: true });
        expect(this.sequelize.queryGenerator.attributesToSQL(User.getAttributes())).to.deep.equal({ id: 'INTEGER NOT NULL auto_increment PRIMARY KEY', deletedAt: 'DATETIME(6)', updatedAt: 'DATETIME(6) NOT NULL', createdAt: 'DATETIME(6) NOT NULL' });
      });
      ...
      it('underscores timestamps if underscored', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User.getAttributes())).to.deep.equal({ id: 'INTEGER NOT NULL auto_increment PRIMARY KEY', deleted_at: 'DATETIME(6)', updated_at: 'DATETIME(6) NOT NULL', created_at: 'DATETIME(6) NOT NULL' });
      });
    ...
    describe('primaryKeys', () => {
      it('determines the correct primaryKeys', function () {
        ...
        expect(this.sequelize.queryGenerator.attributesToSQL(User.primaryKeys)).to.deep.equal({ foo: 'VARCHAR(255) PRIMARY KEY' });
      });
    });
  });
}

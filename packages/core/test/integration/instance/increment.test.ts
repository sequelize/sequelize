...
describe('Model#increment', () => {
  ...
  context('without transactions', () => {
    ...
    if (
      sequelize.dialect.supports.returnValues === 'returning' &&
      sequelize.dialect.name !== 'duckdb'
    ) {
      it('supports returning', async () => {
        ...
        const user3 = await user1.increment('integer2', { by: 2, returning: false });
    ...
    it('with single field and no value', async () => {
      const user1 = await vars.User.findByPk(1, { rejectOnEmpty: true });
      await user1.increment('integer1');
    ...
    it('should still work right with other concurrent updates', async () => {
      ...
      expect(user5.integer1).to.equal(3);
    });
    ...
    it('with key value pair', async () => {
      const user1 = await vars.User.findByPk(1, { rejectOnEmpty: true });
      await user1.increment({ integer1: 1, integer2: 2 });
      const user3 = await vars.User.findByPk(1, { rejectOnEmpty: true });
    ...
    it('with negative value', async () => {
      ...
      await Promise.all([
        user1.increment('integer1', { by: -2 }),
        user1.increment(['integer1', 'integer2'], { by: -2 }),
        user1.increment({ integer1: -1, integer2: -2 }),
      ]);
    ...
    it('supports silent: true', async () => {
      ...
      await user.increment('integer1', { by: 1, silent: true });

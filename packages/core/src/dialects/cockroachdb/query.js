  ...
  async run(sql, parameters, options) {
    ...
    if (this.isShowIndexesQuery()) {
      for (const row of rows) {
        ...
        // CockroachDB does not support array splicing so we need to separate the fields here.
        const indexFields = indkeys.slice(0, row.index_fields_length);
        const includeFields = indkeys.slice(row.index_fields_length, indkeys.length);

        // Indkey is the order of attributes in the index, specified by a string of attribute indexes
        row.fields = indexFields.map((indKey, index) => {
        ...
        row.includes = includeFields.map(indKey => {
          field = columns[indKey];
          // for functional indices indKey = 0
          if (!field) {
            return null;
        ...
        delete row.index_fields_length;
      }
    ...
    switch (code) {
      ...
      case '23505':
        if (errDetail && (match = errDetail.replaceAll(/["']/g, '').match(/Key \((.*?)\)=\((.*?)\)/))) {
          const fields = zipObject(match[1].split(',').map(match => match.trim()), match[2].split(',').map(match => match.trim()));
          const errors = [];

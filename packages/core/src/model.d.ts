...
export interface DropOptions extends Logging {
  ...
  /**
   * Also drop temporal table history tables
   */
...
export interface AttributeOptions<M extends Model = Model> {
  ...
  /**
   * Indicates that the attribute is used for temporal tables.
   */
  [TEMPORAL_SECRET]?: boolean | undefined;
}
...
export interface ModelOptions<M extends Model = Model>
  ...
  /**
   * The type of temporal table to use when creating the model.
   */
  temporalTableType?: Exclude<TemporalTableType, TemporalTableType.NON_TEMPORAL> | undefined;
}

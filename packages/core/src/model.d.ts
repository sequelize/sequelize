...
export interface AttributeOptions<M extends Model = Model> {
   ...
   * Must be a {@link BaseSqlExpression}, such as one created by the `sql` template tag or
   * {@link sql.fn}. Plain strings are not accepted.
   *
   ...
   *     generatedAs: sql`"firstName" || ' ' || "lastName"`,
   *     generatedColumn: 'STORED',
   *   }
   * }
   * ```
   */
  generatedAs?: BaseSqlExpression | undefined;

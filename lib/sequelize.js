...
module.exports = (function() {
  ...
  */
  ...
  Sequelize.prototype.sync = function(options) {
    ...
    var when;
    if (options.truncate) {
      when = this.truncate(options);
    } else {
      when = Promise.resolve();
    }

    if (options.force) {
      when = when.bind(this).then(function() {
   ...
   * @see {Model#truncate} for options
   *
   * @param {object} options The options passed to each call to Model.truncate
   * @return {Promise}
   */

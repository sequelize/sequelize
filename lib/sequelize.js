...
module.exports = (function() {
  ...
  */
  ...
  /**
    Returns and instance of Sequelize, created from a json config file
  */
  Sequelize.createFromConfigFile = function(options) {
    var path = options.path || __dirname + "./config/config"
      , env = options.env || process.env.NODE_ENV
      , config = require(path)[env]

    if (config === undefined) {
      throw new Error('"' + env + '" does not exist in config ' + path + '.')
    }

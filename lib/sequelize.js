...
class Sequelize {
  ...
  query(sql, options) {
    ...
    return Promise.try(() => {
      ...
      const queryBegin = Date.now();
      this.runHooks('beforeQuery', sql, options);
      return retry(() => query.run(sql, bindParameters), retryOptions)
        .then(results => {
          this.runHooks('afterQuery', sql, { results, error: null, duration: Date.now() - queryBegin }, options);
          return results;
        }, error => {
          this.runHooks('afterQuery', sql, { results: null, error, duration: Date.now() - queryBegin }, options);
          throw error;
        })
        .finally(() => {

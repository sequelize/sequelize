...
class Sequelize {
    ...
    return Promise.resolve(retry(retryParameters => Promise.try(() => {
      ...
      if (typeof sql === 'object') {
        ...
        if (sql instanceof Composition) {
          ...
          // Skips formatBindParameters, avoids unneeded reparsing
          options.rawBind = true;
        } else {
  ...
  databaseVersion(options) {
    return this.getQueryInterface().databaseVersion(options).then(version => {
      // Coherce databaseVersion to valid semver format. Some DBs return partial
      // versions that have to be coherced. E.g. postgres returns '10.6'
      return semver.valid(semver.coerce(version)) || 0;
    });
  }

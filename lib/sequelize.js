...
class Sequelize {
   ...
   * @param {Object}   [options.dsn] The datasource name (DSN) for connections using the odbc connector
   */
  constructor(database, username, password, options) {
    ...
    if (arguments.length === 1 && typeof database === 'object') {
      ...
      options = database;
      config = _.pick(options, 'host', 'port', 'database', 'username', 'password', 'dsn');
    ...
    if (this.options.dialect === 'ibmi' && this.options.timezone !== '+00:00') {
      throw new Error('Setting a custom timezone is not supported by IBM i Db2, dates are always returned as UTC. Please remove the custom timezone parameter.');
    }

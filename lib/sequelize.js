...
class Sequelize {
  ...
  async query(sql, options) {
    ...
    return retry(async () => {
      ...
      try {
        await this.hooks.run('beforeQuery', options, query);
        checkTransaction();
        return await query.run(sql, bindParameters);
      } finally {
        await this.hooks.run('afterQuery', options, query);
  ...
  async sync(options) {
    ...
    if (options.hooks) {
      await this.hooks.run('beforeBulkSync', options);
    }
    if (options.force) {
    ...
    if (options.hooks) {
      await this.hooks.run('afterBulkSync', options);
    }
    return this;
  }

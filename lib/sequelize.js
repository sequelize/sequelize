  ...
  */
  ...
  Sequelize.prototype.sync = function(options) {
    ...
    return when.then(function() {
      ...
      // Iterate daos and sync.  Collect results into array
      return Promise.reduce(daos, function(total, dao) {
        return total.push(dao.sync(options));
      }, [])
      // array of tables is ok, but I really want 'self' as DI container for models
      .then(function(syncTables) {
        return self;
      });
    });
  };

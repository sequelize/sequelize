...
const { each: pEach, tryFn } = require('./utils/promise-helpers');
...
class Sequelize {
  ...
  sync(options) {
    ...
    return tryFn(() => {
      ...
      .then(() => {
        ...
        return pEach(models, model => model.sync(options));
      })
      .then(() => {
        if (options.hooks) {
  ...
  authenticate(options) {
    return this.query('SELECT 1+1 AS result', _.assign({ raw: true, plain: true }, options)).then(() => undefined);
  }
  ...
  transaction(options, autoCallback) {
    ...
    if (!autoCallback) {
      return transaction.prepareEnvironment(false).then(() => transaction);
    }

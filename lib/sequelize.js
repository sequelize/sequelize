  ...
  */
  var Sequelize = function(database, username, password, options) {
    ...
    this.options = Utils._.extend({
      ...
      enableFibers: false // by default sequelize won't use fibers
    }, options || {});
    ...
    this.config = {
      ...
      enableFibers: this.options.enableFibers
    };
  ...
  Sequelize.prototype.query = function(sql, callee, options, replacements) {
    ...
    if (this.options.enableFibers !== true || options.forcePromise === true) {
      fiber = undefined; // no fiber enabled or function call requires promises instead of actual result
    }

    if (fiber !== undefined) {
      options.transaction = fiber.transaction; // recover the current Fiber transaction
    }
    ...
    promise = Promise.resolve(
      ...
      innerPromise = query.run(sql).finally(function() {
    ...
    if (fiber === undefined) {
      ...
      return Fiber.yield(); // yielding the query call
    }
  };

  /**
   * Starts a new transaction inside function call function without the need of promises, working 
   * as if each query call is blocking the current thread.
   *
   * To use this function it's required to configure Sequelize with `enableFibers : true`
   *
   * @see https://www.npmjs.org/package/fibers
   * @param {Function} fn the function that will run the transaction
   */
  Sequelize.prototype.begin = function (fn) {
    var self = this;

    if (this.options.enableFibers !== true) {
      throw new Error('To use this function it\'s required to configure Sequelize with `enableFibers : true`');
    }
    ...
    Fiber(function () {
      ...
      self.transaction().then(function (t) {
          fiber.run(t);
      });

      transaction = Fiber.yield();
      fiber.transaction = transaction;

      try {
        fn.apply(self, transaction);
        ...
        // this is gonna throw an exception to the Sequelize.prototype.begin function
        fiber.throwInto(err); 
      }
    }).run();
  };

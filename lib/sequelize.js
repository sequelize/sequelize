...
class Sequelize {
  ...
  import(importPath) {
    ...
    if (path.normalize(importPath) !== path.resolve(importPath)) {
      ...
      // When callerFilename is undefined (callerStackTrace is empty) fallback to current folder.
      // We are probably going to use function instead of requring file
      const callerPath = callerFilename ? path.dirname(callerFilename) : '.';

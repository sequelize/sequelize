...
class Sequelize {
  ...
  constructor(database, username, password, options) {
    ...
    if (this.options.logging === true) {
      ...
      this.options.logging = Utils.log;
    }
  ...
  query(sql, options) {
    ...
    options = _.defaults(options, {
      logging: this.options.hasOwnProperty('logging') ? this.options.logging : Utils.log,
  ...
  log(...args) {
    ...
    if (options.logging) {
      if (options.logging === true) {
        logger.deprecate('The logging-option should be either a function or false. Default: console.log');
        options.logging = Utils.log;
      }

      // second argument is sql-timings, when benchmarking option enabled
      if ((this.options.benchmark || options.benchmark) && options.logging === Utils.log) {

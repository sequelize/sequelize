...
class Sequelize {
  ...
  constructor(database, username, password, options) {
    ...
    this.transactionQueue = new PQueue({
      concurrency: this.connectionManager.pool.maxSize - 1
    });

    this.queued = callback => () => this.transactionQueue.add(callback);
  }
  ...
  transaction(options, autoCallback) {
    ...
    return Sequelize._clsRun(this.queued(() => {

  ...
  */
  ...
  Sequelize.prototype.define = function(daoName, attributes, options) {
    ...
    Utils._.each(attributes, function(dataType, name){
      var type = (!!dataType && dataType.hasOwnProperty("type")) ? dataType.type : dataType;

      if (type === undefined) {
        throw new Error('Unrecognized data type for field ' + name)
      }

      var lowered = type.toString().toLowerCase()

      if (type === String || lowered === "string") {
        if (dataType.type) {
          attributes[name].type = DataTypes.STRING
        } else {
          attributes[name] = DataTypes.STRING
        }
      }
      else if (type === Boolean || lowered === "boolean") {
        if (dataType.type) {
          attributes[name].type = DataTypes.BOOLEAN
        } else {
          attributes[name] = DataTypes.BOOLEAN
        }
      }
      else if (type === Array || lowered === "array") {
        if (dataType.type) {
          attributes[name].type = DataTypes.ARRAY('text')
        } else {
          attributes[name] = DataTypes.ARRAY('text')
        }
      }
      else if (type === Number || lowered === "number") {
        if (dataType.type) {
          attributes[name].type = DataTypes.INTEGER
        } else {
          attributes[name] = DataTypes.INTEGER
        }
      }
      else if (type === Date || lowered === "date") {
        if (dataType.type) {
          attributes[name].type = DataTypes.DATE
        } else {

...
class Sequelize {
  ...
  /**
   * @param {Connection} connection  The connection to log for.
   * @param {string} sql
   * @param {Object} options
   * @param {boolean} [benchmark] If true, will log query and runtime information upon call of returned function.
   * @param {DebugContext} [debug] A debugging function, if set will be called with connection and sql.
   * @private
   * @returns {Function} A function to call after the query was completed.
   */
  logQuery(connection, sql, options, benchmark = false, debug) {
    const startTime = Date.now();
    const fmt = `(${connection.uuid || 'default'}): ${sql}`;
    const msg = `Executing ${fmt}`;
    debug.log(msg);
    if (!benchmark) {
      this.log(`Executing ${fmt}`, options);
    }
    return () => {
      const afterMsg = `Executed ${fmt}`;
      debug.log(afterMsg);
      if (benchmark) {
        this.log(afterMsg, Date.now() - startTime, options);
      }
    };
  }

...
import { each as pEach } from './utils/promise-helpers.js';
...
import QueryInterface from './query-interface.js';
import Transaction from './transaction.js';
import { CLSNamespace, getClsTransactionFor } from './cls.js';
import QueryTypes from './query-types.js';
import * as sequelizeErrors from './errors.js';
import * as Hooks from './hooks.js';
import Association from './associations/index.js';
...
// `Sequelize#import` loads a caller-supplied model path synchronously, which has
// no ESM equivalent.
...
export class Sequelize {
   ...
   * @param {String}   [options.dialect] The dialect of the database you are connecting to. Only `postgres` is supported.
   ...
   * @param {string}   [options.timezone='+00:00'] The timezone used when converting a date from the database into a JavaScript date. The timezone is also used to SET TIMEZONE when connecting to the server, to ensure that the result of NOW, CURRENT_TIMESTAMP and other time related functions have in the right timezone. For best cross platform performance use the format +/-HH:MM. Will also accept IANA timezone names (e.g. 'America/Los_Angeles'); this is useful to capture daylight savings time changes.
   ...
   * @param {Boolean}  [options.releaseSavepointsOnCommit=false] Commit a nested transaction by issuing `RELEASE SAVEPOINT`, instead of leaving its savepoint open until the root transaction ends. Releasing reclaims server-side state, but it also discards every savepoint opened after it, so nested transactions must not overlap — two running concurrently (typically under `Promise.all`) will have one destroy the other's savepoint. When enabled, closing a savepoint that an enclosing one already discarded throws before any SQL is issued, naming the cause and leaving the parent transaction usable.
   *
   */
  constructor(...args) {
    // The overloads are distinguished by arity, so the parameters are read off
    // the rest array rather than declared individually.
    const [database, username, password] = args;
    let options = args[3];
    let config;
    ...
    if (args.length === 1 && typeof database === 'object') {
      ...
      const urlParts = url.parse(database);
    ...
    this.options = Object.assign(
      {
        ...
        retry: {
          max: 5,
          // Nothing is retried by default. Note that `retry-as-promised` treats an EMPTY `match`
          // array as "retry every error", so this must be a pattern that cannot match rather than
          // `[]`. Retrying a failed query in place is wrong for Postgres anyway -- once a statement
          // errors the transaction is aborted, so the retry only yields 25P02. `max` stays at 5 so
          // that callers who supply their own `match` still get a sensible retry budget.
          match: [/$^/]
        },
        ...
        releaseSavepointsOnCommit: false
      },
      options || {}
    );
    ...
    switch (this.getDialect()) {
      case 'postgres':
        ...
        break;
      default:
        throw new Error('The dialect ' + this.getDialect() + ' is not supported. Supported dialects: postgres.');
    }
  ...
  import(path, ...rest) {
    ...
    if (!this.importCache[path]) {
      let defineCall = rest.length > 0 ? rest[0] : require(path);
   ...
   *
   * By default, the function resolves to a two-element array: the results, and a metadata object containing number of affected rows etc. Destructure it to access the results.
   *
   * If you are running a type of query where you don't need the metadata, for example a `SELECT` query, you can pass in a query type to make sequelize format the results:
   *
   * ```js
   * // Raw query - destructure the results and metadata
   * const [results, metadata] = await sequelize.query('SELECT...');
   *
   * // SELECT query - the results are returned directly
   ...
   * @param {String}          [options.attributeBehavior='throw'] Sequelize had to introduce a breaking change to fix vulnerability CVE-2023-22578. This option allows you to revert to the old behavior (unsafe-legacy), or to opt in to the new behavior (escape). The default behavior throws an error to warn you about the change (throw).
   ...
   * @param {Boolean}         [options.nest=false] If true, transforms objects with `.` separated property names into nested objects. For example { 'user.username': 'john' } becomes { user: { username: 'john' }}. When `nest` is true, the query type is assumed to be `'SELECT'`, unless otherwise specified
  ...
  query(sql, options) {
    ...
    // Counted out here rather than around the query itself so that the pair stays balanced no matter
    // how the query ends: one increment per call, one decrement when the whole retry chain settles.
    // Counting inside the retried function instead would leak a permanent +1 whenever it threw before
    // reaching the run (acquiring a connection, say) and would over-decrement once a query was retried.
    const trackRunningQueries = this.test._trackRunningQueries;

    if (trackRunningQueries) {
    ...
    const promise = Promise.resolve(
      retry(async () => {
        ...
        if (options.bind) {
          const bindSql = this.dialect.Query.formatBindParameters(sql, options.bind);
          sql = bindSql[0];
          bindParameters = bindSql[1];
        }

        options = _.defaults(options, {
          logging: Object.hasOwn(this.options, 'logging') ? this.options.logging : console.log,
          searchPath: Object.hasOwn(this.options, 'searchPath') ? this.options.searchPath : 'DEFAULT'
        });

        if (options.transaction === undefined) {
          options.transaction = getClsTransactionFor(this);
        }
        ...
        //if the dialect option to prepend searchPath is not true delete the searchPath option
        if (
  ...
  async sync(options) {
    ...
    await pEach(models, (model) => model.sync(options));
  ...
  truncate(options) {
    ...
    if (options && options.cascade) {
      return pEach(models, truncateModel);
    } else {
  ...
  drop(options) {
    ...
    return pEach(models, (model) => model.drop(options));
  }
  ...
  async authenticate(options) {
    await this.query('SELECT 1+1 AS result', Object.assign({ raw: true, plain: true }, options));
  }
  ...
  /**
   * Creates an object representing nested where conditions for the postgres json data-type.
   * @see {@link Model#findAll}
   *
   * @method json
   ...
   * To enable CLS, create a namespace and set it on the sequelize constructor:
   *
   * ```js
   * import Sequelize from 'sequelize';
   ...
   *
   * With CLS enabled, calling this inside an existing transaction nests the new transaction as a
   * SAVEPOINT of the ambient one. Rolling it back only undoes the work inside it and leaves the outer
   * transaction usable; committing it leaves the work to be committed by the outer transaction.
   *
   * @see {@link Transaction}
   * @param {Object}   [options={}]
   * @param {Transaction|null} [options.transaction] Parent transaction to nest under as a SAVEPOINT. Defaults to the ambient CLS transaction, if any. Pass `null` to start an independent transaction on its own connection.
  ...
  async transaction(options, autoCallback) {
    ...
    // Mirrors the CLS handling in `Sequelize.prototype.query`: with no explicit `transaction` the
    // ambient CLS transaction becomes the parent, so a nested call opens a SAVEPOINT on the same
    // connection rather than a second, independent transaction. Pass `transaction: null` to opt out.
    const clsTransaction = getClsTransactionFor(this);

    if (clsTransaction && (!options || options.transaction === undefined)) {
      options = Object.assign({}, options, { transaction: clsTransaction });
    }
  ...
  /**
   * Creates a CLS namespace to pass to {@link Sequelize.useCLS}, backed by
   * `AsyncLocalStorage`. Callers used to have to supply this themselves, in practice
   * via the unmaintained `cls-hooked`; any object implementing `get`/`set`/`run`/`bind`
   * is still accepted.
   *
   * @param {string} [name] Namespace name, for debugging
   * @returns {CLSNamespace} A new namespace
   */
  static createCLSNamespace(name) {
    return new CLSNamespace(name);
  }

  /**
   * Use CLS with Sequelize. Native promises propagate the namespace via
   * `async_hooks` automatically, so no promise patching is required.
   *
   * @param {Object}   ns   CLS namespace, e.g. from {@link Sequelize.createCLSNamespace}
...
Sequelize.condition = Sequelize.prototype.condition = function (...args) {
  Utils.deprecate('Sequelize.condition has been deprecated, please use Sequelize.where instead');
  return Sequelize.where.apply(this, args);
};
 ...
 */
Sequelize.version = packageJson.version;
...
/**
 * This used to be a reference to Bluebird Promise but is now
 * the native Promise
 */
...
/**
 * A reference to the CLS namespace class, exposed so consumers can subclass it — e.g. to add
 * `enter`/`exit` by overriding `_activeContext`. Use {@link Sequelize.createCLSNamespace} if you
 * just need an instance.
 * @see {@link CLSNamespace}
 */
Sequelize.prototype.CLSNamespace = Sequelize.CLSNamespace = CLSNamespace;

/**
 * Provide alternative version of `inflection` module to be used by `Utils.pluralize` etc.
 *
 * Must be called before any model is defined: inflected names (table names, association
 * aliases, foreign keys, accessor methods) are derived once and then frozen, so a later
 * swap applies to nothing. A late call warns rather than failing silently.
 *
 * @param {Object} _inflection - `inflection` module; must expose `pluralize`, `singularize`
 *   and `underscore`
 * @throws {Error} if any of those three methods is missing
 */

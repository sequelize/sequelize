 ...
 */
  ...
  * @param {Boolean}  [options.noImplicitTransactions=false] Set to `true` to prevent Sequelize from creating transactions when none is provided. All queries must have transactions provided either explicitly, or via CLS.
...
class Sequelize {
  ...
  query(sql, options) {
    ...
    if (!options.transaction && this.options.noImplicitTransactions) {
      throw new sequelizeErrors.NoTransactionError('Transaction required (noImplicitTransactions is enabled).');
    }
...
Sequelize.prototype.NoTransactionError = Sequelize.NoTransactionError =
  sequelizeErrors.NoTransactionError;

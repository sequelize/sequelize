...
// The only three methods lib/ ever calls on the inflection module.
const INFLECTION_METHODS = ['pluralize', 'singularize', 'underscore'];

// Every inflected name (table names, association aliases, foreign keys, accessor
// method names) is derived once at define/associate time and then frozen, so a
// swap that lands after the first derivation silently applies to nothing.
// Tracking first use lets `useInflection` say so instead of no-op'ing.
let inflectionUsed = false;
...
export function useInflection(_inflection) {
  const missing = INFLECTION_METHODS.filter((method) => typeof _inflection?.[method] !== 'function');

  if (missing.length > 0) {
    throw new Error(
      `useInflection expects an object exposing ${INFLECTION_METHODS.join(', ')}; missing: ${missing.join(', ')}`
    );
  }

  if (inflectionUsed) {
    warn(
      'useInflection was called after names had already been inflected. Names derived before this ' +
        'call (table names, association aliases, foreign keys, accessor methods) keep their old ' +
        'spelling — call useInflection before defining any model.'
    );
  }

  inflection = _inflection;
  // A fresh implementation starts a new epoch: only usage of *this* one should
  // make a subsequent swap warn.
  inflectionUsed = false;
}
...
export function getOperators(obj) {
  return Object.getOwnPropertySymbols(obj || {}).filter((symbol) => operatorsSet.has(symbol));
}
  ...
  sources.forEach((source) => {
    if (source) {
      ...
      getComplexKeys(source).forEach((key) => {
        ...
        if (value === undefined || (Object.is(value, Object.prototype[key]) && !Object.hasOwn(object, key))) {

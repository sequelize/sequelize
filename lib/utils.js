...
const Placeholder = composition.Placeholder;
const QueryComposition = composition.Composition;
...
function replacePlaceholder(composition, replacement, name) {
  const newObj = new QueryComposition();

  replacement = replacement instanceof QueryComposition ?
    replacement.items : [replacement];

  newObj.items = composition.items.reduce((a, item) => {
    if (item instanceof Placeholder &&
      (!item.name && !name || item.name === name)) {
      a.push.apply(a, replacement);
    } else {
      a.push(item);
    }
    return a;

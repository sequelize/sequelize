...
class Model {
  ...
  static _addDefaultAttributes() {
    ...
    Utils.forEach(head, (value, attr) => {
      this.rawAttributes[attr] = value;
    });

    Utils.forEach(existingAttributes, (value, attr) => {
      this.rawAttributes[attr] = value;
    });

    Utils.forEach(tail, (value, attr) => {
  ...
  static _expandIncludeAllElement(includes, include) {
    ...
    (function addAllIncludes(parent, includes) {
      Utils.forEach(parent.associations, association => {
  ...
  static _validateIncludedElement(include, tableNames, options) {
    ...
    if (include.attributes && !options.raw) {
      ...
      if (include.attributes.length) {
        Utils.forEach(include.model.primaryKeys, (attr, key) => {
  ...
  static init(attributes, options = {}) {
    ...
    // error check options
    if (options.validate) {
      Utils.forEach(options.validate, (validator, validatorType) => {
  ...
  static refreshAttributes() {
    ...
    ['get', 'set'].forEach(type => {
      ...
      Utils.forEach(funcs, (method, attribute) => {
      ...
      Utils.forEach(this.rawAttributes, (options, attribute) => {
    ...
    Utils.forEach(this.rawAttributes, (definition, name) => {
      ...
      .then(() => {
        ...
        return Promise.all([
          ...
          .then(tableInfos => {
            ...
            Utils.forEach(attributes, (columnDesc, columnName) => {
            ...
            Utils.forEach(columns, (columnDesc, columnName) => {
              ...
              if (!currentAttribute) {
                ...
                if (currentAttribute.references) {
                  ...
                  Utils.forEach(foreignKeyReferences, foreignKeyReference => {
  ...
  static warnOnInvalidOptions(options, validColumnNames) {
    ...
    const unrecognizedOptions = Object.keys(options).filter(o => !validQueryKeywords.has(o));
  ...
  static findOrCreate(options) {
    ...
    return this.sequelize.transaction(options).then(t => {
      ...
      return this.create(values, options).then(instance => {
        ...
        if (defaultFields) {
          if (!Utils.intersects(errFieldKeys, whereFields) && Utils.intersects(errFieldKeys, defaultFields)) {
            throw err;
          }
        }

        if (Utils.intersects(errFieldKeys, whereFields)) {
          Utils.forEach(err.fields, (value, key) => {
      ...
      if (options.individualHooks) {
        return this.findAll({
          ...
          return Promise.map(instances, instance => {
            ...
            Utils.forEach(valuesUse, (newValue, attr) => {
            ...
            return this.runHooks('beforeUpdate', instance, options).then(() => {
              if (!different) {
                ...
                Utils.forEach(instance.dataValues, (newValue, attr) => {
                  if (newValue !== instance._previousDataValues[attr]) {

...
import associationsMixin from './associations/mixin.js';
...
class Model {
  ...
  static _paranoidClause(model, options) {
    ...
    let deletedAtDefaultValue = Object.hasOwn(deletedAtAttribute, 'defaultValue')
  ...
  static _addDefaultAttributes() {
    ...
    if (!Object.values(this.rawAttributes).some((attr) => attr.primaryKey)) {
    ...
    for (const [attr, value] of Object.entries(head)) {
      this.rawAttributes[attr] = value;
    }

    for (const [attr, value] of Object.entries(existingAttributes)) {
      this.rawAttributes[attr] = value;
    }

    for (const [attr, value] of Object.entries(tail)) {
  ...
  static _transformStringAssociation(include, self) {
    if (self && typeof include === 'string') {
      if (!Object.hasOwn(self.associations, include)) {
  ...
  static _expandIncludeAllElement(includes, include) {
    ...
    const used = [];
    (function addAllIncludes(parent, parentIncludes) {
      Object.values(parent.associations).forEach((association) => {
        ...
        if (_.find(parentIncludes, predicate)) {
          return;
        }
        ...
        parentIncludes.push(thisInclude);
  ...
  static _validateIncludedElement(include, tableNames, options) {
    ...
    if (include.attributes && !options.raw) {
      ...
      if (include.attributes.length) {
        for (const [key, attr] of Object.entries(include.model.primaryKeys)) {
    ...
    if (
      ...
      !options.attributes.includes(association.source.primaryKeyAttribute)
    ) {
   ...
   * @param {String}                  [options.indexes[].type] Index type. One of `UNIQUE`, `FULLTEXT` and `SPATIAL`
   * @param {String}                  [options.indexes[].method] The method to create the index by (`USING` statement in SQL). Postgres supports BTREE, HASH, GIST and GIN.
  ...
  static init(attributes, options) {
    ...
    if (!this.options.tableName) {
      this.tableName = this.options.freezeTableName
        ? this.name
        : Utils.underscoredIf(Utils.pluralize(this.name), this.options.underscoredAll);
    } else {
    ...
    // error check options
    for (const [validatorType, validator] of Object.entries(options.validate || {})) {
      if (Object.keys(attributes || {}).includes(validatorType)) {
        throw new Error(
    ...
    if (this.options.timestamps) {
      if (this.options.createdAt !== false) {
        this._timestampAttributes.createdAt =
          this.options.createdAt || Utils.underscoredIf('createdAt', this.options.underscored);
      }
      if (this.options.updatedAt !== false) {
        this._timestampAttributes.updatedAt =
          this.options.updatedAt || Utils.underscoredIf('updatedAt', this.options.underscored);
      }
      if (this.options.paranoid && this.options.deletedAt !== false) {
        this._timestampAttributes.deletedAt =
          this.options.deletedAt || Utils.underscoredIf('deletedAt', this.options.underscored);
      }
    }
    ...
    this._readOnlyAttributes = Object.values(this._timestampAttributes);
    if (this._versionAttribute) {
    ...
    this._isReadOnlyAttribute = _.memoize(
      (key) => this._hasReadOnlyAttributes && this._readOnlyAttributes.indexOf(key) !== -1
    );
    ...
    for (const scope of Object.values(this.options.scopes)) {
  ...
  static refreshAttributes() {
    ...
    ['get', 'set'].forEach((type) => {
      ...
      for (const [attribute, method] of Object.entries(funcs)) {
      ...
      for (const [attribute, options] of Object.entries(this.rawAttributes)) {
      ...
      for (const [name, fct] of Object.entries(funcs)) {
    ...
    for (const [name, definition] of Object.entries(this.rawAttributes)) {
      ...
      if (Object.hasOwn(definition, 'unique') && definition.unique) {
        let idxName;
        if (typeof definition.unique === 'object' && Object.hasOwn(definition.unique, 'name')) {
      ...
      if (Object.hasOwn(definition, 'validate')) {
    ...
    for (const key of Object.keys(attributeManipulation)) {
      if (Object.hasOwn(Model.prototype, key)) {
  ...
  static async sync(options) {
    ...
    if (options.alter) {
      const [columns, foreignKeyReferences] = await Promise.all([
      ...
      const changes = []; // array of thunks to run sequentially
      const removedConstraints = {};

      for (const columnName of Object.keys(attributes)) {
        if (!columns[columnName]) {
          changes.push(() =>
            this.QueryInterface.addColumn(this.getTableName(options), columnName, attributes[columnName])
          );
        }
      }
      for (const columnName of Object.keys(columns)) {
        ...
        if (!currentAttributes) {
          ...
          if (currentAttributes.references) {
            ...
            foreignKeyReferences.forEach((foreignKeyReference) => {
              ...
              if (
                ...
                changes.push(() =>
                  this.QueryInterface.removeConstraint(this.getTableName(options), constraintName, options)
                );
          ...
          changes.push(() =>
            this.QueryInterface.changeColumn(this.getTableName(options), columnName, attributes[columnName])
          );
        }
      }
    ...
    indexes = this.options.indexes.filter((item1) => !indexes.some((item2) => item1.name === item2.name));
    ...
    // `CREATE INDEX CONCURRENTLY` takes a ShareUpdateExclusiveLock and then waits for every
    // transaction already holding a lock on the table to finish. A plain `CREATE INDEX` running
    // at the same time wants a ShareLock on that same table, so the two deadlock. Build all the
    // regular indexes first, then the concurrent ones one at a time.
    const concurrentIndexes = indexes.filter((index) => index.concurrently === true);
    const regularIndexes = indexes.filter((index) => !concurrentIndexes.includes(index));

    await Promise.all(regularIndexes.map(addIndex));
  ...
  static scope(option, ...scopeArgs) {
    ...
    const options = [option, ...scopeArgs].flat();
    ...
    for (const opt of options) {
      ...
      if (_.isPlainObject(opt)) {
        if (opt.method) {
          if (Array.isArray(opt.method) && !!self.options.scopes[opt.method[0]]) {
            scopeName = opt.method[0];
            scope = self.options.scopes[scopeName].apply(self, opt.method.slice(1));
          } else if (self.options.scopes[opt.method]) {
            scopeName = opt.method;
        ...
        if (opt === 'defaultScope' && _.isPlainObject(self.options.defaultScope)) {
   ...
   */
  /**
   * Validate, clone and scope-inject the options handed to `findAll`.
   *
   * This is the first of three normalization steps split out of `findAll` so that the SQL for a
   * find can be generated without executing it (see `buildFindAllQuery`). They are separate rather
   * than one function because `findAll` runs a hook between each, and hooks may mutate `options`.
   *
   * @param {Object} options   Find options, as passed by the caller.
   * @param {Object} tableNames Accumulator for the tables involved in the query.
   * @return {Object}          The cloned, normalized options.
   * @private
   */
  static _prepareFindOptions(options, tableNames) {
  ...
  /**
   * Second normalization step: conform the options and expand `include: all`.
   *
   * @param {Object} options Options returned by `_prepareFindOptions`.
   * @private
   */
  static _conformFindOptions(options) {
    this._conformOptions(options, this);
    this._expandIncludeAll(options);
  }

  /**
   * Final normalization step: resolve includes, default the attribute list, map field names and
   * apply the paranoid clause. After this the options are ready for `selectQuery`.
   *
   * @param {Object} options    Options returned by `_conformFindOptions`.
   * @param {Object} tableNames Accumulator for the tables involved in the query.
   * @return {Object}           The finalized options (`_paranoidClause` may return a new object).
   * @private
   */
  static _finalizeFindOptions(options, tableNames) {
    if (options.include) {
    ...
    return this._paranoidClause(this, options);
  }

  /**
   * Build the SELECT a `findAll` with these options would run, without running it.
   *
   * Normalizes the options exactly as `findAll` does -- the two share `_prepareFindOptions`,
   * `_conformFindOptions` and `_finalizeFindOptions` -- but runs no hooks, performs no I/O and
   * does not apply `rejectOnEmpty` or `findSeparate`, none of which affect the generated SQL.
   *
   * @param {Object} [options] The same options accepted by `findAll`.
   * @return {String}          The generated SQL.
   */
  static buildFindAllQuery(options) {
    const tableNames = {};

    options = this._prepareFindOptions(options, tableNames);
    this._conformFindOptions(options);
    options = this._finalizeFindOptions(options, tableNames);
    options.tableNames = Object.keys(tableNames);

    return this.QueryInterface.QueryGenerator.selectQuery(this.getTableName(options), options, this);
  }
  ...
  static warnOnInvalidOptions(options, validColumnNames) {
    ...
    const unrecognizedOptions = Object.keys(options).filter((key) => !validQueryKeywords.includes(key));
    const unexpectedModelAttributes = unrecognizedOptions.filter((key) => validColumnNames.includes(key));
  ...
  static async _findSeparate(results, options) {
    ...
    await Promise.all(
      options.include.map(async (include) => {
        ...
        const map = await include.association.get(
          results,
          Object.assign(
            {},
  ...
  static findOne(options) {
    ...
    if (options.limit === undefined) {
      ...
      if (
        !options.where ||
        !Object.entries(options.where).some(
          ([key, value]) =>
   ...
   * @param {Transaction}   [options.transaction] Transaction to run query under
   * @param {String}        [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
   * @param  {Boolean|Array<String>} [options.returning=true] Return the affected rows (only for postgres). `true` returns the model's own columns; pass an array of attribute names to return only those
   *
   * @return {Promise<Model>}
   *
   */
   ...
   * The successful result of the promise resolves to `[instance, initialized]` — destructure with `const [instance, initialized] = await ...`.
   *
   ...
   */
  static async findOrBuild(options, ...rest) {
    if (!options || !options.where || rest.length > 0) {
      throw new Error(
    ...
    if (instance === null) {
      ...
      return [this.build(values, options), true];
    }
   ...
   * The successful result of the promise resolves to `[instance, created]` — destructure with `const [instance, created] = await ...`.
   *
   ...
   */
  static async findOrCreate(options, ...rest) {
    if (!options || !options.where || rest.length > 0) {
      throw new Error(
    ...
    if (options.transaction === undefined) {
      const t = getClsTransactionFor(this.sequelize);
      if (t) {
    ...
    try {
      transaction = await this.sequelize.transaction(options);
      ...
      try {
        ...
        const flattenedWhereKeys = Object.keys(flattenedWhere).map((name) => name.split('.').pop());
        const whereFields = flattenedWhereKeys.map((name) => _.get(this.rawAttributes, `${name}.field`, name));
        const defaultFields =
          options.defaults && Object.keys(options.defaults).map((name) => this.rawAttributes[name].field || name);

        if (defaultFields) {
          if (
            !Object.keys(err.fields).some((field) => whereFields.includes(field)) &&
            Object.keys(err.fields).some((field) => defaultFields.includes(field))
          ) {
            throw err;
          }
        }

        if (Object.keys(err.fields).some((field) => whereFields.includes(field))) {
   ...
   * @param  {Boolean}      [options.returning=false] Re-select and return the upserted row (Postgres only). Only `true` is honoured here -- the row comes back from a follow-up select, so narrowing the RETURNING clause would have no effect on the result
   * @param  {Transaction}  [options.transaction] Transaction to run query under
   * @param  {Function}     [options.logging=false] A function that gets executed while running the query to log the sql.
   * @param  {Boolean}      [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
   * @param  {String}       [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
  ...
  static async upsert(values, options) {
    ...
    const [created, primaryKey] = await this.QueryInterface.upsert(
      this.getTableName(options),
      insertValues,
      updateValues,
      instance.where(),
   ...
   *
   * The success handler is passed an array of instances, but please notice that these may not completely represent the state of the rows in the DB, because `returning` defaults to false.
   * Pass `returning: true` to append a RETURNING clause and get back automatically generated IDs and other default values, or query for the records again.
   *
   * If validation fails, the promise is rejected with a native `AggregateError` whose `.errors` is an array of `BulkRecordError`s.
   *
   ...
   * @param  {Boolean}      [options.ignoreDuplicates=false] Skip rows that violate a unique constraint (`ON CONFLICT DO NOTHING`). Skipped rows are not returned by `RETURNING`, so the result can be shorter than the input. Ignored when options.individualHooks is set.
   * @param  {Array}        [options.updateOnDuplicate]      Fields to update if the row already exists. Not implemented.
   * @param  {Transaction}  [options.transaction] Transaction to run query under
   * @param  {Function}     [options.logging=false]          A function that gets executed while running the query to log the sql.
   * @param  {Boolean}      [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
   * @param  {Boolean|Array<String>} [options.returning=false] Append a RETURNING clause to get back auto generated values (Postgres only). `true` returns the model's own columns; pass an array of attribute names to return only those
   * @param  {String}       [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
   *
   * @return {Promise<Array<Model>>}
   */
  static async bulkCreate(records, options) {
    ...
    if (options.updateOnDuplicate) {
      // Guard rather than silently no-op: the query generator ignores this option, so without the
      // throw a bulk upsert would quietly degrade to a plain insert.
      throw new Error("The 'updateOnDuplicate' option is not implemented.");
    }
    ...
    if (options.validate) {
      ...
      delete options.skip;
      if (errors.length) {
        throw new AggregateError(errors, 'Bulk create validation failed');
      }
    }
    ...
    if (options.individualHooks) {
      ...
      Utils.mapReturningFieldNames(options, this);
  ...
  static async destroy(options) {
    ...
    if (options.individualHooks) {
      ...
      instances = await Promise.all(
        found.map(async (instance) => {
          await this.runHooks('beforeDestroy', instance, options);
          return instance;
        })
      );
    }
    ...
    if (this._timestampAttributes.deletedAt && !options.force) {
      ...
      where[field] = Object.hasOwn(deletedAtAttribute, 'defaultValue') ? deletedAtAttribute.defaultValue : null;

      attrValueHash[field] = Utils.now();
  ...
  static async restore(options) {
    ...
    if (options.individualHooks) {
      ...
      instances = await Promise.all(
        found.map(async (instance) => {
          await this.runHooks('beforeRestore', instance, options);
          return instance;
        })
      );
    }
    ...
    const deletedAtDefaultValue = Object.hasOwn(deletedAtAttribute, 'defaultValue')
      ? deletedAtAttribute.defaultValue
      : null;
    ...
    // Run afterRestore hook on each record individually
    if (options.individualHooks) {
   ...
   * @param  {Boolean|Array<String>} [options.returning=false] Return the affected rows. `true` returns the model's own columns; pass an array of attribute names to return only those
   * @param  {Boolean}      [options.raw=false]             Return the affected rows as plain objects instead of instances, skipping getters and virtual attributes. Has no effect without `options.returning`
  ...
  static async update(values, options) {
    ...
    if (options.fields && options.fields instanceof Array) {
      ...
      const updatedAtAttr = this._timestampAttributes.updatedAt;
      const tableAttributeNames = Object.keys(this.tableAttributes);
      options.fields = Object.keys(values).filter((key) => tableAttributeNames.includes(key));
    ...
    if (this._timestampAttributes.updatedAt && !options.silent) {
      values[this._timestampAttributes.updatedAt] =
        this._getDefaultTimestamp(this._timestampAttributes.updatedAt) || Utils.now();
    }
    ...
    if (options.validate) {
      ...
      if (options.sideEffects) {
        ...
        options.fields = [...new Set([...options.fields, ...Object.keys(values)])];
      }

      // We want to skip validations for all other fields
      const valueKeys = Object.keys(values);
      options.skip = Object.keys(this.rawAttributes).filter((key) => !valueKeys.includes(key));
      ...
      if (!instances.length) {
        ...
        if (!different) {
          ...
          if (keys.length) {
            ...
            options.fields = [...new Set([...options.fields, ...keys])];
          }
        } else {
  ...
  static describe(schema, options) {
    return this.QueryInterface.describeTable(
      this.tableName,
      Object.assign({ schema: schema || this._schema || undefined }, options)
    );
  }
        ...
        if (
          ...
          !options.include.some((item) => {
            ...
            if (scopeInclude.as) {
              ...
              const association =
                scopeInclude.association || this.getAssociationForAlias(scopeInclude.model, scopeInclude.as);
  ...
  static async increment(fields, options) {
    ...
    if (!options.silent && updatedAtAttr && !values[updatedAtAttr]) {
      options.attributes[updatedAtAttribute.field || updatedAtAttr] =
        this._getDefaultTimestamp(updatedAtAttr) || Utils.now();
    }
    if (versionAttr) {
  ...
  */
  ...
  _initValues(values, options) {
    ...
    if (options.isNewRecord) {
      ...
      if (this.constructor.primaryKeyAttribute && !Object.hasOwn(defaults, this.constructor.primaryKeyAttribute)) {
  ...
  get(key, options) {
    ...
    if (key) {
      if (Object.hasOwn(this._customGetters, key) && !options.raw) {
    ...
    if (this._hasCustomGetters || (options.plain && this._options.include) || options.clone) {
      ...
      if (this._hasCustomGetters) {
        for (_key in this._customGetters) {
          if (Object.hasOwn(this._customGetters, _key)) {
      ...
      for (_key in this.dataValues) {
        if (!Object.hasOwn(values, _key) && Object.hasOwn(this.dataValues, _key)) {
          values[_key] = this.get(_key, options);
        }
      }
      return values;
    ...
    if (typeof key === 'object' && key !== null) {
      ...
      if (!options.raw && this._customSetters[key]) {
        ...
        if (this._options && this._options.include && this._options.includeNames.indexOf(key) !== -1) {
          ...
          if (!options.raw) {
            ...
            if (!this._isAttribute(key)) {
              if (key.indexOf('.') > -1 && this.constructor._isJsonAttribute(key.split('.')[0])) {
                const previousNestedValue = Dot.get(this.dataValues, key);
                if (!_.isEqual(previousNestedValue, value)) {
                  Dot.set(this.dataValues, key, value);
          ...
          if (!(value instanceof Utils.SequelizeMethod) && Object.hasOwn(this.constructor._dataTypeSanitizers, key)) {
          ...
          if (
            ...
            (value instanceof Utils.SequelizeMethod ||
              ...
              // Check default
              (!this.constructor._dataTypeChanges[key] &&
                ((!Utils.isPrimitive(value) && value !== null) || value !== originalValue)))
          ) {
            this._previousDataValues[key] = originalValue;
            this.changed(key, true);
          }
  ...
  changed(key, value) {
    ...
    const changed = Object.keys(this.dataValues).filter((k) => this.changed(k));
  ...
  previous(key) {
    ...
    return _.pickBy(this._previousDataValues, (value, k) => this.changed(k));
  }
  ...
  _setInclude(key, value, options) {
    ...
    if (include.originalAttributes === undefined || include.originalAttributes.length) {
      if (association.isSingleAssociation) {
        ...
        value = !isEmpty && Object.assign(value, association.scope);
        this[accessor] = this.dataValues[accessor] = isEmpty ? null : include.model.build(value, childOptions);
      } else {
        isEmpty = value[0] && value[0][primaryKeyAttribute] === null;
        value = !isEmpty && value.map((v) => Object.assign(v, association.scope));
  ...
  async save(options, ...rest) {
    ...
    if (!options.fields) {
      if (this.isNewRecord) {
        ...
        const rawAttributeNames = Object.keys(this.constructor.rawAttributes);
        options.fields = (this.changed() || []).filter((field) => rawAttributeNames.includes(field));
      }
    ...
    Utils.mapReturningFieldNames(options, this.constructor);
    ...
    if (options.hooks) {
      ...
      let ignoreChanged = (this.changed() || []).filter((field) => !options.fields.includes(field)); // In case of update where it's only supposed to update the passed values and the hook values
      let hookChanged;
      let afterHookValues;

      if (updatedAtAttr && options.fields.indexOf(updatedAtAttr) !== -1) {
        ignoreChanged = ignoreChanged.filter((field) => field !== updatedAtAttr);
      }
      ...
      if (options.defaultFields && !this.isNewRecord) {
        afterHookValues = _.pick(
          this.dataValues,
          (this.changed() || []).filter((field) => !ignoreChanged.includes(field))
        );
        ...
        options.fields = [...new Set(options.fields.concat(hookChanged))];
      }
      ...
      if (hookChanged) {
        if (options.validate) {
          ...
          options.skip = Object.keys(this.constructor.rawAttributes).filter((key) => !hookChanged.includes(key));
          await this.validate(options);
          delete options.skip;
        }
      }
      ...
      await Promise.all(
        this._options.include
          ...
          .map(async (include) => {
            ...
            return this[include.association.accessors.set](instance, { save: false, logging: options.logging });
          })
      );
    }
    ...
    const results = await this.constructor.QueryInterface[query].apply(this.constructor.QueryInterface, args);
            ...
            return Promise.all(
              instances.map(async (instance) => {
                if (include.association instanceof BelongsToMany) {
                  await instance.save(includeOptions);
                  const _values = {};
                  _values[include.association.foreignKey] = this.get(this.constructor.primaryKeyAttribute, {
                    raw: true
                  });
                  _values[include.association.otherKey] = instance.get(instance.constructor.primaryKeyAttribute, {
                    raw: true
                  });
                  // Include values defined in the scope of the association
                  Object.assign(_values, include.association.through.scope);
  ...
  update(values, options) {
    ...
    const sideEffects = (this.changed() || []).filter((field) => !changedBefore.includes(field));
    const fields = [...new Set([...Object.keys(values), ...sideEffects])];

    if (!options.fields) {
      const changed = this.changed() || [];
      options.fields = fields.filter((field) => changed.includes(field));
      options.defaultFields = options.fields;
    }
  ...
  async destroy(options) {
    ...
    if (this.constructor._timestampAttributes.deletedAt && options.force === false) {
      ...
      where[field] = Object.hasOwn(attribute, 'defaultValue') ? attribute.defaultValue : null;

      this.setDataValue(field, values[field]);

      const results = await this.constructor.QueryInterface.update(
        this,
      ...
      result = await this.constructor.QueryInterface.delete(
        ...
        this.constructor.getTableName(options),
        where,
        Object.assign({ type: QueryTypes.DELETE, limit: null }, options)
      );
    }
  ...
  isSoftDeleted() {
    ...
    const defaultValue = Object.hasOwn(deletedAtAttribute, 'defaultValue') ? deletedAtAttribute.defaultValue : null;
    ...
    const deletedAtIsInTheFuture = new Date(deletedAt) > new Date();

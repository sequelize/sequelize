...
StackTracey.isThirdParty.include(path => path.includes('internal') || path.includes('node_modules'));
function buildStackTraceComment() {
  const stackData = new StackTracey();
  const stackItems = stackData.withSources.clean;

  let stackTraceComment = '';
  for (let i = 0; i < Math.min(6, stackItems.length - 1); ++i) {
    const stackItem = stackItems[i];
    if (stackItem.beforeParse.search('node_modules') !== -1) continue;
    stackTraceComment += `-- ${stackItem.beforeParse} ${stackItem.sourceLine ? stackItem.sourceLine.trim() : ''} \n`;
  }
  return stackTraceComment;
}
...
class Model {
  ...
  static refreshAttributes() {
    ...
    _.each(this.rawAttributes, (definition, name) => {
      ...
      if (definition.type._sanitize && !definition.doNotSanitize) {
  ...
  static findAll(options) {
    ...
    const stackTraceComment = buildStackTraceComment();
    _.defaults(options, { hooks: true, rejectOnEmpty: this.options.rejectOnEmpty, stackTraceComment });
    ...
    return Promise.try(() => {
      ...
      if (options.hooks) {
        return this.runHooks('afterFindSeparate', results, options);
      }
    });
  }
    ...
    return this.sequelize.transaction(options).then(t => {
      ...
      return this.create(values, options).then(instance => {
        if (instance.get(this.primaryKeyAttribute, { raw: true }) === null) {
          ...
          throw new this.sequelize.UniqueConstraintError({ fields: [this.primaryKeyAttribute] });
        }
  ...
  */
  ...
  setDataValue(key, value) {
    ...
    if (!(value instanceof Utils.SequelizeMethod) && this.constructor._dataTypeSanitizers[key]) {
      value = this.constructor._dataTypeSanitizers[key].call(this, value);
    }

    this.changed(key, this._isNewValue(key, value));
  ...
  _isNewValue(key, value) {
    let originalValue = this._previousDataValues[key];
    ...
    return (
        ...
        !(value instanceof Utils.SequelizeMethod) && this.constructor._dataTypeChanges[key] && this.constructor._dataTypeChanges[key].call(this, value, originalValue) ||
        // Check default
  ...
  reload(options) {
    ...
    return this.constructor.findOne(options)
      ...
      // update the internal options of the instance
        ...
        _.forIn(reload.dataValues, (value, key) => this.changed(key, false));
        return this;
      });
  }

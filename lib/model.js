...
class Model {
  ...
  static async sync(options) {
    ...
    if (options.hooks) {
      await this.hooks.run('beforeSync', options);
    }
    if (options.force) {
    ...
    if (options.hooks) {
      await this.hooks.run('afterSync', options);
    }
    return this;
  }
  ...
  static async findAll(options) {
    ...
    if (options.hooks) {
      await this.hooks.run('beforeFind', options);
    }
    ...
    if (options.hooks) {
      await this.hooks.run('beforeFindAfterExpandIncludeAll', options);
    }
    ...
    if (options.hooks) {
      await this.hooks.run('beforeFindAfterOptions', options);
    }
    ...
    if (options.hooks) {
      await this.hooks.run('afterFind', results, options);
    }
  ...
  static async count(options) {
    ...
    options.raw = true;
    if (options.hooks) {
      await this.hooks.run('beforeCount', options);
    }
    let col = options.col || '*';
    if (options.include) {
  ...
  static async upsert(values, options) {
    ...
    if (options.hooks) {
      await this.hooks.run('beforeUpsert', values, options);
    }
    ...
    if (options.hooks) {
      await this.hooks.run('afterUpsert', result, options);
      return result;
    }
    return result;
  }
  ...
  static async bulkCreate(records, options = {}) {
    ...
    const recursiveBulkCreate = async (instances, options) => {
      ...
      // Run before hook
      if (options.hooks) {
        await model.hooks.run('beforeBulkCreate', instances, options);
      }
      // Validate
      if (options.validate) {
        const errors = [];
      ...
      // Run after hook
      if (options.hooks) {
        await model.hooks.run('afterBulkCreate', instances, options);
      }
  ...
  static async destroy(options) {
    ...
    // Run before hook
    if (options.hooks) {
      await this.hooks.run('beforeBulkDestroy', options);
    }
    let instances;
    ...
    if (options.individualHooks) {
      ...
      await Promise.all(instances.map(instance => this.hooks.run('beforeDestroy', instance, options)));
    }
    let result;
    ...
    if (options.individualHooks) {
      await Promise.all(
        instances.map(instance => this.hooks.run('afterDestroy', instance, options))
      );
    }
    // Run after hook
    if (options.hooks) {
      await this.hooks.run('afterBulkDestroy', options);
    }
    return result;
  }
  ...
  static async restore(options) {
    ...
    // Run before hook
    if (options.hooks) {
      await this.hooks.run('beforeBulkRestore', options);
    }
    ...
    if (options.individualHooks) {
      ...
      await Promise.all(instances.map(instance => this.hooks.run('beforeRestore', instance, options)));
    }
    // Run undelete query
    const attrValueHash = {};
    ...
    if (options.individualHooks) {
      await Promise.all(
        instances.map(instance => this.hooks.run('afterRestore', instance, options))
      );
    }
    // Run after hook
    if (options.hooks) {
      await this.hooks.run('afterBulkRestore', options);
    }
    return result;
  }
  ...
  static async update(values, options) {
    ...
    if (options.hooks) {
      ...
      await this.hooks.run('beforeBulkUpdate', options);
    ...
    if (options.individualHooks) {
      ...
      if (instances.length) {
        ...
        instances = await Promise.all(instances.map(async instance => {
          ...
          await this.hooks.run('beforeUpdate', instance, options);
          if (!different) {
    ...
    if (options.individualHooks) {
      await Promise.all(instances.map(instance => this.hooks.run('afterUpdate', instance, options)));
      result[1] = instances;
    }
    // Run after hook
    if (options.hooks) {
      options.attributes = values;
      await this.hooks.run('afterBulkUpdate', options);
      delete options.attributes;
    }
    return result;
  }
  ...
  async save(options) {
    ...
    if (options.hooks) {
      ...
      await this.constructor.hooks.run(`before${hook}`, this, options);
    ...
    // Run after hook
    if (options.hooks) {
      await this.constructor.hooks.run(`after${hook}`, result, options);
    }
  ...
  */
  ...
  async destroy(options) {
    ...
    // Run before hook
    if (options.hooks) {
      await this.constructor.hooks.run('beforeDestroy', this, options);
    }
    ...
    // Run after hook
    if (options.hooks) {
      await this.constructor.hooks.run('afterDestroy', this, options);
    }
    return result;
  }
  ...
  async restore(options) {
    ...
    // Run before hook
    if (options.hooks) {
      await this.constructor.hooks.run('beforeRestore', this, options);
    }
    ...
    // Run after hook
    if (options.hooks) {
      await this.constructor.hooks.run('afterRestore', this, options);
      return result;
    }
    return result;
  }

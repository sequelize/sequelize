...
class Model {
  ...
  static init(attributes, options) {
    ...
    for (const [validatorType, validator] of Object.entries(options.validate || {})) {
      if (_.keys(attributes).includes(validatorType)) {
        throw new Error(
  ...
  static async sync(options) {
    ...
    indexes = _.filter(this.options.indexes, (item1) => !indexes.some((item2) => item1.name === item2.name));
  ...
  static async findOrCreate(options, ...rest) {
    ...
    try {
      ...
      try {
        ...
        const flattenedWhereKeys = _.keys(flattenedWhere).map((name) => name.split('.').pop());

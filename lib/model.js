  ...
  Model.prototype.bulkCreate = function(records, fieldsOrOptions, options) {
    ...
    records = records.map(function(values) {
      ...
      options.fields.forEach(function(field) {
        valuesFiltered[field] = values[field]
      })
    ...
    return Promise.resolve().then(function() {
      ...
      var runQuery = function(records) {
        return self.QueryInterface.bulkUpdate(self.getTableName(), attrValueHash, where, options, self.rawAttributes).then(function(results) {
          if (options.hooks !== true || !records || !records.length) {
            return results
          }

          return Promise.map(records, function(record) {
            return self.runHooks(self.options.hooks.afterUpdate, record).then(function(newValues) {
              if (!!newValues && newValues.dataValues) {
                record.dataValues = newValues.dataValues
              }
            })
          }).return(results)
        })
      ...
      return self.all({where: where}).then(function(records) {
        ...
        return Promise.map(records, function(record) {
          // record updates in record's dataValues
          Utils._.forIn(attrValueHash, function(value, attr) {
            record.dataValues[attr] = value
          });

          return self.runHooks(self.options.hooks.beforeUpdate, record).spread(function(newValues) {
            if (newValues && newValues.dataValues) {
              record.dataValues = newValues.dataValues

              if (!different) {
                var thisChangedValues = {};
                Utils._.forIn(record.dataValues, function(newValue, attr) {
                  if (newValue !== record._previousDataValues[attr]) {
          ...
          if (!different) {
            ...
            if (changedValues) {
              // hooks change values - record changes in attrValueHash so runQuery executes them
              attrValueHash = changedValues
            }
            ...
            return Promise.map(records, function(record) {
              return record.save({transaction: options.transaction, hooks: false}).then(function(result) {
                return self.runHooks('afterUpdate', result).return(result)
              })
            })
          }
        })

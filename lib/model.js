...
class Model {
   ...
   * @param  {Boolean}                   [options.hooks=true] Run before and after findAll hooks?
   *
   * @see    {Sequelize#query}
   ...
   * @param {Boolean}         [options.hooks=true] Run before and after aggregate hooks?
   * @param {Function}        [options.logging=false] A function that gets executed while running the query to log the sql.
   * @param {Boolean}         [options.benchmark=false] Pass query execution time in milliseconds as second argument to logging function (options.logging).
  ...
  static aggregate(attribute, aggregateFunction, options) {
    ...
    return Promise.try(() => {
      if (options.hooks) {
        return this.runHooks('beforeAggregate', values, options);
      }
    }).then(() => (
      this.QueryInterface.rawSelect(this.getTableName(options), options, aggregateFunction, this);
    )).tap(result => {
      if (options.hooks) {
        return this.runHooks('afterAggregate', result, options);
      }
    });
  }

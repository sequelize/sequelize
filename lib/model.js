...
class Model {
  ...
  static refreshAttributes() {
    ...
    Utils._.each(['get', 'set'], type => {
      ...
      // Use the getters and setters defined in the model options
      ...
      // Use the getters and setters defined in model attributes
    ...
    this._dataTypeComparators = {};
    this._dataTypeGetters = {};
    this._dataTypeSetters = {};
    ...
    _.each(this.rawAttributes, (definition, name) => {
      ...
      ['get', 'set'].forEach(type => {
        const _dataType = type === 'get' ? this._dataTypeGetters : this._dataTypeSetters;

        if (definition.type[type]) {
          _dataType[name] = definition.type[type];
        }
      });

      if (definition.type.compare) {
        this._dataTypeComparators[name] = definition.type.compare;
      }
    ...
    this._hasDataTypeComparators = !Utils._.isEmpty(this._dataTypeComparators);
    this._hasDataTypeGetters = !Utils._.isEmpty(this._dataTypeGetters);
    this._hasDataTypeSetters = !Utils._.isEmpty(this._dataTypeSetters);
  ...
  */
  ...
  set(key, value, options) { // testhint options:none
    ...
    if (typeof key === 'object' && key !== null) {
      ...
      // If there's a data type setter
      if (
        (this.isNewRecord || this.constructor._hasReadOnlyAttributes && !this.constructor._isReadOnlyAttribute(key)) &&
        this.constructor._hasDataTypeSetters && this.constructor._dataTypeSetters[key]
      ) {
        this.dataValues[key] = this.constructor._dataTypeSetters[key].call(this, value, key);
      }
      ...
      if (!options.raw && this._customSetters[key]) {
        ...
        if (
          // this.constructor._hasDataTypeComparators && this.constructor._dataTypeComparators[key] && this.constructor._dataTypeComparators[key].call(this, value, originalValue, key, options) ||
          this.rawAttributes[key].type.hasOwnProperty('compare') && this.rawAttributes[key].type.compare.call(this, value, originalValue, key, options) ||
  ...
  //     // If there's a data type setter
  //     if (this.constructor._hasDataTypeSetters && this.constructor._dataTypeSetters[key]) {
  //       this.dataValues[key] = this.constructor._dataTypeSetters[key].call(this, value, key);
  //     }
  ...
  //         this._setInclude(key, value, options);
  //         return this;
  //       } else if (this.constructor._hasDataTypeComparators && this.constructor._dataTypeComparators[key] && this.constructor._dataTypeComparators[key].call(this, value, originalValue, key, options)) {
  //         // if (key === 'createdAt' || key === 'created_at' || key === 'updatedAt' || key === 'updated_at') { debug('%O data type compare', key); }
  //         if (!Utils.isPrimitive(value) && value !== null || value !== originalValue) {
  //           // if (key === 'createdAt' || key === 'created_at' || key === 'updatedAt' || key === 'updated_at') { debug('%O data type compare CHANGED!', key); }
  //           this._previousDataValues[key] = originalValue;
  //           this.changed(key, true);
  //         }

...
class Model {
  ...
  static _mergeOptions(...args) {
    _.mergeWith(...args, (objValue, srcValue, key) => {
  ...
  static scope(option) {
    ...
    for (const option of options) {
      ...
      if (scope) {
        this._mergeOptions(self._scope, scope);
  ...
  static _injectScope(options) {
    ...
    Utils.defaults(options, scope);
    Utils.defaults(options.where, scope.where);

    // Includes need special treatment
    if (scope.include) {
      options.include = _.concat(scope.include, options.include || []);
    }
    if (options.include) {
      this._uniqIncludes(options);
    }
  }

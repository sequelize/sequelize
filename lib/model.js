  ...
  static findOrCreate(options) {
    ...
    return this.sequelize.transaction(options).then(t => {
      ...
      return this.create(values, options).then(instance => {
        ...
        const flattenedWhere = _.map(_.keys(flat(options.where)), name => _.last(_.split(name, '.')));
        const whereFields = flattenedWhere.map(name => _.get(this.rawAttributes, `${name}.field`, name));

...
class Model {
    ...
    static refreshAttributes() {
        ...
        _.each(this.rawAttributes, (definition, name) => {
            ...
            if (Object.prototype.hasOwnProperty.call(definition, 'unique') &&
                definition.unique) {
                let idxName;
    ...
    static _injectDependentVirtualAttributes(attributes) {
        ...
        for (const attribute of attributes) {
            if (this._virtualAttributes.has(attribute) &&
    ...
    static async findOne(options) {
        ...
        if (options.limit === undefined) {
            ...
            if (!options.where ||
                !_.some(options.where, (value, key) => (key === this.primaryKeyAttribute ||
                        ...
                        for (const associationInstance of associated) {
                            if (associationInstance) {
                                if (!(include.association instanceof BelongsToMany)) {
                                    associationInstance.set(include.association.foreignKey, instance.get(include.association.sourceKey ||
                                        instance.constructor.primaryKeyAttribute, { raw: true }), { raw: true });
                    ...
                    if (include.association instanceof BelongsToMany) {
                        ...
                        for (const idx in createdAssociationInstances) {
                            ...
                            const values = {
                                ...
                                [include.association
                                    .otherKey]: associationInstance.get(associationInstance.constructor.primaryKeyAttribute, { raw: true }),
                                // Include values defined in the association
                                ...include.association.through.scope,
                            };
                            if (associationInstance[include.association.through.model.name]) {
                                for (const attr of Object.keys(include.association.through.model.rawAttributes)) {
                                    if (include.association.through.model.rawAttributes[attr]
                                        ._autoGenerated ||
                                    ...
                                    values[attr] =
                                        associationInstance[include.association.through.model.name][attr];
                                }
                            }
    ...
    static _optionsMustContainWhere(options) {
        ...
        assert(_.isPlainObject(options.where) ||
            Array.isArray(options.where) ||
            options.where instanceof Utils.SequelizeMethod, 'Expected plain object, array or sequelize method in the options.where parameter');
    }
    /**
    ...
    get(key, options) {
        ...
        options = options || {};
        if (key) {
            if (Object.prototype.hasOwnProperty.call(this._customGetters, key) &&
                !options.raw) {
    ...
    set(key, value, options) {
        ...
        if (!options.raw && this._customSetters[key]) {
            ...
            if (!options.raw) {
                ...
                if (this.constructor._hasPrimaryKeys &&
                    originalValue &&
    ...
    _setInclude(key, value, options) {
        ...
        let isEmpty;
        if (include.originalAttributes === undefined ||
    ...
    async save(options) {
        ...
        if (!this.changed() && !this.isNewRecord)
            return this;
        const versionFieldName = _.get(this.constructor.rawAttributes[versionAttr], 'field') ||
            versionAttr;
            ...
            await Promise.all(this._options.include
                ...
                await Promise.all(instances.map(async (instance) => {
                    if (include.association instanceof BelongsToMany) {
                        ...
                        instance.set(include.association.foreignKey, this.get(include.association.sourceKey ||
                            this.constructor.primaryKeyAttribute, { raw: true }), { raw: true });
    ...
    equals(other) {
        ...
        return this.constructor.primaryKeyAttributes.every((attribute) => this.get(attribute, { raw: true }) ===

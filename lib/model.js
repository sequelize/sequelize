...
class Model {
  ...
  */
  ...
  update(values, options) {
    ...
    // We want to keep track of the fields changed via side effects
    const pushed_changed = this._changed;
    this._changed = {};
    this.set(values, setOptions);

    // Now we need to figure out which fields were actually affected by the setter.
    const sideEffects = this.changed();
    this._changed = _.union(Object.keys(pushed_changed), Object.keys(this._changed)).reduce((acc, key) => {
      acc[key] = pushed_changed[key] || this._changed[key] || false;
      return acc;
    }, {});

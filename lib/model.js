...
class Model {
  ...
  static _conformOptions(options, self) {
    ...
    [
      ['cte', this._conformCTE.bind(this)],
      ['include', this._conformInclude.bind(this)]
    ].forEach(([prop, conformFunc]) => {
      if (options[prop]) {
        options[prop] = (Array.isArray(options[prop])) ? options[prop] : [options[prop]];

        if (options[prop].length > 0) {
          options[prop] = options[prop].map(propElement => conformFunc(propElement, self));
        } else {
          delete options[prop];
        }
      }

...
class Model {
   ...
   * @param  {Array}        [options.updateOnDuplicate]      Fields to update if row key already exists (on update of record with existing primary key). Only supported by MySQL, MariaDB & Postgres >= 9.5). Is undefined by default, which results in a SequelizeUniqueConstraintError. To update all fields, use `updateOnDuplicate=Object.keys(Model.attributes)` or `updateOnDuplicate=Object.keys(Model.rawAttributes).filter(attr => attr !== 'createdAt' && attr !== 'updatedAt' && attr !== 'deletedAt')`
   * @param  {Transaction}  [options.transaction]            Transaction to run query under
   * @param  {Function}     [options.logging=false]          A function that gets executed while running the query to log the sql.
   * @param  {boolean}      [options.benchmark=false]        Pass query execution time in milliseconds as second argument to logging function (options.logging).

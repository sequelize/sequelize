...
const {and, or, gt, eq} = require('./operators');
...
class Model {
   ...
   *       const {like, gte} = Sequelize.Operators;
   *
   *       return {
   *         where: {
   *           email: {
   ...
   * ```js
   * const {gt, lte, ne, in: opIn} = Sequelize.Operators;
   * Model.findAll({
   *   where: {
   *     attr1: {
   *       [gt]: 50
   ...
   * ```js
   * const {or, and, gt, lt} = Sequelize.Operators;
   * Model.findAll({
   *   where: {
   *     name: 'a project',
   *     [or]: [

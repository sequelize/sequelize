...
class Model {
  ...
  static aggregate(attribute, aggregateFunction, options) {
    ...
    if (typeof attribute === 'object' && attribute.length) {
      let isDataTypeSet = false;
      const attributeSelector = [];
      attribute.forEach(attr => {
        const attrOptions = this.rawAttributes[attr];
        const field = attrOptions && attrOptions.field || attr;
        let aggregateColumn = this.sequelize.col(field);

        const fieldSelector = `${aggregateFunction}_${field}`;
        options.attributes.push([this.sequelize.fn(aggregateFunction, aggregateColumn), fieldSelector]);
        attributeSelector.push(fieldSelector);
      ...
      return this.QueryInterface.rawSelect(this.getTableName(options), options, attributeSelector, this).then(res => {
        const result = {} 
        if (res) {
          Object.keys(res).forEach(key => {
            const fieldArr = key.split(`${aggregateFunction}_`);
            result[fieldArr[1]] = res[key];
          })
        }

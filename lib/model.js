...
class Model {
   ...
   * @param  {string}         [options.searchPath=DEFAULT]     An optional parameter to specify the schema search_path (Postgres only)
   * @param  {Array}          [options.upsertKeys]             Fields to use as the unique keys (on duplicate key update)? (only supported by MySQL, MariaDB, SQLite >= 3.24.0 & Postgres >= 9.5). By default, unique keys and/or primary keys are used.
   *
   * @returns {Promise<Array<Model>>}
   */
    ...
    const recursiveBulkCreate = async (instances, options) => {
      ...
      if (options.individualHooks) {
        ...
        if (options.updateOnDuplicate) {
          ...
          if (!options.upsertKeys || options.upsertKeys.length === 0) {
            const upsertKeys = [];
            ...
            // use custom field name if provided, otherwise use provided name
            options.upsertKeys = options.upsertKeys.map(x => model.tableAttributes[x] && model.tableAttributes[x].field || x);
          }
        }

...
class Model {
  ...
  /**
   * Iterate through included models and run the included models' hooks
   * if the `hooks: true` attribute is set within the `include` property.
   */
  static _runIncludedModelHooks(include, hook, result, options, path) {
    if (!include) return;
    if (!path) path = [];

    include.forEach(inclusion => {
      if (inclusion.hooks) {
        const _result = inclusion.model._getNestedResult(result, path.concat([inclusion.as]));
        inclusion.model.runHooks(hook, _result, options);
      }

      inclusion.model._runIncludedModelHooks(inclusion.include, hook, result, options, path.concat([inclusion.as]));
    });
  }

  /**
   * Fetch the nested included data to be passed into a hook.
   */
  static _getNestedResult(result, _path) {
    const path = _path;
    const prop = path.shift();

    if (prop) return this._getNestedResult(result[prop], path);
    return result;
  }
  ...
  static findAll(options) {
    ...
    return Promise.try(() => {
      ...
      // Iterate through included models and run hooks if specified
      if (Array.isArray(results)) {
        results.forEach(result => {
          this._runIncludedModelHooks(options.include, 'afterFind', result, options);
        });
      } else {
        this._runIncludedModelHooks(options.include, 'afterFind', results, options);
      }

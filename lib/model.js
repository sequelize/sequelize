...
class Model {
  ...
  static _conformOptions(options, self) {
    ...
    [
      { property: 'cte', method: this._conformCTE.bind(this)},
      { property: 'include', method: this._conformInclude.bind(this)}
    ].forEach(check => {
      if (options[check.property]) {
        options[check.property] = (Array.isArray(options[check.property])) ? options[check.property] : [options[check.property]];

        if (options[check.property].length > 0) {
          options[check.property] = options[check.property].map(propertyElement => check.method(propertyElement, self));
        } else {
          delete options[check.property];
        }
      }
    });
  ...
  static _conformCTE(cte, self) {
    // fill in missing model or fulfill `useModel` option
    if (self && _.isUndefined(cte.model) && (_.isUndefined(cte.useModel) || cte.useModel === true)) {
      cte.model = self;
    }

    // conform both properties 
    ['initial', 'recursive'].forEach(prop => {
      if (_.has(cte, prop) && _.has(cte[prop], 'include')) {
        // if include is not an array, wrap in an array
        if (!Array.isArray(cte[prop].include)) {
          cte[prop].include = [cte[prop].include];
        }

        if (!cte[prop].include.length) {
          delete cte[prop].include;
        } else {
          // convert all included elements to { model: Model } form
          cte[prop].include = cte[prop].include.map(include => this._conformInclude(include, self));
        }
      }
    });

    // conform recursive.next
    if (_.has(cte, 'recursive.next')) {
      if (_.isString(cte.recursive.next)) {
        cte.recursive.next = { model: self, as: cte.recursive.next };
      }
      cte.recursive.next = this._conformInclude(cte.recursive.next, self);
    }
  ...
  static _validateCTEElements(options) {
    options.cte = options.cte.map(cte => {
      if (_.has(cte, 'model')) {
        ...
        ['initial', 'recursive'].forEach(prop => {
          if (_.has(cte, `${prop}.include`)) {
            cte[prop].include = cte[prop].include.map(include => {
              include.parent = cte;
              include.required = true;

              this._validateIncludedElement.call(cte.model, include, {}, options);
        ...
        if (_.has(cte, 'recursive.next')) {
          ...
          this._validateIncludedElement.call(cte.model, cte.recursive.next, {}, options);
        }
      }
      return cte;
    });
      ...
      if (_.isPlainObject(option)) {
        if (option.method) {
          if (Array.isArray(option.method) && !!self.options.scopes[option.method[0]]) {
            ...
            // options will not be conformed here unless we do it ourselves
            this._conformOptions(scope, this);
          }
  ...
  static findAll(options) {
    ...
    return Promise.try(() => {
      ...
      if (options.cte) {
        this._validateCTEElements(options);
      }

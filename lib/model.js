...
class Model {
  ...
  static update(values, options) {
    ...
    return Promise.try(() => {
      ...
      // Run before hook
      if (options.hooks) {
        const beforeBulkUpdateOptions = Utils.cloneDeep();
        beforeBulkUpdateOptions.attributes = values;
        return this.runHooks('beforeBulkUpdate', beforeBulkUpdateOptions).then(() => {

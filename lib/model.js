...
module.exports = (function() {
  ...
  */
   ...
   * @param  {Boolean}      [options.fieldsOnDuplicateKey]   Fields to update if row key already exists (on duplicate key update)? (only supported by mysql)
   *
   * @return {Promise<Array<Instance>>}
   */
  Model.prototype.bulkCreate = function(records, fieldsOrOptions, options) {
    ...
    if (this.sequelize.options.dialect !== 'mysql' && options.fieldsOnDuplicateKey) {
      return Promise.reject(new Error('Only mysql supports the \'fieldsOnDuplicateKey\' option.'));
    }

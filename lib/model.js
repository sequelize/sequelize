...
var addDefaultAttributes = function() {
  ...
  ['createdAt', 'updatedAt', 'deletedAt'].forEach(function(attr) {
    if (self._timestampAttributes[attr]) {
      tail[self._timestampAttributes[attr]] = {
        type: DataTypes.DATE,
        allowNull: attr !== 'deletedAt' ? false : true,
        _autoGenerated: true
      };
    }
  });

  var existingAttributes = _.clone(self.rawAttributes);
  self.rawAttributes = {};

  _.each(head, function(value, attr) {
    self.rawAttributes[attr] = value;
  });

  _.each(existingAttributes, function(value, attr) {
    self.rawAttributes[attr] = value;
  });

  _.each(tail, function(value, attr) {
    if (_.isUndefined(self.rawAttributes[attr])) {
      self.rawAttributes[attr] = value;
    }
  });
  ...
  options.forEach(function(option) {
    ...
    if (!!scope) {
      _.assign(self.$scope, scope, function scopeCustomizer(objectValue, sourceValue, key) {
        ...
        if (key === 'include' && Array.isArray(objectValue) && Array.isArray(sourceValue)) {
 ...
 * @param {Boolean}  [options.paranoid=true] If true, only non-deleted records will be returned. If false, both deleted and non-deleted records will be returned. Only applies if `options.paranoid` is true for the model.
 * @param {Object}   [options.group] For creating complex counts. Will return multiple rows as needed. The result will be an array with one element
 * @param {Function} [options.logging=false] A function that gets executed while running the query to log the sql.
 *
 * @return {Promise<Integer|Array>}
 */
...
Model.prototype.upsert = function (values, options) {
  ...
  var createdAtAttr = this._timestampAttributes.createdAt
    ...
    , hadPrimary = values[this.primaryKeyField] || values[this.primaryKeyAttribute]
  ...
  return Promise.try(function() {
    ...
    if (options.individualHooks) {
      ...
      return Promise.map(instances, function(instance) {
        ...
        individualOptions.fields = undefined;
        individualOptions.ignoreDuplicates = undefined;
        individualOptions.individualHooks = undefined;
      ...
      var attributes = {};
      var keys = Object.keys(self.tableAttributes);

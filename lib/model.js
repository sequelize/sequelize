  ...
  static upsert(values, options) {
    ...
    return instance.validate(options).then(() => {
      ...
      if (createdAtAttr && !updateValues[createdAtAttr]) {
        const attribute = this.rawAttributes[createdAtAttr].field || createdAtAttr;
        insertValues[attribute] = this._getDefaultTimestamp(attribute) || now;
      }
      if (updatedAtAttr && !insertValues[updatedAtAttr]) {
        const attribute = this.rawAttributes[updatedAtAttr].field || updatedAtAttr;
        insertValues[attribute] = updateValues[attribute] = this._getDefaultTimestamp(attribute) || now;
      }

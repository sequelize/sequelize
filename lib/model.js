...
class Model {
   ...
   *
   * * SQLite: A TimeoutError will be thrown if one transaction inserts and another tries to select before the first one has committed.
   * * PostgreSQL: requires PostgreSQL 9.5+ to correctly handle multiple concurrent findOrCreate over the same row inside the same transaction. Insert makes use of `ON CONFLICT DO NOTHING` not to abort the transaction.
   *
  ...
  static findOrCreate(options) {
    ...
    return this.sequelize.transaction(options).then(t => {
      ...
      // Concurrent findOrCreate should not abort shared transaction in postgres
      if (!internalTransaction && this.sequelize.options.dialect === 'postgres') {
        const databaseVersion = _.get(this, 'sequelize.options.databaseVersion', 0);
        if (semver.gte(databaseVersion, '9.5.0')) {
          options.ignoreDuplicates = true;
        } else {
          logger.warn('findOrCreate may abort shared transactions unexpectedly on PostgreSQL 9.4 or lower');
        }
      }
      ...
      return this.create(values, options).then(instance => {
        ...
        if (err.fields) { 
          ...
          const whereFields = Object.keys(flattenedWhere).map(name => {
            const lastName = _.last(name.split('.'));
            return _.get(this.rawAttributes, `${lastName}.field`, lastName);
          });
          ...
          const badFieldKeys = Object.keys(err.fields);

          if (defaultFields) {
            if (!_.intersection(badFieldKeys, whereFields).length && _.intersection(badFieldKeys, defaultFields).length) {
              throw err;
            }
          }

          if (_.intersection(badFieldKeys, whereFields).length) {
        ...
        return this.findOne(Utils.defaults({
          ...
          // Sanity check, ideally we caught this at the defaultFields/err.fields check
   ...
   *
   * The operation is guaranteed to be idempotent. At least one of update or insert should succeed. Otherwise an error is thrown and the current transaction may be aborted.
   *
   * * MySQL and MariaDB - Implemented as a single query `INSERT values ON DUPLICATE KEY UPDATE values`
   * * PostgreSQL - First update, then `INSERT INTO (columns) SELECT values WHERE NOT EXISTS (update_outcome)`. If update succeeds, insert becomes empty and current transaction is not aborted.
   * * SQLite - Implemented in query interface. Run upsert and insert separatedly and "asynchronously". Wait for both, at least one must succeed.
   * * MSSQL - Implemented as a single query using `MERGE` and `WHEN (NOT) MATCHED THEN`
   * **Note** Postgres and SQLite do not use `ON CONFLICT DO UPDATE`. It cannot deal with multiple constraints.
   *

...
function conformOptions(options, self) {
  ...
  if (options.cte) {
    ...
    options.cte = options.cte.map(function (cte) {
      ...
      if (_.has(cte, 'initial.include')) {
        cte.initial.include = cte.initial.include.map(function (include) {
          return conformInclude(include, self);
        });
      }

      if (_.has(cte, 'recursive.include')) {
        cte.recursive.include = cte.recursive.include.map(function (include) {
          return conformInclude(include, self);
        });
      }
 ...
 * @param  {Array<String>}             [options.cteAttributes] An array of attributes from the cteSelect CTE to return along with the results. Will be accessible via `get`
 * 
 * @see    {Sequelize#query}

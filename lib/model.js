...
class Model {
   ...
   * @param  {String}     [options.searchPath=DEFAULT] An optional parameter to specify the schema search_path (Postgres only)
   * @param {string[]}    [options.where] An optional object with where conditions.
   * @param {Boolean}     [options.rejectOnNonAffected=true] If true, throws errors if no rows are affected (Only Postgres supported right now).
   *
    ...
    return Promise.try(() => {
      ...
      if (this.isNewRecord) {
        ...
        if (options.where) {
          ...
          if( options.rejectOnNonAffected === undefined ){
            options.rejectOnNonAffected = true;
          }
        }

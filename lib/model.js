...
class Model {
  ...
  */
  ...
  update(values, options) {
    ...
    const pushedChanged = this.changed();
    this._changed = {};
    this.set(values, setOptions);

    // Now we need to figure out which fields were actually affected by the setter.
    const sideEffects = this.changed();
    this._changed = _.union(Object.keys(pushedChanged), Object.keys(this._changed)).reduce((acc, key) => {
      acc[key] = pushedChanged[key] || this._changed[key] || false;
      return acc;
    }, {});

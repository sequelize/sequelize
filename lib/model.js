    ...
    return this.sequelize
      ...
      .then(instance => {
        ...
        return this.create(values, options)
          ...
          .catch(err => {
            if (!(err instanceof this.sequelize.UniqueConstraintError)) throw err;
      ...
      .then(() => {
        ...
        if (options.individualHooks) {
          return this.findAll({
            ...
            .then(found => Promise.all(found.map(instance => this.runHooks('beforeDestroy', instance, options).then(() => instance))))
            .then(_instances => {
              instances = _instances;
            });
        }
      ...
      .then(() => {
        ...
        if (options.individualHooks) {
          return this.findAll({
            ...
            .then(found => Promise.all(found.map(instance => this.runHooks('beforeRestore', instance, options).then(() => instance))))
            .then(_instances => {
              instances = _instances;
            });
        }
  ...
  save(options) {
    ...
    return tryFn(() => {
      ...
      .then(() => {
        ...
        return Promise.all(
          this._options.include.filter(include => include.association instanceof BelongsTo).map(include => {
            ...
            return instance
              .save(includeOptions)
              .then(() => this[include.association.accessors.set](instance, { save: false, logging: options.logging }));
          })
        );
      })
      .then(() => {
        ...
        return this.constructor.QueryInterface[query]
          ...
          .then(async result => {
            if (!wasNewRecord) return result;
            if (!this._options.include || !this._options.include.length) return result;

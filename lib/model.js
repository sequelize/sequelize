...
class Model {
  ...
  static _findSeparate(results, options) {
    ...
    // Filter attribute aliases with the same name
    const includes = options.attributes?
      options.include.filter(include => {
        return !options.attributes.some(attr => Array.isArray(attr) && attr[1] === include.as);
      }) :
      options.include
  ...
  set(key, value, options) {
    ...
    if (!options.raw && this._customSetters[key]) {
      ...
      if (this._options && this._options.include && this._options.includeNames.includes(key) && !(this._options.attributes && this._options.attributes.includes(key))) {

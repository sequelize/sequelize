...
module.exports = (function() {
   ...
   * @param  {Boolean}                   [options.subQuery] Toggle sub query logic for entire query, default is based on whether or not there any any row duplicating associations that also need to filter the main table
   ...
   * @param  {Boolean}                   [options.include[].duplicating] Whether or not this association will result in duplicated rows (cartesian products). Default will be determined by includes (true if any :M includes), can be set to false if absolutely certain only a single row will be returned for an association
   * @param  {Boolean}                   [options.include[].subQuery] Toggle sub query logic for this include, default is based on whether or not there any any row duplicating associations that also need to filter the main table
  ...
  */
  ...
  var conformOptions = function(options, mainModel) {
    if (!options.include) {
      return options;
    }
    ...
    options.include = options.include.map(function(include) {
      ...
      if (include instanceof Association) {
        if (include.target.name === mainModel.name) {
        ...
        if (include.association) {
          if (include.association.target.name === mainModel.name) {
  ...
  var validateIncludedElements = function(options, tableNames) {
    options = Model.$conformOptions(options);

    tableNames = tableNames || {};
    options.includeNames = [];
    options.includeMap = {};
    
    if (options.duplicating === undefined) {
    ...
    options.include.map(function (include) {
      ...
      validateIncludedElement.call(this, include, tableNames);
    ...
    options.include.map(function (include) {
      ...
      if (include.subQuery !== false && options.hasDuplicating && options.topLimit) {
        ...
        if (include.association.isSingleAssociation) {
          include.subQueryFull = include.hasRequired;
        }
      } else {
        include.subQueryFilter = include.subQueryFull = include.subQuery || false;
      }
  ...
  Model.$validateIncludedElement = validateIncludedElement; 

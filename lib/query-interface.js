  ...
  dropAllSchemas(options) {
    ...
    if (!this.QueryGenerator._dialect.supports.schemas) {
      ...
      return this.showAllSchemas(options).then(schemaNames =>
        Promise.all(schemaNames.map(schemaName => this.dropSchema(schemaName, options)))
      );
    }
  }
  ...
  delete(instance, tableName, identifier, options) {
    ...
    return pEach(cascades, cascade => {
      return instance[cascade](options).then(instances => {
        ...
        return pEach(instances, instance => instance.destroy(options));
      });
    }).then(() => {
      options.instance = instance;
      return this.sequelize.query(sql, options);

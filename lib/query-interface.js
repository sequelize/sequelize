...
class QueryInterface {
        ...
        if (instanceTable) {
          ...
          for (let i = 0; i < keyLen; i++) {
            if (instanceTable.rawAttributes[keys[i]].type instanceof DataTypes.ENUM) {
              sql = this.QueryGenerator.pgEnumDrop(getTableName, keys[i], options);
  ...
  dropEnum(enumName, options) {
    ...
    return this.sequelize.query(
      this.QueryGenerator.pgEnumDrop(null, null, this.QueryGenerator.pgEscapeAndQuote(enumName), options),
      { ...options, raw: true }
    );
  }
  ...
  dropAllEnums(options) {
    ...
    return this.pgListEnums(null, options).map(result => this.sequelize.query(
      this.QueryGenerator.pgEnumDrop(null, null, this.QueryGenerator.pgEscapeAndQuote(result.enum_name), options),
      { ...options, raw: true }
    ));
  }
  ...
  addIndex(tableName, attributes, options, rawTablename) {
    ...
    return this.sequelize.query(sql, { ...options, supportsSearchPath: false });
  }
  ...
  showIndex(tableName, options) {
    ...
    return this.sequelize.query(sql, { ...options, type: QueryTypes.SHOWINDEXES });
  }
  ...
  showConstraint(tableName, constraintName, options) {
    ...
    return this.sequelize.query(sql, { ...options, type: QueryTypes.SHOWCONSTRAINTS });
  }

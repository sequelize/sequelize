...
class QueryInterface {
  ...
  async dropAllSchemas(options) {
    ...
    if (!this.QueryGenerator._dialect.supports.schemas) {
      return await this.sequelize.drop(options);
    }
  ...
  async dropAllTables(options) {
    ...
    for (const q of queries) {
      await this.sequelize.query(q, options);
    }
    await dropAllTables(tableNames);
  }
  ...
  async pgListEnums(tableName, options) {
    ...
    return await this.sequelize.query(sql, Object.assign({}, options, { plain: false, raw: true, type: QueryTypes.SELECT }));
  }
  ...
  async describeTable(tableName, options) {
    ...
    let data;
    try {
      data = await this.sequelize.query(sql, options);
    } catch (e) {
  ...
  async changeColumn(tableName, attributeName, dataTypeOrOptions, options) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      return await SQLiteQueryInterface.changeColumn(this, tableName, attributes, options);
    }
  ...
  async renameColumn(tableName, attrNameBefore, attrNameAfter, options) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      return await SQLiteQueryInterface.renameColumn(this, tableName, attrNameBefore, attrNameAfter, options);
    }

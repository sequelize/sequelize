            ...
            if (!results[enumIdx]) {
              ...
              vals.forEach((value, idx) => {
                ...
                if (enumVals.indexOf(value) === -1) {
                  ...
                  promises.push(this.sequelize.query(this.QueryGenerator.pgEnumAdd(tableName, attributes[keys[i]].field || keys[i], value, valueOptions), valueOptions));
                }
              });
              enumIdx++;
            }
        ...
        if (instanceTable) {
          ...
          for (let i = 0; i < keyLen; i++) {
            if (instanceTable.rawAttributes[keys[i]].type instanceof DataTypes.ENUM) {
              sql = this.QueryGenerator.pgEnumDrop(getTableName, instanceTable.rawAttributes[keys[i]].field || keys[i]);

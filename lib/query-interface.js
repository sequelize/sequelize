...
QueryInterface.prototype.createTable = function(tableName, attributes, options, model) {
  ...
  if (self.sequelize.options.dialect === 'postgres') {
    ...
    if (self.sequelize.options.dialect === 'oracle') {
      ...
      if (autoincattr) {
        self.sequelize.query(self.QueryGenerator.dropSequence(tableName, autoincattr));
        self.sequelize.query(self.QueryGenerator.createSequence(tableName, autoincattr, 1));

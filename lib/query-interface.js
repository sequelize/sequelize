    ...
    return this.sequelize.query(sql, options).then(function(data) {
      ...
      if (options && options.dataType) {
        ...
        if (
          ...
          dataType instanceof DataTypes.REAL
          // Attention: Don't convert decimals, as the precision would be lost!
        ) {
            result = +result;
        } else if (dataType instanceof DataTypes.BIGINT && +result !== Infinity) {
          // Convert the result string to a number if it fits into a js number (up to 308 digits)
            result = +result;
        } else if (dataType instanceof DataTypes.DATE && !_.isDate(result)) {
          result = new Date(result);
        }
        // Else: Nothing to do, result is already a string.
      }

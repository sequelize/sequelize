...
class QueryInterface {
  ...
  async dropAllSchemas(options) {
    ...
    const schemaNames = await this.showAllSchemas(options);
  ...
  async showAllSchemas(options) {
    ...
    return schemaNames.map((value) => (value.schema_name ? value.schema_name : value)).flat();
  }
    ...
    if (this.sequelize.options.dialect === 'postgres') {
      ...
      for (i = 0; i < keyLen; i++) {
        ...
        if (
          ...
          enumPromises.push(
            this.sequelize.query(sql, Object.assign({}, options, { plain: true, raw: true, type: QueryTypes.SELECT }))
          );
        }
      }
  ...
  async dropAllEnums(options) {
    ...
    const results = await this.pgListEnums(null, options);
    return Promise.all(
      results.map((result) =>
        this.sequelize.query(
  ...
  async describeTable(tableName, options) {
    ...
    const data = await this.sequelize.query(sql, Object.assign({}, options, { type: QueryTypes.DESCRIBE }));
  ...
  async getForeignKeysForTables(tableNames, options) {
    ...
    tableNames.forEach((tableName, i) => {
      ...
      result[tableName] = result[tableName].filter(Boolean);
    });
  ...
  async upsert(tableName, insertValues, updateValues, where, model, options) {
    ...
    indexes = Object.values(model.options.uniqueKeys).map((value) => {
      return value.fields;
    });

    model.options.indexes.forEach((value) => {
      if (value.unique) {
  ...
  async delete(instance, tableName, identifier, options) {
    ...
    await pEach(cascades, async (cascade) => {
      ...
      return pEach(instances, (i) => i.destroy(options));
    });
  ...
  async rawSelect(tableName, options, attributeSelector, Model) {
    ...
    if (options && options.dataType) {
      ...
      if (dataType instanceof DataTypes.DECIMAL || dataType instanceof DataTypes.FLOAT) {
        ...
        if (result !== null && !_.isDate(result)) {
  ...
  startTransaction(transaction, options) {
    ...
    options = Object.assign({}, options, {
      transaction: transaction.parent || transaction,
      // Only the mssql driver needs the savepoint name. It used to be read off `options.transaction`,
      // which meant assigning it here overwrote the PARENT transaction's own name — so a savepoint that
      // opened a deeper savepoint could no longer name itself, and released/rolled back the wrong one.
      savepointName: transaction.parent ? transaction.name : undefined
    });
  ...
  commitTransaction(transaction, options) {
    ...
    // A savepoint commits by being released, which keeps the subtransaction stack from growing for the
    // life of the root transaction. Dialects with no equivalent (mssql) generate no query, so the
    // savepoint stays open until the root transaction ends — the previous behaviour everywhere.

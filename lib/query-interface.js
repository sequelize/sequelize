    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      ...
      if (self.sequelize.options.dialect === "postgres") {
        ...
        chainer.runSerially().success(function(results) {
          ...
          for (i = 0; i < keyLen; i++) {
            if (attributes[keys[i]].toString().match(/^ENUM\(/)) {
              var existingEnum = null;
              for (var x = 0; x < results.length; x++) 
                if (results[x] && results[x].enum_name == self.QueryGenerator.pgEscapeAndQuote("enum_" + tableName + "_" + keys[i])) existingEnum = results[x];
              ...
              } else if (!!existingEnum && !!daoTable) {
                var enumVals = self.QueryGenerator.fromArray(existingEnum.enum_value)

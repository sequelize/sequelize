...
class QueryInterface {
  ...
  upsert(tableName, insertValues, updateValues, where, model, options) {
    ...
    // For database versions not supporting UPSERT do INSERT + UPDATE
    if (this.sequelize.options.dialect === 'sqlite') {
      options.ignoreDuplicates = true;
      const insert = this.QueryGenerator.insertQuery(tableName, insertValues, model.rawAttributes, options);
      const update = this.QueryGenerator.updateQuery(tableName, updateValues, where, options, model.rawAttributes);

      // Sqlite in reality is syncronous(server-less)
      return Promise.all([
        this.sequelize.query(update, Object.assign({}, options, { type: QueryTypes.UPDATE })),
        this.sequelize.query(insert, Object.assign({}, options, { type: QueryTypes.INSERT }))
      ]).then(([updateRes, insertRes]) => {

        if (insertRes && insertRes[1] || updateRes && updateRes[1]) {
          return [!!insertRes[1], undefined];
        }

        throw new Error('Upsert query did not update or insert anything');
      });
    }
  ...
  select(model, tableName, options) {
    ...
    // node-mysql2 .execute() API cannot process returned data types correctly
    // Do not bind parameters to use regular .query() API
    if (this.sequelize.options.dialect === 'mysql') {
      query = this.QueryGenerator.composeString(query);
    }

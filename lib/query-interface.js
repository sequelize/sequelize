...
QueryInterface.prototype.createTable = function(tableName, attributes, options, model) {
  ...
  if (self.sequelize.options.dialect === 'postgres') {
    ...
    for (i = 0; i < keyLen; i++) {
      var type = attributes[keys[i]].type;

      if (
        type instanceof DataTypes.ENUM ||
        (type instanceof DataTypes.ARRAY && type.type instanceof DataTypes.ENUM) //ARRAY sub type is ENUM
      ) {
        sql = self.QueryGenerator.pgListEnums(tableName, type.field || keys[i], options);
    ...
    return Promise.all(promises).then(function(results) {
      ...
      for (i = 0; i < keyLen; i++) {
        ...
        if (
          ...
          if (!results[enumIdx]) {
            sql = self.QueryGenerator.pgEnum(tableName, type.field || keys[i], enumType, options);

...
QueryInterface.prototype.createTable = function(tableName, attributes, options, model) {
  ...
  if (self.sequelize.options.dialect === 'postgres') {
    ...
    var result = self.sequelize.query(sql, options);
    if (self.sequelize.options.dialect === 'oracle') {
      var autoincattr = self.QueryGenerator.checkTableAutoinc(tableName, attributes);
      if (autoincattr) {
        self.sequelize.query(self.QueryGenerator.dropSequence(tableName + '_' + autoincattr + '_PK_SEQUENCE'));
        self.sequelize.query(self.QueryGenerator.createSequence(tableName  + '_' + autoincattr + '_PK_SEQUENCE', 1));
        self.sequelize.query(self.QueryGenerator.createPKTrigger(tableName, autoincattr));
      }
    }

...
/**
 * The interface that Sequelize uses to talk to the database
 *
...
export class QueryInterface {
  ...
  async getForeignKeyReferencesForTable(tableName, options) {
    ...
    // Postgres returns all field names lowercased, so camelize them back.
  ...
  async upsert(tableName, insertValues, updateValues, where, model, options) {
    ...
    for (const index of indexes) {
      if (attributes.filter((attribute) => index.includes(attribute)).length === index.length) {
        where = {};
  ...
  bulkUpdate(tableName, values, identifier, options, attributes) {
    ...
    const model = this.sequelize.modelManager.models.find((m) => m.tableName === table.tableName);

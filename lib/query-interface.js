  ...
  createTable(tableName, attributes, options, model) {
    ...
    if (this.sequelize.options.dialect === 'postgres') {
      ...
      return Promise.all(promises).then(results => {
        ...
        for (i = 0; i < keyLen; i++) {
          type = attributes[keys[i]].type;
          // Handle arrays of enum type.
          if (type instanceof DataTypes.ARRAY) type = type.type;

          const attr = attributes[keys[i]].field || keys[i];
          if (type instanceof DataTypes.ENUM) {
            // If the enum type doesn't exist then create it
            if (!results[enumIdx]) {
              sql = this.QueryGenerator.pgEnum(tableName, attr, type, options);
              promises.push(this.sequelize.query(
                sql,

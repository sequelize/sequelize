...
class QueryInterface {
  ...
  createTable(tableName, attributes, options, model) {
    ...
    const promise = this.sequelize.options.dialect === 'postgres' ?
      PostgresQueryInterface.ensureEnums(this, tableName, attributes, options, model) :
      Promise.resolve();

    const canAddSchema = !tableName.schema && (options.schema || !!model && model._schema);
    if (canAddSchema) {

...
const IBMiQueryInterface = require('./dialects/ibmi/query-interface');
...
class QueryInterface {
  ...
  startTransaction(transaction, options) {
    switch (this.sequelize.options.dialect) {
      case 'ibmi':
        return Promise.resolve(IBMiQueryInterface.startTransaction(transaction, options));
      default:
        if (!transaction || !(transaction instanceof Transaction)) {
  ...
  commitTransaction(transaction, options) {
    switch (this.sequelize.options.dialect) {
      case 'ibmi':
        return Promise.resolve(IBMiQueryInterface.commitTransaction(transaction, options));
      default:
        if (!transaction || !(transaction instanceof Transaction)) {
  ...
  rollbackTransaction(transaction, options) {
    switch (this.sequelize.options.dialect) {
      case 'ibmi':
        return Promise.resolve(IBMiQueryInterface.rollbackTransaction(transaction, options));
      default:
        if (!transaction || !(transaction instanceof Transaction)) {

...
module.exports = (function() {
  ...
  QueryInterface.prototype.dropAllSchemas = function() {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      var chainer = new Utils.QueryChainer(null, self.sequelize.options)
    ...
    }, self.sequelize.options.continuationLocationStorageNamespace).run()
  }
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      ...
      if (self.sequelize.options.dialect === "postgres") {
        ...
        chainer.runSerially().success(function(results) {
          var chainer2 = new Utils.QueryChainer(null, self.sequelize.options)
  ...
  QueryInterface.prototype.dropAllTables = function(options) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      return new Utils.CustomEventEmitter(function(dropAllTablesEmitter) {
        var events  = []
          , chainer = new Utils.QueryChainer(null, self.sequelize.options)
  ...
  QueryInterface.prototype.dropAllEnums = function(options) {
    ...
    var emitter = new Utils.CustomEventEmitter(null, self.sequelize.options.continuationLocationStorageNamespace)
      , chainer = new Utils.QueryChainer(null, self.sequelize.options)
  ...
  QueryInterface.prototype.removeColumn = function(tableName, attributeName) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      }.bind(this), self.sequelize.options.continuationLocationStorageNamespace).run()
    } else {
  ...
  var queryAndEmit = QueryInterface.prototype.queryAndEmit = function(sqlOrQueryParams, methodName, options, emitter) {
    ...
    if (!!emitter) {
      execQuery(emitter)
       ...
       return new Utils.CustomEventEmitter(execQuery,
           this.sequelize.options.continuationLocationStorageNamespace).run();
    }
  }

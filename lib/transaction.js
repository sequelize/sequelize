...
var Transaction = module.exports = function(sequelize, options) {
  ...
  // main propose is to allow chainable logic inside hooks
  this.promise = new Promise(function () {});

  // sugar promise methods into `transaction`
  if(this.options.sugarPromiseMethods) {
    ([ 'then', 'done', 'nodeify', 'finally', 'error', 'catch' ])
    .forEach(function (n) {
      self[n] = self.promise[n].bind( self.promise );
    });
  }
...
Transaction.prototype.commit = function() {
  ...
  return this
    ...
    .tap(
      function (val) { self.promise.seqResolve(val); },
      function (err) { self.promise.seqReject(err); }
    );
};
...
Transaction.prototype.rollback = function() {
  ...
  return this
    ...
    .tap(
      function () { self.promise.seqReject(new Error("transaction rollbacked")); },
      function (err) { self.promise.seqReject(err); }
    );
};

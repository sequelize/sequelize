...
var Transaction = module.exports = function(sequelize, options) {
  ...
  // `transaction.promise` implementation #2805
  // main purpose is to allow chainable logic inside hooks
  this.promise = new Promise(function () {});

  // sugar promise methods into `transaction`
  if(this.options.sugarPromiseMethods) {
    var methods = [ 'then', 'done', 'nodeify', 'finally', 'error', 'catch' ];

    for (var i = 0; i < methods.length; i++) {
      this[methods[i]] = this.promise[methods[i]].bind( this.promise );
    }
  }
};
...
Transaction.prototype.commit = function() {
  ...
  return this
    ...
    .tap(
      self.promise._resolveCallback,
      self.promise._rejectCallback
    );
};
...
Transaction.prototype.rollback = function() {
  ...
  return this
    ...
    .tap(
      function () { self.promise._rejectCallback(new Error("transaction rolled back")); },
      self.promise._rejectCallback
    );
};

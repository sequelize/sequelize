...
class Transaction {
  ...
  async rollback() {
    ...
    if (!this.connection) {
      throw new Error(new Error('Transaction cannot be rolled back because it never started'));
    }
  ...
  async cleanup() {
    const res = await this.sequelize.connectionManager.releaseConnection(this.connection);

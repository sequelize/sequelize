...
class Transaction {
  ...
  /**
   * A hook that is run after a transaction is rolled back
   *
   * @param {Function} fn   A callback function that is called with the rolled back transaction
   * @name afterRollback
   * @memberof Sequelize.Transaction
   */
  afterRollback(fn) {
    ...
    this._afterRollbackHooks.push(fn);
  }

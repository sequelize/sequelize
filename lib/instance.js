...
module.exports = (function() {
  ...
  Instance.prototype.save = function(fieldsOrOptions, options) {
    ...
    var self = this
      ...
      , validAttr = Object.keys(this.Model.attributes);

    // Remove fields to skip unknown ones
    options.fields.forEach(function ( attr ) {
      if( validAttr.indexOf( attr ) === -1 )
        options.fields.splice( options.fields.indexOf( attr ), 1 );
    });
    ...
    return Promise.try(function() {
      ...
      return Promise.try(function() {
        ...
        return self.QueryInterface[query].apply(self.QueryInterface, args).catch(self.sequelize.UniqueConstraintError, function(err) {
          ...
          // And pass only the saved ones into _previousDataValues
          options.fields.forEach(function(attr) {
            result._previousDataValues[ attr ] = result.dataValues[ attr ];
          });
  ...
  */
  ...
  Instance.prototype.updateAttributes = function(updates, options) {
    options = typeof options instanceof Array && { fields: options } || options || {};

    options.fields = options.fields || Object.keys( updates );

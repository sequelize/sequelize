...
var initValues = function(values, options) {
  ...
  if (options.isNewRecord) {
    ...
    ['createdAt', 'updatedAt', 'deletedAt'].forEach(function(attr) {
      if (self.Model._timestampAttributes[attr] && defaults[self.Model._timestampAttributes[attr]]) {
        self.dataValues[self.Model._timestampAttributes[attr]] = Utils.toDefaultValue(defaults[self.Model._timestampAttributes[attr]]);
        delete defaults[self.Model._timestampAttributes[attr]];
      }
    });

    Object.keys(defaults).forEach(function(key) {
      if (values[key] === undefined) {
        self.set(key, Utils.toDefaultValue(defaults[key]), defaultsOptions);
      }
    });
  }
...
Instance.prototype.get = function(key, options) {
  ...
  if (key) {
    ...
    if (options.plain && this.options.include && this.options.includeNames.indexOf(key) !== -1) {
      ...
      if (this.dataValues[key] instanceof Instance) {
  ...
  if (this._hasCustomGetters || (options.plain && this.options.include) || (options && options.clone)) {
    var values = {}
      , _key
      , keys
      , i = 0;
    ...
    if (this._hasCustomGetters) {
      keys = Object.keys(this._customGetters);
      for (i = 0; i < keys.length; i++) {
        values[keys[i]] = this.get(keys[i], options);
      }
    }

    keys = Object.keys(this.dataValues);
    for (i = 0; i < keys.length; i++) {
      _key = keys[i];
...
Instance.prototype.set = function(key, value, options) {
  ...
  if (typeof key === 'object' && key !== null) {
    ...
    if (options.raw && !(this.options && this.options.include) && !(options.attributes) && !this.Model._hasBooleanAttributes && !this.Model._hasDateAttributes) {
    ...
    if (!options.raw && this._customSetters[key]) {
      ...
      if (!options.raw) {
        ...
        if (this.Model._hasDateAttributes && this.Model._isDateAttribute(key) && value && !(value instanceof Date) && !value._isSequelizeMethod) {
      ...
      if (this.Model._hasBooleanAttributes && this.Model._isBooleanAttribute(key) && value !== null && value !== undefined && !value._isSequelizeMethod) {
        ...
        if (_.isString(value) || _.isNumber(value)) {
          var tmp = ({ 'true': true, 'false': false, 1: true, 0: false })[value];
          if (tmp !== undefined) {
            value = tmp;
          }
...
Instance.prototype.save = function(options) {
  ...
  return Promise.bind(this).then(function() {
    ...
    return Promise.bind(this).then(function() {
      ...
      if (!options.fields.length || !this.isNewRecord || !this.options.include || !this.options.include.length) {
        return this;
      }
...
Instance.prototype.increment = function(fields, options) {
  ...
  if (identifier) {
    var keys = Object.keys(identifier);

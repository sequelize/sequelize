  ...
  Instance.prototype.set = function(key, value, options) {
    ...
    if (typeof key === 'object') {
      ...
      if (options.raw && !(this.options && this.options.include) && !(options && options.attributes) && !this.Model._hasBooleanAttributes && !this.Model._hasDateAttributes) {
        ...
        if (options.raw || this.sequelize.options.jsonDeepCompare) {
          // If raw, .changed() shouldn't be true
          this._previousDataValues = _.clone(this.dataValues);
        }
      }
  ...
  Instance.prototype.changed = function(key) {
    if (key) {
      ...
      if (this.Model._isJsonAttribute(key) && this._previousDataValues[key] && this.dataValues[key]) {
        if (this.sequelize.options.jsonDeepCompare) {
          return _.isEqual(this._previousDataValues[key], this.dataValues[key]);
        } else {
          //TODO: some other methods that do not use clone and deep compare.
        }
      }
        ...
        return self.QueryInterface[query].apply(self.QueryInterface, args)
          ...
          .then(function(result) {
            options.fields.forEach(function (field) {
              if (self.sequelize.options.jsonDeepCompare) {
                result._previousDataValues[field] = _.clone(result.dataValues[field]);
              }

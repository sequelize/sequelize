...
var initValues = function(values, options) {
  ...
  if (this.Model._hasVirtualAttributes) {
    // set virtual attributes to their default values
    this.Model._virtualAttributes.forEach(function (key) {
      if (values[key] === undefined) {
        this.set(key, Utils.toDefaultValue(this.Model._defaultValues[key]), defaultsOptions);
        delete values[key];
      }
    }, this);
  }
...
Instance.prototype.reload = function(options) {
  ...
  .then(function(reload) {
    // persist virtual fields that have been previously set
    this.Model._virtualAttributes.forEach(function (key) {
      var virtualAttribute = this.get(key, {raw: true});
      if (virtualAttribute !== undefined) {
        reload.set(key, virtualAttribute, {raw: true});
      }
    }, this);
    return reload;
  })

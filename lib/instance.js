 ...
 * @return {Object}
 */
Instance.prototype.where = function(checkVersion) {
  var where;
  ...
  var versionAttr = this.Model._versionAttribute;
...
Instance.prototype.save = function(options) {
  ...
  var self = this
    ...
    , versionAttr = this.Model._versionAttribute
  ...
  return Promise.bind(this).then(function() {
    ...
    .then(function() {
      ...
      if (self.isNewRecord) {
        ...
        var where = this.where(true);
        where = Utils.mapValueFieldNames(where, Object.keys(where), this.Model);
        if (versionAttr) {
          // TODO: Need to find the DB field name in case it's different than the attribute name
      ...
      return self.sequelize.getQueryInterface()[query].apply(self.sequelize.getQueryInterface(), args)
        .then(function(results) {
          ...
          if (versionAttr) {
            result.dataValues[versionAttr] += 1;
          }
  ...
  return Promise.bind(this).then(function() {
    ...
    if (this.Model._timestampAttributes.deletedAt && options.force === false) {
      ...
      return this.sequelize.getQueryInterface().update(
        this, this.$Model.getTableName(options), values, where, _.defaults({ hooks: false }, options)
      ).then(function(results) {
        var rowsUpdated = results[1];
        if (rowsUpdated < 1) {
          throw new sequelizeErrors.OptimisticLockError({

...
Stack.defaultOptions.breakOn = function ( value ) {
  return false;
};
...
var shouldBeHookified = function () {
  if( ! this.hooks ) {
    throw Error("You should initialize with Hook.ify( this )");
  }
};
...
Hooks.ifyPrototype = function ( Constructor, hooksPerStacks ) {
  ...
  proto.hookify = function ( options ) {
    ...
    if( options && options.hooks ) {
      Utils._.each( options.hooks, function ( fnsOrFn, hookName ) {
        if( hooks[ hookName ] ){
          hooks[ hookName ].queue( fnsOrFn );
        }
      });
    }

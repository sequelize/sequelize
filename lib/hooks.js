...
const Hooks = {
   ...
   */
  _setupHooks(userHooks) {
    this.options.hooks = {};
    Object.entries(userHooks || {}).forEach(([hookName, hooksArray]) => {
  ...
  runHooks(hooksToRun, ...hookArgs) {
    ...
    if (typeof hooksToRun === 'string') {
      hookType = hooksToRun;
      hooksToRun = getHooks.call(this, hookType);

      if (this.sequelize) {
        hooksToRun = hooksToRun.concat(getHooks.call(this.sequelize, hookType));
      }
    }
    ...
    // asynchronous hooks (default): run sequentially, ignoring per-hook return values.
    return pEach(hooksToRun, (entry) => {
      const hook = typeof entry === 'object' ? entry.fn : entry;

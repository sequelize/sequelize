...
const debug = Logger.getLogger().debugContext().extend('hooks');
...
const Hooks = {
  ...
  runHooks(hooks, ...hookArgs) {
    ...
    if (hookTypes[hookType] && hookTypes[hookType].sync) {
      for (let hook of hooks) {
        ...
        debug.log(`running hook(sync) ${hookType}`);
    ...
    return Promise.each(hooks, hook => {
      ...
      debug.log(`running hook ${hookType}`);
  ...
  addHook(hookType, name, fn) {
    ...
    debug.log(`adding hook ${hookType}`);
  ...
  removeHook(hookType, name) {
    ...
    debug.log(`removing hook ${hookType}`);

...
const escapeSqlValue = require("../sql-string").escape;
...
function injectReplacements(sqlString, dialect, replacements) {
  ...
  if (!Array.isArray(replacements) && !isPlainObject(replacements)) {
    throw new TypeError(
      `"replacements" must be an array or a plain object, but received ${JSON.stringify(
        replacements
      )} instead.`
    )
  }
  ...
  for (let i = 0; i < sqlString.length; i++) {
    ...
    if (currentDollarStringTagName !== null) {
      ...
      const dollarStringStartMatch = remainingString.match(
        /^\$(?<name>[a-z_][0-9a-z_])?(\$)/i
      )
      const tagName = !!dollarStringStartMatch && !!dollarStringStartMatch.groups && dollarStringStartMatch.groups.name;
    ...
    if (char === "$") {
      ...
      if (dollarStringStartMatch) {
        currentDollarStringTagName = (!!dollarStringStartMatch && !!dollarStringStartMatch.groups && dollarStringStartMatch.groups.name) || ""
    ...
    if (isNamedReplacements && char === ":") {
      ...
      const match = const match = remainingString.match(
        /^:(?<name>[a-z_][0-9a-z_]*)(?:\)|,|\;|$|\s|::)/i
      );
      const replacementName = !!match && !!match.groups && match.groups.name
      if (!replacementName) {
        continue
      }
      ...
      if (
        !Object.prototype.hasOwnProperty.call(replacements, replacementName) ||
      ...
      const escapedReplacement = escapeSqlValue(
        replacementValue,
        undefined,
        dialect.name,
        true
    ...
    if (isPositionalReplacements && char === "?") {
      ...
      const replacementValue =
        replacements[lastConsumedPositionalReplacementIndex]
...
module.exports.injectReplacements = injectReplacements;
module.exports.default = injectReplacements;

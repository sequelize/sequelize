...
import { escape as escapeSqlValue } from '../sql-string.js';
 ...
 */
export function injectReplacements(sqlString, dialect, replacements) {
  ...
  for (let i = 0; i < sqlString.length; i++) {
    ...
    if (isNamedReplacements && char === ':') {
      ...
      if (!Object.hasOwn(replacements, replacementName) || replacementValue === undefined) {
    ...
    if (char === '$') {
      ...
      if (dollarStringStartMatch) {
        currentDollarStringTagName =
          (!!dollarStringStartMatch && !!dollarStringStartMatch.groups && dollarStringStartMatch.groups.name) || '';
...
export default injectReplacements;

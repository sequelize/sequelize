...
/**
 * A column storing a modified uuidv1 as binary(16).
 * @see https://www.percona.com/blog/2014/12/19/store-uuid-optimized-way/
 *
 * @namespace DataTypes.BINARYUUID
 */
function BINARYUUID() {
  if (!(this instanceof BINARYUUID)) return new BINARYUUID();
}
inherits(BINARYUUID, ABSTRACT);

BINARYUUID.prototype.key = UUID.key = 'BINARY(16)';
...
BINARYUUID.prototype.validate = function validate(value) {
  if (!Buffer.isBuffer(value) || value.length !== 16) {
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid binary uuid (buffer)', value));
  }
  return true;
};

 ...
 */
function YEAR() {
  if (!(this instanceof YEAR)) return new YEAR();
}
util.inherits(YEAR, ABSTRACT);

YEAR.prototype.key = YEAR.key = 'YEAR';
YEAR.prototype.toSql = function() {
  return 'YEAR';
};

YEAR.prototype._stringify = function _stringify(date) {
  return date;
};

YEAR.prototype.validate = function(value){
  if (!Validator.isInt(String(value) || String(value).length !== 4 || String(value).length !== 2)){
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid year', value));
  }
  return true;
};

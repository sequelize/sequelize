...
const fromBinaryUUID = require('binary-uuid').fromBinaryUUID;
...
BINARY.prototype.validate = function validate(value) {
  if (!Buffer.isBuffer(value) && typeof value !== 'number') {
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid binary buffer', value));
  }
  return true;
};
...
Object.defineProperty(UUID.prototype, 'BINARY', {
  get() {
    this._binary = true;
    return this;
  }
...
UUID.prototype.toSql = function toSql() {
  return this._binary ? 'binary(16)' : this.key;
};

UUID.prototype._stringify = function _stringify(value) {
  if (!this._binary) return value;
  if (!Buffer.isBuffer(value)) {
    if (Array.isArray(value)) {
      value = Buffer.from(value);
    } else {
 ...
 *
 * @param {boolean} [binary=false] Whether UUID should be transformed to binary(16)
 * @namespace DataTypes.UUIDV1
 */
function UUIDV1(binary) {
  if (!(this instanceof UUIDV1)) return new UUIDV1(binary);
  this._binary = binary;
}
...
Object.defineProperty(UUIDV1.prototype, 'BINARY', {
  get() {
    if (!(this instanceof UUIDV1)) return new UUIDV1(true);
    this._binary = true;
    return this;
  }
});
...
const helpers = {
  BINARY: [STRING, CHAR, UUID, UUIDV1],
 ...
 * * The `CHAR`, `STRING`, and `UUID` types expose the `BINARY` property
 *

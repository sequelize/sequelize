...
class TIME extends ABSTRACT {
  ...
  validate(value) {
    if (!Validator.isDate(String(value)) && !Validator.isDate(`01-01-1970 ${String(value)}`)) {
  ...
  _sanitize(value, options) {
    if ((!options || !options.raw) && value instanceof Date ) {
    ...
    if (
      ...
      !!value &&
        (
          ...
          Math.floor(value.getTime() / 1000) === Math.floor(originalValue.getTime() / 1000)
        )
    ) {
      return false;
    }

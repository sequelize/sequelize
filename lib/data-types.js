...
class DATE extends ABSTRACT {
  ...
  _isChanged(value, originalValue) {
    ...
    const isInstanceOfDate =
      value instanceof Date
      && originalValue instanceof Date
      && value.getTime() === originalValue.getTime();

    const isValuesEqualsOrInstanceOfDate = value === originalValue || isInstanceOfDate;

    if (originalValue && value && isValuesEqualsOrInstanceOfDate) {
      return false;
    }
...
class ENUM extends ABSTRACT {
  ...
  constructor(...args) {
    ...
    const options = typeof value === 'object' && !Array.isArray(value) && value || {
      values: args.reduce((result, element) => result.concat(Array.isArray(element) ? element : [element]), [])
    };

...
/**
 * NCHAR A fixed length string with unicode string support
 *
 * @param {number} [length=255] length of string
 * @param {boolean} [binary=false] Is this binary?
 *
 * @namespace DataTypes.NCHAR
 */
function NCHAR(length) {
  const options = typeof length === 'object' && length || {length};

  if (!(this instanceof NCHAR)) return new NCHAR(options);
  STRING.apply(this, arguments);
}
inherits(NCHAR, STRING);

NCHAR.prototype.key = NCHAR.key = 'NCHAR';
NCHAR.prototype.toSql = function toSql() {
  return `NCHAR(${this._length})`;
};
 ...
 * Date column with timezone, default is UTC
 *
 * @namespace DataTypes.SMALLDATETIME
 */
function SMALLDATETIME(length) {
  const options = typeof length === 'object' && length || {length};

  if (!(this instanceof SMALLDATETIME)) return new SMALLDATETIME(options);

  this.options = options;
  this._length = options.length || '';
}
inherits(SMALLDATETIME, ABSTRACT);

SMALLDATETIME.prototype.key = SMALLDATETIME.key = 'SMALLDATETIME';
SMALLDATETIME.prototype.toSql = function toSql() {
  return 'SMALLDATETIME';
};
SMALLDATETIME.prototype.validate = function validate(value) {
  if (!Validator.isDate(String(value))) {
    throw new sequelizeErrors.ValidationError(util.format('%j is not a valid date', value));
  }
...
SMALLDATETIME.prototype._sanitize = function _sanitize(value, options) {
  if ((!options || options && !options.raw) && !(value instanceof Date) && !!value) {
    return new Date(value);
  }
...
SMALLDATETIME.prototype._isChanged = function _isChanged(value, originalValue) {
  if (
    originalValue && !!value &&
    (
      value === originalValue ||
...
SMALLDATETIME.prototype._applyTimezone = function _applyTimezone(date, options) {
  if (options.timezone) {
    if (momentTz.tz.zone(options.timezone)) {
      return momentTz(date).tz(options.timezone);
    }

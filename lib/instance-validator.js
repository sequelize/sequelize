...
class InstanceValidator {
  ...
  async _validateAndRunHooks() {
    const hooks = this.modelInstance.constructor.hooks;
    await hooks.run('beforeValidate', this.modelInstance, this.options);

    try {
      await this._validate();
    } catch (error) {
      const newError = await hooks.run('validationFailed', this.modelInstance, this.options, error);
      throw newError || error;
    }

    await hooks.run('afterValidate', this.modelInstance, this.options);

...
const Mixin = {
  hasMany(target, options = {}) { // testhint options:none
    ...
    if (options.useHooks) {
      this.runHooks('afterHasManyAssociate', association);
    }
  ...
  belongsToMany(target, options = {}) { // testhint options:none
    ...
    if (options.useHooks) {
      this.runHooks('beforeBelongsToManyAssociate', target, options);
    }
    // the id is in the foreign table or in a connecting table
    ...
    if (options.useHooks) {
      this.runHooks('afterBelongsToManyAssociate', association);
    }
...
function singleLinked(Type) {
  return function(target, options = {}) { // testhint options:none
    ...
    if (options.useHooks) {
      this.runHooks(`before${Type.name}Associate`, target, options);
    }
    ...
    if (options.useHooks) {
      this.runHooks(`after${Type.name}Associate`, association);
    }

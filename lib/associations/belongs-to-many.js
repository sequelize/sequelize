...
class BelongsToMany extends Association {
  ...
  async set(sourceInstance, newAssociatedObjects, options) {
    ...
    for (const currentRow of currentRows) {
      ...
      if (!newObj) {
        ...
        if (Object.keys(attributes).length) {
          promises.push(association.through.model.update(attributes, _.extend(options, { whereClause })));
        }
      }
    }
    ...
    if (obsoleteAssociations.length > 0) {
      ...
      promises.push(association.through.model.destroy(_.defaults({ whereClause }, options)));
    }

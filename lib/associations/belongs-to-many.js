  ...
  if (typeof this.through.model === 'string') {
    if (!this.sequelize.isDefined(this.through.model)) {
      this.through.model = this.sequelize.define(this.through.model, {}, _.extend({}, this.options, {
        ...
        as: undefined, // Don't name through model after target model
        name: undefined,
        through: undefined
      }));
    } else {
...
BelongsToMany.prototype.get = function(instances, options) {
  var throughModel = this.throughModel;

  var throughOptions = _.assign({
    where: _.set({}, this.foreignKey, {$in: _.map(instances, this.source.primaryKeyAttribute)}),
    logging: options.logging
  });

  var association = this;

  if(_.isUndefined(options.limit) && _.isUndefined(options.orderBy)) {
    return throughModel.findAll(throughOptions).then(function (throughs) {
      var otherKey = association.otherKey
          , targetIds = _.chain(throughs).map(otherKey).uniq().value();

      var targetOptions = _.chain({
            where: _.set({}, association.target.primaryKeyAttribute, {$in: targetIds})
          })
          .assign(association.target.$optClone(options))
          .omit('include', 'includeMap', 'includeNames')
          .value();

      return Utils.Promise.join(throughs, _.isEmpty(targetIds) ? null : association.target.findAll(targetOptions));
    }).then(_.spread(function (throughs, targets) {
      targets = _.groupBy(targets, association.target.primaryKeyAttribute);

      return _.chain(instances)
          .map(association.source.primaryKeyAttribute)
          .map(function (sourceId) {
            var instanceThroughs = _.filter(throughs, function (through) {
              return through.get(association.foreignKey, {raw: true}) === sourceId;
            })
                , instanceTargets = _.at(targets, _.map(instanceThroughs, association.otherKey));

            return [sourceId, _(instanceTargets).compact().flatten().value()];
          })
          .fromPairs()
          .value();
    }));
  } else {
    var sourcePrimaryKeys = _.map(instances, function (instance) {
      return (instance instanceof instance.Model.Instance) ?
          instance.get(instance.Model.primaryKeyAttribute, {raw: true}) :
          instance;
    });

    var targetInclude = _.extend({
      association: association.manyFromTarget,
      where: _.set({}, association.foreignKey, {$in: sourcePrimaryKeys}),
      limit: options.limit
    });

    var targetOptions = _.set(_.omit(options, ['limit']), 'include', targetInclude);

    var results = {};

    return association.target
      .findAll(targetOptions)
      .each(function(target) {
        _.each(target.get(association.manyFromTarget.as), function(throughInstance) {
          var primaryKey = throughInstance.get(association.foreignKey);
          results[primaryKey] = (results[primaryKey] || []).concat(target);
        });
      })
      .then(function() {
        return results;

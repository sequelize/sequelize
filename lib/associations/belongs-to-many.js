...
class BelongsToMany extends Association {
  ...
  count(instance, options) {
    ...
    const primaryKeyField = model.attributes[model.primaryKeyAttribute].field || model.primaryKeyAttribute;

    options = Utils.cloneDeep(options);
    options.attributes = [
      [sequelize.fn('COUNT', sequelize.col([association.target.name, primaryKeyField].join('.'))), 'count']
    ];

...
export class BelongsToMany extends Association {
  constructor(source, target, options) {
     ...
     */
    for (const association of Object.values(this.target.associations)) {
  ...
  injectAttributes() {
    ...
    for (const [attributeName, attribute] of Object.entries(this.through.model.rawAttributes)) {
    ...
    this.through.model.rawAttributes[this.foreignKey] = _.extend(
      this.through.model.rawAttributes[this.foreignKey],
      sourceAttribute
    );
    this.through.model.rawAttributes[this.otherKey] = _.extend(
      this.through.model.rawAttributes[this.otherKey],
      targetAttribute
    );
  ...
  async has(sourceInstance, instances, options) {
    ...
    where[Op.or] = instances.map((instance) => {
      if (instance instanceof association.target) {
        ...
        const whereClause = {};
        whereClause[association.target.primaryKeyAttribute] = instance;
  ...
  async set(sourceInstance, newAssociatedObjects, options) {
    ...
    defaultAttributes = _.omit(defaultAttributes, [
      'transaction',
      'hooks',
      'individualHooks',
      'ignoreDuplicates',
    ...
    const unassociatedObjects = newAssociatedObjects.filter(
      (obj) => !_.find(currentRows, (currentRow) => currentRow[foreignIdentifier] === obj.get(targetKey))
    );
    ...
    for (const currentRow of currentRows) {
      ...
      if (!newObj) {
        ...
        whereClause[identifier] = sourceInstance.get(sourceKey);
        whereClause[foreignIdentifier] = newObj.get(targetKey);

        if (Object.keys(attributes).length) {
          promises.push(association.through.model.update(attributes, _.extend(options, { where: whereClause })));
        }
      }
    }
    ...
    if (obsoleteAssociations.length > 0) {
      let whereClause = {};
      whereClause[identifier] = sourceInstance.get(sourceKey);
      whereClause[foreignIdentifier] = obsoleteAssociations.map(
        (obsoleteAssociation) => obsoleteAssociation[foreignIdentifier]
      );
      whereClause = Object.assign(whereClause, association.through.scope);
      promises.push(association.through.model.destroy(_.defaults({ where: whereClause }, options)));
    }
  ...
  async add(sourceInstance, newInstances, options) {
    ...
    const identifier = association.identifier;
    const foreignIdentifier = association.foreignIdentifier;
    const defaultAttributes = _.omit(options.through || {}, [
      'transaction',
      'hooks',
      'individualHooks',
      'ignoreDuplicates',
    ...
    for (const assoc of changedAssociations) {
      ...
      whereClause[identifier] = sourceInstance.get(sourceKey);
      whereClause[foreignIdentifier] = assoc.get(targetKey);
  ...
  remove(sourceInstance, oldAssociatedObjects, options) {
    ...
    where[association.foreignIdentifier] = oldAssociatedObjects.map((newInstance) =>
      newInstance.get(association.target.primaryKeyAttribute)
    );

...
BelongsToMany.prototype.get = function(instances, options) {
  ...
  if(_.isUndefined(options.limit) && _.isUndefined(options.orderBy)) {
    ...
    return Utils.Promise.resolve(instances)
      .map(function (instance) {
        var throughOptions = _.assign(
          {
            include: [
              {
                association: association.target.hasOne(throughModel),
                where: _.set({}, association.foreignKey, instance.get(association.source.primaryKeyAttribute))
              }
            ]
          },
          _.pick(options, 'order', 'limit', 'logging')
        );

        return association.target.findAll(throughOptions)
          .then(function(results) {
            return [instance.get(association.source.primaryKeyAttribute), results];
          });
      })
      .then(function(pairs) {

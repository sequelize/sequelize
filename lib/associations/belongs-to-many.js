...
class BelongsToMany extends Association {
  constructor(source, target, options) {
    ...
    */
    ...
    /*
    * Default/generated source/target PrimaryKeyAttributes
    */
    this.hasNonPrimaryBelongsTo = this.options.targetKey || this.options.sourceKey;
    this.targetPrimaryKeyAttribute = this.options.targetKey || this.target.primaryKeyAttribute;
    this.sourcePrimaryKeyAttribute = this.options.sourceKey || this.source.primaryKeyAttribute;
    ...
    if (_.isObject(this.options.foreignKey)) {
      ...
      this.foreignKey = this.options.foreignKey || Utils.camelizeIf(
        [
          ...
          this.sourcePrimaryKeyAttribute
        ].join('_'),
    ...
    if (_.isObject(this.options.otherKey)) {
      ...
      this.otherKey = this.options.otherKey || Utils.camelizeIf(
        [
          ...
          this.targetPrimaryKeyAttribute
        ].join('_'),
        !this.target.options.underscored
      );
    }
  ...
  injectAttributes() {
    ...
    const sourceKey = this.source.rawAttributes[this.sourcePrimaryKeyAttribute];
    const sourceKeyType = sourceKey.type;
    const sourceKeyField = sourceKey.field || this.sourcePrimaryKeyAttribute;
    const targetKey = this.target.rawAttributes[this.targetPrimaryKeyAttribute];
    const targetKeyType = targetKey.type;
    const targetKeyField = targetKey.field || this.targetPrimaryKeyAttribute;
  ...
  get(instance, options) {
    ...
    if (Object(through.model) === through.model) {
      throughWhere = {};
      throughWhere[association.foreignKey] = instance.get(association.sourcePrimaryKeyAttribute);
  ...
  count(instance, options) {
    ...
    options = Utils.cloneDeep(options);
    options.attributes = [
      [sequelize.fn('COUNT', sequelize.col([association.target.name, association.targetPrimaryKeyAttribute].join('.'))), 'count']
    ];
  ...
  has(sourceInstance, instances, options) {
    ...
    where.$or = instances.map(instance => {
      if (instance instanceof association.target) {
        ...
        const where = {};
        where[association.targetPrimaryKeyAttribute] = instance;
        return where;
      }
    });
  ...
  set(sourceInstance, newAssociatedObjects, options) {
    ...
    const association = this;
    const sourceKey = association.sourcePrimaryKeyAttribute;
    const targetKey = association.targetPrimaryKeyAttribute;
  ...
  remove(sourceInstance, oldAssociatedObjects, options) {
    ...
    const where = {};
    where[association.identifier] = sourceInstance.get(association.sourcePrimaryKeyAttribute);
    where[association.foreignIdentifier] = oldAssociatedObjects.map(newInstance => newInstance.get(association.targetPrimaryKeyAttribute));

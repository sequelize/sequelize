...
class BelongsToMany extends Association {
  ...
  has(sourceInstance, instances, options) {
    ...
    return association
      .get(sourceInstance, options)
      .then(associatedObjects => associatedObjects.length === instances.length);
  }
  ...
  set(sourceInstance, newAssociatedObjects, options) {
    ...
    return association.through.model.findAll(_.defaults({ where, raw: true }, options)).then(currentRows => {
      ...
      if (obsoleteAssociations.length > 0) {
        let where = {};
        where[identifier] = sourceInstance.get(sourceKey);
        where[foreignIdentifier] = obsoleteAssociations.map(
  ...
  create(sourceInstance, values, options) {
    ...
    return association.target
      ...
      .then(newAssociatedObject =>
        sourceInstance[association.accessors.add](newAssociatedObject, _.omit(options, ['fields'])).then(
          () => newAssociatedObject
        )
      );
  }

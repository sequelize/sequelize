...
module.exports = (function() {
  ...
  HasOne.prototype.injectAttributes = function() {
    ...
    if (this.target.rawAttributes.hasOwnProperty(this.identifier)) {
        if (DataTypes.INTEGER !== this.target.rawAttributes[this.identifier].type)
            throw new Error("Field " + this.identifier + " of type " + this.target.tableName + " must be of type INT in order for it to be usable as a foreign key")
    } else {
        var newAttributes = {}
        newAttributes[this.identifier] = { type: DataTypes.INTEGER }
        newAttributes = Utils.simplifyAttributes(newAttributes)
        Utils._.extend(this.target.attributes, newAttributes)
    }

  ...
  constructor(source, target, options) {
    ...
    if (this.tenantKeys) {
      if (!this.tenantKeys.source || !this.tenantKeys.foreign) {
        throw new Error('tenantKeys for a Hasone association must have source and foreign keys');
      }
    ...
    if (instances) {
      ...
      if (association.tenantKeys) {
        where[association.tenantKeys.foreign] = {
          [Op.in]: instances.map(instance => instance.get(association.tenantKeys.source))  
        }
      }
    } else {
      ...
      if (association.tenantKeys) {
        where[association.tenantKeys.foreign] = instance.get(association.tenantKeys.source);
      }
    }

...
class HasOne extends Association {
  ...
  async get(instances, options) {
    ...
    if (instances) {
      where[association.foreignKey] = {
        [Op.in]: instances.map(inst => inst.get(association.sourceKey))
      };
    } else {
    ...
    if (instances) {
      ...
      for (const inst of instances) {
        result[inst.get(association.sourceKey, { raw: true })] = null;
      }

      for (const inst of results) {
        result[inst.get(association.foreignKey, { raw: true })] = inst;
      }
  ...
  async set(sourceInstance, associatedInstance, options) {
    ...
    const oldInstance = await sourceInstance[association.accessors.get](options);

    // TODO Use equals method once #5605 is resolved
    const alreadyAssociated =
      oldInstance &&
      associatedInstance &&
      association.target.primaryKeyAttributes.every(
        attribute =>
    ...
    if (associatedInstance && !alreadyAssociated) {
      ...
      Object.assign(associatedInstance, association.scope);

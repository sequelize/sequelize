...
class Association {
   ...
   */
  async toInstanceArray(input) {
    if (!Array.isArray(input)) {
      input = [input];
    }
    return await Promise.all(input.map(async element => {
        if (element instanceof this.target) return element;
        let tmpInstance = {};
        const instanceOptions = { isNewRecord: false };
        if (typeof element === 'object') {
          tmpInstance = element;
          return await this.target.create(tmpInstance, instanceOptions);
        }
        tmpInstance[this.target.primaryKeyAttribute] = element;
        return this.target.build(tmpInstance, instanceOptions);
      }));
  }

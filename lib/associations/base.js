...
class Association {
  ...
  /**
   * Normalize input and create the model if necessary
   *
   * @param {Array|string} input it may be array or single obj, instance or primary key
   *
   * @private
   * @returns {Array} built or create objects
   */
  toInstanceCreate(input) {
    if (!Array.isArray(input)) {
      input = [input];
    }
    return Promise.all(input.map(async element => {
      if (element instanceof this.target) return element;
      let tmpInstance = {};
      // checks if the element is not a model, if so creates it.
      if (typeof element === 'object') {
        tmpInstance = element;
        return await this.target.create(tmpInstance);
      }

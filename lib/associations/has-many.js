...
class HasMany extends Association {
  ...
  has(sourceInstance, targetInstances, options) {
    ...
    return association
      ...
      .then(associatedObjects => associatedObjects.length === targetInstances.length);
  }
  ...
  set(sourceInstance, targetInstances, options) {
    ...
    return association.get(sourceInstance, _.defaults({ scope: false, raw: true }, options)).then(oldAssociations => {
      ...
      return Promise.all(promises).then(() => sourceInstance);
    });
  }
  ...
  add(sourceInstance, targetInstances, options) {
    ...
    return association.target
      .unscoped()
      .update(update, _.defaults({ where }, options))
      .then(() => sourceInstance);
  }

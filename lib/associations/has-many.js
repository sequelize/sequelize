...
class HasMany extends Association {
  constructor(source, target, options) {
    ...
    */
    ...
    this.accessors = {
      ...
      hasAll: `has${plural}`,
      count: `count${plural}`,
      getAndCount: `getAndCount${plural}`
    };
  }
  ...
  mixin(obj) {
    const methods = ['get', 'count', 'hasSingle', 'hasAll', 'set', 'add', 'addMultiple', 'remove', 'removeMultiple', 'create', 'getAndCount'];
    const aliases = {
      hasSingle: 'has',
      hasAll: 'has',
      addMultiple: 'add',
  ...
  count(instance, options) {
    ...
    options.raw = true;
    options.plain = typeof options.plain === 'undefined' ? true : options.plain;

    return this.get(instance, options).then(result => Array.isArray(result) ? result.length : parseInt(result.count, 10));
  }

  /**
   * Find and count everything currently associated with this, using an optional where clause.
   * 
   * @param {Model}        instance the source instance
   * @param {Object}         [options] find & count options
   * @param {Object}         [options.where] An optional where clause to limit the associated models
   ...
   */
  getAndCount(instance, options) {
    if (options !== undefined && !_.isPlainObject(options)) {
      throw new Error('The argument passed to getAndCount must be an options object, use findByPk if you wish to pass a single primary key value');
    }
    ...
    countOptions.offset = null;
    countOptions.order = null;

    return Promise.all([
      this.count(instance, countOptions),

...
class HasMany extends Association {
  constructor(source, target, options) {
    ...
    this.extraForeignKeys = options.extraForeignKeys;
    ...
    */
    ...
    if (this.extraForeignKeys) {
      if (!this.extraForeignKeys.source || !this.extraForeignKeys.target) {
        throw new Error('extraForeignKeys must have source and target keys');
      }

      if (!source.rawAttributes[this.extraForeignKeys.source]) {
        throw new Error('Extra foreign key: ' + this.extraForeignKeys.source + ' is not an attribute of ' + source.name);
      }

      if (!target.rawAttributes[this.extraForeignKeys.target]) {
        throw new Error('Extra foreign key: ' + this.extraForeignKeys.target + ' is not an attribute of ' + target.name);
      }      
    }
  ...
  get(instances, options) {
    ...
    if (instances) {
      ...
      if (options.limit && instances.length > 1 && !association.extraForeignKeys) {
        ...
        if (association.extraForeignKeys) {
          where[association.extraForeignKeys.target] = {
            [Op.in]: instances.map(instance => instance.get(association.extraForeignKeys.source, { raw: true }))
          };
        }
      ...
      if (association.extraForeignKeys) {
        where[association.extraForeignKeys.target] = instance.get(association.extraForeignKeys.source, { raw: true });
      }
    }

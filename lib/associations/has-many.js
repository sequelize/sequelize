...
module.exports = (function() {
  var HasMany = function(source, target, options) {
    ...
    this.combinedModelName = Utils.combineTableNames(
      this.source.name,
      (this.isSelfAssociation && this.as) ? Utils.singularize(this.as) : this.target.name
    );
    ...
    var freezeTableName = this.options.freezeTableName;
    if (freezeTableName === undefined && this.sequelize.options.define) freezeTableName = this.sequelize.options.define.freezeTableName;
    ...
    if (!this.associationAccessor) {
      if (typeof this.through.model === 'string') {
        this.associationAccessor = (freezeTableName ? this.through.model : Utils.pluralize(this.through.model));
    ...
    if (this.doubleLinked && this.through.model === true) {
      this.through.model = this.combinedModelName;
      this.through.tableName = this.combinedTableName;
    }

    if (typeof this.through.model === 'string') {
      if (!this.through.tableName) this.through.tableName = (freezeTableName ? this.through.model : Utils.pluralize(this.through.model));

      this.through.model = this.sequelize.define(this.through.model, {}, _.extend(this.options, {
        tableName: this.through.tableName,
    ...
    this.options.tableName = this.combinedName = (this.through.model === Object(this.through.model) ? this.through.model.tableName : this.combinedTableName);

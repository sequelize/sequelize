...
class HasMany extends Association {
  constructor(source, target, options) {
    ...
    this.tenantKeys = options.tenantKeys;
    ...
    */
    ...
    if (this.tenantKeys) {
      if (!this.tenantKeys.source || !this.tenantKeys.foreign) {
        throw new Error('tenantKeys for a HasMany association must have source and foreign keys');
      }

      if (!source.rawAttributes[this.tenantKeys.foreign]) {
        throw new Error('Tenant key: ' + this.tenantKeys.foreign + ' is not an attribute of ' + source.name);
      }

      if (!target.rawAttributes[this.tenantKeys.source]) {
        throw new Error('Tenant key: ' + this.tenantKeys.source + ' is not an attribute of ' + target.name);
      }      
    }
  ...
  injectAttributes() {
    ...
    if (this.tenantKeys) {
      newAttributes[this.tenantKeys.foreign] = {
        name: this.tenantKeys.foreign,
        type: this.source.rawAttributes[this.tenantKeys.foreign].type,
      };
    }
  ...
  get(instances, options) {
    ...
    if (instances) {
      ...
      if (options.limit && instances.length > 1 && !association.tenantKeys) {
        ...
        if (association.tenantKeys) {
          where[association.tenantKeys.foreign] = {
            [Op.in]: instances.map(instance => instance.get(association.tenantKeys.source, { raw: true }))
          };
        }
      ...
      if (association.tenantKeys) {
        where[association.tenantKeys.foreign] = instance.get(association.tenantKeys.source, { raw: true });
      }
    }

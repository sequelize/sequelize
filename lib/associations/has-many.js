...
class HasMany extends Association {
  ...
  /**
   * Make associations of targetInstances with sourceInstances.
   * @param {Model|Model[]|string[]|string|number[]|number} [targetInstances] Can be an array of instances or their primary keys
   * @param {Model} sourceInstance the source instance
   * @param {Object} [options] Options passed to getAssociations
   *
  ...
  makeAssociations(targetInstances, sourceInstance, options) {
   ...
   * @param {Model|Model[]|string[]|string|number[]|number|Object[]} [targetInstances] An array of persisted instances or an array of raw javascript objects without IDs or primary key of instances to associate with this. Pass `null` or `undefined` to remove all associations.
  ...
  set(sourceInstance, targetInstances, options) {
    if (targetInstances === null) {
      return this.makeAssociations([], sourceInstance, options);
    } 
    return this.toInstanceCreate(targetInstances)
      .then(data => this.makeAssociations(data, sourceInstance, options));
  }
  ...
  add(sourceInstance, targetInstances, options = {}) {
    ...
    return this.toInstanceCreate(targetInstances).then(resultInstances => {
      update[this.foreignKey] = sourceInstance.get(this.sourceKey);
      Object.assign(update, this.scope);
  
      const where = {
        [this.target.primaryKeyAttribute]: resultInstances.map(unassociatedObject =>

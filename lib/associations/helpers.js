...
function validateExtraForeignKeys(association) {
  if (association.extraForeignKeys) {
    if (!Array.isArray(association.extraForeignKeys)) {
      throw new Error('extraForeignKeys must be an array');
    }

    association.extraForeignKeys.forEach(key => {
      if (!key.source || !key.target) {
        throw new Error('extraForeignKeys must have source and target keys');
      }

      if (!association.source.rawAttributes[key.source]) {
        throw new Error(`Extra foreign key: ${key.source} is not an attribute of ${association.source.name}`);
      }

      if (!association.target.rawAttributes[key.target]) {
        throw new Error(`Extra foreign key: ${key.target} is not an attribute of ${association.target.name}`);
      }      
    });
  }
}
exports.validateExtraForeignKeys = validateExtraForeignKeys;

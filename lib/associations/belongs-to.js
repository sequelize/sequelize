    ...
    if (instances) {
      ...
      if (association.extraForeignKeys) {
        where[association.extraForeignKeys.target] = {
          [Op.in]: instances.map(instance => instance.get(association.extraForeignKeys.source))
        };
      }
    } else {
      if (association.targetKeyIsPrimary && !options.where && !association.extraForeignKeys) {
      ...
      if (association.extraForeignKeys) {
        where[association.extraForeignKeys.target] = instance.get(association.extraForeignKeys.source);
      }
    }

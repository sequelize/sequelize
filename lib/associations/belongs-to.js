...
class BelongsTo extends Association {
  constructor(source, target, options) {
    ...
    if (this.tenantKeys) {
      if (!this.tenantKeys.target || !this.tenantKeys.foreign) {
        throw new Error('tenantKeys for a BelongsTo association must have target and foreign keys');
      }
      ...
      if (!target.rawAttributes[this.tenantKeys.target]) {
        throw new Error('Tenant key: ' + this.tenantKeys.target + ' is not an attribute of ' + target.name);
      }
    }
  ...
  get(instances, options) {
    ...
    if (instances) {
      ...
      if (association.tenantKeys) {
        where[association.tenantKeys.target] = {
          [Op.in]: instances.map(instance => instance.get(association.tenantKeys.foreign))
        };
      }
    } else {
      if (association.targetKeyIsPrimary && !options.where && !association.tenantKeys) {
      ...
      if (association.tenantKeys) {
        where[association.tenantKeys.target] = instance.get(association.tenantKeys.foreign);
      }
    }

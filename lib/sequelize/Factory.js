    ...
    addOneToOneMethods: function(table1, table2, assocName, backAssocName) {
      ...
      table1.prototype[getterName] = function(callback) {
        ...
        if (typeof(this.primaryKey)=='string')
          whereConditions[Sequelize.Helper.SQL.asTableIdentifier(assocName)] = this[this.primaryKey]
        else { // Join the primary keys of this table to create association on a single column
           whereConditions[Sequelize.Helper.SQL.asTableIdentifier(assocName)]=
             Sequelize.Helper.Array.map(this.primaryKey,function(element){return this[element]}).join("\000");
        }
        ...
        if(_options.force || !this.hasFetchedAssociationFor(assocName)) {
          var Association = sequelize.tables[Sequelize.Helper.SQL.manyToManyTableName(assocName, backAssocName)].klass,
              ...
              if (typeof(this.primaryKey)=='string')
                ...
                whereConditions = Sequelize.Helper.Array.map(this.primaryKey,function(element){return element+"="+Sequelize.Helper.SQL.transformValueByDataType(this[element], this.attributes[key])}).
              }

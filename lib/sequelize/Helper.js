exports.Helper = function(Sequelize) {
  var Helper = {
    ...
    SQL: {
      escape: function(value,fieldType) {
	if (fieldType===Sequelize.STRING || fieldType===Sequelize.TEXT) {
	...
	return value;
      ...
      fieldsForInsertQuery: function(object,without) {
        without=without||[];
        return Helper.Array.without(Helper.Hash.keys(object.values),without).join(", ")
      },
      ...
      transformValueByDataType: function(value, attributeOptions) {
        ...
        return ("'" + Helper.SQL.escape(value,dataType) + "'")
      },

      hashToWhereConditions: function(conditions, attributes) {
        if(typeof conditions == 'number')
          Helper.log("Sending a number as a where condition is no longer supported. Please correct this!\n"+(new Error().stack));
        else {
          var result = []

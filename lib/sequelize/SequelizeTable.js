...
exports.SequelizeTable = function(Sequelize, sequelize, tableName, attributes, options) {
  ...
  /**
   * Enable over-ride of default identifier when saving & reading from database
   * e.g. idField:{
   * 				name:'id',				// The name referred to it in the object
   * 				field:'nid',			// The actual column name in the database (simple rename)
   * 				composite:true,			// is it a composite key (if so, field is ignored for fields)
   * 				fields:['nid','vid']    // Array of database columns that make up the composite key - must be integers
   * 		}
   * Can be added to the schema options parameter (along with the created/updated).
   */  
  var idField =  {name:'id',field:'id',composite:false};
  if(options.idField) idField = options.idField;
  ...
  var table = function(values) {
    ...
    this.idField = idField // Store for later use
    this[idField] = null // specify id as null to declare this object as unsaved and as not present in the database
    this.table = table
  ...
  var classMethods = {
    ...
    sync: function(callback) {
      ...
      var fields =  Sequelize.Helper.ID.hashToCreateSql(idField);
    ...
    find: function(conditions, options, callback) {
      ...
      sequelize.query(
        Sequelize.sqlQueryFor('select', {
          table: table.tableName,
          where: Sequelize.Helper.SQL.hashToWhereConditions(conditions, table.attributes, idField),          
          limit: 1
        }), function(result) {
    ...
    sqlResultToObject: function(result) {
      ...
      object[idField.name] = Sequelize.Helper.ID.mapResultIdtoObjectId(idField,result);      
      return object
    },
    ...
    belongsTo: function(assocName, _table, backAssociation) {
      ...
      table.prototype[Sequelize.Helper.SQL.addPrefix('set', assocName, true)] = function(object, callback) {
        ...
        attr[Sequelize.Helper.SQL.asTableIdentifier(assocName)] = object[identifier]
  ...
  table.prototype = {
    ...
    save: function(callback) {
      ...
      if(this.isNewRecord) {
        ...
        query = Sequelize.sqlQueryFor('insert', {
    	  ...
    	  query = Sequelize.sqlQueryFor('update', { table: table.tableName, values: Sequelize.Helper.SQL.valuesForUpdate(this), where: Sequelize.Helper.ID.valuesForUpdate(idField,this) })
      }

      sequelize.query(query, function(result, stats) {
    	if(!idField.composite) {
    		self[idField.name] = self[idField.name] || stats.insert_id
    	}
    ...
    destroy: function(callback) {
      sequelize.query(
        Sequelize.sqlQueryFor('delete', { table: table.tableName, where: Sequelize.Helper.ID.valuesForUpdate(idField,this)  }),
        callback
      )
    }
  }

...
exports.SequelizeTable = function(Sequelize, sequelize, tableName, attributes, options) {
  ...
  if (options.trackDelete) {
    attributes.deletedAt = { type: Sequelize.DATE, allowNull: true}
  }
  ...
  var classMethods = {
    ...
    sync: function(callback) {
      ...
      if (typeof (attributes[table.primaryKey]) == 'undefined')
        fields.unshift(table.primaryKey+" INT NOT NULL auto_increment PRIMARY KEY");
    ...
    if (table.primaryKey==name) { //single column PK if defined as a string.
      type+=" PRIMARY KEY";
    } else {
// TODO: Allow more complex options on index definition. See: http://dev.mysql.com/doc/refman/5.1/en/create-table.html
      ...
      if (typeof(table.primaryKey)=='object') {
        //var pk='PRIMARY KEY('+table.primaryKey.join(",")+")";
    ...
    findAll: function(options, callback) {
      ...
      if (table.options.trackDelete) {
        queryOptions.where+=" AND deletedAt IS NULL";
      }
      ...
      sequelize.query(
        Sequelize.sqlQueryFor('select', {
          ...
          where: Sequelize.Helper.SQL.hashToWhereConditions(conditions, table.attributes) +
                    options.trackDelete?" AND deletedAt IS NULL":"",
          order: pk+' DESC',
          limit: 1
        }), function(result) {
...
*/
    ...
    destroy: function(callback) {
      var values=Sequelize.Helper.SQL.valuesForUpdate(this);
      var id=values[table.primaryKey];
      var where;
      if (typeof(table.primaryKey)=='string')
        where=table.primaryKey+'='
         +Sequelize.Helper.SQL.transformValueByDataType(this._key[table.primaryKey],table.attributes[table.primaryKey])
      ...
      if (options.trackDelete) {
        ...
        query=Sequelize.sqlQueryFor('update', { 
            table: table.tableName, 
            values: 'deletedAt='+Sequelize.Helper.SQL.transformValueByDataType(new Date(),table.attributes['deletedAt']),
            where: where
        })
      } else {
        query=Sequelize.sqlQueryFor('delete', { table: table.tableName, where: where })
      }
      sequelize.query(
        query,
        callback

  ...
  */
  checkConfigObject(config) {
    ...
    if(config.database.length === 0) {
      ...
      errorToThrow +=' or specify the full connectString with the format : HOST:[PORT]/SERVICE_NAME';
      errorToThrow += '\n from tnsnames.ora : (HOST = mymachine.example.com)(PORT = 1521)(SERVICE_NAME = orcl)'; 
      throw new Error(errorToThrow);
    }

    if(!config.host || config.host.length === 0) {
      throw new Error('You have to specify the host or the fully qualified database');
    }
...
//TODO - see if this is called and if still has to be defined

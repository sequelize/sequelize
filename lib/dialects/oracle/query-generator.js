  ...
  */
  ...
  changeColumnQuery(tableName, attributes) {
      ...
      if (definition.match(/REFERENCES/)) {
        constraintString.push(
          _.template('<%= fkName %> FOREIGN KEY (<%= attrName %>) <%= definition %>')({
            fkName: this.quoteIdentifier("FK" + tableName + attributeName),
    ...
    if (constraintString.length) {
      let match = constraintString[0].match(/^(.+) (FOREIGN KEY.*)$/);
      constraintName = match[1];
      finalQuery += 'DROP CONSTRAINT ' + this.quoteIdentifier(constraintName);
    ...
    if (constraintString.length) {
      const secondQuery = 'ADD CONSTRAINT ' + constraintString.join(', ');
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    const bind = options.upsertbind === undefined ? []: options.upsertbind;
    ...
    if (options.returning && primaryKey === '') {
      ...
      if ((!currentModel || !currentModel.hasPrimaryKeys) && modelAttributes) {
        ...
        const pkReturn = `$:${this.quoteIdentifier(field)};string$`;
  ...
  /**
   * It causes bindbyName by using a like :a1 .
   * For bindbyPos, we can keep :1
   */
  bindParam(bind) {
    return value => {
      bind.push(value);
      return `:a${bind.length}`;
    };
  }
}

...
var QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    if (options && !!options.uniqueKeys) {
      Utils._.each(options.uniqueKeys, (columns, indexName) => {
        ...
        //The test on the PrimaryKey has been done, we have to check if there is no UNIQUE in the column definition
        if(canBeUniq) {
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    var returningQuery = "<%=valueQuery %> RETURNING <%=primaryKey %> INTO $:<%=primaryKeyReturn %>;<%=primaryKeyType %>$;";
    ...
    primaryKeys.forEach(element => {
      if(element.field.toLowerCase() === 'uid') {
        primaryKey += '"uid"';
        primaryKeyReturn.push("pkReturnVal");
      } else {
        primaryKey += element.field;
        primaryKeyReturn.push(element.field);
      }
      primaryKeyType += element.type.toSql();
    });
  ...
  bulkInsertQuery(tableName, attrValueHashes, options, attributes) {
    var query = 'INSERT ALL INTO <%= table %> (<%= attributes %>) WITH rowAttr AS (<%= rows %>) SELECT * FROM rowAttr;'
      ...
      if(tableName.name === 'product_tag') {
        // console.log(' - ATTRHASHVALUES - ');
        // console.dir(attrValueHashes);
  ...
  getForeignKeysQuery(table) {
    ...
    var sql = [
      ...
      table.schema ? "where r_owner = '" + table.schema + "' and " : "where ",
      "constraint_type = 'R' ",

              ...
              if (currUnique.fields.length === fields.length) {
                ...
                for (let i = 0; i < currUnique.fields.length; i++) {
                  ...
                  if (Utils._.includes(fields, field)) {
                    ...
                    //We have at least one different column, even if we found the same column previously, we let the constraint be created
                    canContinue = true;
                    break;
                  }
                }
  ...
  */
  ...
  getPrimaryKeyConstraintQuery(tableName) {
    ...
    let sql = ['SELECT cols.column_name, atc.identity_column ',
      ...
      '\' ',
      tableName.schema ? 'AND cols.owner = \'' + tableName.schema.toUpperCase() + '\' ' : ' ',

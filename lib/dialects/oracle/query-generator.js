...
class OracleQueryGenerator extends AbstractQueryGenerator {
    ...
    if (options && !!options.uniqueKeys) {
      _.each(options.uniqueKeys, (columns, indexName) => {
        ...
        if (canBeUniq) {
          ...
          values.attributes +=
            ', CONSTRAINT ' + indexName + ' UNIQUE (' + _.map(columns.fields, self.quoteIdentifier).join(', ') + ')';
        }
      });
    }
  ...
  attributeToSQL(attribute) {
    ...
    if (attribute.onUpdate === 'cascade' || attribute.onUpdate === 'CASCADE') {
      //[25/11/2021] Hasan - Add this to documentation when merging to sequelize-main
      throw new Error("Oracle doesn't support ON UPDATE CASCADE");
    }

...
//TODO
  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query
      , valueQuery = '<%= tmpTable %>INSERT<%= ignore %> INTO <%= table %> (<%= attributes %>) VALUES (<%= values %>);'
      , emptyQuery = '<%= tmpTable %>INSERT<%= ignore %> INTO <%= table %> VALUES (<%= values %>);'
      , outputFragment
      , fields = []
      , values = []
      , key
    ...
    //Request to get the last inserted row
    var output = "SELECT * FROM (SELECT * FROM <%= table %> ORDER BY 1 DESC) <%= table %>2 WHERE rownum <= 1;"
    var lockingTable = "LOCK TABLE <%= table %> IS SHARE ROW EXCLUSIVE MODE;"; //Not used but to keep in case of inconsistent returned datas problem

...
var QueryGenerator = {
  ...
  showSchemasQuery() {
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.gt(this.sequelize.options.databaseVersion, '12.0.0')) {
      //Version 12 and below, searching from all_users
      return `SELECT USERNAME AS "schema_name" FROM ALL_USERS WHERE COMMON = ('NO') AND USERNAME != user`;
    } else {
      //Version before 12, searching from dba_users
      return `SELECT USERNAME as "schema_name" FROM DBA_USERS WHERE DEFAULT_TABLESPACE NOT IN ('SYSTEM', 'SYSAUX') AND USERNAME NOT IN ('SYS','SYSTEM', user)`;
    }
  },
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    completeQuery = "BEGIN EXECUTE IMMEDIATE '<%= createTableQuery %>'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;",
    oldVersionQuery = "CREATE TABLE <%= table %> (<%= attributes %>)", 
    completeOldVersionQuery = "BEGIN EXECUTE IMMEDIATE '<%= createTableQuery %>'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF;END; /MORE/ <%= seqQuery %>",
    primaryKeys = [], seqQuery = "", foreignKeys = {}, attrStr = [], checkStr = [], self = this;
    ...
    const isMinorTwelve = semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '12.0.0');
    ...
    for (let attr in attributes) {
      if (attributes.hasOwnProperty(attr)) {
        ...
        //Specific actions for version < 12, we setted the IDENTITY inside the declaration of the attribute, but this has to be removed as it doesn't exist in version < 12
        if (isMinorTwelve) {
          if (dataType.indexOf('IDENTITY') > -1) {
            //We remove the identity of the declaration AND tell the code that it has to generate sequence / trigger on the PK
            hasIdentityColumns = true;
            dataType = dataType.replace(' IDENTITY', '');
          } 
        }
    ...
    //Special sql for dealing with autoincrement columns with Oracle minor 12.0.0
    if (isMinorTwelve && hasIdentityColumns) {
      query = oldVersionQuery;
      completeQuery = completeOldVersionQuery;
      let seqName = this._generateSeqName(values.table);

      seqQuery = `BEGIN EXECUTE IMMEDIATE 'CREATE SEQUENCE ${seqName}'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF;END;`;
    }

    values["seqQuery"] = seqQuery;
      ...
      options.indexes.forEach((index, idx) => {
        if (index.unique === true) {
          ...
          if (options.uniqueKeys) {
            let keys = Object.keys(options.uniqueKeys);

            for (let fieldIdx = 0; fieldIdx < keys.length; fieldIdx++) {
              let currUnique = options.uniqueKeys[keys[fieldIdx]];
  ...
  _generateUniqueConstraintName(table, columns) {
    ...
    if (indexName.length > 30) {
      ...
      if (indexName.length > 30) {
        let crcName = crc32(`${table}_${columns.join('')}`);
  ...
  showTablesQuery() {
    ...
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.gt(this.sequelize.options.databaseVersion, '12.0.0')) {
      ...
      //Version before 12, searching from dba_users
      subQuery = `SELECT USERNAME as "schema_name" FROM DBA_USERS WHERE DEFAULT_TABLESPACE NOT IN ('SYSTEM', 'SYSAUX') AND USERNAME NOT IN ('SYS','SYSTEM')`;
    }

    let sql = [`SELECT DISTINCT table_name, lvl, owner as table_schema `,
    `FROM ( `,
  ...
  dropTableQuery(tableName) {
    ...
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '12.0.0')) {
      //Second query to delete the sequence, don't know if it's really a table with autoincrement, in doubt, we drop the sequence and catch the potential not exist exception
      let seqName = this._generateSeqName(table);
      //Virer les ""
      query.push(`/MORE/BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE ${seqName}';`);
      query.push(" EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN");
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    var query
      ...
      , isMinorTwelve = semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '12.0.0');
    ...
    for (key in valueHash) {
      if (valueHash.hasOwnProperty(key)) {
        ...
        if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && !value) {
          if (isMinorTwelve) {
            let seqName = this._generateSeqName(realTableName);
            values.push(`${seqName}.nextval`);
          } else {
            values.push('DEFAULT');
          }
    ...
    primaryKeys.forEach(element => {
      if (isMinorTwelve && element.autoIncrement) {
        ...
        if (fields.indexOf(fieldName) === -1) {
          let seqName = this._generateSeqName(realTableName);
          values.unshift(`${seqName}.nextval`);
          fields.unshift(element.fieldName);
        }
      }
      if (element.field.toLowerCase() === 'uid') {
  ...
  bulkInsertQuery(tableName, attrValueHashes, options, attributes) {
    var query = 'INSERT ALL INTO <%= table %> (<%= attributes %>) WITH rowAttr AS (<%= rows %>) SELECT * FROM rowAttr;'
      ...
      , fullInsertQuery = "EXECUTE IMMEDIATE 'INSERT INTO <%= table %> (<%= attributes %>) VALUES (<%= values %>)'"
      , tuples = []
      , allAttributes = []
      , needIdentityInsertWrapper = false
      , allQueries = []
      , realTableName = this.quoteTable(tableName)
      , self = this
      , isMinorTwelve = semver.valid(self.sequelize.options.databaseVersion) && semver.lt(self.sequelize.options.databaseVersion, '12.0.0');
    ...
    Utils._.forEach(attrValueHashes, function (attrValueHash) {
      ...
      if (fields.length === 1 && attributes[fields[0]].autoIncrement && attrValueHash[fields[0]] === null) {
        ...
        if (isMinorTwelve) {
          let seqName = self._generateSeqName(realTableName);
          attrValueHash[fields[0]] = `${seqName}.nextval`;
      ...
      Utils._.forOwn(attrValueHash, function (value, key) {
        if (value === null && attributes[key].autoIncrement) {
          if (isMinorTwelve) {
            let seqName = self._generateSeqName(realTableName);
            attrValueHash[key] = `${seqName}.nextval`;
            needIdentityInsertWrapper = true;
          }
        }
    ...
    if (isMinorTwelve && needIdentityInsertWrapper) {
      //Oracle < 12 doesn't support select union with sequence.nextval, so we have to chain multiple insert 
        let queryAttributes = [];
        let allQueries = [];
        Utils._.forEach(attrValueHashes, function (attrValueHash, idx, array) {
          let values = allAttributes.map(function (key) {
            if (idx === 0) {
              queryAttributes.push(this.quoteIdentifier(key));
            }
            if (attributes[key].autoIncrement) {
              return attrValueHash[key];
            } 
            let value = this.escape(attrValueHash[key]);
          ...
          let generatedQuery = Utils._.template(fullInsertQuery)(replacements);
          allQueries.push(generatedQuery);
        }.bind(this));
        
        let completeQuery = "BEGIN " + allQueries.join(';') + "; END;";
      ...
      if (allAttributes.length > 0) {
        ...
        Utils._.forEach(attrValueHashes, function (attrValueHash, idx, array) {
          //Generating the row
          let row = "SELECT ";
          let attrs = allAttributes.map(function (key) {
    ...
    if (attribute.type instanceof DataTypes.ENUM) {
      ...
      if (attribute.autoIncrement) {
        if (semver.valid(this.sequelize.options.databaseVersion) && !semver.lt(this.sequelize.options.databaseVersion, '12.0.0')) {
          ...
          //We set identity BUT this is just an information for the create table, it will be removed
          template = ' NUMBER(*,0) NULL IDENTITY';
        }
      } else {
  ...
  attributesToSQL(attributes, options) {
    ...
    for (const key in attributes) {
      ...
      let attributeName = attribute.field || key;
  ...
  getConstraintsOnColumn(table, column) {
    ...
    var sql = ['SELECT CONSTRAINT_NAME FROM user_cons_columns WHERE TABLE_NAME = \'',
  ...
  getPrimaryKeyConstraintQuery(tableName) {
    ...
    let sql = ['SELECT cols.column_name, atc.identity_column ',
  ...
  dropIdentityColumn(tableName, columnName) {
    let table = this.quoteTable(tableName);
  ...
  selectFromTableFragment(options, model, attributes, tables, mainTableAs, where) {
    let topFragment = '';
    // let mainFragment = 'SELECT ' + attributes.map(f => {return this.quoteIdentifier(f);}).join(', ') + ' FROM ' + tables;
    let mainFragment = 'SELECT ' + attributes.join(', ') + ' FROM ' + tables;
    /*if (mainTableAs) {
      mainFragment += " " + mainTableAs;
    }
    return mainFragment;*/
    //delete options.where; // if where ici
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '12.0.0')) {
      if ((options.limit && !options.include) || (options.limit > 1 && options.include)) {
        let keys = Object.keys(options);
        if (keys.length === 3) {
          if (keys.includes("attributes") && keys.includes("where") && keys.includes("limit") && options.limit === 1) {
            //Case include, doing nothing
            if (mainTableAs) {
              mainFragment += " " + mainTableAs;
            }
            return mainFragment;
        ...
        const tmpTable = tables + (mainTableAs ? ' '+ mainTableAs : 'OffsetTable');
        const whereFragment = where ? ' WHERE ' + where : '';
        let fullOffsetCount = offset + options.limit;
        let whereFrag = `WHERE ROWNUM <=${fullOffsetCount} `;

        const frag = [
        'SELECT ' + attributes.map(f => { return 'a.' + f;}).join(', ') +' FROM (',
        'SELECT ' + attributes.map(f => {
          if (f.indexOf(' ') > -1) {
            return f.split(' ')[0];
          }
          return f;
        ...
        if (options.offset) {
          frag.push(` WHERE rnum >= ${options.offset} `);
        }
        return frag.join('');
        ...
        let order = this.getQueryOrders(options, model, "")["mainQueryOrder"];
        if (order) {
        ...
        if (options.offset) {
          query.push(` WHERE rnum >= ${options.offset} `);
        }*/
  ...
  addLimitAndOffset(options, model) {
    ...
    if (semver.valid(this.sequelize.options.databaseVersion) && semver.lt(this.sequelize.options.databaseVersion, '12.0.0')) {
      if ((options.limit && !options.include) || (options.limit > 1 && options.include)) {
        ...
        return `) a WHERE ROWNUM <=${options.limit} `;
      }
      return '';
    }
    var fragment = '';

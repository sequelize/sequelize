...
//TODO add vérification avant création de la non existence de la table
  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query
      ...
      //With Oracle, to return to get the inserted row, we have to pass through a anonymous block that prints the id
      //then the id is read in the callback of execute and a select statement of the row is made (in the execute statement) 
      var returningQuery = "DECLARE\n" + "Inserted_Value <%=primaryKeyType %>;\n" + "v <%=primaryKeyType %>;\n";
	    returningQuery += "BEGIN \n" + "<%=valueQuery %> RETURNING <%=primaryKey %> INTO Inserted_Value;\n";
	    returningQuery += "SELECT Inserted_Value INTO v FROM DUAL;\n" + "DBMS_OUTPUT.PUT_LINE(v);\nEND;\n";
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, attributes) {
    var query = 'INSERT INTO <%= table %> (<%= attributes %>) VALUES <%= tuples %>;'
      ...
      //In oracle bulk create, we don't return the rows inserted

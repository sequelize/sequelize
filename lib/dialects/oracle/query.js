...
class Query extends AbstractQuery {
  ...
  getExecOptions() {
    let self = this;
    let execOpts = { outFormat : self.outFormat, autoCommit : self.autoCommit};
  ...
  _run(connection, sql, parameters) {
    ...
    // we use bindbyname for binding in and out params.
    // TODO: modify to use bindbypos.
    const outParameters = {};
    let bindParameters = (parameters === undefined) ? {}: parameters; // input and output bindParameters
    ...
    if (!this.options.raw) {
      ...
      if ((Object.keys(outParameters).length > 0)) {
        bindParameters = self.outParameters;
        if (parameters != undefined ) {
          // construct input binds from parameters
          // ex: {a1: {val:3, dir: BIND_IN}, a2: {val:4, dir: BIND_IN},...
          let bindNamestr = 'a'; // appending character to make it bindbyname
          for (var key in parameters) {
            if (parameters.hasOwnProperty(key)) {
              let bindpos = parseInt(key, 10);
              bindpos = bindpos + 1;
              let ret = bindNamestr.concat(bindpos.toString());
              bindParameters[ret] = parameters[key];
            }
          }
        }
      }
    ...
    if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
      ...
      return connection
        .execute(self.sql, bindParameters, { autoCommit: self.autoCommit })
        .then(() => {
          return {};
        })
        .catch(error => {
      ...
      return connection
        .execute(self.sql, self.bindParameters, execOpts)
        .then(result => {
          if (benchmark) {
            self.sequelize.log(

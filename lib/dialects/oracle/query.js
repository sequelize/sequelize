...
_run(connection, sql, parameters) {
  ...
  if (benchmark) {
    ...
    if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
      ...
      let promises = [];
      if (self.sql.indexOf('/MORE/') > -1) {
        //Separating chained calls (Ora 11 -> create table / sequence / trigger)

        const doAfter = function(sqls, idx, connection, connectionOpts) {
          if (sqls.length > 0) {
            if (idx < sqls.length) {
              let sql = sqls[idx];
              idx++;
              //While elements, we execute the query
              return connection.execute(sql, [], connectionOpts)
              .then(() => {
                return doAfter(sqls, idx, connection, connectionOpts);
              });
            } else {
              //Done, we get out
        ...
        let sqls = self.sql.split('/MORE/');
        return doAfter(sqls, 0, connection, { outFormat: self.outFormat, autoCommit : self.autoCommit });
      } else {

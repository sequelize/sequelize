  ...
  */
  formatResults(data) {
    ...
    if (this.isInsertQuery(data)) {
      ...
      const keys = [];
      if (data.outBinds) {
        keys.push(...Object.keys(this.options.outBindAttributes));
        insertData = data.outBinds;
      }
      // For single row insert statment this.instance is valid and we map the
      // values received from DB to this.options.outBindAttributes keys
      if (this.instance) {
        let obj = {};
        for (const k in keys) {
          obj[keys[k]] = insertData[k][0];
        }
        insertData = [obj];
      }

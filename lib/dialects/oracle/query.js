  ...
  formatResults(data) {
    ...
    if (this.isInsertQuery(data)) {
      ...
      data.rows.forEach(_result => {
        ...
        if (!(_result.COLUMN_NAME in result)) {
          let key = _result.COLUMN_NAME;

          if (modelAttributes.length > 0) {
            for (let i = 0; i < modelAttributes.length; i++) {
              if (modelAttributes[i] === key) {
      ...
      if (rows.length > 0) {
        ...
        if (!this.model) {
          ...
          if (this.isSelectCountQuery() && this.sql.toLowerCase().indexOf('group') === -1) {
            ...
            if (this.sequelize.options.quoteIdentifiers === true) {
            ...
            rows.forEach(row => {
              ...
              rowKeys.forEach(rowKey => {
                ...
                if (this.sequelize.options.quoteIdentifiers === true) {
                  ...
                  fieldMapKey = rowKey.toLowerCase();
                }
                if (_.includes(mapKeys, fieldMapKey)) {
                  //We have a fieldMap for the names
                  outKey = this.options.fieldMap[fieldMapKey];
                } else {
                  outKey = fieldMapKey;
                }
                const value = {};
        ...
        // Attribute keys obtained from model raw attributes map        
        ...
        attrKeys.forEach(attrKey => {
          ...
          if (this.sequelize.options.quoteIdentifiers === true) {
            attrs[attribute.fieldName] = attribute.fieldName;
          } else {
      ...
      for (let rowsIdx = 0; rowsIdx < rows.length; rowsIdx++) {
        ...
        for (let keysIdx = 0; keysIdx < keys.length; keysIdx++) {
          ...
          //Oracle returns everything in uppercase until quoted, so we have to transform this if quote identifier option is false
          ...
          if (key.indexOf('.') > -1) {
            ...
            if (this.options && this.options.includeMap) {
              ...
              for (let i = 0; i < includeKeys.length; i++) {
                let parts0 = parts[0];
                let includeKeysi = includeKeys[i];
                if (this.sequelize.options.quoteIdentifiers === false) {
                  parts0 = parts0.toUpperCase();
                  includeKeysi = includeKeysi.toUpperCase();
                }
                if (parts0 === includeKeysi) {
            ...
            if (attrs[keyVal] === undefined) {
              ...
              let typeid;
              if (this.sequelize.options.quoteIdentifiers === true) {
                typeid = this.model.rawAttributes[attrs[key]].type.toLocaleString();
              } else {
                typeid = this.model.rawAttributes[attrs[key.toLowerCase()]].type.toLocaleString();
              }
      ...
      result = [{isNewRecord: data[0].isUpdate, value: data[0]}, data[0].isUpdate == 0];

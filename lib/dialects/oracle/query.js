...
Query.prototype._run = function (connection, sql, parameters) {
  ...
  //Method to generate the object for Oracle out bindings
  //Deal with out parameters, specified format -> {:name{type}} 
  var outParameters = {};
  while (this.sql.indexOf('{') > -1) { //if we have multiple params
    var firstIdx = this.sql.indexOf('{');
    var lastIdx = this.sql.indexOf('}') + 1;
    //We extract the full param
    var fullParam = this.sql.substring(firstIdx, lastIdx + 1);
    var param = fullParam.replace('{', '').replace('}', '');
    //We extract the type
    var type = param.substring(param.indexOf('{'), param.indexOf('}') + 1);
    //We extract the name of the parameter
    var fullName = param.replace(type, '').replace(':', '');
    type = type.replace('{', '').replace('}', ''); //remove { and } from the type

    switch (type) {
      case 'INTEGER':
      case 'NUMBER':
        outParameters[fullName] = { dir: oracledb.BIND_OUT, type: oracledb.NUMBER };
        break;
      case 'STRING':
        outParameters[fullName] = { dir: oracledb.BIND_OUT, type: oracledb.STRING };
        break;
      default:
    ...
    //Finally we replace the param in the sql :  ':name{type}'  -> :name

    if(this.sql.indexOf(fullParam) > -1 && this.sql.indexOf("'" + fullParam + "'") > -1) {
      //Case for stored procedures
      this.sql = this.sql.replace("'" + fullParam + "'", `:${fullName}`);
    } else {
      this.sql = this.sql.replace(fullParam, `:${fullName}`);
    }
  ...
  var promise = new Promise(function (resolve, reject) {
    ...
    if (_.includes(self.sql, 'BEGIN TRANSACTION')) {
      ...
      return connection.execute(self.sql, outParameters)
        .then(result => {

...
ConnectionManager = function(dialect, sequelize) {
  ...
  this.sequelize.config.port = this.sequelize.config.port || 5433;
  try {
  ...
  this.refreshTypeParser(dataTypes.vertica);
};
...
ConnectionManager.prototype.$refreshTypeParser = function (dataType) {
  ...
  if (dataType.types.vertica.oids) {
    dataType.types.vertica.oids.forEach(function (oid) {
  ...
  if (dataType.types.vertica.array_oids) {
    dataType.types.vertica.array_oids.forEach(function (oid) {
      self.lib.types.setTypeParser(oid, function (value) {
...
ConnectionManager.prototype.connect = function(config) {
  ...
  if (config.dialectOptions) {
    Utils._.merge(connectionConfig,
      ...
      // see [http://www.verticaql.org/docs/9.3/static/runtime-config-logging.html#GUC-APPLICATION-NAME]
      'application_name',
      // choose the SSL mode with the PGSSLMODE environment variable
      // object format: [https://github.com/brianc/node-vertica/blob/master/lib/connection.js#L79]
      // see also [http://www.verticaql.org/docs/9.3/static/libpq-ssl.html]
      'ssl',
      ...
      // see [http://www.verticaql.org/message-id/flat/bc9549a50706040852u27633f41ib1e6b09f8339d845@mail.gmail.com#bc9549a50706040852u27633f41ib1e6b09f8339d845@mail.gmail.com]
      'binary'
    ]));
  }
  ...
  return new Promise(function (resolve, reject) {
    ...
    // If we didn't ever hear from the client.connect() callback the connection timeout, node-vertica does not treat this as an error since no active query was ever emitted
    ...
    //if (dataTypes.HSTORE.types.vertica.oids.length === 0) {
    ...
    return new Promise(function (resolve, reject) {
      connection.query(query).on('error', function (err) {
        ...
        if (row.typname === 'geometry') {
          type = dataTypes.vertica.GEOMETRY;
        } else if (row.typname === 'hstore') {
          type = dataTypes.vertica.HSTORE;
        } else if (row.typname === 'geography'){
          type = dataTypes.vertica.GEOGRAPHY;
        }

        type.types.vertica.oids.push(row.oid);
        type.types.vertica.array_oids.push(row.typarray);

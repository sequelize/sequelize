...
var QueryGenerator = {
  ...
  handleSequelizeMethod: function (smth, tableName, factory, options, prepend) {
    if (smth instanceof Utils.json) {
      ...
      if (smth.conditions) {
        ...
        // Allow specifying conditions using the vertica json syntax
  ...
  // http://www.maori.geek.nz/post/vertica_upsert_update_or_insert_in_ger_using_knex_js
  ...
  attributeToSQL: function(attribute) {
    ...
    if(_.values(attribute).indexOf('fraudSeveritySource') > -1) {
  ...
  dataTypeMapping: function(tableName, attr, dataType) {
    ...
    if (Utils._.includes(dataType, 'INTEGER')) {
      if (Utils._.includes(dataType, 'BIGINT')) {
        dataType = dataType.replace(/INTEGER/, 'BIGINTEGER');
        dataType = dataType.replace(/BIGINT/, '');
      } else {
        dataType = dataType.replace(/INTEGER/, 'INT');
      }
  ...
  setIsolationLevelQuery: function(value, options) {
    ...
    return 'SET SESSION CHARACTERISTICS AS TRANSACTION ISOLATION LEVEL ' + value + ';';
  }
};

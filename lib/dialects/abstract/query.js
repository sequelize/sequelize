...
module.exports = (function() {
      ...
      if (this.sql.toLowerCase().indexOf('describe') === 0) {
        ...
        result = Utils._.uniq(result.map(function(result) {
          return {
            ...
            // these names align with addIndex parameters:
            indexName:  result.Key_name,
            attributes: [result.Column_name],
          }
        }), false, function(row) {
          return row.name
        })
  ...
  */
  ...
  var buildAssociatedDaoInstances = function(tableName, associationData, dao) {
    ...
    var accessor = associatedDaoFactory.options.underscored ? tableName : Utils._.camelize(tableName)
    ...
    associationData.forEach(function(data) {
      ...
      if (association && ['BelongsTo', 'HasOne'].indexOf(association.associationType) > -1) {

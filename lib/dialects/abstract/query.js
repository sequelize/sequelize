...
const logger = require('../../utils/logger').Logger.getLogger();
...
class AbstractQuery {
  ...
  _logQuery(sql, debugContext) {
    ...
    let fmt = `(${connection.uuid || 'default'}): ${sql}`;
    if (options.bind) {
      fmt += `\n  BOUND:\n  ${options.bind.join('\n  ')}`;
    }
  ...
  /**
   * Gets a debug context for sub classes.
   *
   * @param {string} dialect
   * @private
   */
  static makeDebugContext(dialect) {
    return logger.debugContext(`sql:${dialect}`);
  }

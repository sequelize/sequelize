  ...
  var handleInsertQuery = function(results, metaData) {
    if (this.callee) {
      ...
      var autoIncrementField = this.callee.__factory.autoIncrementField
        , attributes         = this.callee.daoFactory.rawAttributes
        , id                 = null
      ...
      Object.keys(this.callee.daoFactory.rawAttributes).forEach(function(key){
        var fct = attributes[key].get || (attributes[key].type && attributes[key].type.get) || undefined
        if (fct) {
          this.callee[key] = fct(this.callee[key]);
        }
      }.bind(this))
    }
  }

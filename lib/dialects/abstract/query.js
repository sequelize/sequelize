...
class AbstractQuery {
   ...
   */
  _logQuery(sql, parameters, debugContext) {
    const { connection, options } = this;
    const benchmark = this.sequelize.options.benchmark || options.benchmark;
    const startTime = Date.now();
    const fmt = `(${connection.uuid || 'default'}): ${sql}`;
    // For performance of non-debug queries, inspect parameters with formatter
    debugContext(`Executing ${fmt} with parameters %O`, parameters);
    if (!benchmark) {
      this.sequelize.log(`Executing ${fmt} with parameters ${logger.inspect(parameters)}`, options);
    }
    return () => {
      debugContext(`Executed ${fmt} with parameters %O`, parameters);
      if (benchmark) {
        this.sequelize.log(`Executed ${fmt} with parameters ${logger.inspect(parameters)}`, Date.now() - startTime, options);
      }
    };
  }

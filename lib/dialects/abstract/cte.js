...
const debug = Utils.getLogger().debugContext('abstract:cte');
  ...
  generateCTEQuery(topLevelInfo) {
    ...
    topLevelInfo.options.cte.forEach(cte => {
      ...
      if (cteModel) {
        ...
        // _.forIn(cteModel.rawAttributes, (attr, name) => {
          cteModelAttributes.push(name);
          // cteModelAttributes.push(attr);
        });
      }

      // debug('cteModelAttributes %O', cteModelAttributes);

      cteAllAttributes = cteModelAttributes.concat(cteAdditionalAttributes);

      debug('cteAllAttributes %O', cteAllAttributes);

      const cteAttributes = cteAllAttributes.map(this.escapeAttribute, this).join(',');


      debug('cteAttributes %O', cteAttributes);

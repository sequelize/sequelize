...
function quoteIdentifier(dialect, identifier, options) {
  ...
  switch (dialect) {
    ...
    case 'oracle':
      const rawIdentifierOracle = Utils.removeTicks(identifier, '"');
      const regExp = (/^(([\w][\w\d_]*))$/g);
      if (
        options.force !== true &&
        options.quoteIdentifiers === false &&
        regExp.test(rawIdentifierOracle) === true &&
        !oracleReservedWords.includes(rawIdentifierOracle.toUpperCase())
      ) {
        ...
        return rawIdentifierOracle;
      }
      return Utils.addTicks(rawIdentifierOracle, '"');

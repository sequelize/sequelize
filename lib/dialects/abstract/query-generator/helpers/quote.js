...
function quoteIdentifier(dialect, identifier, options) {
  ...
  switch (dialect) {
    ...
    case 'mssql':
      let rawIdentifier = Utils.removeTicks(identifier, '"');
        // In MSSQL, the default quoted identifier is double-quoted,
        // So we need to specify it. However, we include the option
        // to strip quotes for users who have specified a custom schema.
      let modIdentifier;
      if (
        options.force !== true &&
        options.quoteIdentifiers === false
      ) {
        modIdentifier = rIdentifier;
      } else {
        modIdentifier = Utils.addTicks(rIdentifier, '"');
      }
      return `[${modIdentifier.replace(/[[\]']+/g, '')}]`;

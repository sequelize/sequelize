...
const QueryGenerator = {
  ...
  */
  arithmeticQuery(operator, tableName, attrValueHash, where, options, attributes) {
    ...
    for (const key in attrValueHash) {
      const value = attrValueHash[key];
      const negativeValueIsNotSqlComment = (typeof value === 'number' && value < 0) ? ' ' : '';
      values.push(this.quoteIdentifier(key) + '=' + this.quoteIdentifier(key) + operator + negativeValueIsNotSqlComment + this.escape(value));
    }

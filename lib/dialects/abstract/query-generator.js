  ...
  */
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      let attributes = include.attributes.map(attr => {
        let attrAs = attr;
        let verbatim = false;
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    if (Array.isArray(options.order)) {
      ...
      mainQueryOrder.push(this.quote(typeof options.order === 'string' ? new Utils.Literal(options.order) : options.order, model, false, '->'));
    }

    ...
    const quoteOrder = order => {
      ...
      if (Array.isArray(order)) {
        ...
        if (typeof order[order.length - offset] === 'string') {
          ...
          // if the attribute identifiers length is > 1
          if (attributeIdentifiers.length > 1 && model !== undefined) {
            ...
            attributeIdentifiers.forEach((identifier, identifierIndex) => {
              ...
              // this is the first assoication so use the model
              if (identifierIndex === 0 && model.associations !== undefined && model.associations[identifier] !== undefined) {
                association = {
                  model: model.associations[identifier].target,
                  as: model.associations[identifier].as
                };
                ...
                association = {
                  model: order[previousOrderIndex].model.associations[identifier].target,
                  as: order[previousOrderIndex].model.associations[identifier].as
                };
              }

              // splice in the order association before the attribute
              if (association.model) {
                order.splice(order.length - offset, 0, association);
              } else if (identifier !== attribute && association.model === undefined) {
                throw new Error(util.format('Order association cannot be found for identifier, \'%s\' given', identifier));
              }
            });

            // set order model as the last order model

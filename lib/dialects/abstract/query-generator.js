...
class QueryGenerator {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (this.options.dialectOptions && this.options.dialectOptions.isSelectQueryBindingAvailable === false) {
      bindParam = false;
    }
    ...
    if (options.include) {
      for (const include of options.include) {
        ...
        let joinQueries;
        if (this.dialect === 'mssql') {
          joinQueries = this.generateInclude(include, { externalAs: mainTable.as, internalAs: mainTable.as }, topLevelInfo, bindParam, bind);
        } else {
          joinQueries = this.generateInclude(include, { externalAs: mainTable.as, internalAs: mainTable.as }, topLevelInfo)
        }
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        let baseQuery; // Every derived table must have its own alias
        if (this.dialect === 'mssql') {
          if (Array.isArray(selectQuery.bind)) {
            bind.push(...selectQuery.bind);
          } else {
            bind.push(selectQuery.bind);
          }

          baseQuery = `SELECT * FROM (${selectQuery.query.replace(/;$/, '')}) AS sub`;
        } else {
          baseQuery = `SELECT * FROM (${selectQuery.replace(/;$/, '')}) AS sub`;
        }
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'where') && !options.groupedLimit) {
      if (this.dialect === 'mssql') {
        ...
        if (_.get(options.where, [Op.placeholder])) {
          // Not currently supported with placeholder operation
          options.bindParam = false;
        }
      }
  ...
  generateInclude(include, parentTableName, topLevelInfo, bindParam, bind) {
    const joinQueries = {
      mainQuery: [],
      subQuery: []
    };
    ...
    if (include.through) {
      if (bindParam) {
        joinQuery = this.generateThroughJoin(include, includeAs, parentTableName.internalAs, topLevelInfo, bindParam, bind);
      } else {
      ...
      if (bindParam) {
        ...
        this._generateSubQueryFilter(include, includeAs, topLevelInfo, bind);
      } else {
    ...
    if (include.include) {
      for (const childInclude of include.include) {
        ...
        let childJoinQueries;
        if (bindParam) {
          childJoinQueries = this.generateInclude(childInclude, includeAs, topLevelInfo, bindParam, bind);
        } else {
          childJoinQueries = this.generateInclude(childInclude, includeAs, topLevelInfo);
        }
  ...
  generateJoin(include, topLevelInfo, bindParam) {
    ...
    if (include.where) {
      if (bindParam) {
        if (_.get(include.where, [Op.placeholder])) {
          // Not currently supported with placeholder operation
          bindParam = false;
        }
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo, bindParam, bind) {
    ...
    if (include.where || include.through.where) {
      if (include.where) {
        if (bindParam) {
          targetWhere = this.getWhereConditions(include.where, this.sequelize.literal(this.quoteIdentifier(includeAs.internalAs)), include.model, { ...topLevelInfo.options, bindParam });
        } else {
   ...
   * given include.
   */
  _generateSubQueryFilter(include, includeAs, topLevelInfo, bind) {
    ...
    if (topInclude.through && Object(topInclude.through.model) === topInclude.through.model) {
      selectQuery = this.selectQuery(topInclude.through.model.getTableName(), {
      ...
      selectQuery = this.selectQuery(topInclude.model.getTableName(), {
    ...
    const query = _.isObject(selectQuery) ? selectQuery.query : selectQuery;
  ...
  _whereBind(binding, key, value, options) {
    if (_.isPlainObject(value)) {
      value = Utils.getComplexKeys(value).map(prop => {
        ...
        if (item === null) {
          return this.whereItemQuery(key, { [prop]: item }, { ...options, bindParam: false });
        }
  ...
  _whereParseSingleValueObject(key, field, prop, value, options) {
    ...
    switch (prop) {
      ...
      case Op.notIn:
        ...
        if (value.length) {
          const hasLiterals = value.some((item) => item instanceof Utils.Literal);

          if (hasLiterals) {
            return this._joinKeyValue(key, `(${value.map(item => this.escape(item, field)).join(', ')})`, comparator, options.prefix);
          }

          const opValueIn = options.bindParam ? value.map(item => this.format(item, field, options, options.bindParam)).join(', ') : value.map(item => this.escape(item, field)).join(', ');
          return this._joinKeyValue(key, `(${opValueIn})`, comparator, options.prefix);
        }
      ...
      case Op.all:
        ...
        if (value[Op.values]) {
          const opValueAny = options.bindParam ? value[Op.values].map(item => `(${this.format(item, null, options, options.bindParam)})`).join(', ') : value[Op.values].map(item => `(${this.escape(item)})`).join(', ');
          return this._joinKeyValue(key, `(VALUES ${opValueAny})`, comparator, options.prefix);
        }

        return this._joinKeyValue(key, `(${options.bindParam ? this.format(value, field, options, options.bindParam) : this.escape(value, field)})`, comparator, options.prefix);
      case Op.between:
      case Op.notBetween:
        const opValueBetween1 = options.bindParam ? this.format(value[0], field, options, options.bindParam) : this.escape(value[0], field);
        const opValueBetween2 = options.bindParam ? this.format(value[1], field, options, options.bindParam) : this.escape(value[1], field);
        return this._joinKeyValue(key, `${opValueBetween1} AND ${opValueBetween2}`, comparator, options.prefix);
      case Op.raw:
      ...
      case Op.col:
        ...
        const opValueCol = options.bindParam ? value.map(identifier => this.format(identifier, null, options, options.bindParam)).join('.') : value.map(identifier => this.quoteIdentifier(identifier)).join('.');
        return this._joinKeyValue(key, opValueCol, comparator, options.prefix);
      case Op.startsWith:
      case Op.endsWith:
      case Op.substring:
        ...
        const opValueSubstr = options.bindParam ? this.format(pattern, null, options, options.bindParam) : this.escape(pattern);
        return this._joinKeyValue(key, opValueSubstr, comparator, options.prefix);
    }
    ...
    if (_.isPlainObject(value)) {
      ...
      if (value[Op.any]) {
        ...
        const opValue = options.bindParam ? this.format(value[Op.any], field, { ...options, ...escapeOptions }, options.bindParam) : this.escape(value[Op.any], field, escapeOptions);
        return this._joinKeyValue(key, `(${opValue})`, `${comparator} ${this.OperatorMap[Op.any]}`, options.prefix);
      }
      if (value[Op.all]) {
        escapeOptions.isList = true;
        const opValue = options.bindParam ? this.format(value, field, { ...options, ...escapeOptions }, options.bindParam) : this.escape(value[Op.all], field, escapeOptions);
        return this._joinKeyValue(key, `(${opValue})`, `${comparator} ${this.OperatorMap[Op.all]}`, options.prefix);
      }
    }
  ...
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    if (Array.isArray(smth)) {
      ...
      if (Utils.canTreatArrayAsAnd(smth)) {
        ...
        if (options.bindParam) {
          return this.getWhereConditions(_smth, tableName, factory, { ...options, bindParam: options.bindParam }, prepend);
        }

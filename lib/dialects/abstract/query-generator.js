...
var QueryGenerator = {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    if (subQuery) {
      subQueryItems.push(this.selectSqlFragmentBuilder(options, model, subQueryAttributes, table, mainTableAs));
    ...
    // Else do it the reguar way
      ...
      if (options.groupedLimit) {
        ...
        mainQueryItems.push(this.selectSqlFragmentBuilder(options, model, mainAttributes, '('+
        ...
        mainQueryItems.push(this.selectSqlFragmentBuilder(options, model, mainAttributes, table, mainTableAs));
      }
    ...
    if (options.hasOwnProperty('where') && !options.groupedLimit) {
      ...
      if (options.where) {
        if (subQuery) {
          ...
          // Walk the array backwards to find the last instance
          for(var i = mainQueryItems.length - 1; i >= 0; --i) {
            if(mainQueryItems[i].match(/^SELECT/)) {
              var string = this.selectSqlFragmentBuilder(options, model, mainAttributes, table, mainTableAs, options.where);
  ...
  selectSqlFragmentBuilder: function(options, model, attributes, tables, mainTableAs, whereClause) {

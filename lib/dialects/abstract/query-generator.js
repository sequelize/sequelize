...
const QueryGenerator = {
  ...
  */
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    const getOrderOption = order => {
      const upperOrder = order.toUpperCase();
      let index = -1;
      let option = false;

      for (const v of validOrderOptions) {
        if(_.endsWith(upperOrder, v)){
          option = v;
          index = upperOrder.indexOf(v);
        }
      }
    ...
    if (Array.isArray(options.order)) {
      ...
      // split
      for (let order of options.order.split(',')) {
        // get options
        const orderOption = getOrderOption(order);
        if(orderOption.index > -1){
          order = order.substring(0, orderOption.index);
        }
        order = _.trim(order);
        let sql = this.quote(order, model, false, '->');
        if(orderOption.option){
          sql += ' ' + orderOption.option;
        }
        if (subQuery) {
      ...
      var sql = this.quote(options.order, model, false, '->');
      if (subQuery) {
        subQueryOrder.push(sql);
      }
      mainQueryOrder.push(sql);

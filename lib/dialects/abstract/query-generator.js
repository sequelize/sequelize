...
/**
 * Does a where condition belong on the main table (as opposed to a joined table)?
 *
 * @param {string|Symbol} [key] where condition key
 * @param {Array<string>} [subTableNames] array of joined table names
 *
 * @returns {boolean}
 */
function conditionIsMain(key, subTableNames) {
  if (typeof key !== 'string') {
    return true;
  }

  const literalMatch = key.match(/^\$(\w+)\.\w+\$$/);
  if (!literalMatch) {
    return true;
  }

  const [, table] = literalMatch;

  return !subTableNames.includes(table);
}

/**
 * Does an object or array of where conditions belong on the main table (as opposed to a joined table)?
 *
 * @param {Object|Array|any} [conditionList] where condition group
 * @param {Array<string>} [subTableNames] array of joined table names
 *
 * @returns {boolean}
 */
function conditionListIsMain(conditionList, subTableNames) {
  if (Array.isArray(conditionList)) {
    return conditionList.some(subConditionList => conditionListIsMain(subConditionList, subTableNames));
  }
  if (typeof conditionList !== 'object') {
    return true;
  }

  return Object.keys(conditionList).some(key => conditionIsMain(key, subTableNames));
}

/**
 * Does a root key on the where condition belong on the main table?
 *
 * @param {Object|Array} [where] where conditions
 * @param {Array<string>} [subTableNames] array of joined table names
 *
 * @returns {boolean}
 */
const conditionKeyIsMain = (where, subTableNames) => key =>
  conditionIsMain(key, subTableNames) &&
  conditionListIsMain(where[key], subTableNames);

/**
 * Does a root key on the where condition belong on a joined table?
 *
 * @param {Object|Array} [where] where conditions
 * @param {Array<string>} [subTableNames] array of joined table names
 *
 * @returns {boolean}
 */
const conditionKeyIsJoin = (where, subTableNames) => {
  const isMain = conditionKeyIsMain(where, subTableNames);

  return key => !isMain(key);
};

/**
 * Removes all the dollar symbols on joined table where condition keys
 *
 * @param {Object|Array} [where] where conditions
 * @param {Array<string>} [subTableNames] array of joined table names
 *
 * @returns {Object}
 */
function whereJoinMapper(where, subTableNames) {
  const objectMapper = object => {
    ...
    getAllKeys(newObject).forEach(key => {
      const { key: newKey, value } = mapper(key, object[key]);
      if (key !== newKey) {
  ...
  const mapper = (key, object) => {
    let newKey = key;
    if (!conditionIsMain(key, subTableNames)) {
      newKey = key.substring(1, key.length - 1);
    }

    const value = objectMapper(object);
...
/**
 * Get all keys of an object, including keys which are symbols
 *
 * @param {Object} [object] the object to get keys from
 *
 * @returns {Array}
 */
const getAllKeys = object => Object.getOwnPropertySymbols(object).concat(Object.keys(object));

/**
 * Filters a where condition based on a specified filter function,
 * optionally mapping the result based on a specified mapper function
 *
 * @param {Function} [makeFilter]
 * @param {Function} [makeMapper]
 *
 * @returns {Object}
 */
function recursiveWhereFilter(makeFilter, makeMapper = null) {
  const objectHandler = (where, subTableNames) => {
    if (!where) {
      return where;
    }
    if (Array.isArray(where)) {
      return where.map(object => objectHandler(object, subTableNames))
        .filter(object => Object.keys(object).length);
    }

    const whereFiltered = Object.assign({}, where);

    const filter = makeFilter(where, subTableNames);

    let mapper = null;
    if (makeMapper) {
      mapper = makeMapper(where, subTableNames);
    }

    getAllKeys(where).forEach(key => {
      if (!filter(key)) {
        Reflect.deleteProperty(whereFiltered, key);
      } else if (mapper) {
        const { key: newKey, value } = mapper(key, where[key]);

        if (key !== newKey) {
          Reflect.deleteProperty(whereFiltered, key);
        }

        whereFiltered[newKey] = value;
      }
    });
...
/**
 * Filters a where condition, returning only items which should be on
 * the main table
 *
 * @returns {Object}
 */
const getMainTableWhere = recursiveWhereFilter(conditionKeyIsMain);
 ...
 * @returns {Object}
 */
const getJoinTableWhere = recursiveWhereFilter(conditionKeyIsJoin, whereJoinMapper);
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.hasOwnProperty('where') && !options.groupedLimit) {
      const mainTableWhereOptions = subQuery
        ? getMainTableWhere(options.where, subTableNames)
        : options.where;

      const mainTableWhere = this.getWhereConditions(
        mainTableWhereOptions,
      ...
      if (mainTableWhere) {
        if (subQuery) {
          subQueryItems.push(` WHERE ${mainTableWhere}`);
        } else {
          mainQueryItems.push(` WHERE ${mainTableWhere}`);
          // Walk the main query to update all selects
          mainQueryItems.forEach((value, key) => {
            if (value.startsWith('SELECT')) {
              mainQueryItems[key] = this.selectFromTableFragment(options, model, attributes.main, mainTable.quotedName, mainTable.as, mainTableWhere);
            }
          });
        }
      }

      if (subQuery && subTableNames.length) {
        const joinTableWhereOptions = getJoinTableWhere(options.where, subTableNames);

        const joinTableWhere = this.getWhereConditions(joinTableWhereOptions, null, model, options);

        if (joinTableWhere) {
          mainQueryItems.push(` WHERE ${joinTableWhere}`);
        }
      }

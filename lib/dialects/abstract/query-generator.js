  ...
  generateJoin(include, topLevelInfo) {
    ...
    if (association.extraForeignKeys) {
      const extraForeignKeysJoinConditions = association.extraForeignKeys.map(key => {
        const extraForeignKeyConditionLeft = `${this.quoteTable(asLeft)}.${this.quoteIdentifier(key.source)}`;
        const extraForeignKeyConditionRight = `${this.quoteTable(asRight)}.${this.quoteIdentifier(key.target)}`;

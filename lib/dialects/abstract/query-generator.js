  ...
  */
  quote: function(obj, parent, force) {
    if (Utils._.isString(obj)) {
      ...
      return obj.raw;
    } else if (Utils._.isObject(obj) && 'fn' in obj) {
      return '' + obj.fn + '(' + (Array.isArray(obj.cols) ? obj.cols : []).map(function (col) {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    mainAttributes = mainAttributes && mainAttributes.map(function(attr) {
      ...
      } else if (Utils._.isObject(attr) && 'fn' in attr) {
        addTable = false;
        if (options.include && Array.isArray(attr.cols)) {
          attr.cols = attr.cols.map(function (col) {
            return ('string' === typeof(col) && -1 === col.indexOf('.')) ? mainTableAs + '.' + col : col;
          });
        }
        if (attr.as) {
          attr = self.quote(attr) + ' AS ' + self.quoteIdentifier(attr.as);
        } else {
          attr = self.quote(attr);
        }
      } else {
      ...
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (options.includeIgnoreAttributes !== false) {
          ...
          attributes = include.attributes.map(function(attr) {
            var attrAs = attr,
                verbatim = false;
            if (include.model._isGeometryAttribute(attr)) {
              attr = self.geometrySelect(attr);
            }
            ...
            if (Array.isArray(attr) && attr.length === 2) {
              ...
              verbatim = true;
              if (Array.isArray(attr.cols)) {
                attr.cols = attr.cols.map(function(col) {
                  return ('string' === typeof 'col') ? self.quoteIdentifier(as) + '.' + self.quoteIdentifier(col) : col;
                });
              }
              attr = self.quote(attr);
            }

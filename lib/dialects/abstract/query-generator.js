  ...
  generateCTEQuery(topLevelInfo) {
    ...
    topLevelInfo.options.cte.forEach(cte => {
      ...
      if (cteModel) {
        ...
        } else if (!Array.isArray(topLevelInfo.names.name) && topLevelInfo.names.model) {
      ...
      if (initial.where) {
        ...
        cteQueryItems.push(`${this.getWhereConditions(initial.where, modelAs || modelName, topLevelInfo.names.model, topLevelInfo.options)} `);
      }
      ...
      if (recursive) {
        ...
        if (recursive.where) {
          ...
          if (modelConditions) {
            recursiveWhereItems.push(`${this.getWhereConditions(modelConditions, modelAs || modelName, topLevelInfo.names.model, topLevelInfo.options)} `);
          }
          if (cteConditions) {
            if (recursiveWhereItems.length > 1) {
              recursiveWhereItems.push(`AND `);
            }
            recursiveWhereItems.push(`${this.getWhereConditions(cteConditions, cteName, topLevelInfo.names.model, topLevelInfo.options)} `);
          }

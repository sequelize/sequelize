...
var QueryGenerator = {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    mainAttributes = mainAttributes || (options.include || cteSelect ? [mainTableAs + '.*'] : ['*']);
  ...
  generateCTEQuery: function (tableName, options, model) {
    ...
    options.cte.forEach(function (cte) {
      ...
      if (self._dialect.supports.cteLimitOffsetOrder && cte.order && !_.has(cte, 'recursive')) {
      ...
      if (recursive) {
        ...
        if (_.has(cte, 'unique')) {
          cteQueryItems.push(cte.unique ? 'UNION' : 'UNION ALL');
        }
        else {

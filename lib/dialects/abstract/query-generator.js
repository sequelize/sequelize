...
const QueryGenerator = {
  ...
  extractTableDetails(tableName, options) {
    ...
    return {
      ...
      tableName: Utils._.isPlainObject(tableName) ? tableName.tableName : tableName,
  ...
  */
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    if (options.prefix && Utils._.isString(options.prefix)) {
      ...
      if (value._isSequelizeMethod) {
        ...
        if (field && field.type) {
          ...
          if (field.type.stringify) {
            ...
            const simpleEscape = Utils._.partialRight(SqlString.escape, this.options.timezone, this.dialect);
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (subQuery && attributes.main) {
      for (const keyAtt of mainTable.model.primaryKeyAttributes) {
        ...
        if (!Utils._.find(attributes.main, attr => keyAtt === attr || keyAtt === attr[0] || keyAtt === attr[1])) {
    ...
    if (options.hasOwnProperty('where') && !options.groupedLimit) {
      ...
      if (options.where) {
        if (subQuery) {
          ...
          Utils._.each(mainQueryItems, (value, key) => {
    ...
    if (include.where || include.through.where) {
      ...
      if (topLevelInfo.subQuery && include.required) {
        ...
        while (parent = parent.parent) {
          nestedIncludes = [Utils._.extend({}, child, { include: nestedIncludes })];
          child = parent;
        }
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    const methodOrder = order => {
      let result = false;
      if (
          order._isSequelizeMethod
          && (
            order instanceof Utils.Fn
            || order instanceof Utils.Col
            || order instanceof Utils.Literal
          )
      ){
        result = true;
      }
    ...
    const validateOrder = order => {
      ...
      if (!Utils._.includes(validOrderOptions, order.toUpperCase())) {
    ...
    if (Array.isArray(options.order)) {
      for (const order of options.order) {
        if (Array.isArray(order) && Utils._.size(order) > 1) {
          if (Utils._.isFunction(order[0]) && order[0].prototype instanceof Model || Utils._.isFunction(order[0].model) && order[0].model.prototype instanceof Model) {
            if (Utils._.isString(order[order.length - 2])) {
              validateOrder(Utils._.last(order));
            }
          } else {
            validateOrder(Utils._.last(order));
          }
        ...
        if (subQuery && (Array.isArray(order) && !(Utils._.isFunction(order[0]) && order[0].prototype instanceof Model) && !(order[0] && Utils._.isFunction(order[0].model) && order[0].model.prototype instanceof Model))) {
          subQueryOrder.push(this.quote(order, model, false, '->'));
        }

        mainQueryOrder.push(this.quote(order, model, false, '->'));
      }
    } else if (methodOrder(options.order)){
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Where) {
      ...
      } else if (Utils._.isPlainObject(value)) {
      ...
      } else if (Utils._.isPlainObject(smth.val)) {
      ...
      result = smth.fn + '(' + smth.args.map(arg => {
        ...
        } else if (Utils._.isPlainObject(arg)) {
  ...
  whereItemsQuery(where, options, binding) {
    if (
      ...
      (Utils._.isPlainObject(where) && Utils._.isEmpty(where)) ||
      where === null ||
      where === undefined
    ) {
      // NO OP
    ...
    if (Utils._.isPlainObject(where)) {
      Utils._.forOwn(where, (value, key) => {
  ...
  whereItemQuery(key, value, options) {
    ...
    if (Utils._.isPlainObject(value)) {
      Utils._.forOwn(value, (item, key) => {
        if (aliasMap[key]) {
    ...
    if (key === undefined) {
      ...
      if (Utils._.isPlainObject(value) && Utils._.size(value) === 1) {
        key = Object.keys(value)[0];
        value = Utils._.values(value)[0];
      }
    }
    ...
    if (key === '$or' || key === '$and' || key === '$not') {
      ...
      if (Array.isArray(value)) {
        value = value.map(item => {
          ...
          if ((Array.isArray(item) || Utils._.isPlainObject(item)) && Utils._.size(item) > 1) {
            itemQuery = '('+itemQuery+')';
          }
          return itemQuery;
    ...
    if (value && (value.$or || value.$and)) {
      ...
      if (Utils._.isPlainObject(value)) {
        value = Utils._.reduce(value, (result, _value, key) => {
          result.push(Utils._.zipObject([key], [_value]));
          return result;
        }, []);
      }
    ...
    if (Utils._.isPlainObject(value) && fieldType instanceof DataTypes.JSON && options.json !== false) {
      const items = [];
      const traverse = (prop, item, path) => {
        ...
        if (Utils._.isPlainObject(item)) {
          Utils._.forOwn(item, (item, prop) => {
            if (prop.indexOf('$') === 0) {
              where[prop] = item;
              const key = castKey(item);

              items.push(this.whereItemQuery(new Utils.Literal(key), where/*, Utils._.pick(options, 'prefix')*/));
            } else {
      ...
      Utils._.forOwn(value, (item, prop) => {
        if (prop.indexOf('$') === 0) {
          const where = {};
          where[prop] = item;
          items.push(this.whereItemQuery(key, where, Utils._.assign({}, options, {json: false})));
          return;
        }
    ...
    if (Utils._.isPlainObject(value) && Object.keys(value).length > 1) {
      const items = [];
      Utils._.forOwn(value, (item, logic) => {
        const where = {};
        where[logic] = item;
      ...
      if (Utils._.isPlainObject(value)) {
        Utils._.forOwn(value, (item, key) => {
          if (comparatorMap[key]) {
            ...
            if (Utils._.isPlainObject(value) && value.$any) {
              comparator += ' ANY';
              escapeOptions.isList = true;
              value = value.$any;
            } else if (Utils._.isPlainObject(value) && value.$all) {
              comparator += ' ALL';
              escapeOptions.isList = true;
              value = value.$all;
            } else if (value && value.$col) {

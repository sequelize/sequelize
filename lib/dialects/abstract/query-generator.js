...
const QueryGenerator = {
  ...
  */
  ...
  /*
   ...
   @private
  */
  quote(collection, parent, force, connector) {
    // init
    ...
    if (typeof collection === 'string'){
      return this.quoteIdentifiers(collection, force);
      ...
      collection.forEach((item, index) => {
        ...
        if(previous === undefined && parent !== undefined){
          previous = parent;
        } else if (previous !== undefined && previous instanceof Association) {
          previous = previous.target;
        }

        if (previous !== undefined && previous.prototype instanceof Model && typeof item === 'function' && item.prototype instanceof Model) {
          // turn into association assuming the modle name is the alias
          item = previous.getAssociationForAlias(item, item.name);

        } else if (previous !== undefined && previous.prototype instanceof Model && _.isPlainObject(item) && item.model && item.model.prototype instanceof Model) {
          // if ther as property isn't set, then default it to the model name
          if(!item.as){
            item.as = item.model.name;
          }

          // turn object with model and as into an association
          item = previous.getAssociationForAlias(item.model, item.as);
        } else if (_.isPlainObject(item) && item.raw) {
          ...
          } else if (previous !== undefined && previous.prototype instanceof Model && previous.associations !== undefined && previous.associations[item]) {
            // convert the item to an association
            item = previous.associations[item];
          } else if (previous !== undefined && previous.prototype instanceof Model && previous.rawAttributes !== undefined && previous.rawAttributes[item] && item !== previous.rawAttributes[item].field) {
            // convert the item attribute from it's alias
            item = previous.rawAttributes[item].field;
          }
        }
      ...
      for (i = 0; i < collectionLength - 1; i++) {
        item = collection[i];
        if (item._modelAttribute || typeof item === 'string' || item instanceof Utils.SequelizeMethod) {
          break;
      ...
      collection.forEach((collectionItem) => {
        if (collectionItem instanceof Utils.SequelizeMethod) {
          sql += this.handleSequelizeMethod(collectionItem);
        } else if (collectionItem._modelAttribute) {
          sql += this.quoteIdentifier(collection.fieldName);
        } else if (typeof collectionItem === 'string') {
          sql += this.quoteIdentifiers(collectionItem, force);
        }
      }, this);

      return sql;
    } else if (collection._modelAttribute) {
      return this.quoteTable(collection.Model.name) + '.' + collection.fieldName;
      ...
      throw new Error('Unknown structure passed to order / group: ' + JSON.stringify(collection));
    }
  },
  ...
  */
  quoteIdentifiers(identifiers, force) {
    if (identifiers.indexOf('.') !== -1) {
      ...
      return this.quoteIdentifier(identifiers.slice(0, identifiers.length - 1).join('.'), force) + '.' + this.quoteIdentifier(identifiers[identifiers.length - 1], force);
    } else {
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        if (subQuery && Array.isArray(order)) {

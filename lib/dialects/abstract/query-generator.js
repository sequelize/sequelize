...
const QueryGenerator = {
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this.dialect === "postgres") {
      valueQuery = '<%= tmpTable %>INSERT INTO <%= table %> (<%= attributes %>)<%= output %> VALUES (<%= values %>)<%= ignoreDuplicates %>';
      emptyQuery = '<%= tmpTable %>INSERT INTO <%= table %><%= output %><%= ignoreDuplicates %>';
   }
    let outputFragment;
  ...
  */
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    let query = 'INSERT<%= ignoreDuplicates %> INTO <%= table %> (<%= attributes %>) VALUES <%= tuples %><%= onDuplicateKeyUpdate %><%= returning %>;';
    if (this.dialect === "postgres") {
      query = 'INSERT INTO <%= table %> (<%= attributes %>) VALUES <%= tuples %><%= onDuplicateKeyUpdate %><%= ignoreDuplicates %><%= returning %>;';
    }
    const tuples = [];
    const serials = {};
    const allAttributes = [];

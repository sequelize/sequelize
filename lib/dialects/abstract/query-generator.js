  ...
  */
  ...
  generateJoin(include, topLevelInfo) {
    ...
    if (association.tenantKeys) {
      const tenantKeyConditionLeft = `${this.quoteTable(asLeft)}.${this.quoteIdentifier(association.tenantKeys.foreign)}`;
      const tenantKeyConditionRight = `${this.quoteTable(asRight)}.${this.quoteIdentifier(association.tenantKeys.source || association.tenantKeys.target)}`;
      joinOn = `(${joinOn} AND ${tenantKeyConditionLeft} = ${tenantKeyConditionRight})`
    }

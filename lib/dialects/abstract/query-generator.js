    ...
    insertQuery: function(table, valueHash, modelAttributes, options) {
      ...
      if (this._dialect.supports.returnValues && options.returning) {
        if (!!this._dialect.supports.returnValues.returning) {
          ...
          Utils._.each(modelAttributes, function (attribute, key) {
            if (attribute.Model._isGeometryAttribute(key)) {
              // 15 and 2 refer to maxdecimaldigits and crs return options in:
              // http://www.postgis.org/docs/ST_AsGeoJSON.html
              // Required to make sure we're always comparing the same SRID where needed
              adds.push('ST_AsGeoJson('+key+', 15, 2) AS ' + key );
            }
          });

          if (adds.length) {
            valueQuery += ', ' + adds.join(',');
            emptyQuery += ', ' + adds.join(',');
          }
    ...
    */
    ...
    selectQuery: function(tableName, options, model) {
      ...
      mainAttributes = mainAttributes && mainAttributes.map(function(attr) {
        ...
        } else if (model && model._isGeometryAttribute(attr)) {
          geoAttr = true;
          attr = 'ST_AsGeoJSON(' + mainTableAs + '.' + self.quoteIdentifiers(attr)+', 15, 2) AS '+ self.quoteIdentifiers(attr);
        } else {
          attr = attr.indexOf(Utils.TICK_CHAR) < 0 && attr.indexOf('"') < 0 ? self.quoteIdentifiers(attr) : attr;
        }

        if (options.include && attr.indexOf('.') === -1 && addTable && !geoAttr) {
          ...
          if (options.includeIgnoreAttributes !== false) {
            ...
            attributes = include.attributes.map(function(attr) {
              ...
              if (Array.isArray(attr) && attr.length === 2) {
                ...
                attrAs = attr[1];
                attr = attr[0];
              } else if (include.model._isGeometryAttribute(attr)) {
                reverseConcat = true;
                attr = 'ST_AsGeoJSON(' + self.quoteIdentifier(as) + '.' + self.quoteIdentifiers(attr)+', 15, 2)';
              ...
              if (reverseConcat) {
                return attr + ' AS ' + self.quoteIdentifier(as + '.' + attrAs);
              } else {
    ...
    whereItemQuery: function(key, value, options) {
      ...
      if ( value && Utils._.isPlainObject(value)) {

        var geoCompares = ['$geoIn', '$geoNotIn', '$geoWithout', '$geoWithin'];
        var isGeoCompare = Object.keys(value).some(function (compare) {
          return geoCompares.indexOf(compare) !== -1;
        });

        if (isGeoCompare) {
          if (!(fieldType instanceof DataTypes.GEOMETRY)) {
            throw new Error('Geo comparison called on none Geo field');
          }
          var bool, geoFunc, parameters;

          if (value.$geoWithin || value.$geoWithout) {
            var obj = value.$geoWithin || value.$geoWithout;
            geoFunc = 'ST_DWithin';
            parameters = [
              'ST_GeomFromGeoJson(\'' + JSON.stringify(obj.point) +'\')',
              ( options.prefix ) ? this.quoteTable(options.prefix) + '.' + key : key,
              obj.distance || obj.distance
            ];
            bool = !!value.$geoWithin;
          } else if ( value.$geoIn || value.$geoNotIn ) {
            geoFunc = 'ST_Contains';
            parameters = [
              'ST_GeomFromGeoJson(\'' + JSON.stringify(value.$geoIn || value.$geoNotIn) + '\')',
              ( options.prefix ) ? this.quoteTable(options.prefix) + '.' + key : key
            ];
            bool = !!value.$geoIn;
          }

          return geoFunc + '(' + parameters.join(',') + ') = ' + this.escape(bool);
        }
      }

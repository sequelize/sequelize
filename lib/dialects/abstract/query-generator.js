  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query
      ...
      , modelKey
      , primaryKey = '';           //primaryKey field for which we have to get back value
    ...
    if (modelAttributes) {
      Utils._.each(modelAttributes, function(attribute, key) {
        ...
        if (attribute.field) {
          ...
          if (modelAttributeMap[attribute.field].primaryKey) {
            primaryKey = attribute.field;
          }
        }
      });
    }
    ...
    if (this._dialect.supports['DEFAULT VALUES']) {
      ...
      emptyQuery += ' VALUES ()';
    } else if (this._dialect.supports['("id") VALUES (default)']) {
      emptyQuery += ' ("id") VALUES (default)';
    }

    if (this._dialect.supports.returnValues && options.returning) {
      if (!!this._dialect.supports.returnValues.returninginto) {
        if (modelAttributeMap[primaryKey]) {
          if(modelAttributeMap[primaryKey].autoIncrement) {
            valueQuery += ' RETURNING "' + primaryKey + '" INTO :' + primaryKey;
            emptyQuery += ' RETURNING "' + primaryKey + '" INTO :' + primaryKey;
          }
        }
      } else if (!!this._dialect.supports.returnValues.returning) {
    ...
    query = (this._dialect.supports.autosemi ? query : query + ';');
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    if (subQuery) {
      ...
      if (mainTableAs) {
        subQueryItems.push((this._dialect.supports.as ? ' AS ' : ' ') + mainTableAs);
      }
    ...
    if (this._dialect.supports.limitwhere) {
      this.addWhereLimitAndOffset(options, model);
    }
    ...
    if (!this._dialect.supports.autosemi) {
      query += ';';
    }

  ...
  */
  selectQuery(tableName, options, model) {
    ...
    attributes.main = attributes.main && attributes.main.map(attr => {
      let addTable = !(Array.isArray(attr) && attr.length === 2 && attr[0] instanceof Utils.SequelizeMethod);
  ...
  generateCTEQuery(topLevelInfo) {
    ...
    topLevelInfo.options.cte.forEach(cte => {
      ...
      quotedCteName = cteName.indexOf(Utils.TICK_CHAR) < 0 && cteName.indexOf('"') < 0 ? this.quoteIdentifiers(cteName) : cteName;
      ...
      cteAttributes = cteAllAttributes.map(this.escapeAttribute, this).join(',');
      ...
      initialSelect = (() => {
        ...
        initialJoins = initial.include === undefined ? '' : initial.include.map(include => {
          return this.generateInclude(include, modelAs, topLevelInfo, undefined).mainQuery;
        }).join('');

        // 3. if there is an initial where add it too
        initialWhere = initial.where === undefined ? '' : ` WHERE ${this.getWhereConditions(initial.where, modelAs || modelName, topLevelInfo.model, topLevelInfo.options)}`;
      ...
      recursiveSelect = (() => {
        ...
        recursiveJoins = recursiveJoinItems.length > 0 ? `${recursiveJoinItems.join('')}` : '';

        // 6. handle the where if there is one
        if (recursive.where !== undefined) {

          if (recursive.where.model) {
            recursiveWhereItems.push(`${this.getWhereConditions(recursive.where.model, modelAs || modelName, topLevelInfo.model, topLevelInfo.options)}`);
          }
          if (recursive.where.cte) {
            if (recursiveWhereItems.length > 0) {
              recursiveWhereItems.push(`AND`);
            }
            recursiveWhereItems.push(`${this.getWhereConditions(recursive.where.cte, quotedCteName, topLevelInfo.model, topLevelInfo.options)}`);
          }
          if (recursiveWhereItems.length > 0) {
      ...
      limitAndOffset = this._dialect.supports.cteLimitOffsetOrder ? this.addLimitAndOffset(cte) : '';

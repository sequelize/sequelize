  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Else do it the reguar way
      ...
      if (options.groupedLimit) {
        ...
        var field = (options.order && typeof options.order && options.order[0][0]) || 'id';
        var distinct = options.choozLimit ? 'DISTINCT ON ("Chooz"."'+ field + '") ' : '';
    ...
    if(options.choozLimit) query = '(' + query + ') LIMIT ' + options.choozLimit;
    query += ';';

...
var QueryGenerator = {
  ...
  */
  dropTableQuery: function(tableName, options) {
    ...
    var query = 'DROP TABLE IF EXISTS <%= data.table %>;';
  ...
  */
  renameTableQuery: function(before, after) {
    var query = 'ALTER TABLE <%= data.before %> RENAME TO <%= data.after %>;';
    return Utils.template(query, {
  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query
      , valueQuery = 'INSERT<%= data.ignore %> INTO <%= data.table %> (<%= data.attributes %>)<%= data.output %> VALUES (<%= data.values %>)'
      , emptyQuery = 'INSERT<%= data.ignore %> INTO <%= data.table %><%= data.output %>'
      , outputFragment
      , fields = []
      , values = []
      , key
    ...
    if (this._dialect.supports.EXCEPTION && options.exception) {
      ...
      //valueQuery = 'CREATE OR REPLACE FUNCTION pg_temp.testfunc() RETURNS SETOF <%= data.table %> AS $body$ BEGIN RETURN QUERY ' + valueQuery + '; EXCEPTION ' + options.exception + ' END; $body$ LANGUAGE plpgsql; SELECT * FROM pg_temp.testfunc(); DROP FUNCTION IF EXISTS pg_temp.testfunc();';
      ...
      valueQuery = 'CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response <%= data.table %>, OUT sequelize_caught_exception text) RETURNS RECORD AS ' + delimiter +
  ...
  */
  updateQuery: function(tableName, attrValueHash, where, options, attributes) {
    ...
    query = 'UPDATE <%= data.table %> SET <%= data.values %><%= data.output %> <%= data.where %>';
    ...
    var keys = Object.keys(attrValueHash);
    for (var i = 0; i < keys.length; i++) {
      var key = keys[i];
  ...
  */
  incrementQuery: function(tableName, attrValueHash, where, options) {
    ...
    for (var i = 0; i < keys.length; i++) {
      value = attrValueHash[keys[i]];
      values.push(this.quoteIdentifier(keys[i]) + '=' + this.quoteIdentifier(keys[i]) + ' + ' + this.escape(value));
    }
  ...
  quote: function(obj, parent, force) {
    ...
    if (Array.isArray(obj)) {
      ...
      for (var i = 0; i < len - 1; i++) {
        item = obj[i];
        if (item._modelAttribute || Utils._.isString(item) || item._isSequelizeMethod || item.raw) {
          break;
        }

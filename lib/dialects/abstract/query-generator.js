        ...
        if (typeof item === 'string') {
          ...
          if (index > 0 && orderIndex !== -1) {
            ...
            if (previousModel.associations !== undefined && previousModel.associations[item]) {
              ...
              && previousModel.rawAttributes[item.split('.')[0]].type instanceof DataTypes.JSON
            ) {
              // just quote identifiers for now
              const identifier = this.quoteIdentifiers(previousModel.name  + '.' + previousModel.rawAttributes[item.split('.')[0]].field);

...
const debug = Utils.getLogger().debugContext('abstract:query-generator');
...
const QueryGenerator = {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (!this._dialect.supports.ctes && (options.cte || cteSelect)) {
    ...
    debug('attributes.main %O', attributes.main);

    // generate cte query
    if (this._dialect.supports.ctes && options.cte) {
      query = this.generateCTEQuery(topLevelInfo);
      debug('cte query %O', query);
    }
  ...
  attributeItemQuery(object, options, modelName, cteName) {
    ...
    const arrayOperators = [Op.cteAdd, Op.cteSub, Op.cteMul, Op.cteDiv];
    ...
    if (arrayOperators.indexOf(operator) === -1) {
      throw new Error(`Unrecognized cte array operator ${operator} in attribute`);
    }
    ...
    return '(' + operands.join(' ' + operatorMap[operator] + ' ') + ')';
  },

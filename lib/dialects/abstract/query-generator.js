...
const QueryGenerator = {
  ...
  */
  ...
  addConstraintQuery(tableName, options) {
    ...
    const fieldsSqlString = options.fields.join('_');
    let constraintSnippet, constraintName;
    switch(options.type.toUpperCase()) {
      case 'UNIQUE':
        constraintSnippet = `UNIQUE (${fieldsSqlQuotedString})`;
        constraintName = `UK__${tableName}__${fieldsSqlString}`;
        break;
      case 'CHECK':
        constraintSnippet = `CHECK (${options.where})`;
        constraintName = `CK__${tableName}__${fieldsSqlString}`;
        break;
      case 'DEFAULT':
        ...
        constraintSnippet = `DEFAULT (${this.escape(options.defaultValue)}) FOR ${fieldsSql[0]}`;
        constraintName = `DF__${tableName}__${fieldsSqlString}`;
        break;
      case 'PRIMARY KEY':
        constraintSnippet = `PRIMARY KEY (${fieldsSqlQuotedString})`;
        constraintName = `PK__${tableName}__${fieldsSqlString}`;
        break;
      case 'FOREIGN KEY':
        const references = options.references;
        if(!references || !references.model || !references.key) {
          throw new Error('references object with model and key must be specified');
        }
        const referencesSnippet = `${this.quoteTable(references.model)} (${this.quoteIdentifier(references.key)})`;
        constraintSnippet = `FOREIGN KEY (${fieldsSqlQuotedString}) REFERENCES ${referencesSnippet}`;
        if (references.onUpdate) {
          constraintSnippet+= ` ON UPDATE ${references.onUpdate.toUpperCase()}`;
        }
        if (references.onDelete) {
          constraintSnippet+= ` ON DELETE ${references.onDelete.toUpperCase()}`;
        }
        constraintName = `FK__${tableName}__${fieldsSqlString}__${references.model}`;
        break;
    ...
    constraintName = this.quoteIdentifier(options.name || constraintName);

    return `ALTER TABLE ${tableName} ADD CONSTRAINT ${constraintName} ${constraintSnippet};`;
  },

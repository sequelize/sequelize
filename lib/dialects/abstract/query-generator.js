  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    if (this._dialect.supports.EXCEPTION && options.exception) {
      ...
      if (semver.gte(this.sequelize.options.databaseVersion, '9.2.0')) {
        ...
        valueQuery =
          'CREATE OR REPLACE FUNCTION pg_temp.testfunc() RETURNS SETOF <%= table %> AS $body$ BEGIN RETURN QUERY ' +
          valueQuery +
          '; EXCEPTION ' +
          options.exception +
...
https://github.com/sequelize/sequelize/discussions/15694`);
  ...
  */
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    throw new Error(
      `Unsupported where option value: ${NodeUtil.inspect(smth)}. Please refer to the Sequelize documentation to learn more about which values are accepted as part of the where option.`
    );
  },

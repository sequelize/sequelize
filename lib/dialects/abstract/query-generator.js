...
var QueryGenerator = {
  ...
  */
  dropTableQuery: function(tableName, options) {
    ...
    var query = 'DROP TABLE IF EXISTS <%= d.table %>;';
  ...
  */
  renameTableQuery: function(before, after) {
    var query = 'ALTER TABLE <%= d.before %> RENAME TO <%= d.after %>;';
    return Utils.compileOrRunTemplate(query, {
  ...
  */
  insertQuery: function(table, valueHash, modelAttributes, options) {
    ...
    var query
      , valueQuery = 'INSERT<%= d.ignore %> INTO <%= d.table %> (<%= d.attributes %>)<%= d.output %> VALUES (<%= d.values %>)'
      , emptyQuery = 'INSERT<%= d.ignore %> INTO <%= d.table %><%= d.output %>'
      , outputFragment
      , fields = []
      , values = []
      , key
    ...
    if (this._dialect.supports.EXCEPTION && options.exception) {
      ...
      //valueQuery = 'CREATE OR REPLACE FUNCTION pg_temp.testfunc() RETURNS SETOF <%= d.table %> AS $body$ BEGIN RETURN QUERY ' + valueQuery + '; EXCEPTION ' + options.exception + ' END; $body$ LANGUAGE plpgsql; SELECT * FROM pg_temp.testfunc(); DROP FUNCTION IF EXISTS pg_temp.testfunc();';
      ...
      valueQuery = 'CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response <%= d.table %>, OUT sequelize_caught_exception text) RETURNS RECORD AS ' + delimiter +
  ...
  */
  updateQuery: function(tableName, attrValueHash, where, options, attributes) {
    ...
    query = 'UPDATE <%= d.table %> SET <%= d.values %><%= d.output %> <%= d.where %>';

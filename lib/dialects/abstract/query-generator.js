    ...
    */
      ...
      if (options.include) {
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (through) {
            ...
            if (subQuery && include.duplicating && include.subQuery) {
    ...
    joinIncludeQuery: function(options) {
      var subQuery = options.subQuery
        ...
        , attrRight = association instanceof BelongsTo ?
                      right.rawAttributes[right.primaryKeyAttribute].field || right.primaryKeyAttribute :
                      association.identifierField;
      ...
      if (subQuery && include.parent.subQuery && !include.subQuery) {
        if (include.parent.model.name === options.model.name) {
          ...
          joinOn = this.quoteIdentifier((parent.as || parent.model.name) + '.' + attrLeft);
        }
      }

      joinOn += ' = ' + this.quoteTable(include.as) + '.' + this.quoteIdentifier(attrRight);


      if (include.where) {
        joinWhere = this.whereItemsQuery(include.where, {
          prefix: this.sequelize.literal(this.quoteIdentifier(include.as)),
          model: include.model
        });
        if (joinWhere) {
      ...
      return joinType + this.quoteTable(tableRight, include.as) + ' ON ' + joinOn;
    },

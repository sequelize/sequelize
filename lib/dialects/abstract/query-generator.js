...
class QueryGenerator {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.include) {
      // To support cross database joins on mysql, prefix the database name of the model in case the model is for a different database
      if (this.dialect === 'mysql') {
        mainTable.quotedName = `${this.quoteIdentifier(model.sequelize.config.database)}.${mainTable.quotedName}`;
      }
  ...
  generateJoin(include, topLevelInfo) {
    ...
    let joinTableName = this.quoteTable(tableRight, asRight);

    // For mysql, prepend the table name with database name
    if (this.dialect === 'mysql') {
      joinTableName = `${this.quoteIdentifier(right.sequelize.config.database)}.${joinTableName}`;
    }
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    let throughTableName = this.quoteTable(throughTable, throughAs);

    let joiningTableName = this.quoteTable(
    ...
    // For mysql, prepend the table name with database name
    if (this.dialect === 'mysql') {
      joiningTableName = `${this.quoteIdentifier(
        include.model.sequelize.config.database
      )}.${joiningTableName}`;

      throughTableName = `${this.quoteIdentifier(through.model.sequelize.config.database)}.${throughTableName}`;
    }
      
    if (this._dialect.supports.joinTableDependent) {
      // Generate a wrapped join so that the through table join can be dependent on the target join
      joinBody = `( ${throughTableName} INNER JOIN ${joiningTableName} ON ${targetJoinOn}`;
      if (throughWhere) {
      ...
      joinBody = `${throughTableName} ON ${sourceJoinOn} ${joinType} ${joiningTableName}`;

...
var QueryGenerator = {
  ...
  */
    ...
    if (options.hasOwnProperty('where') && !options.groupedLimit) {
      ...
      if (options.where) {
        if (subQuery) {
          ...
          _.forEach(subQueryItems, function(value) {
            if(value.match(/^SELECT/i)) {
              value = self.selectSqlFragmentBuilder(options, model, mainAttributes, table, mainTableAs, options.where);
            }
          });
        } else {
          mainQueryItems.push(' WHERE ' + options.where);
          _.forEach(mainQueryItems, function(value) {
            if(value.match(/^SELECT/i)) {
              value = self.selectSqlFragmentBuilder(options, model, mainAttributes, table, mainTableAs, options.where);
            }
          });
  ...
  selectSqlFragmentBuilder: function(options, model, attributes, tables, mainTableAs, where) {

  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (this._dialect.supports.cteQueries && options.cte) {
      // automagically select from cte if we only have one
      if (_.isUndefined(options.cteSelect) && options.cte.length === 1) {
        cteSelect = options.cte[0].name;
      }
      if (cteSelect) {
        cteAs = cteSelect.indexOf(Utils.TICK_CHAR) < 0 && cteSelect.indexOf('"') < 0 ? this.quoteIdentifiers(cteSelect) : cteSelect;
      }
      query = this.generateCTEQuery(topLevelInfo);
    }

    attributes.main = attributes.main && attributes.main.map(attr => {
      let addTable = !(Array.isArray(attr), attr.length > 0 && attr[0]._isSequelizeMethod);
    ...
    attributes.main = attributes.main || ((options.include || cteSelect) ? [`${mainTable.as}.*`] : ['*']);
    ...
    if (cteSelect && options.includeCTEAttributes) {
      attributes.main = attributes.main.concat(options.includeCTEAttributes.map(attr => {
        return `${this.quoteIdentifier(cteSelect)}.${this.quoteIdentifier(attr)}`;
      }));
    }
    if (subQuery) {
      ...
      if (cteSelect) {
        ...
        let primaryKey = model.rawAttributes[model.primaryKeyAttribute].field;
        primaryKey = primaryKey.indexOf(Utils.TICK_CHAR) < 0 && primaryKey.indexOf('"') < 0 ? this.quoteIdentifiers(primaryKey) : primaryKey;
        mainQueryItems.push(`${mainTable.as || mainTable.name}.${primaryKey} = ${cteAs}.${primaryKey}`);
      }
  ...
  generateCTEQuery(topLevelInfo) {
    ...
    topLevelInfo.options.cte.forEach(cte => {
      ...
      const cteAttributes = cte.cteAttributes ? cte.cteAttributes.slice() : [];
      const initial = cte.initial;
      const initialSelectItems = [];
      const initialJoinQueries = [];
      const recursive = cte.recursive;
      const recursiveSelectItems = [];
      const recursiveJoinQueries = [];
      const queryOrder = [];
      ...
      // the CTE is going to use the model of the findA;; unlesss a different one is given or useModel is false
      if (useModel) {
        if (_.isUndefined(cte.model)) {
          cteModel = topLevelInfo.model;
        } else {
          cteModel = cte.model;
        }
      }
      ...
      if (cteModel) {
        if (topLevelInfo.names.as) {
          modelAs = this.quoteTable(topLevelInfo.names.as);
        } else if (!Array.isArray(topLevelInfo.names.name) && topLevelInfo.model) {
          modelAs = this.quoteTable(topLevelInfo.model.name);
        }
        modelName = !Array.isArray(topLevelInfo.names.name) ? this.quoteTable(topLevelInfo.names.name) : topLevelInfo.names.name.map(t => {
          if (Array.isArray(t)) {
        ...
        _.forIn(cteModel.tableAttributes, (attr, name) => {
      ...
      if (_.isUndefined(cteName) || cteName.length < 1) {
        throw new Error(`No 'name' property given to CTE.`);
      }

      cteQueryItems.push(`${cteName}( ${cteAllAttributes.map(this.escapeAttribute, this).join(',')} ) AS ( `);

      cteName = cteName.indexOf(Utils.TICK_CHAR) < 0 && cteName.indexOf('"') < 0 ? this.quoteIdentifiers(cteName) : cteName;
      ...
      cteModelAttributes.forEach(attr => {
        let addTable = !(Array.isArray(attr) && attr.length === 2 && attr[0]._isSequelizeMethod);
        attr = this.escapeAttribute(attr);
      ...
      cteAttributes.forEach(attr => {
        if (_.has(initial, attr)) {
          if (_.isPlainObject(initial[attr])) {
            initialSelectItems.push(this.attributeItemQuery(initial[attr], topLevelInfo.options, modelAs));
          } else {
      ...
      if (_.has(initial, 'include')) {
        initial.include.forEach(include => {
          let results = this.generateInclude(include, modelAs, topLevelInfo, undefined);
          initialJoinQueries.push(results.mainQuery);
        });
      }

      cteQueryItems.push(`${initialSelectItems.join(', ')} FROM ${modelName}`);
      if (modelAs) {
      ...
      if (initial.where) {
        ...
        cteQueryItems.push(`${this.getWhereConditions(initial.where, modelAs || modelName, topLevelInfo.model, topLevelInfo.options)} `);
      }
      // we need the order clause here if there is no recursive query since we depend on the recursive query for order otherwise
      if (this._dialect.supports.cteLimitOffsetOrder && cte.order && !recursive) {
        if (Array.isArray(cte.order)) {
          cte.order.forEach(e => {
            if (Array.isArray(e) && _.size(e) > 1 && _.indexOf(cteAllAttributes, e[0]) !== -1) {
              this.validateOrder(_.last(e));
              queryOrder.push(`${(_.indexOf(cteAllAttributes, e[0]) + 1).toString()} ${_.last(e)}`);
            }
          });
        }
      ...
      if (recursive) {
        if (_.has(recursive, 'next')) {
          ...
          let nextResults = this.generateInclude(recursive.next, recursiveJoinTable, topLevelInfo, cte);
          recursiveJoinQueries.push(nextResults.mainQuery);

          if (_.has(recursive, 'include')) {
            recursive.include.forEach(include => {
              let includeResults = this.generateInclude(include, include.as, topLevelInfo, _.assign(cte, {name: recursive.next.as}));
              recursiveJoinQueries.push(includeResults.mainQuery);
            });
          }
        }
        if (_.has(cte, 'unique')) {
        ...
        cteQueryItems.push(` SELECT `);
        cteModelAttributes.forEach(attr => {
          recursiveSelectItems.push(`${recursiveJoinTable}.${this.escapeAttribute(attr)}`);
        });
        ...
        cteAttributes.forEach(attr => {
          ...
          if (_.isPlainObject(recursive[attr])) {
            recursiveSelectItems.push(this.attributeItemQuery(recursive[attr], topLevelInfo.options, recursiveJoinTable, cteName));
          } else {
            recursiveSelectItems.push(recursive[attr]);
          }
        });

        cteQueryItems.push(`${recursiveSelectItems.join(', ')} FROM ${cteName}`);
        if (recursiveJoinQueries.length > 0) {
          cteQueryItems.push(`${recursiveJoinQueries.join('')} `);
        }

        if (recursive.where) {
          let modelConditions = recursive.where.model;
          let cteConditions = recursive.where.cte;
          let recursiveWhereItems = [` WHERE `];

          if (modelConditions) {
            recursiveWhereItems.push(`${this.getWhereConditions(modelConditions, modelAs || modelName, topLevelInfo.model, topLevelInfo.options)} `);
          }
          if (cteConditions) {
            if (recursiveWhereItems.length > 1) {
              recursiveWhereItems.push(`AND `);
            }
            recursiveWhereItems.push(`${this.getWhereConditions(cteConditions, cteName, topLevelInfo.model, topLevelInfo.options)} `);
          }
          ...
          if (Array.isArray(cte.order)) {
            cte.order.forEach(order => {
              if (Array.isArray(order) && order.length > 1 && _.indexOf(cteAllAttributes, order[0]) !== -1) {
                ...
                queryOrder.push(`${(_.indexOf(cteAllAttributes, order[0]) + 1).toString()} ${_.last(order)}`);
              }
            });
          }
      ...
      // add LIMIT to recurisve query
      if (this._dialect.supports.cteLimitOffsetOrder) {

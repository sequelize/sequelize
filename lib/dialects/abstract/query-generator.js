...
class QueryGenerator {
  ...
  */
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    const on = joinQuery.condition === undefined ? '' : `ON ${joinQuery.condition}`;
    if (include.subQuery && topLevelInfo.subQuery) {
      if (requiredMismatch && subChildIncludes.length > 0) {
        joinQueries.subQuery.push(` ${joinQuery.join} ( ${joinQuery.body}${subChildIncludes.join('')} ) ${on}`);
      } else {
        joinQueries.subQuery.push(` ${joinQuery.join} ${joinQuery.body} ${on}`);
      ...
      if (requiredMismatch && mainChildIncludes.length > 0) {
        joinQueries.mainQuery.push(` ${joinQuery.join} ( ${joinQuery.body}${mainChildIncludes.join('')} ) ${on}`);
      } else {
        joinQueries.mainQuery.push(` ${joinQuery.join} ${joinQuery.body} ${on}`);
  ...
  generateJoin(include, topLevelInfo) {
    ...
    if (!include.separate && (include.limit !== undefined || include.offset != undefined)) {
      const newBody = `(${this.selectQuery(
        tableRight,
        {
          tableAs: asRight,
          where: new PrebuiltWhere(joinOn),
          limit: include.limit,
          offset: include.offset,
          order: include.order
        },
        include.model
      ).replace(/;$/, '')}) as ${this.quoteIdentifier(asRight)}`;

      return {
        join: include.required ? this._dialect.supports.LATERAL.cross : this._dialect.supports.LATERAL.outer,
        body: newBody,
        condition: this._dialect.supports.LATERAL.on,
        attributes: {
          main: [],
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        if (
          ...
          subQueryOrder.push(this.quote(order, options.tableAs ? { name: options.tableAs } : model, '->'));
        }
        ...
        mainQueryOrder.push(this.quote(order, options.tableAs ? { name: options.tableAs } : model, '->'));
      }
  ...
  */
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof PrebuiltWhere)
    {

  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (subQuery && attributes.main) {
      ...
      // appending fields which are required by 'BelongsTo' associations when main attributes filter is in use
      if (options.include) {
        for (const include of options.include) {
          if (include.association && include.association.associationType && include.association.foreignKey) {
            const { associationType, foreignKey } = include.association;
            if (associationType === 'BelongsTo' && attributes.main.indexOf(foreignKey) === -1) {
              attributes.main.push(foreignKey);
            }
          }
        }
      }

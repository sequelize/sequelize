...
class QueryGenerator {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (subQuery) {
      this._ensureNonemptySelectClause(attributes.main, { modelName: model && model.name, as: mainTable.as });
  ...
  _ensureNonemptySelectClause(attributes, extraInfo = {}) {
    ...
    let message = 'Attempted a SELECT query ';
    if (extraInfo.modelName && extraInfo.as) {
      message += `for model '${extraInfo.modelName}' (as ${extraInfo.as}) `;
    } else if (extraInfo.modelName) {
      message += `for model '${extraInfo.modelName}' `;
    } else if (extraInfo.as) {
      message += `as ${extraInfo.as} `;
    }
    message += 'without selecting any attributes.';
    const error = new sequelizeError.QueryError(message);
    error.modelName = extraInfo.modelName;
    error.as = extraInfo.as;
    throw error;
  }

  selectFromTableFragment(options, model, attributes, tables, mainTableAs) {
    this._ensureNonemptySelectClause(attributes, { modelName: model && model.name, as: mainTableAs });

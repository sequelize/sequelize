...
const QueryGenerator = {
  ...
  */
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    // Determine the attrTarget, if there is a NonPrimaryBelongsTo in our association
    // we need to use the targetPrimaryKeyAttribute to ensure we do not default to the standard
    // primary key.
    let attrTarget;
    if (association.hasNonPrimaryBelongsTo) {
      attrTarget = association.target.rawAttributes[association.targetPrimaryKeyAttribute].field || association.targetPrimaryKeyAttribute;
    } else {
    ...
    // Figure out if we need to use the given source name for a belongs-to-many
    if (association.hasNonPrimaryBelongsTo) {
      attrSource = association.sourcePrimaryKeyAttribute;

      // Figure out if a field should be used
      if (!topLevelInfo.subQuery) {
        attrSource = association.source.rawAttributes[association.sourcePrimaryKeyAttribute].field;
      }
      if (topLevelInfo.subQuery && !include.subQuery && !include.parent.subQuery && include.parent.model !== topLevelInfo.options.mainModel) {
        attrSource = association.source.rawAttributes[association.sourcePrimaryKeyAttribute].field;
      }

...
const QueryGenerator = {
  ...
  */
  ...
  _traverseJSON(items, baseKey, prop, item, path) {
    ...
    if (_.isPlainObject(item)) {
      _.forOwn(item, (value, itemProp) => {
        if (itemProp.indexOf('$') === 0) {
          const tmpCast = cast || this._getJsonCast(value);
          const key = tmpCast ? this.handleSequelizeMethod(new Utils.Cast(new Utils.Literal(pathKey), tmpCast)) : pathKey;

          items.push(this.whereItemQuery(new Utils.Literal(key), {[itemProp]: value}));
          return;
        }
    ...
    if (!cast) {
      cast = this._getJsonCast(item);
    }
    const key = cast ? this.handleSequelizeMethod(new Utils.Cast(new Utils.Literal(pathKey), cast)) : pathKey;

    items.push(this.whereItemQuery(new Utils.Literal(key), {$eq: item}));
  },

...
class QueryGenerator {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (subQuery) {
      query = `SELECT ${attributes.main.straightJoin ? `STRAIGHT_JOIN` : ``} ${attributes.main.join(', ')} FROM (${subQueryItems.join('')}) AS ${mainTable.as}${mainJoinQueries.join('')}${mainQueryItems.join('')}`;
    } else {

...
const QueryGenerator = {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.hasOwnProperty('indexHint') && Array.isArray(options.indexHint)) {
      _.each(options.indexHint, (value) => {
        let line = ' ';
        if (value.hasOwnProperty('val')) {
          line += value.val;
        } else {
          line += String(value[0]).toUpperCase();
          line += ' INDEX(`' + String(value[1]) + '`)';
          if (value[2]) line += ' ' + String(value[2]);
        }

        if (subQuery) {
          subQueryItems.push(line);
        } else {
          mainQueryItems.push(line);
        }
      });
    }

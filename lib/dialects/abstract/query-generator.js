...
const QueryGenerator = {
  ...
  */
  quote(obj, parent, force, connector) {
    ...
    if (_.isString(obj)) {
      ...
      for (i = 0; i < len - 1; i++) {
        item = obj[i];
        if (_.isString(item) || item instanceof Utils.SequelizeMethod) {
          break;
        }
      ...
      return sql;
    // } else if (obj instanceof Model.Attribute) {
    //  return this.quoteTable(obj.Model.name) + '.' + this.quoteIdentifier(obj.fieldName);
  ...
  */
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    } else if (smth instanceof Utils.Attribute){
      return this.quoteTable(smth.Model.name) + '.' + this.quoteIdentifier(smth.field || smth.fieldName);
    } else {

  ...
  */
  quote(obj, parent, force, connector) {
    ...
    if (_.isString(obj)) {
      ...
      let sql = (i > 0 ? this.quoteIdentifier(tableNames.join(connector)) + '.' : (_.isString(obj[0]) && parent ? this.quoteIdentifier(parent.name) + '.' : '')) + this.quote(obj[i], parent, force, connector);
      if (i < len - 1) {
  ...
  */
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    const quoteOrder = order => {
      ...
      // only continue if array
      if (Array.isArray(order)) {
        // Check if we have ['attr', 'DESC'] or [Model, 'attr', 'DESC']
        if (typeof order[order.length - 2] === 'string') {
          offset = 2;
        ...
        // split the attribute at . and get the attribute name
        if (typeof order[order.length - offset] === 'string') {
          const attributeIdentifiers = order[order.length - offset].split(/\.|->/);
          const attribute = _.last(attributeIdentifiers);

          // if the attribute identifiers are greater than 1
          if (attributeIdentifiers.length > 1 && model !== undefined) {
            // set the order as the attribute without identifiers
            order[order.length - offset] = attribute;

            // loop through the identifiers
            attributeIdentifiers.forEach((identifier, identifierIndex) => {
              const previousOrderIndex = order.length - (offset + 1);
              let association = {};

              // this is the first assoication so use the model and make sure the identifier isnt the current model
              if (identifierIndex === 0 && identifier !== model.name && model.associations !== undefined && model.associations[identifier]) {
                association = model.associations[identifier];
              } else if (identifier !== attribute && order[previousOrderIndex] && order[previousOrderIndex].model !== undefined && order[previousOrderIndex].model.associations[identifier]) {
                association = order[previousOrderIndex].model.associations[identifier];
              }

              // splice in the order association and set the order model
              if (association.target && association.as) {
                order.splice(order.length - offset, 0, {
                  model: association.target,
                  as: association.as
                });
                orderModel = order[order.length - (offset + 1)].model;
              } else if (identifier !== model.name && identifier !== attribute && association.model === undefined) {
                throw new Error(util.format('Order association cannot be found for identifier, \'%s\' given', identifier));
              } else if (identifier === model.name) {
                orderModel = model;
              }
            });
          } else {
            // if we have a model, then use it
            if (order.length > offset) {
              orderModel = order[order.length - (offset + 1)];
              if(orderModel !== undefined && orderModel.model) {
                orderModel = orderModel.model;
              }
            }

            // if undefined then set as current model
          ...
          // set fields
          if (orderModel !== undefined && orderModel.rawAttributes !== undefined && orderModel.rawAttributes[attribute] && attribute !== orderModel.rawAttributes[attribute].field) {
            order[order.length - offset] = orderModel.rawAttributes[attribute].field;
          }
        }
      }

      // order with our new order array
      return self.quote(order, model, false, '->');
    };
    ...
    if (Array.isArray(options.order)) {
      for (const order of options.order) {
        ...
        if (subQuery && (Array.isArray(order) && !(typeof order[0] === 'function' && order[0].prototype instanceof Model) && !(order[0] && typeof order[0].model === 'function' && order[0].model.prototype instanceof Model))) {
          ...
          subQueryOrder.push(quoteOrder(order));
        }

        // mainQueryOrder.push(this.quote(order, model, false, '->'));
        mainQueryOrder.push(quoteOrder(order));
      }
    } else if (options.order instanceof Utils.SequelizeMethod){
      // const sql = this.quote(options.order, model, false, '->');
      const sql = quoteOrder(options.order);
      if (subQuery) {

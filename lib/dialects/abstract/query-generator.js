  ...
  */
  ...
  _getAliasForField(tableName, field, options) {
    if (this.options.minifyAliases) {
      ...
      if (Array.isArray(field)) {
        if (field.length === 1) {
          ...
          field.forEach((item, index) => {
            ...
            // set the previous as the parent when previous is undefined or the target of the association
            if (previous && previous instanceof Association) {
              previousAssociation = previous;
              previousModel = previous.target;
            }
          ...
          // loop through array, adding table names of models to quoted
          const collectionLength = field.length;
          const tableNames = [];
          let item;
          let i = 0;
          ...
          // loop through everything past i and append to the sql
          field.slice(i).forEach(collectionItem => {
            sql += this.quote(collectionItem, undefined, '->');
          }, this);
      ...
      if (aliased instanceof Utils.Literal) {
        aliased = aliased.val;
      }
      else if (aliased instanceof Utils.Col) {
        aliased = aliased.col;
      }
      else if (aliased._modelAttribute) {
        aliased = aliased.fieldName;
      }
      aliased = aliased.replace(/["']/g, '');
      
      if (options.aliasesByTable[`${tableName}${aliased}`]) {
        return options.aliasesByTable[`${tableName}${aliased}`];
      }
    }
    return null;
  }
  ...
  getQueryOrders(options, model, subQuery) {
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        const quotedModel = model && this.quoteIdentifier(model.name);    
        
        // wrap if not array
        if (!Array.isArray(order)) {
          order = [order];
        }
        const alias = this._getAliasForField(quotedModel, order, options);
        ...
        if (subQuery) {
          ...
          if (subQueryAttribute) {
            order[0] = new Utils.Col(this._getAliasForField(quotedModel, subQueryAttribute[1], options) || subQueryAttribute[1]);
          }
        }

        if (alias) {
          mainQueryOrder.push(alias);
        }
        else {

...
const QueryGenerator = {
  ...
  */
  ...
  generateJoin(include, topLevelInfo) {
    ...
    const parentIsTop = !!parent && !include.parent.association && include.parent.model.name === topLevelInfo.names.model.name;
    let $parent;
    let joinWhere;
  ...
  generateCTEQuery(topLevelInfo) {
    ...
    topLevelInfo.options.cte.forEach(cte => {
      ...
      if (cteModel) {
        ...
        } else if (!Array.isArray(topLevelInfo.names.name) && model) {
      ...
      cteAllAttributes = cteAllAttributes.concat(cteModelAttributes);
      ...
      if (_.has(initial, 'include')) {
        initial.include.forEach(include => {
          let results = this.generateInclude(include, modelAs, topLevelInfo);
          intialJoinQueries.push(results.mainQuery);
        });
      }

      cteQueryItems.push(`${initialSelectItems.join(', ')} FROM ${modelName}`);
      if (modelAs) {
        cteQueryItems.push(` AS ${modelName}`);
      }
      ...
      if (recursive) {
        if (_.has(recursive, 'next')) {
          ...
          let nextResults = this.generateInclude(recursive.next, recursiveJoinTable, topLevelInfo);
          recursiveJoinQueries.push(nextResults.mainQuery);

          if (_.has(recursive, 'include')) {
            recursive.include.forEach(include => {
              let includeResults = this.generateInclude(include, include.as, topLevelInfo);
        ...
        cteAttributes.forEach(attr => {
          ...
          if (_.isPlainObject(recursive[attr])) {
            recursiveSelectItems.push(this.attributeItemQuery(recursive[attr], options, recursiveJoinTable, cteName));
          } else {
        ...
        if (recursive.where) {
          // do recursive where stuff
        }
        // add ORDER BY to recursive query
        if (this._dialect.support.cteLimitOffsetOrder && cte.order) {
          // ^
        }

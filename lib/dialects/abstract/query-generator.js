...
var QueryGenerator = {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Handle CTEs
    if (self._dialect.supports.cteQueries && options.cte) {
      
      //automagically select from a cte if we have only one
      if (_.isUndefined(options.cteSelect) && !_.isUndefined(options, 'cte') && options.cte.length === 1) {
        cteSelect = options.cte[0].name;
      }

      if (cteSelect) {
        cteAs = cteSelect.indexOf(Utils.TICK_CHAR) < 0 && cteSelect.indexOf('"') < 0
          ? self.quoteIdentifiers(cteSelect) : cteSelect;
      }


      cteQuery = self.generateCTEQuery(tableName, options, model);
    }
    ...
    mainAttributes = mainAttributes && mainAttributes.map(function (attr) {
      ...
      if ((options.include || cteSelect) && attr.indexOf('.') === -1 && addTable) {
    ...
    if (options.include) {
      ...
      options.include.filter(function (include) {
        ...
        var results = self.generateJoinQueries(include, mainTableAs,
            { tableName: tableName, mainTableAs: mainTableAs, model: model}, options);

        subJoinQueries = subJoinQueries.concat(results.subQuery);
        mainJoinQueries = mainJoinQueries.concat(results.mainQuery);
        mainAttributes = mainAttributes.concat(results.mainAttributes);
        if (subQuery) {
            subQueryAttributes = subQueryAttributes.concat(results.subQueryAttributes);
        }

      }.bind(this));
    }

    // Add CTE attributes if they were specified
    if (cteSelect && options.includeCTEAttributes) {
      mainAttributes = mainAttributes.concat(options.includeCTEAttributes.map(function (e) {
        return self.quoteIdentifier(cteSelect) + '.' + self.quoteIdentifier(e);
      }));
    }
    ...
    // Else do it the reguar way
      ...
      // if we are selecting from the cte add cte to the FROM list
      if (self._dialect.supports.cteQueries && cteSelect) {
        mainQueryItems.push(' INNER JOIN ' + cteAs + ' ON ');
        var primaryKey = model.rawAttributes[model.primaryKeyAttribute].field;
        primaryKey = primaryKey.indexOf(Utils.TICK_CHAR) < 0 && primaryKey.indexOf('"') < 0
          ? self.quoteIdentifiers(primaryKey) : primaryKey;

        mainQueryItems.push((mainTableAs || tableName) + '.' + primaryKey + ' = ' + cteAs + '.' + primaryKey);
      }
  ...
  // create a SQL WITH clause using the given options
  generateCTEQuery: function (tableName, options, model) {
    ...
    cteQueryItems.push('WITH RECURSIVE ');

    // Iterate over each defined cte table
    options.cte.forEach(function (cte) {

      var cteModelAttributes = cte.attributes ? cte.attributes.slice() : []
        , cteAttributes = cte.cteAttributes ? cte.cteAttributes.slice() : []
        , cteAllAttributes = []
        , cteName = cte.name
        , initial = cte.initial
        , initialSelectItems = []
        ...
        , cteModel
        , useModel = cte.useModel || true
        , modelName = null
        , modelAs = null
        , unique = cte.unique
        , recursiveSelectItems = []
        , recursiveJoinTable
        , recursiveJoinQueries = []
        , queryOrder = [];

      // Assume that cte is going to use the model of the findall, unless a different one is given
      // or useModel is false
      if (useModel) {
      ...
      if (cteModel) {
        ...
        if (options.tableAs) {
          modelAs = this.quoteTable(options.tableAs);
        } else if (!Array.isArray(tableName) && model) {
          modelAs = this.quoteTable(model.name);
        }

        modelName = !Array.isArray(tableName) ? this.quoteTable(tableName) : tableName.map(function (t) {
          if (Array.isArray(t)) {
            return this.quoteTable(t[0], t[1]);
          }
          return this.quoteTable(t, true);
        }.bind(this)).join(', ');
        modelName = modelName.indexOf(Utils.TICK_CHAR) < 0 && modelName.indexOf('"') < 0
          ? self.quoteIdentifiers(modelName) : modelName;


        // If the query defined which model attributes to include, make sure we also have the primary keys
        if (cteModelAttributes.length > 0) {
          model.primaryKeyAttributes.forEach(function (keyAtt) {
            if (!_.find(cteModelAttributes, function (attr) { return keyAtt === attr || keyAtt === attr[0] || keyAtt === attr[1]; })) {
              cteModelAttributes.push(cteModel.rawAttributes[keyAtt].field ? [keyAtt, cteModel.rawAttributes[keyAtt].field] : keyAtt);
            }
          });
          // Otherwise include all model attributes
        } else {
          _.forIn(cteModel.tableAttributes, function (attr, name) {
            cteModelAttributes.push(name);
          });
        }
      }

      cteAllAttributes = cteModelAttributes.concat(cteAttributes);

      if (_.isUndefined(cteName)) {
        throw new Error('No \'name\' property given to CTE.');
      }

      cteQueryItems.push(cteName);
      cteQueryItems.push('( ');
      cteQueryItems.push(cteAllAttributes.map(self.escapeAttributes, this).join(','));
      cteQueryItems.push(' ) AS ( ');

      cteName = cteName.indexOf(Utils.TICK_CHAR) < 0 && cteName.indexOf('"') < 0
        ? self.quoteIdentifiers(cteName) : cteName;
      ...
      cteQueryItems.push('SELECT ');

      cteModelAttributes.forEach(function (attr) {

        var addTable = !(Array.isArray(attr) && attr.length === 2 && attr[0]._isSequelizeMethod);

        attr = self.escapeAttributes(attr);

        if ((options.include || options.cteSelect) && attr.indexOf('.') === -1 && addTable) {
          attr = modelAs + '.' + attr;
        }

        initialSelectItems.push(attr);

      });

      cteAttributes.forEach(function (attr) {

        if (typeof initial[attr] !== 'undefined') {
          if (_.isPlainObject(initial[attr])) {
            initialSelectItems.push(self.attributeItemQuery(initial[attr], options, modelAs));
          } else {
            initialSelectItems.push(initial[attr]);
          }
        } else {
      ...
      if (_.has(initial, 'include')) {

        initial.include.forEach(function (include) {
          var results = self.generateJoinQueries(include, modelAs,
            { tableName: modelName, mainTableAs: modelAs, model: model },
            _.assign(cte, { isCTE: false }));
          initialJoinQueries = initialJoinQueries.concat(results.mainQuery);
        });

      }


      cteQueryItems.push(initialSelectItems.join(', '));
      cteQueryItems.push(' FROM ');
      cteQueryItems.push(modelName);
      if (modelAs) {
        cteQueryItems.push(' AS ' + modelAs);
      }

      if (initialJoinQueries.length > 0) {
        cteQueryItems.push(initialJoinQueries.join('') + ' ');
      }

      if (initial.where) {

        cteQueryItems.push(' WHERE ');
        cteQueryItems.push(self.getWhereConditions(initial.where, modelAs || modelName, model, options) + ' ');
      }
          
      // we need the order clause here if there is no recursive query since we depened on the recursive query for order otherwise
      if (self._dialect.supports.cteLimitOffsetOrder && cte.order && _.isUndefined(cte, 'recursive')) {

        if (Array.isArray(cte.order)) {
          cte.order.forEach(function (t) {
            if (Array.isArray(t) && _.size(t) > 1 && _.indexOf(cteAllAttributes, t[0]) !== - 1) {
              self.validateOrder(_.last(t));
              queryOrder.push((_.indexOf(cteAllAttributes, t[0]) + 1).toString() + ' ' + _.last(t));
            }
          }.bind(this));
        }
        if (queryOrder.length) {
          cteQueryItems.push(' ORDER BY ' + queryOrder.join(', '));
        }
      }
      ...
      if (recursive) {
        ...
        if (_.has(recursive, 'next')) {

          recursiveJoinTable = recursive.next.as;
          recursiveJoinTable = recursiveJoinTable.indexOf(Utils.TICK_CHAR) < 0 && recursiveJoinTable.indexOf('"') < 0
            ? self.quoteIdentifiers(recursiveJoinTable) : recursiveJoinTable;

          recursiveJoinQueries = recursiveJoinQueries.concat(self.generateJoinQueries(
            recursive.next,
            recursiveJoinTable,
            { tableName: modelName, mainTableAs: modelAs, model: model },
            _.assign(cte, { isCTE: true })
            ).mainQuery);

          if (_.has(recursive, 'include')) {
            recursive.include.forEach(function (include) {
              recursiveJoinQueries = recursiveJoinQueries.concat(self.generateJoinQueries(
                include,
                include.as,
                { tableName: modelName, mainTableAs: modelAs, model: model },
                _.assign(cte, { isCTE: true, name: recursive.next.as })
                ).mainQuery);
            });
          }
        }
        ...
        if (typeof unique !== 'undefined') {
          cteQueryItems.push(unique ? 'UNION' : 'UNION ALL');
        }
        else {
          cteQueryItems.push('UNION');
        }
        ...
        cteModelAttributes.forEach(function (attr) {
          ...
          attr = self.escapeAttributes(attr);
          attr = recursiveJoinTable + '.' + attr;

          recursiveSelectItems.push(attr);

        });

        cteAttributes.forEach(function (attr) {

          if (!_.has(recursive, attr)) {
            throw new Error('Missing attribute ' + attr + ' value in CTE definition of recursive selection.');
          }

          if (_.isPlainObject(recursive[attr])) {
            recursiveSelectItems.push(self.attributeItemQuery(recursive[attr], options, recursiveJoinTable, cteName));
          } else {
            recursiveSelectItems.push(recursive[attr]);
          }

        });

        cteQueryItems.push(recursiveSelectItems.join(', '));
        cteQueryItems.push(' FROM ');
        cteQueryItems.push(cteName);

        if (recursiveJoinQueries.length > 0) {
          cteQueryItems.push(recursiveJoinQueries.join('') + ' ');
        }


        if (recursive.where) {

          var modelConditions = recursive.where.model
            , cteConditions = recursive.where.cte
            , recursiveWhereItems = [' WHERE '];

          if (modelConditions) {
            recursiveWhereItems.push(self.getWhereConditions(modelConditions, modelAs || modelName, model, options) + ' ');
          }

          if (cteConditions) {
            if (recursiveWhereItems.length > 1) {
              recursiveWhereItems.push(' AND ');
            }
            recursiveWhereItems.push(self.getWhereConditions(cteConditions, cteName, model, options) + ' ');
          }
          cteQueryItems.push(recursiveWhereItems.join(''));
        }

        //Add ORDER BY to recursive query
        if (self._dialect.supports.cteLimitOffsetOrder && cte.order) {
      ...
      //Add LIMIT to recursive query
      if (self._dialect.supports.cteLimitOffsetOrder) {
        cteQueryItems.push(this.addLimitAndOffset(cte));
      }

      cteQueryItems.push(' ) ');
    }, this);

    return cteQueryItems.join('');
  },
  ...
  escapeAttributes: function(attr) {
      var self = this;
  ...
  */
  attributeItemQuery: function (object, options, modelName, cteName) {
    options = options || {};
    ...
    if (!_.isArray(operands)) {
      if (key === '$model') {
        return modelName + '.' + self.quoteIdentifier(operands);
      } else if (key === '$cte') {
        return cteName + '.' + self.quoteIdentifier(operands);
      } else if (key === '$col') {
        return self.handleSequelizeMethod(self.sequelize.col(operands));
      } else {
    ...
    operands = operands.map(function (operand) {
      ...
      if (_.isPlainObject(operand)) {
        return self.attributeItemQuery(operand, options, modelName, cteName);
  ...
  generateJoinQueries: function (include, parentTable, model, options) {

    var self = this
      , table = include.model.getTableName()
      , as = include.as
      , joinQueryItem = ''
      , subQuery = options.subQuery === undefined ?
    ...
    if (options.isCTE) {
      as = parentTable;
    } else if (model.tableName !== parentTable && model.mainTableAs !== parentTable) {
    ...
    if (options.includeIgnoreAttributes !== false) {
      ...
      if (include.subQuery && subQuery) {
        results.subQueryAttributes = results.subQueryAttributes.concat(attributes);
      } else {
        results.mainAttributes = results.mainAttributes.concat(attributes);
      }
    }
    ...
    if (through) {
      ...
      if (options.includeIgnoreAttributes !== false) {
        ...
        results.mainAttributes = results.mainAttributes.concat(throughAttributes);
      }
      ...
      if (subQuery && !include.subQuery && !include.parent.subQuery && include.parent.model !== model.model) {
        attrSource = association.source.rawAttributes[primaryKeysSource[0]].field;
      }
      ...
      } else if (options.isCTE) {
        sourceJoinOn = self.quoteIdentifier(options.name) + '.' +
        self.quoteIdentifier(attrSource) + ' = ';
      } else {
    ...
    if (include.subQuery && subQuery) {
      results.subQuery.push(joinQueryItem);
    } else {
      results.mainQuery.push(joinQueryItem);
    }
    ...
    if (include.include) {
      include.include.filter(function (include) {
        ...
        var childResults = self.generateJoinQueries(childInclude, as, model, options);

        if (childInclude.subQuery && subQuery) {
          results.subQuery = results.subQuery.concat(childResults.subQuery);
        }
        if (childResults.mainQuery) {
          results.mainQuery = results.mainQuery.concat(childResults.mainQuery);
        }
        if (childResults.subQueryAttributes.length > 0) {
          results.subQueryAttributes = results.subQueryAttributes.concat(childResults.subQueryAttributes);
        }
        if (childResults.mainAttributes.length > 0) {
          results.mainAttributes = results.mainAttributes.concat(childResults.mainAttributes);
        }

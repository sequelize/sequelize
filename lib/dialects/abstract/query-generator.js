...
class QueryGenerator {
  ...
  */
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    // EDIT BY GUIDO: add inner joins to the subquery, to prevent missing joins errors
    if (!include.subQuery
      && joinQueries.subQuery.filter((j) => j !== '').length > 0
      && joinQueries.subQuery.filter((j) => j === ` ${joinQuery.join} ${joinQuery.body} ON ${joinQuery.condition}`).length === 0) {
      joinQueries.subQuery.unshift(` ${joinQuery.join} ${joinQuery.body} ON ${joinQuery.condition}`);
      // joinQueries.mainQuery = _.uniq(joinQueries.mainQuery.concat(joinQueries.subQuery));
      // attributes.main = _.uniq(attributes.main.concat(attributes.subQuery));
    }

  ...
  */
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      let attrs = include.attributes;
      if (attrs.exclude) {
        attrs = Object.keys(include.model.rawAttributes).filter(elem => !attrs.exclude.includes(elem));
      }
      const includeAttributes = attrs.map(attr => {
        let attrAs = attr;
        let verbatim = false;

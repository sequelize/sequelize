...
const QueryGenerator = {
  ...
  */
  quote(obj, parent, force, internal) {
    if (Utils._.isString(obj)) {
      ...
      for (i = 0; i < len - 1; i++) {
        ...
        if (association) {
          ...
          throw new Error('\'' + tableNames.join(tableConnector) + '\' in order / group clause is not valid association');
        }
      }

      // add 1st string as quoted, 2nd as unquoted raw
      let sql = (i > 0 ? this.quoteIdentifier(tableNames.join(tableConnector)) + '.' : (Utils._.isString(obj[0]) && parent ? this.quoteIdentifier(parent.name) + '.' : '')) + this.quote(obj[i], parent, force);
      if (i < len - 1) {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (options.include) {
      ...
      for (const include of options.include) {
        ...
        const joinQueries = this.generateInclude(include, { externalAs: mainTable.as, internalAs: mainTable.as }, options, mainTable, attributes, subQuery);
  ...
  generateInclude(include, parentTableName, topLevelOptions, topLevelTableInfo, topLevelTableAttributes, topLevelSubQuery) {
    ...
    topLevelOptions.keysEscaped = true;

    if (topLevelTableInfo.name !== parentTableName.externalAs && topLevelTableInfo.as !== parentTableName.externalAs) {
      includeAs.internalAs = `${parentTableName.internalAs}_${include.as}`;
      includeAs.externalAs = `${parentTableName.externalAs}.${include.as}`;
    }

    // includeIgnoreAttributes is used by aggregate functions
    if (topLevelOptions.includeIgnoreAttributes !== false) {
      ...
      if (include.subQuery && topLevelSubQuery) {
        for (let attr of attributes) {
          topLevelTableAttributes.subQuery.push(attr);
        }
      } else {
        for (let attr of attributes) {
          topLevelTableAttributes.main.push(attr);
        }
      }
    }
    ...
    //through
    if (include.through) {
      joinQuery = this.generateThroughJoin(include, includeAs, parentTableName.internalAs, topLevelTableInfo, topLevelOptions, topLevelTableAttributes, topLevelSubQuery);
    } else {

      if (topLevelSubQuery && include.subQueryFilter) {
        ...
        if (!topLevelOptions.where) topLevelOptions.where = {};
        ...
        if (Utils._.isPlainObject(topLevelOptions.where)) {
          topLevelOptions.where['__' + includeAs] = subQueryWhere;
        } else {
          topLevelOptions.where = { $and: [topLevelOptions.where, subQueryWhere] };
        }
      }

      joinQuery = this.generateJoin(include, topLevelOptions, topLevelSubQuery);
    ...
    if (include.include) {
      ...
      for (const childInclude of include.include) {
        ...
        if (childInclude.seperate || childInclude._pseudo) {
          continue;
        }

        const childJoinQueries = this.generateInclude(childInclude, includeAs, topLevelOptions, topLevelTableInfo, topLevelTableAttributes, topLevelSubQuery);
        ...
        if (childInclude.subQuery && topLevelSubQuery) {
  ...
  generateJoin(include, topLevelOptions, topLevelSubQuery) {

    const association = include.association;
    const parent = include.parent;
    const parentIsTop = !!parent && !include.parent.association && include.parent.model.name === topLevelOptions.model.name;
    ...
    if ((topLevelOptions.groupedLimit && parentIsTop) || (topLevelSubQuery && include.parent.subQuery && !include.subQuery)) {
      if (parentIsTop) {
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelTableInfo, topLevelOptions, topLevelAttributes, topLevelSubQuery) {
    ...
    const association = include.association;
    const parentIsTop = !include.parent.association && include.parent.model.name === topLevelTableInfo.model.name;
    ...
    if (topLevelOptions.includeIgnoreAttributes !== false) {
      ...
      for (let attr of throughAttributes) {
        topLevelAttributes.main.push(attr);
      }
    ...
    if (topLevelSubQuery && !include.subQuery && !include.parent.subQuery && include.parent.model !== topLevelOptions.mainModel) {
      attrSource = association.source.rawAttributes[primaryKeysSource[0]].field;
    }
    ...
    if (topLevelSubQuery && !include.subQuery && include.parent.subQuery && !parentIsTop) {
    ...
    if (include.where || include.through.where) {
      if (include.where) {
        targetWhere = this.getWhereConditions(include.where, this.sequelize.literal(this.quoteIdentifier(includeAs.internalAs)), include.model, topLevelOptions);
        if (targetWhere) {
          joinCondition += ` AND ${targetWhere}`;
        }
      }
      if (topLevelSubQuery && include.required) {
        ...
        topLevelOptions.where['__' + throughAs] = this.sequelize.asIs([
          '(',
          $query.replace(/\;$/, ''),
          ')',
          'IS NOT NULL'
    ...
    if (Array.isArray(options.order)) {
      for (const t of options.order) {
        ...
        if (subQuery && (Array.isArray(t) && !(typeof t[0] === 'function' && t[0].prototype instanceof Model) && !(t[0] && typeof t[0].model === 'function' && t[0].model.prototype instanceof Model))) {
          subQueryOrder.push(this.quote(t, model, false, true));
        }

        mainQueryOrder.push(this.quote(t, model, false, true));
      }
    } else {
      mainQueryOrder.push(this.quote(typeof options.order === 'string' ? new Utils.Literal(options.order) : options.order, model, false, true));
    }

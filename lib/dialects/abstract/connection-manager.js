...
class ConnectionManager {
  constructor(dialect, sequelize) {
    ...
    this.config = _.cloneDeep(sequelize.config);
    this.dialectName = this.sequelize.options.dialect;
    this.versionPromise = null;

    if (this.config.pool === false) {
      throw new Error('Support for pool:false was removed in v4.0');
    }

    this.config.pool = _.defaults(
      this.config.pool || {}, {
        max: 5,
        min: 0,
        idle: 10000,
  ...
  connect() {
    throw new Error('Please implement "connect" method in connection manager');
  }

  disconnect() {
    throw new Error('Please implement "disconnect" method in connection manager');
  }
}

...
const debug = Logger.getLogger().debugContext().extend('pool');
...
class ConnectionManager {
  ...
  _onProcessExit() {
    ...
    return this.pool.drain().then(() => {
      debug.log('connection drain due to process exit');
  ...
  initPools() {
    ...
    if (!config.replication) {
      this.pool = new Pool({
        ...
        destroy: connection => {
          return this._disconnect(connection)
            .tap(() => { debug.log('connection destroy'); });
        },
      ...
      debug.log(`pool created with max/min: ${config.pool.max}/${config.pool.min}, no replication`);
    ...
    this.pool = {
      ...
      destroy: connection => {
        ...
        debug.log('connection destroy');
      },
      destroyAllNow: () => {
        return Promise.join(
          this.pool.read.destroyAllNow(),
          this.pool.write.destroyAllNow()
        ).tap(() => { debug.log('all connections destroyed'); });
      },
      drain: () => {
        return Promise.join(
    ...
    debug.log(`pool created with max/min: ${config.pool.max}/${config.pool.min}, with replication`);
  }
  ...
  getConnection(options) {
    ...
    return promise.then(() => {
      return this.pool.acquire(options.type, options.useMaster)
        ...
        .tap(() => { debug.log('connection acquired'); });
    });
  }

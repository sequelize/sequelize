...
class ConnectionManager {
      ...
      this.pool = Pooling.createPool(
        {
          ...
          destroy: mayBeConnection => {
            ...
            return this._disconnect(mayBeConnection).then(result => {
    ...
    this.pool = {
      ...
      destroy: mayBeConnection => {
        if (mayBeConnection.queryType === undefined) {
          return Promise.all([
            this.pool.read.destroy(mayBeConnection).catch(err => { if (!/Resource not currently part of this pool/.test(err.message)) throw err; }),
            this.pool.write.destroy(mayBeConnection).catch(err => { if (!/Resource not currently part of this pool/.test(err.message)) throw err; })
          ]);
        }
      ...
      clear: () => {
        return Promise.all([this.pool.read.clear(), this.pool.write.clear()]).then(result => {
      ...
      read: Pooling.createPool(
        {
          create: () => {
            ...
            return this._connect(config.replication.read[nextRead])
              .then(connection => {
      ...
      write: Pooling.createPool(
        {
          create: () => {
            return this._connect(config.replication.write)
              .then(connection => {
  ...
  getConnection(options) {
    ...
    return promise.then(() => {
      return this.pool
        .acquire(options.priority, options.type, options.useMaster)
  ...
  releaseConnection(connection) {
    return this.pool
      ...
      .catch(err => {
        if (!/Resource not currently part of this pool/.test(err.message)) throw err;
      });
  }
  ...
  _determineConnection(mayBeConnection) {
    if (mayBeConnection instanceof Error) {
      return Promise.resolve(this.pool.destroy(mayBeConnection))
        .catch(err => { if (!/Resource not currently part of this pool/.test(err.message)) throw err; })
        .then(() => {

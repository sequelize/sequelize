...
class ConnectionManager {
  ...
  async _connect(config) {
    await this.sequelize.hooks.run('beforeConnect', config);
    const connection = await this.dialect.connectionManager.connect(config);
    await this.sequelize.hooks.run('afterConnect', connection, config);
    return connection;
  }
  ...
  async _disconnect(connection) {
    await this.sequelize.hooks.run('beforeDisconnect', connection);
    await this.dialect.connectionManager.disconnect(connection);
    return this.sequelize.hooks.run('afterDisconnect', connection);
  }

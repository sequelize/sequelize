...
ConnectionManager.prototype.initPools = function () {
  ...
  if (!config.replication) {
    this.pool = Pooling.Pool({
	  ...
	  // Hook something on connection
	  if (config.dialectOptions && config.dialectOptions.onConnect && typeof(config.dialectOptions.onConnect) === 'function') {
	      // Do not forget to call the callback
	      config.dialectOptions.onConnect(connection, err, callback);
	  } else {

...
class Db2QueryGenerator extends AbstractQueryGenerator {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    let outputFragment;
    const tmpTable = '';         //tmpTable declaration for trigger
    ...
    for (const attr in attributes) {
      if (attributes.hasOwnProperty(attr)) {
        ...
        if (_.includes(dataType, 'PRIMARY KEY')) {
          ...
          if (options && options.indexes && options.indexes.length) {
            let indx;
            for (let i = 0; i < options.indexes.length; i++) {
              indx = options.indexes[i];
              if (_.includes(indx.fields, attr) && ! _.includes(dataType, 'NOT NULL')) {
                dataType += ' NOT NULL';
              }
              if (_.includes(indx.fields, attr) && indx.unique && ! _.includes(dataType, 'UNIQUE')) {
                dataType += ' UNIQUE';
              }
            }
          }

...
class Query extends AbstractQuery {
  getInsertIdField() {
    return 'SELECT IDENTITY_VAL_LOCAL() AS ID FROM SYSIBM.SYSDUMMY1';
  }
  ...
  run(sql, parameters) {
    sql = sql.replace(/\;$/, '');
    // console.log(`IBMiQuery::run(${sql}, ${parameters})`);
    // return this.connection.query(sql, parameters)
    return new Promise(async (resolve, reject) => {
      try {
        this.connection.query(sql, parameters, function(error, result) {
    ...
    //   // sql = AbstractQuery.formatBindParameters(sql, this.options.bind, this.options.dialect || 'ibmi', { skipUnescape: true })[0];
    //   this.sql = sql;
    // }

  ...
  handleShowIndexesQuery(data) {
    ...
    data.forEach(item => {
      ...
      if (Object.prototype.hasOwnProperty.call(indexes, item['NAME'])) {
        indexes[item['NAME']].fields.push({ attribute: item['COLUMN_NAME'], length: undefined, order: undefined });
      } else {
        indexes[item['NAME']] = {
          primary: item['CONSTRAINT_TYPE'] === 'PRIMARY KEY',
          fields: [{ attribute: item['COLUMN_NAME'], length: undefined, order: undefined }],
          name: item['NAME'],
  ...
  formatError(err) {
    ...
    if (Object.prototype.hasOwnProperty.call(err, 'odbcErrors') && err.odbcErrors.length > 0)
    {
      ...
      if (odbcError.code === -204) {
        ...
        let type = undefined;
        const constraintNameRegex = /"([a-zA-Z0-9]+?)" in [^]+? type (\*\w+?) not found./;
        const constraintNameRegexMatches = odbcError.message.match(constraintNameRegex);
        if (constraintNameRegexMatches.length === 3)
        {

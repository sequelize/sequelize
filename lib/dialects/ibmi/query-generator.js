...
class IBMiQueryGenerator extends AbstractQueryGenerator {
  ...
  dropSchema() {
    ...
    //return `BEGIN IF EXISTS (SELECT * FROM SYSIBM.SQLSCHEMAS WHERE TABLE_SCHEM = '${schema}') THEN RELEASE SAVEPOINT A; DROP SCHEMA "${schema}" CASCADE; COMMIT; END IF; END`;
  }

  showSchemasQuery() {
    return 'SELECT DISTINCT SCHEMA_NAME FROM QSYS2.SYSSCHEMAAUTH WHERE GRANTEE = CURRENT USER';
  }
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    return `CREATE TABLE ${this.quoteTable(tableName)} (${attributesClause})`;
  }
  ...
  dropTableQuery(tableName, options) {
    ...
    return `BEGIN IF EXISTS (SELECT * FROM SYSIBM.SQLTABLES WHERE TABLE_SCHEM = '${schema}' AND TABLE_NAME = '${table}') THEN DROP TABLE "${schema}"."${table}"; COMMIT; END IF; END`;
  }

  describeTableQuery(tableName, schema) {
    return `SELECT * FROM SYSIBM.COLUMNS WHERE TABLE_SCHEMA = '${schema}' AND TABLE_NAME = '${tableName}'`;
  }
  ...
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    if (this._dialect.supports.indexViaAlter) {
      ind = [
        ...
        tableName,
        concurrently,
        `ADD CONSTRAINT ${this.quoteIdentifiers(options.name)}`
      ];
    } else {
      ind = ['CREATE'];
    }

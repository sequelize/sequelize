...
class IBMiQueryGenerator extends AbstractQueryGenerator {
  ...
  */
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    return `CREATE${options.unique ? ' UNIQUE' : ''} INDEX ${schema ? ` ${schema}.` : ''}${this.quoteIdentifiers(options.name)} ON ${tableName} (${fieldsSql.join(', ')}${options.operator ? ` ${options.operator}` : ''})`;
  }

...
class IBMiQueryGenerator extends AbstractQueryGenerator {
  ...
  dropSchema(schema) {
    return `BEGIN IF EXISTS (SELECT * FROM SYSIBM.SQLSCHEMAS WHERE TABLE_SCHEM = '${schema}') THEN DROP SCHEMA "${schema}"; COMMIT; END IF; END`;
  }
  ...
  showTablesQuery(schema) {
    return `SELECT * FROM SYSIBM.SQLTABLES ${schema ? ` WHERE TABLE_SCHEM = '${schema}'`: ''}`;
  }

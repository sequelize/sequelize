...
const QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    let query = "IF OBJECT_ID('<%= table %>', 'U') IS NULL CREATE TABLE <%= table %> (<%= attributes %>)"
      , primaryKeys = []
      , foreignKeys = {}
      , attrStr = []
      , self = this;
  ...
  addColumnQuery(table, key, dataType) {
    ...
    let query = 'ALTER TABLE <%= table %> ADD <%= attribute %>;'
  ...
  changeColumnQuery(tableName, attributes) {
    ...
    let attrString = [], constraintString = [];
  ...
  renameColumnQuery(tableName, attrBefore, attributes) {
    let query = "EXEC sp_rename '<%= tableName %>.<%= before %>', '<%= after %>', 'COLUMN';"

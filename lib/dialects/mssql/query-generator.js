...
const QueryGenerator = {
   ...
   * @returns {String}
   */
  _getForeignKeysQueryPrefix() {
    return 'SELECT ' +
        'constraint_name = TC.CONSTRAINT_NAME, ' +
        'constraintName = TC.CONSTRAINT_NAME, ' +
        'constraintSchema = TC.CONSTRAINT_SCHEMA, ' +
        'constraintCatalog = TC.CONSTRAINT_CATALOG, ' +
        'tableName = TC.TABLE_NAME, ' +
        'tableSchema = TC.TABLE_SCHEMA, ' +
        'tableCatalog = TC.TABLE_CATALOG, ' +
        'columnName = KCU.COLUMN_NAME, ' +
        'referencedTableSchema = RTC.TABLE_SCHEMA, ' +
        'referencedTableCatalog = RTC.TABLE_CATALOG, ' +
        'referencedTableName = RTC.TABLE_NAME, ' +
        'referencedColumnName = RKCU.COLUMN_NAME ' +
      'FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS TC ' +
        ...
        'INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS RTC ' +
          'ON RC.UNIQUE_CONSTRAINT_SCHEMA = RTC.CONSTRAINT_SCHEMA ' +
          'AND RC.UNIQUE_CONSTRAINT_NAME = RTC.CONSTRAINT_NAME ' +
        'INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE RKCU ' +
          'ON RTC.CONSTRAINT_SCHEMA = RKCU.CONSTRAINT_SCHEMA ' +
          'AND RTC.CONSTRAINT_NAME = RKCU.CONSTRAINT_NAME ' +
  ...
  getForeignKeysQuery(table, catalogName) {
    ...
    if (catalogName) {
      sql += ' AND TC.TABLE_CATALOG =' + wrapSingleQuote(catalogName);
    }
  ...
  getForeignKeyQuery(table, attributeName) {
    ...
    let sql = this._getForeignKeysQueryPrefix() +
      ' AND TC.TABLE_NAME =' + wrapSingleQuote(tableName) +
      ' AND KCU.COLUMN_NAME =' + wrapSingleQuote(attributeName);

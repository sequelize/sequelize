  ...
  selectSqlFragmentBuilder: function(options, model, attributes, tables, mainTableAs) {
    ...
    if (semver.satisfies(this.sequelize.options.databaseVersion, '<11.0.0')) {
      ...
      if (options.offset) {
        ...
        var fragment = 'WITH ' + tmpTable + ' AS (' +
          'SELECT ROW_NUMBER() OVER (ORDER BY ' + mainQueryOrder.join(', ') + ') as row_num, ' + attributes.join(', ') +
          ' FROM ' + tables + ')' +
          'SELECT ' + topFragment + ' * FROM ' + tmpTable + ' WHERE row_num > ' + options.offset;
        return fragment;
      } else {

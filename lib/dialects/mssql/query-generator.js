...
var QueryGenerator = {
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, attributes) {
    ...
    var query = '<%= tmpTable %>INSERT INTO <%= table %> (<%= attributes %>)<%= output %> VALUES <%= tuples %>;'
      , emptyQuery = '<%= tmpTable %>INSERT INTO <%= table %><%= output %> DEFAULT VALUES'
      , tuples = []
      , allAttributes = []
    ...
    if (options.returning && !!!options.hasTrigger) {
      outputFragment = ' OUTPUT INSERTED.*';
    } else if (options.returning && options.hasTrigger && this._dialect.supports.tmpTableTrigger) {
      tmpTable = 'declare @tmp table (<%= columns %>); ';
      
      for (modelKey in attributes){
        attribute = attributes[modelKey];
        if(!(attribute.type instanceof DataTypes.VIRTUAL || attribute.type instanceof DataTypes.TIMESTAMP)){
      ...
      tmpTable = Utils._.template(tmpTable)({columns : tmpColumns}).trim();

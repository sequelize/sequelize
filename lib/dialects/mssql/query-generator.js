...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    var query = "IF OBJECT_ID('<%= d.table %>', 'U') IS NULL CREATE TABLE <%= d.table %> (<%= d.attributes %>)"
      , primaryKeys = []
      , foreignKeys = {}
      , attrStr = []
      , self = this;
    ...
    keys = Object.keys(foreignKeys);
    for (i = 0; i < keys.length; i++) {
      values.attributes += ', FOREIGN KEY (' + this.quoteIdentifier(keys[i]) + ') ' + foreignKeys[keys[i]];
    }
  ...
  renameTableQuery: function(before, after) {
    var query = 'EXEC sp_rename <%= d.before %>, <%= d.after %>;';
  ...
  dropTableQuery: function(tableName) {
    var query = "IF OBJECT_ID('<%= d.table %>', 'U') IS NOT NULL DROP TABLE <%= d.table %>";
    var values = {
      table: this.quoteTable(tableName)
    };
  ...
  removeColumnQuery: function(tableName, attributeName) {
    var query = 'ALTER TABLE <%= d.tableName %> DROP <%= d.attributeName %>;';
    return Utils.compileOrRunTemplate(query, {
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= d.tableName %> ALTER COLUMN <%= d.attributes %>;';
    var attrString = [];

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      var definition = attributes[keys[i]];

      attrString.push(Utils.compileOrRunTemplate('<%= d.attrName %> <%= d.definition %>', {
        attrName: this.quoteIdentifier(keys[i]),
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    var query = "EXEC sp_rename '<%= d.tableName %>.<%= d.before %>', '<%= d.after %>', 'COLUMN';"
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, attributes) {
    var query = 'INSERT INTO <%= d.table %> (<%= d.attributes %>)<%= d.output %> VALUES <%= d.tuples %>;'
      , emptyQuery = 'INSERT INTO <%= d.table %><%= d.output %> DEFAULT VALUES'
      , tuples = []
      , allAttributes = []
    ...
    var generatedQuery = Utils.compileOrRunTemplate(allQueries.join(';'), replacements);
  ...
  deleteQuery: function(tableName, where, options) {
    ...
    var limit = ''
      , query = 'DELETE<%= d.limit %> FROM <%= d.table %><%= d.where %>; ' +
  ...
  showIndexesQuery: function(tableName) {
    var sql = "EXEC sys.sp_helpindex @objname = N'<%= d.tableName %>';";
  ...
  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    var sql = 'DROP INDEX <%= d.indexName %> ON <%= d.tableName %>'
    ...
    return Utils.compileOrRunTemplate(sql, values);
  },
  ...
  attributesToSQL: function(attributes, options) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (attribute.references) {
        attribute = Utils.formatReferences(attributes[keys[i]]);
      ...
      if (keys[i] && !attribute.field) attribute.field = keys[i];
  ...
  dropForeignKeyQuery: function(tableName, foreignKey) {
    return Utils.compileOrRunTemplate('ALTER TABLE <%= d.table %> DROP <%= d.key %>', {
      table: this.quoteTable(tableName),

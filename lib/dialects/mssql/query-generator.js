...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    var query = "IF OBJECT_ID('<%= data.table %>', 'U') IS NULL CREATE TABLE <%= data.table %> (<%= data.attributes %>)"
      , primaryKeys = []
      , foreignKeys = {}
      , attrStr = []
      , self = this;
  ...
  renameTableQuery: function(before, after) {
    var query = 'EXEC sp_rename <%= data.before %>, <%= data.after %>;';
  ...
  dropTableQuery: function(tableName) {
    var query = "IF OBJECT_ID('<%= data.table %>', 'U') IS NOT NULL DROP TABLE <%= data.table %>";
    var values = {
      table: this.quoteTable(tableName)
    };
  ...
  removeColumnQuery: function(tableName, attributeName) {
    var query = 'ALTER TABLE <%= data.tableName %> DROP <%= data.attributeName %>;';
    return Utils.template(query, {
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= data.tableName %> ALTER COLUMN <%= data.attributes %>;';
    var attrString = [];
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      attrString.push(Utils.template('<%= data.attrName %> <%= data.definition %>', {
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    var query = "EXEC sp_rename '<%= data.tableName %>.<%= data.before %>', '<%= data.after %>', 'COLUMN';"
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, attributes) {
    var query = 'INSERT INTO <%= data.table %> (<%= data.attributes %>)<%= data.output %> VALUES <%= data.tuples %>;'
      , emptyQuery = 'INSERT INTO <%= data.table %><%= data.output %> DEFAULT VALUES'
      , tuples = []
      , allAttributes = []
    ...
    var generatedQuery = Utils.template(allQueries.join(';'), replacements);
  ...
  deleteQuery: function(tableName, where, options) {
    ...
    var limit = ''
      , query = 'DELETE<%= data.limit %> FROM <%= data.table %><%= data.where %>; ' +
  ...
  showIndexesQuery: function(tableName) {
    var sql = "EXEC sys.sp_helpindex @objname = N'<%= data.tableName %>';";
  ...
  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    var sql = 'DROP INDEX <%= data.indexName %> ON <%= data.tableName %>'
    ...
    return Utils.template(sql, values);
  },
  ...
  dropForeignKeyQuery: function(tableName, foreignKey) {
    return Utils.template('ALTER TABLE <%= data.table %> DROP <%= data.key %>', {
      table: this.quoteTable(tableName),

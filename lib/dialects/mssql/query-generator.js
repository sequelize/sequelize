...
class MSSQLQueryGenerator extends AbstractQueryGenerator {
  ...
  addLimitAndOffset(options, model) {
    ...
    console.log("isSubQuery------isSubQuery",isSubQuery);
    ...
    if (options.limit || options.offset) {
      ...
      if (model !== undefined) {
        if (model.primaryKeys !== undefined && !options.order.length && !isSubQuery) {
          Object.keys(model.primaryKeys).map((key) => {
            let primaryKey = model.primaryKeys[key];
            primaryKeys.push(`${this.quoteTable(options.tableAs || model.name)}.${this.quoteIdentifier(primaryKey.field)}`);
          });
          fragment += ' ORDER BY HERE!!' + primaryKeys.join(',');
        }
      }

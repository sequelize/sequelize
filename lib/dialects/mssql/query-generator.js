  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit || options.offset) {
      if (!options.order || options.include && !orders.subQueryOrder.length) {
        const orderByPrimaryKeyField = `${this.quoteTable(options.tableAs || model.name)}.${this.quoteIdentifier(model.primaryKeyField)}`;
        if (!orders.mainQueryOrder || orders.mainQueryOrder && orders.mainQueryOrder.includes(orderByPrimaryKeyField) ) {
          fragment += options.order && !isSubQuery ? ', ' : ' ORDER BY ';
          fragment += orderByPrimaryKeyField;
        }
      }

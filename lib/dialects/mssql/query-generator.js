    ...
    if (semver.satisfies(this.sequelize.options.databaseVersion, '<11.0.0')) {
      ...
      if (options.offset) {
        ...
        if (options.order) {
          ...
          mainQueryOrder.push(this.quoteIdentifier(model.primaryKeyField));
        }

        var tmpTable = (mainTableAs) ? mainTableAs : 'OffsetTable';
        var whereFragment = (where) ? ' WHERE ' + where : '';
        var fragment = 'SELECT TOP 100 PERCENT ' + attributes.join(', ') + ' FROM (SELECT ' + topFragment + '*' +
          ' FROM (SELECT ROW_NUMBER() OVER (ORDER BY ' + mainQueryOrder.join(', ') + ') as row_num, * ' +
          ' FROM ' + tables + ' AS ' + tmpTable + whereFragment + ') AS ' + tmpTable + ' WHERE row_num > ' + offset + ') AS ' + tmpTable;
        return fragment;
      } else {

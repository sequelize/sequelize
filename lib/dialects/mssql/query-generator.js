  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit || options.offset) {
      ...
      if (model.primaryKeys) {
        for (let primaryKey of Object.values(model.primaryKeys)) {
          console.log((options.tableAs || model.name) + '.' + primaryKey.field);
          primaryKeys.push(this.quoteTable(options.tableAs || model.name) + '.' + this.quoteIdentifier(primaryKey.field));
        }
      }
      else {
        primaryKeys.push(this.quoteTable(options.tableAs || model.name) + '.' + this.quoteIdentifier(model.primaryKeyField));
      }
      fragment += ' ORDER BY ' + primaryKeys.join(',');

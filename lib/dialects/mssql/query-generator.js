  ...
  var QueryGenerator = {
    ...
    */
    ...
    selectQuery: function(tableName, options, model) {
      ...
      options.table = table = !Array.isArray(tableName) ? SqlGenerator.quoteIdentifier(tableName) : tableName.map(function(t) {
        if (Array.isArray(t)) {
      ...
      if (options.group) {
        options.group = Array.isArray(options.group) ? options.group.map(function(t) { return SqlGenerator.quoteIdentifier(t, model); }.bind(this)).join(', ') : options.group;
        if (subQuery) {
        ...
        if (Array.isArray(options.order)) {
          options.order.forEach(function(t) {
            if (subQuery && !(t[0] instanceof Model) && !(t[0].model instanceof Model)) {
              subQueryOrder.push(SqlGenerator.quoteIdentifier(t, model));
            }
            mainQueryOrder.push(SqlGenerator.quoteIdentifier(t, model));
          }.bind(this));
        } else {
      ...
      if (options.offset) {
        ...
        if (subQuery) {
          if(!options.order){
            subQueryItems(' ORDER BY ' + model.primaryKeyAttribute);
          }
          subQueryItems.push(limitOrder);
        } else {
          if(!options.order){
            mainQueryItems.push(' ORDER BY ' + model.primaryKeyAttribute);
          }
    ...
    */
    ...
    handleSequelizeMethod: function (smth, tableName, factory, options, prepend) {
      ...
      if ((smth instanceof Utils.and) || (smth instanceof Utils.or)) {
        ...
        return SqlGenerator.quoteIdentifier(smth.col, factory);
      } else {

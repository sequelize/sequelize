...
class Query extends AbstractQuery {
  ...
  _run(connection, sql, parameters) {
    ...
    return new Promise((resolve, reject) => {
      ...
      } else if (this.sql.startsWith('ROLLBACK TRANSACTION')) {
      ...
      } else if (this.sql.startsWith('SAVE TRANSACTION')) {
  ...
  async run(sql, parameters) {
    const { acquire, release } = this.connection.lock();
    const connection = await acquire;
    try {
      return await this._run(connection, sql, parameters);
    } finally {
      release(connection);
    }
  }

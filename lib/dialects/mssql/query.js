...
Query.prototype.isShowOrDescribeQuery = function() {
  return (this.sql.toLowerCase().indexOf("select c.column_name as 'name', c.data_type as 'type', c.is_nullable as 'isnull'") === 0) || /* jshint ignore: line */
    (this.sql.toLowerCase().indexOf('select tablename = t.name, name = ind.name,') === 0) ||
    (this.sql.toLowerCase().indexOf('exec sys.sp_helpindex @objname') === 0);
};
...
Query.prototype.handleInsertQuery = function(results, metaData) {
  if (this.instance) {
    ...
    this.instance[autoIncrementField] = (results && results[0][this.getInsertIdField()]) ||
      (metaData && metaData[this.getInsertIdField()]) ||
      (results && results[0][autoIncrementField]) ||
      (autoIncrementFieldAlias && results && results[0][autoIncrementFieldAlias]);
  }
};

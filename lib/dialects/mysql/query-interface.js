...
function removeConstraint(tableName, constraintName, options) {
  ...
  return this.sequelize.query(sql, _.assign({}, options, { type: this.sequelize.QueryTypes.SHOWCONSTRAINTS }))
    .then(constraints => {
      ...
      switch (constraint.constraint_type) {
        case 'UNIQUE':
        case 'PRIMARY KEY':
          query = this.QueryGenerator.removeIndexQuery(constraint.table_name, constraint.constraint_name);
          break;
        case 'FOREIGN KEY':
          query = this.QueryGenerator.dropForeignKeyQuery(tableName, constraintName);
          break;
        default:
          throw new Error(`Constraint ${constraintName} on table ${tableName} does not exist`);
          break;
      }

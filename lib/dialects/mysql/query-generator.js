...
module.exports = (function() {
  var QueryGenerator = {
    ...
    selectQuery: function(tableName, options) {
      ...
      attributes = options.attributes && options.attributes.map(function(attr){
        if(Array.isArray(attr) && attr.length == 2) {
          return [attr[0], this.quoteIdentifier(attr[1])].join(' as ')
        } else if(attr.indexOf(Utils.TICK_CHAR) !== -1) {
          return attr
        } else {
          var daoFactory = options.daoFactory,
              updated = false
          attr = attr.split('.').filter(function(tok, i, attr) {
            var association,
                include

            // process deep table relationships into include
            if(daoFactory && (i > 0 || tok !== daoFactory.tableName)) {
              // get the association of the last table with this token
              association = daoFactory.getAssociationByAlias( tok )
              if(association) {
                if(!options.include) {
                  options.include = []
                ...
                // add to includes
                daoFactory = association.target
                include = { daoFactory: daoFactory, as: daoFactory.tableName, association: association }
                options.include.push(include)

                // replace options.attributes
                if(!updated && attr.length > 2) {
                  options.attributes.splice( options.attributes.indexOf( attr.join('.')), 1, attr.slice(-2).join('.'))
                  updated = true
                }
              } else {
                daoFactory = null
              }
            } else if(!daoFactory && i < attr.length - 2) {
                throw new Error('Attribute relationship lookup failed for "' + tok + '" in ' + util.inspect(attr))
            }

            // keep last two elements as-is
            if(i >= attr.length - 2) {
              daoFactory = null
              return true
            }
          ...
          return this.quoteIdentifiers(attr)
        }
      }.bind(this))

      if (options.include) {
        if( !options.attributes ) {
          attributes = [options.table + '.*']
        }
        ...
        options.include.forEach(function(include) {
          ...
          var tableLeft = include.association.source.tableName
          var attrLeft = ((include.association.associationType === 'BelongsTo') ?
            include.association.identifier :
            ((include.association.source.primaryKeyCount === 1) ?  Object.keys(include.association.source.primaryKeys)[0] : 'id'))

          var tableRight = table
          var attrRight = ((include.association.associationType === 'BelongsTo') ?
            ((include.daoFactory.primaryKeyCount === 1) ?  Object.keys(include.daoFactory.primaryKeys)[0] : 'id')
             : include.association.identifier)
          ...
          attributes = attributes.concat(
            Object.keys(include.daoFactory.attributes).filter(function(attr) {
              return (!options.attributes)
                || (attributes.indexOf(include.as + "." + attr) !== -1)
                || (attributes.indexOf(this.quoteIdentifiers(include.as + "." + attr)) !== -1)
                || (attr == attrLeft)
            }.bind(this)).map(function(attr) {
              if (options.attributes) {
                rmAttributes.push(this.quoteIdentifiers(include.as + "." + attr))
              }
              return this.quoteIdentifier(include.as) + "." + this.quoteIdentifier(attr) + " AS " + this.quoteIdentifier(include.as + "." + attr)
            }.bind(this)))
          rmAttributes.map(function(attr) {
              attr = attributes.indexOf(attr)
              if (attr !== -1) {
      ...
      var query = "SELECT " + attributes + " FROM " + options.table
      query += joinQuery
    ...
    showReferencesQuery: function(tableName, options) {
      var sql = "SELECT constraint_name, column_name, referenced_table_name, referenced_column_name FROM information_schema.key_column_usage WHERE table_schema = '<%= database %>' AND table_name = '<%= tableName %>' AND referenced_column_name is not null"
      return Utils._.template(sql)({
        tableName: tableName,

...
const QueryGenerator = {
  ...
  addColumnQuery(table, key, dataType, options) {
    ...
    let query = `ALTER TABLE ${this.quoteTable(table)} ADD ${this.quoteIdentifier(key)} ${definition}`;

    if (options && typeof options.after === 'string') {
      query += ` AFTER ${this.quoteIdentifier(options.after)};`;
    } else if (options && options.first === true) {
      query += ' FIRST;';
    } else {
      query += ';';
    }

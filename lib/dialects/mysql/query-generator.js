...
const QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const query =
      'CREATE TABLE IF NOT EXISTS <%= table %> (<%= attributes%>) ENGINE=<%= engine %><%= comment %><%= charset %><%= collation %><%= initialAutoIncrement %><%= rowFormat %>';
    const primaryKeys = [];
    const foreignKeys = {};
    const attrStr = [];
    ...
    const values = {
      ...
      comment: options.comment && typeof options.comment === 'string' ? ' COMMENT ' + this.escape(options.comment) : '',
    ...
    if (options.uniqueKeys) {
      for (const [key, columns] of Object.entries(options.uniqueKeys)) {
  ...
  showIndexesQuery(tableName, options) {
    return (
      ...
      ((options || {}).database ? ' FROM `' + options.database + '`' : '')
    );
  },
  ...
  attributeToSQL(attribute, options) {
    ...
    if (
      !['BLOB', 'TEXT', 'GEOMETRY', 'JSON'].includes(attributeString) &&

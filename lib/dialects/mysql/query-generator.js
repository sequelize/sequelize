        ...
        for (var daoName in options.include) {
          if (options.include.hasOwnProperty(daoName)) {
            ...
            if (association.connectorDAO) {
              ...
              leftTableName = dao.tableName;
              leftColumnName = 'id';
              rightTableName = association.connectorDAO.tableName;
              rightColumnName = association.identifier;
            } else if (association.associationType === 'BelongsTo') {
              leftTableName = dao.tableName;
              leftColumnName = association.identifier;
              rightTableName = tableName;
              rightColumnName = 'id';
            } else if (association.associationType === 'HasMany') {
              leftTableName = tableName;
              leftColumnName = association.identifier;
              rightTableName = dao.tableName;
              rightColumnName = 'id';
            } else {
              leftTableName = tableName;
              leftColumnName = 'id';
            ...
            query += ' LEFT OUTER JOIN ' + Utils.addTicks(rightTableName) + ' ON '
            query += Utils.addTicks(leftTableName) + '.' + Utils.addTicks(leftColumnName) + '='
            query += Utils.addTicks(rightTableName) + '.' + Utils.addTicks(rightColumnName)

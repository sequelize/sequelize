...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= d.table %> (<%= d.attributes %>) ENGINE=<%= d.engine %><%= d.comment %><%= d.charset %><%= d.collation %><%= d.initialAutoIncrement %>'
      , primaryKeys = []
      , foreignKeys = {}
      , attrStr = [];
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= d.tableName %> CHANGE <%= d.attributes %>;';
    var attrString = [];

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      var definition = attributes[keys[i]];
      attrString.push(Utils.compileOrRunTemplate('`<%= d.attrName %>` `<%= d.attrName %>` <%= d.definition %>', {
        attrName: keys[i],
        definition: definition
      }));
    }

    return Utils.compileOrRunTemplate(query, { tableName: this.quoteTable(tableName), attributes: attrString.join(', ') });
  },
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var definition = attributes[keys[i]];
      attrString.push(Utils.compileOrRunTemplate('`<%= d.before %>` `<%= d.after %>` <%= d.definition %>', {
        before: attrBefore,
        after: keys[i],
        definition: definition
      }));
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options) {
    var query = 'INSERT<%= d.ignoreDuplicates %> INTO <%= d.table %> (<%= d.attributes %>) VALUES <%= d.tuples %><%= d.onDuplicateKeyUpdate %>;'
      , tuples = []
      , allAttributes = []
  ...
  showIndexesQuery: function(tableName, options) {
    var sql = 'SHOW INDEX FROM <%= d.tableName %><%= d.options %>';
  ...
  findAutoIncrementField: function(factory) {
    var fields = []
      , names = Object.keys(factory.attributes);

    ...
    selectQuery: function(tableName, options) {
      ...
      attributes = options.attributes && options.attributes.map(function(attr){
        if(Array.isArray(attr) && attr.length == 2) {
          ...
          attr = attr.split('.').filter(function(tok, i, attr) {
            ...
            if(i > 0 || tok !== daoFactory.tableName) {
              ...
              if(association) {
                ...
                // add to includes
                include = { daoFactory: association.target, tableName: association.source.tableName, as: association.target.tableName, association: association }
                options.include.push(include)

                // replace options.attributes
                if(!updated && attr.length > 2) {
                  var o = options.attributes.splice( options.attributes.indexOf( attr.join('.')), 1, attr.slice(-2).join('.'))
                  updated = true
                }

                // next
                daoFactory = (association.associationType === 'BelongsTo') ? association.source : association.target
              } else {
                daoFactory = null
                return true
              }
      ...
      if (options.include) {
        ...
        options.include.forEach(function(include) {
          var srcTableName = include.tableName || tableName
          var table = include.daoFactory.tableName
          var as = include.as
          var tableLeft = ((include.association.associationType === 'BelongsTo') ? include.as : srcTableName)
          var attrLeft = 'id'

          if (include.daoFactory.primaryKeyCount === 1) {
            attrLeft = Object.keys(include.daoFactory.primaryKeys)[0]
          }

          var tableRight = ((include.association.associationType === 'BelongsTo') ? srcTableName : include.as)

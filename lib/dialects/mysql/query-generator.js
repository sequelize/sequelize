...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= data.table %> (<%= data.attributes %>) ENGINE=<%= data.engine %><%= data.comment %><%= data.charset %><%= data.collation %><%= data.initialAutoIncrement %>'
      , primaryKeys = []
      , foreignKeys = {}
      , attrStr = [];
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= data.tableName %> CHANGE <%= data.attributes %>;';
    var attrString = [];

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      var definition = attributes[keys[i]];
      attrString.push(Utils.template('`<%= data.attrName %>` `<%= data.attrName %>` <%= data.definition %>', {
        attrName: keys[i],
        definition: definition
      }));
    }

    return Utils.template(query, { tableName: this.quoteTable(tableName), attributes: attrString.join(', ') });
  },
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var definition = attributes[keys[i]];
      attrString.push(Utils.template('`<%= data.before %>` `<%= data.after %>` <%= data.definition %>', {
        before: attrBefore,
        after: keys[i],
        definition: definition
      }));
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options) {
    var query = 'INSERT<%= data.ignoreDuplicates %> INTO <%= data.table %> (<%= data.attributes %>) VALUES <%= data.tuples %><%= data.onDuplicateKeyUpdate %>;'
      , tuples = []
      , allAttributes = []
  ...
  showIndexesQuery: function(tableName, options) {
    var sql = 'SHOW INDEX FROM <%= data.tableName %><%= data.options %>';

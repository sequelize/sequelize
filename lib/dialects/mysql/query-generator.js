    ...
    selectQuery: function(tableName, options) {
      ...
      if (options.include) {
        ...
        options.include.forEach(function(include) {
          ...
          if (!options.join || options.join.indexOf(include) === -1) {
            options.join ? options.join.push(include) : options.join = [include]
          }
        })
      ...
      if (options.join) {
        options.join.forEach(function(join) {
          var type = 'LEFT OUTER'

          if (Array.isArray(join)) {
            // support for [Model, ('INNER' | 'LEFT INNER' | etc.) ]
            type = join[1]
            join = join[0]
          }

          var table = join.daoFactory.tableName
          var as = join.as
          var tableLeft = ((join.association.associationType === 'BelongsTo') ? join.as : tableName)
          var attrLeft = 'id'
          var tableRight = ((join.association.associationType === 'BelongsTo') ? tableName : join.as)
          var attrRight = join.association.identifier

          joinQuery += " " + type + " JOIN `" + table + "` AS `" + as + "` ON `" + tableLeft + "`.`" + attrLeft + "` = `" + tableRight + "`.`" + attrRight + "`"
        })
      }
      ...
      if (options.group) {
        ...
        options.group = options.group.map(function (group) {
            return group.split('.').map(function (identifier) {
                return QueryGenerator.addQuotes(identifier)
            }).join('.')
        }).join(', ')

...
const QueryGenerator = {
  ...
  _traverseJSON(items, baseKey, prop, item, path) {
    ...
    if (_.isPlainObject(item)) {
      _.forOwn(item, (value, itemProp) => {
        if (itemProp.indexOf('$') === 0) {
          items.push(this.whereItemQuery(this._castKey(pathKey, value, cast, true), {[itemProp]: value}));
          return;
        }
    ...
    items.push(this.whereItemQuery(this._castKey(pathKey, item, cast, true), {$eq: item}));
  },

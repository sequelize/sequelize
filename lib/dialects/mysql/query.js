...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    if (benchmark) {
      ...
      this.sequelize.log(`Executing (${this.connection.uuid || 'default'}): ${this.sql} with parameters ${logger.inspect(parameters)}`, this.options);
    }

    debug(`executing(${this.connection.uuid || 'default'}) : ${this.sql} with parameters ${logger.inspect(parameters)}`);

    return new Utils.Promise((resolve, reject) => {
      const handler = (err, results) => {
        debug(`executed(${this.connection.uuid || 'default'}) : ${this.sql} with parameters ${logger.inspect(parameters)}`);

        if (benchmark) {
          this.sequelize.log(`Executed (${this.connection.uuid || 'default'}): ${this.sql} with parameters ${logger.inspect(parameters)}`, Date.now() - queryBegin, this.options);
        }

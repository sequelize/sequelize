...
class Query extends AbstractQuery {
            ...
            resolve(
              new Promise((resolve, reject) => {
                ...
                function afterExecute(err, results) {
                  ...
                  if (benchmark) {
                    query.sequelize.log(
                      'Executed (' + (query.database.uuid || 'default') + '): ' + query.sql,
                      Date.now() - queryBegin,
                      query.options
                    );
                  }
                        ...
                        results = results.map((result) => {
                          return _.mapValues(result, (value, name) => {
                            ...
                            if (name.indexOf('.') !== -1) {
                              ...
                              model = prefixes[name.slice(0, lastind)];
  ...
  getDatabaseMethod() {
    if (this.isUpsertQuery()) {
      ...
      this.sql.toLowerCase().indexOf('CREATE TEMPORARY TABLE'.toLowerCase()) !== -1 ||

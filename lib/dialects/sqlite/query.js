                ...
                if (err) {
                  ...
                  if (this.sql.indexOf('sqlite_master') !== -1) {
                    ...
                    if (!this.options.raw) {
                      ...
                      const prefixes = this.$collectModels(this.options.include);

                      results = results.map(result => _.mapValues(result, (value, name) => {
                        let model;
                        if (name.indexOf('.') !== -1) {
                    ...
                    result = this.handleSelectQuery(results);
                  } else if (this.isShowOrDescribeQuery()) {
                    result = results;
                  } else if (this.sql.indexOf('PRAGMA INDEX_LIST') !== -1) {
                    result = this.handleShowIndexesQuery(results);
                  } else if (this.sql.indexOf('PRAGMA INDEX_INFO') !== -1) {
                    result = results;
                    ...
                    result = results;
                  } else if (this.sql.indexOf('PRAGMA foreign_key_list') !== -1) {
                    result = results;
                  } else if ([QueryTypes.BULKUPDATE, QueryTypes.BULKDELETE].indexOf(this.options.type) !== -1) {
                    result = metaData.changes;
                  } else if (this.options.type === QueryTypes.UPSERT) {
                    result = undefined;
                  } else if (this.options.type === QueryTypes.VERSION) {
                    result = results[0].version;
                  } else if (this.options.type === QueryTypes.RAW) {

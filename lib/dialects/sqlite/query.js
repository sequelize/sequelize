...
class Query extends AbstractQuery {
                ...
                if (err) {
                  ...
                  if (query.sql.indexOf('sqlite_master') !== -1) {
                    if (query.sql.indexOf('SELECT sql FROM sqlite_master WHERE tbl_name') !== -1) {
                      result = results;
                      if (result[0].sql.indexOf('CONSTRAINT') !== -1) {
  ...
  parseConstraintsFromSql(sql) {
    ...
    let referenceTableName, referenceTableKeys, update_action, delete_action;
    constraints.splice(0, 1);
    constraints = constraints.map(constraintSql => {
      //Parse foreign key snippets
      if (constraintSql.indexOf('REFERENCES') !== -1) {
        //Parse out the constraint condition form sql string
        update_action = constraintSql.match(/ON UPDATE (CASCADE|SET NULL|RESTRICT|NO ACTION|SET DEFAULT){1}/);
        delete_action = constraintSql.match(/ON DELETE (CASCADE|SET NULL|RESTRICT|NO ACTION|SET DEFAULT){1}/);
        if (update_action) {
      ...
      return {
        constraint_name: Utils.removeTicks(constraint[0]),
  ...
  handleShowIndexesQuery(data) {
    ...
    return this.sequelize.Promise.map(data.reverse(), item => {
      ...
      item.constraint_name = item.name;

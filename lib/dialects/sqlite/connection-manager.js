...
ConnectionManager.prototype.getConnection = function(options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (self.sequelize.options.key) {
			self.sequelize.options.logger('Found a key to use: ' + self.sequelize.options.key);
			connection.run('PRAGMA KEY = \'' + self.sequelize.options.key + '\'');
		}
		
		if (self.sequelize.options.cipher) {
			self.sequelize.options.logger('Found a cipher to use: ' + self.sequelize.options.cipher);

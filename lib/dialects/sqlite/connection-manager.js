...
ConnectionManager.prototype.getConnection = function(options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    // Check for sqlcihper = {} (object) then for the key to set
    if (self.sequelize.options.sqlcipher  && self.sequelize.options.sqlcipher.key) {
      connection.run('PRAGMA KEY = \'' + self.sequelize.options.sqlcipher.key + '\'');
    }

    // Check for sqlcipher = {} (object then for the cipher to set)
    if (self.sequelize.options.sqlcipher && self.sequelize.options.sqlcipher.cipher ) {
      connection.run('PRAGMA CIPHER = \'' + self.sequelize.options.sqlcipher.cipher + '\'');
    }

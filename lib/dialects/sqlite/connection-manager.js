...
const debug = Logger.getLogger().debugContext().extend('connection:sqlite');
...
class ConnectionManager extends AbstractConnectionManager {
    ...
    return new Promise((resolve, reject) => {
      this.connections[options.inMemory || options.uuid] = new this.lib.Database(
        ...
        err => {
          ...
          debug.log(`connection acquired ${options.uuid}`);
  ...
  releaseConnection(connection, force) {
    ...
    if (connection.uuid) {
      connection.close();
      debug.log(`connection released ${connection.uuid}`);

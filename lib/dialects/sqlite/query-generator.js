...
class SQLiteQueryGenerator extends MySqlQueryGenerator {
  ...
  dataTypeMapping(dataType) {
    ...
    const typesRegex = [/TINYINT|SMALLINT|MEDIUMINT|BIGINT/, /TINYINT(d+)/, /INTEGER\(\d+\)/];

    for (const typeRegex of typesRegex) {
      typeRegex.test(dataType) && (dataType = dataType.replace(typeRegex, 'INTEGER'));
    }

    // remove unsigned which is not allowed on Sqlite
    _.includes(dataType, 'UNSIGNED') && (dataType = dataType.replace(/UNSIGNED\s/, ''));

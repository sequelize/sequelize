...
const QueryGenerator = {
  ...
  checkValidJsonStatement(stmt) {
    const jsonFunctionRegex = /^\s*(json(?:_\w+){0,2})\([^)]*\)/i;
    const tokenCaptureRegex = /^\s*('(?:[^']|'')*'|"(?:[^"]|"")*"|`(?:[^`]|``)*`|[\w\d\s]+|[().,;+-])/i;
  ...
  /**
   * Generate JSON path extraction query
   *
   * @param   {String|Array<String>} path The path to property
   * @returns {String}               path The generated sql query
   * @private
   */
  jsonPathExtractionQuery(path) {
    const paths = _.toPath(path);
    const column = _.first(paths);
    const pathStr = ['$']
      .concat(_.tail(paths))
      .join('.')
      .replace(/\.(\d+)(?:(?=\.)|$)/g, (_, number) => `[${number}]`);

    const quotedColumn = /`[^`]*`/.test(column) ? column : this.quoteIdentifier(column);
    ...
    if (smth instanceof Utils.Json) {
      ...
      if (smth.conditions) {
        const conditions = this.parseConditionObject(smth.conditions).map(condition =>
          `${this.jsonPathExtractionQuery(condition.path)} = '${condition.value}'`
        );
        ...
        if (this.checkValidJsonStatement(smth.path)) {
          ...
          str = this.jsonPathExtractionQuery(smth.path);
        }
  ...
  whereItemQuery(key, value, options) {
    ...
    if (_.isPlainObject(value) && fieldType instanceof DataTypes.JSON && options.json !== false) {
      ...
      const traverse = (prop, item, path) => {
        ...
        baseKey = this.jsonPathExtractionQuery([baseKey].concat(path));

...
const QueryGenerator = {
  ...
  getConstraintSnippet(tableName, options) {
    ...
    switch (options.type.toUpperCase()) {
      case 'UNIQUE':
        constraintName = this.quoteIdentifier(options.name || `UK__${tableName}__${fieldsSqlString}`);      
        constraintSnippet = `CONSTRAINT ${constraintName} UNIQUE (${fieldsSqlQuotedString})`;
        break;
      case 'CHECK':
        options.where = this.whereItemsQuery(options.where);
        constraintName = this.quoteIdentifier(options.name || `CK__${tableName}__${fieldsSqlString}`);              
        constraintSnippet = `CONSTRAINT ${constraintName} CHECK (${options.where})`;
        break;
      case 'PRIMARY KEY':
        constraintName = this.quoteIdentifier(options.name || `PK__${tableName}__${fieldsSqlString}`);
      ...
      case 'FOREIGN KEY':
        ...
        constraintName = this.quoteIdentifier(options.name || `FK__${tableName}__${fieldsSqlString}__${references.model}`);
  ...
  alterConstraintQuery(tableName, attributes, createTableSql) {
    let backupTableName;

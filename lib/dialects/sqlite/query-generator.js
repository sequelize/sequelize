...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= d.table %> (<%= d.attributes %>)'
      , primaryKeys = []
    ...
    for (var i = 0; i < keys.length; i++) {
      var dataType = attributes[keys[i]];
      ...
      if (Utils._.includes(dataType, 'PRIMARY KEY')) {
        ...
        if (needsMultiplePrimaryKeys) {
          primaryKeys.push(keys[i]);
          dataTypeString = dataType.replace(/PRIMARY KEY/, 'NOT NULL');
        }
      }
      attrStr.push(this.quoteIdentifier(keys[i]) + ' ' + dataTypeString);
    }
    ...
    var sql = Utils.compileOrRunTemplate(query, values).trim() + ';';
  ...
  addColumnQuery: function(table, key, dataType) {
    var query = 'ALTER TABLE <%= d.table %> ADD <%= d.attribute %>;'
      , attributes = {};
    ...
    var attribute = Utils.compileOrRunTemplate('<%= d.key %> <%= d.definition %>', {
        key: this.quoteIdentifier(key),
        definition: fields[key]
      });

    var sql =  Utils.compileOrRunTemplate(query, {
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options) {
    var query = 'INSERT<%= d.ignoreDuplicates %> INTO <%= d.table %> (<%= d.attributes %>) VALUES <%= d.tuples %>;'
      , tuples = []
      , allAttributes = [];
  ...
  updateQuery: function(tableName, attrValueHash, where, options) {
    ...
    var query  = 'UPDATE <%= d.table %> SET <%= d.values %> <%= d.where %>'
      , values = []
      , keys = Object.keys(attrValueHash);

    for (var i = 0; i < keys.length; i++) {
      var value = attrValueHash[keys[i]];
      values.push(this.quoteIdentifier(keys[i]) + '=' + this.escape(value));
    }
    ...
    return Utils.compileOrRunTemplate(query, replacements).trim();
  },

  deleteQuery: function(tableName, where, options) {
    options = options || {};

    var query = 'DELETE FROM <%= d.table %><%= d.where %>';
    var replacements = {
      table: this.quoteTable(tableName),
  ...
  attributesToSQL: function(attributes) {
    ...
    for (var i = 0; i < names.length; i++) {
      var dataType = attributes[names[i]];
      var fieldName = dataType.field || names[i];
      ...
      if (Utils._.isObject(dataType)) {
        ...
        result[fieldName] = Utils.compileOrRunTemplate(template, replacements);
      } else {
  ...
  showIndexesQuery: function(tableName) {
    var sql = 'PRAGMA INDEX_LIST(<%= d.tableName %>)';
    return Utils.compileOrRunTemplate(sql, { tableName: this.quoteTable(tableName) });
  },
  ...
  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    ...
    return Utils.compileOrRunTemplate(sql,  { tableName: this.quoteIdentifiers(tableName), indexName: indexName });
  },
  ...
  describeTableQuery: function(tableName, schema, schemaDelimiter) {
    ...
    var sql = 'PRAGMA TABLE_INFO(<%= d.tableName %>);';
    return Utils.compileOrRunTemplate(sql, { tableName: this.addSchema({tableName: this.quoteIdentifiers(tableName), options: options})});
  },
  ...
  removeColumnQuery: function(tableName, attributes) {
    ...
    query = [
      ...
      'INSERT INTO <%= d.backupTableName %> SELECT <%= d.attributeNames %> FROM <%= d.tableName %>;',
      'DROP TABLE <%= d.tableName %>;',
      this.createTableQuery(tableName, attributes),
      'INSERT INTO <%= d.tableName %> SELECT <%= d.attributeNames %> FROM <%= d.backupTableName %>;',
      'DROP TABLE <%= d.backupTableName %>;'
    ].join('');
  ...
  renameColumnQuery: function(tableName, attrNameBefore, attrNameAfter, attributes) {
    ...
    query = [
      this.createTableQuery(backupTableName, attributes).replace('CREATE TABLE', 'CREATE TEMPORARY TABLE'),
      'INSERT INTO <%= d.backupTableName %> SELECT <%= d.attributeNamesImport %> FROM <%= d.tableName %>;',
      'DROP TABLE <%= d.tableName %>;',
      this.createTableQuery(tableName, attributes),
      'INSERT INTO <%= d.tableName %> SELECT <%= d.attributeNamesExport %> FROM <%= d.backupTableName %>;',
      'DROP TABLE <%= d.backupTableName %>;'
    ].join('');
  ...
  getForeignKeysQuery: function(tableName, schemaName) {
    var sql = 'PRAGMA foreign_key_list(<%= d.tableName %>)';
    return Utils.compileOrRunTemplate(sql, { tableName: tableName });
  }
};

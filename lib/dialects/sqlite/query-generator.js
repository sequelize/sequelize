...
const QueryGenerator = {
  ...
  checkValidJsonStatement(stmt) {
    ...
    let openingCount = 0;
    let closingCount = 0;
    while (nextString && nextString.length > 0) {
      const openingCapture = /^\s*(json(?:_\w+)*)\(\s*([\s\S]+)/im.exec(nextString);
      if (openingCapture) {
        openingCount++;
        nextString = openingCapture[2];
      } else {
        const closingCapture = /(?:([`"'])(?:(?!\1).)*\1)*\)([\s\S]*)/im.exec(nextString);
        if (closingCapture) {
          closingCount++;
          nextString = closingCapture[2];
        } else {
          if (nextString.indexOf(';') >= 0) {
            throw new Error('Cannot use separator(;) in sequelize.json method.');
          }
          break;
        }
      }
    ...
    if (openingCount !== closingCount) {
      throw new Error('Invalid json statement provided: ' + stmt);
    }
    ...
    if (smth instanceof Utils.Json) {
      ...
      if (smth.conditions) {
        const conditions = this.parseConditionObject(smth.conditions).map(condition =>
          `json_extract(${this.quoteIdentifier(_.first(condition.path))}, '$.${_.tail(condition.path).join('.')}') = '${condition.value}'`
        );
        ...
        // Allow specifying conditions using the sqlite json1 functions
        ...
        if (this.checkValidJsonStatement(smth.path)) {
          ...
          str = `json_extract(${this.quoteIdentifier(_.first(path))}, '$.${_.tail(path).join('.')}')`;
        }
  ...
  whereItemQuery(key, value, options) {
    ...
    if (key && typeof key === 'string' && key.indexOf('.') !== -1 && options.model) {
      if (options.model.rawAttributes[key.split('.')[0]] && options.model.rawAttributes[key.split('.')[0]].type instanceof DataTypes.JSON) {
        ...
        _.set(value, key.split('.').slice(1), tmp);
    ...
    if (_.isPlainObject(value) && fieldType instanceof DataTypes.JSON && options.json !== false) {
      ...
      const traverse = (prop, item, path) => {
        ...
        baseKey = `json_extract(${baseKey}, '$.${path.join('.')}')`;

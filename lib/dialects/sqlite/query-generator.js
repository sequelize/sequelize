...
const QueryGenerator = {
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    if (smth instanceof Utils.Json) {
      // Parse nested object
      const convertToJsonQuery = path =>
        `json_extract(${this.quoteIdentifier(_.first(path))}, '${['$'].concat(_.tail(path)).join('.')}')`;

      if (smth.conditions) {
        const conditions = this.parseConditionObject(smth.conditions).map(condition =>
          `${convertToJsonQuery(condition.path)} = '${condition.value}'`
        );

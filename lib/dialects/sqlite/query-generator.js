...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= data.table %> (<%= data.attributes %>)'
      , primaryKeys = []
    ...
    var sql = Utils.template(query, values).trim() + ';';
  ...
  addColumnQuery: function(table, key, dataType) {
    var query = 'ALTER TABLE <%= data.table %> ADD <%= data.attribute %>;'
      , attributes = {};
    ...
    var attribute = Utils.template('<%= data.key %> <%= data.definition %>', {
        key: this.quoteIdentifier(key),
        definition: fields[key]
      });

    var sql =  Utils.template(query, {
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options) {
    var query = 'INSERT<%= data.ignoreDuplicates %> INTO <%= data.table %> (<%= data.attributes %>) VALUES <%= data.tuples %>;'
      , tuples = []
      , allAttributes = [];
  ...
  updateQuery: function(tableName, attrValueHash, where, options) {
    ...
    var query  = 'UPDATE <%= data.table %> SET <%= data.values %> <%= data.where %>'
      , values = []
    ...
    return Utils.template(query, replacements).trim();
  },

  deleteQuery: function(tableName, where, options) {
    options = options || {};

    var query = 'DELETE FROM <%= data.table %><%= data.where %>';
    var replacements = {
      table: this.quoteTable(tableName),
  ...
  attributesToSQL: function(attributes) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      if (Utils._.isObject(dataType)) {
        ...
        result[fieldName] = Utils.template(template, replacements);
      } else {
  ...
  showIndexesQuery: function(tableName) {
    var sql = 'PRAGMA INDEX_LIST(<%= data.tableName %>)';
    return Utils.template(sql, { tableName: this.quoteTable(tableName) });
  },
  ...
  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    ...
    return Utils.template(sql,  { tableName: this.quoteIdentifiers(tableName), indexName: indexName });
  },
  ...
  describeTableQuery: function(tableName, schema, schemaDelimiter) {
    ...
    var sql = 'PRAGMA TABLE_INFO(<%= data.tableName %>);';
    return Utils.template(sql, { tableName: this.addSchema({tableName: this.quoteIdentifiers(tableName), options: options})});
  },
  ...
  removeColumnQuery: function(tableName, attributes) {
    ...
    query = [
      ...
      'INSERT INTO <%= data.backupTableName %> SELECT <%= data.attributeNames %> FROM <%= data.tableName %>;',
      'DROP TABLE <%= data.tableName %>;',
      this.createTableQuery(tableName, attributes),
      'INSERT INTO <%= data.tableName %> SELECT <%= data.attributeNames %> FROM <%= data.backupTableName %>;',
      'DROP TABLE <%= data.backupTableName %>;'
    ].join('');
  ...
  renameColumnQuery: function(tableName, attrNameBefore, attrNameAfter, attributes) {
    ...
    query = [
      this.createTableQuery(backupTableName, attributes).replace('CREATE TABLE', 'CREATE TEMPORARY TABLE'),
      'INSERT INTO <%= data.backupTableName %> SELECT <%= data.attributeNamesImport %> FROM <%= data.tableName %>;',
      'DROP TABLE <%= data.tableName %>;',
      this.createTableQuery(tableName, attributes),
      'INSERT INTO <%= data.tableName %> SELECT <%= data.attributeNamesExport %> FROM <%= data.backupTableName %>;',
      'DROP TABLE <%= data.backupTableName %>;'
    ].join('');
  ...
  getForeignKeysQuery: function(tableName, schemaName) {
    var sql = 'PRAGMA foreign_key_list(<%= data.tableName %>)';
    return Utils.template(sql, { tableName: tableName });
  }
};

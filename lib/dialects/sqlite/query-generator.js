...
const QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const needsMultiplePrimaryKeys =
      _.values(attributes).filter((definition) => definition.includes('PRIMARY KEY')).length > 1;
    const attrArray = [];
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    ...
    const sql =
      this.insertQuery(tableName, insertValues, model.rawAttributes, options) +
      ' ' +
      this.updateQuery(tableName, updateValues, where, options, model.rawAttributes);

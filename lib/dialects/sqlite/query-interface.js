  ...
  return this.showConstraint(tableName, constraintName)
    .then(constraints => {
      ...
      if (constraint) {
        ...
        constraint.constraint_name = this.QueryGenerator.quoteIdentifier(constraint.constraint_name);
        let constraintSnippet = `, CONSTRAINT ${constraint.constraint_name} ${constraint.constraint_type} ${constraint.constraintCondition}`;
        if (constraint.constraint_type === 'FOREIGN KEY') {
          const referenceTableName = this.QueryGenerator.quoteTable(constraint.referenceTableName);
          constraint.referenceTableKeys = constraint.referenceTableKeys.map(columnName => this.QueryGenerator.quoteIdentifier(columnName));
          const referenceTableKeys = constraint.referenceTableKeys.join(', ');
          constraintSnippet+= ` REFERENCES ${referenceTableName} (${referenceTableKeys})`;
          constraintSnippet+= ` ON UPDATE ${constraint.update_action}`;
          constraintSnippet+= ` ON DELETE ${constraint.delete_action}`;
        }
    ...
    .then(fields => {
      const sql = this.QueryGenerator.alterConstraintQuery(tableName, fields, createTableSql);
      const subQueries = sql.split(';').filter(q => q !== '');

...
function removeConstraint(tableName, constraintName, options) {
  ...
  return this.showConstraint(tableName, constraintName)
    .then(constraints => {
      ...
      const constraintSnippet = `, CONSTRAINT ${constraint.constraint_name} ${constraint.constraint_type} ${constraint.constraintCondition}`;
      createTableSql = createTableSql.replace(constraintSnippet, '');
      if (constraint.constraint_type === 'FOREIGN KEY') {
        createTableSql = createTableSql.replace(/\sREFERENCES.+\((?:[^)(]+|\((?:[^)(]+|\([^)(]*\))*\))*\)/, '');
      }
      createTableSql+= ';';

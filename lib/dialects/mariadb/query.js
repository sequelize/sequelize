...
class Query extends AbstractQuery {
  ...
  handleJsonSelectQuery(rows) {
    ...
    // Occurs when using mariadb driver with mysql
    const wasParsed = rows.meta.some(m => m.type === 'JSON');
    for (const _field of Object.keys(this.model.fieldRawAttributesMap)) {
      const modelField = this.model.fieldRawAttributesMap[_field];
      const { type, fieldName } = modelField;
      if (type instanceof DataTypes.JSON) {
        //value is return as String, no JSON
        for (const r of rows) {
          if (!wasParsed) {
            r[fieldName] = r[fieldName] ? JSON.parse(r[fieldName]) : null;
          }
          if (DataTypes.JSON.parse) {
            DataTypes.JSON.parse(modelField, this.sequelize.options, r[fieldName]);
          }
        }
      }
    }

...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    if (benchmark) {
      ...
      this.sequelize.log(`Executing (${this.connection.threadId || 'default'}): ${this.sql}`, this.options);
    }

    debug(`executing(${this.connection.threadId || 'default'}) : ${this.sql}`);
    ...
    return new Utils.Promise((resolve, reject) => {
      this.connection.query(this.sql, parameters)
        .then(results => {
          debug(`executed(${this.connection.threadId || 'default'}) : ${this.sql}`);

          if (benchmark) {
            this.sequelize.log(`Executed (${this.connection.threadId || 'default'}): ${this.sql}`, Date.now() - queryBegin, this.options);
          }

          // Log warnings if we've got them.
          if (showWarnings && results && results.warningStatus > 0) {
            resolve(this.logWarnings(results));
            return;
          }
          resolve(results);
        })
  ...
  logWarnings(results) {
    return this.run('SHOW WARNINGS').then(warningResults => {
      const warningMessage = `MariaDB Warnings (${this.connection.threadId||'default'}): `;
      const messages = [];
  ...
  formatError(err) {
    switch (err.errno) {
      case 1062: {
        const match = err.message.match(/Duplicate entry '(.*)' for key '?((.|\s)*?)'?\s.*$/);

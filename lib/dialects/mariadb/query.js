...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    if (benchmark) {
      ...
      this.sequelize.log(
        `Executing (${this.connection.threadId || 'default'}): ${this.sql}`,
        this.options);
    }
    ...
    return new Utils.Promise((resolve, reject) => {
      this.connection.query(this.sql, parameters)
        .then(results => {
          debug(
            `executed(${this.connection.threadId || 'default'}) : ${this.sql}`);
    ...
    if (this.isDescribeQuery()) {
      ...
      for (const _result of data) {
        ...
        result[_result.Field] = {
          type: enumRegex.test(_result.Type) ? _result.Type.replace(enumRegex,
  ...
  logWarnings(results) {
    return this.run('SHOW WARNINGS').then(warningResults => {
      const warningMessage = `MariaDB Warnings (${this.connection.threadId
      || 'default'}): `;
      const messages = [];

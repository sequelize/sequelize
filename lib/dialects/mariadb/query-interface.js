...
/**
 Returns an object that treats MariaDB foreign key constraint
...
/**
 A wrapper to remove columns from existing tables if they have a foreign key constraint.
 TODO check about unique key constraint too.
  ...
  return this.sequelize.query(
    ...
    .then(([results]) => {
      ...
      return Promise.map(results, constraint => this.sequelize.query(
        this.QueryGenerator.dropForeignKeyQuery(tableName,
  ...
  return this.sequelize.query(sql, Object.assign({}, options,
    ...
    .then(constraints => {
      ...
      if (constraint.constraintType === 'FOREIGN KEY') {
        query = this.QueryGenerator.dropForeignKeyQuery(tableName,
          constraintName);
      } else {
        query = this.QueryGenerator.removeIndexQuery(constraint.tableName,
          constraint.constraintName);
      }

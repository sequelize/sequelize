...
class MariaDBQueryGenerator extends AbstractQueryGenerator {
  constructor(options) {
    super(options);
  ...
  showSchemasQuery() {
    return "SELECT SCHEMA_NAME as schema_name FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME NOT IN ('MYSQL', 'INFORMATION_SCHEMA', 'PERFORMANCE_SCHEMA');";
  }
    ...
    if (smth instanceof Utils.Json) {
      ...
      if (smth.conditions) {
        const conditions = this.parseConditionObject(smth.conditions).map(condition =>
          `json_unquote(json_extract(${this.quoteIdentifier(condition.path[0])},'\$.${_.tail(condition.path).join('.')}')) = '${condition.value}'`
        );
      ...
      if (smth.path) {
        ...
        if (this._checkValidJsonStatement(smth.path)) {
          ...
          str = `json_unquote(json_extract(${this.quoteIdentifier(columnName)},'\$${startWithDot ? '.' : ''}${path.join('.')}'))`;
        }
  ...
  getForeignKeysQuery(table, schemaName) {
    const tableName = table.tableName || table;
    return `SELECT ${this._getForeignKeysQueryFields()} FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE where TABLE_NAME = '${tableName  /* jshint ignore: line */}' AND CONSTRAINT_NAME!='PRIMARY' AND CONSTRAINT_SCHEMA='${schemaName}' AND REFERENCED_TABLE_NAME IS NOT NULL;`; /* jshint ignore: line */
  }

...
class MariaDBQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const initialAutoIncrement = options.initialAutoIncrement
      ...
      _.each(options.uniqueKeys, (columns, indexName) => {
        if (columns.customIndex) {
          ...
          attributesClause += `, UNIQUE ${this.quoteIdentifier(
            indexName)} (${columns.fields.map(
            field => this.quoteIdentifier(field)).join(', ')})`;
        }
      });
    }
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    if (smth instanceof Utils.Json) {
      ...
      if (/timestamp/i.test(smth.type)) {
        ...
        smth.type = 'char';
      } else if (/double precision/i.test(smth.type) || /boolean/i.test(
        smth.type) || /integer/i.test(smth.type)) {
        smth.type = 'decimal';
  ...
  showIndexesQuery(tableName, options) {
    return `SHOW INDEX FROM ${this.quoteTable(tableName)}${(options
      || {}).database ? ` FROM \`${options.database}\`` : ''}`;
  }
  ...
  getForeignKeyQuery(table, columnName) {
    ...
    return `SELECT ${foreignKeyFields} FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE`
      + ` WHERE (REFERENCED_TABLE_NAME = ${quotedTableName}${table.schema
       ...
       OR (TABLE_NAME = ${quotedTableName}${table.schema ? ` AND TABLE_SCHEMA = ${quotedSchemaName}` : ''} AND COLUMN_NAME = ${quotedColumnName} AND REFERENCED_TABLE_NAME IS NOT NULL)`;
  }

...
module.exports = BaseTypes => {
  ...
  function INTERVAL(...args) {
    if (!(this instanceof INTERVAL)) return new INTERVAL(...args);
    BaseTypes.INTERVAL.apply(this, args);
  }

  inherits(INTERVAL, BaseTypes.INTERVAL);

  INTERVAL.parse = function parse(value) {
    return moment.duration(value);
  };

  INTERVAL.prototype.toSql = function toSql() {
    let sql = 'INTERVAL';
    if (typeof this._precision === 'number') {
      sql += ` (${this._precision})`;
    }
    return sql;
  };

  INTERVAL.prototype._stringify = function _stringify(value) {
    return value.toISOString();
  };

  INTERVAL.prototype._sanitize = INTERVAL.parse;

  BaseTypes.INTERVAL.types.postgres = ['interval'];

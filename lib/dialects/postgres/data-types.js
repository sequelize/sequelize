...
module.exports = BaseTypes => {
  ...
  DATEONLY.prototype.set = function set(value) {
  ...
  DATE.prototype.stringify = function stringify(date, options) {
    ...
    return BaseTypes.DATE.prototype.stringify.call(this, date, options);
  };

  DATE.prototype.set = function set(value) {
    if (!(value instanceof Date) && !!value && value !== Infinity && value !== -Infinity) {
  ...
  DATE.prototype.compare = function compare(value, originalValue, key, options) {
    if (!options.raw && !!value) {
      if (!(value instanceof Date) && value !== Infinity && value !== -Infinity) {
        value = new Date(value);
      }
      if (originalValue) {
        if (!(originalValue instanceof Date) && originalValue !== Infinity && originalValue !== -Infinity) {
          originalValue = new Date(originalValue);
        }
        if (value === originalValue || value instanceof Date && originalValue instanceof Date && value.getTime() === originalValue.getTime()) {
          return false;
        }
      }

...
import pg from 'pg';
import BaseConnectionManager from './base/connection-manager.js';
import * as Utils from '../../utils.js';
import * as sequelizeErrors from '../../errors.js';
import semver from 'semver';
import dataTypes from '../../data-types.js';
import { isIanaZone } from '../../utils/timezone.js';

// `dialectModulePath` resolves a user-supplied path synchronously inside a
// constructor, which has no ESM equivalent.
...
export class ConnectionManager extends BaseConnectionManager {
  constructor(dialect, sequelize) {
    ...
    try {
      const pgLib = sequelize.config.dialectModulePath ? require(sequelize.config.dialectModulePath) : pg;
    ...
    if (dataType.types.postgres.array_oids) {
      for (const oid of dataType.types.postgres.array_oids) {
        this.arrayOidMap[oid] = (value) => {
          return this.lib.types.arrayParser
            .create(value, (v) => dataType.parse(v, oid, this.lib.types.getTypeParser))
            .parse();
        };
      }
    }
  ...
  getTypeParser(oid, ...rest) {
    ...
    return this.lib.types.getTypeParser(oid, ...rest);
  }

  async connect(config) {
    config.user = config.username;
    const connectionConfig = pick(config, ['user', 'password', 'host', 'database', 'port']);
    ...
    const connection = await new Promise((resolve, reject) => {
      const client = new this.lib.Client(connectionConfig);
      ...
      const parameterHandler = (message) => {
        switch (message.parameterName) {
          ...
          case 'standard_conforming_strings':
            client['standard_conforming_strings'] = message.parameterValue;
            break;
        }
      };
      ...
      if (!this.sequelize.config.native) {
        ...
        client.connection.on('parameterStatus', parameterHandler);
      }

      client.connect((err) => {
        if (!this.sequelize.config.native) {
          // remove parameter handler
          client.connection.removeListener('parameterStatus', parameterHandler);
        }
        if (err) {
          if (err.code) {
            switch (err.code) {
    ...
    if (!this.sequelize.config.keepDefaultTimezone) {
      // A named zone and a UTC offset need different statements: Postgres reads a
      // bare offset under the POSIX convention, so sending '+07:00' down the named
      // branch would silently land the session on UTC-07.
      const isZone = isIanaZone(this.sequelize.options.timezone);
      if (isZone) {

...
const debug = Logger.getLogger().debugContext().extend('connection:pg');
...
class ConnectionManager extends AbstractConnectionManager {
  ...
  connect(config) {
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.on('error', error => {
        ...
        debug.log(`connection error ${error.code || error.message}`);
  ...
  disconnect(connection) {
    if (connection._ending) {
      debug.log('connection tried to disconnect but was already at ENDING state');

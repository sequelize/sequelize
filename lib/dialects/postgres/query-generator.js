...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  dropTableQuery(tableName, options) {
    ...
    if (options.schema) {
      tableName = `${this.quoteIdentifier(options.schema) }.${ tableName}`;
    }
  ...
  addColumnQuery(table, key, dataType) {
    ...
    if (
      dataType.type && dataType.type instanceof DataTypes.ENUM ||
      dataType instanceof DataTypes.ENUM
    ) {
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    ...
    if (options.returning) {
      ...
      insert.query = insert.query.replace(
        returningRegex,
        `RETURNING ${primaryField} INTO primary_key`
      );
      update.query = update.query.replace(
        returningRegex,
        `RETURNING ${primaryField} INTO primary_key`
      );
    }
  ...
  attributesToSQL(attributes, options) {
    ...
    for (const key in attributes) {
      ...
      result[attribute.field || key] = this.attributeToSQL(
        attribute,
        { key, ...options || {} }
      );
    }
  ...
  createFunction(
    ...
    const variableList =
      options && options.variables
        ? this.expandFunctionVariableList(options.variables)
        : '';
  ...
  expandFunctionVariableList(variables) {
    if (!Array.isArray(variables)) {
      throw new Error(
        'expandFunctionVariableList: function variables must be an array'
      );
    }
  ...
  pgEnum(tableName, attr, dataType, options) {
    ...
    if (!!options && options.force === true) {
      sql = this.pgEnumDrop(tableName, attr, options) + sql;
    }
    return sql;
  }
  ...
  pgEnumDrop(tableName, attr, enumName, options) {
    enumName = enumName || this.pgEnumName(tableName, attr);

    options = { ...options || {} };
    if (options.schema) {
      enumName = `${this.quoteIdentifier(options.schema) }.${ enumName}`;
    }

...
var QueryGenerator = {
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    if (!!options.inherits) {
      attributes = _.omitBy(attributes, function(attribute, key) {
        // Do not omit primary keys.
        if (_.includes(attribute, 'PRIMARY KEY')) {
          return false;
        }

        // Omit if already in inherited attributes.
        return key in options.inherits.attributes;
      });
    }
    ...
    var query = 'CREATE TABLE ' +
                ...
                '<%= table %> (<%= attributes %>)<%= parameters %><%= comments %>'
      , comments = ''
      , attrStr = []
      , i;
    ...
    if (!!options.inherits) {
      values.parameters += ' INHERITS (' + this.quoteTable(options.inherits.tableName) + ')';
    }

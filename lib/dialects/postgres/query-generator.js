  ...
  var QueryGenerator = {
    ...
    selectQuery: function(tableName, options, factory) {
      ...
      options.attributes = options.attributes && options.attributes.map(function(attr) {
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          return attr.indexOf('"') < 0 ? options.table.split(', ')[0] + '.' +this.quoteIdentifiers(attr) : attr
        }
      }.bind(this)).join(", ")
      ...
      if (options.include) {
        ...
        options.include.forEach(function(include) {
          var attributes = Object.keys(include.daoFactory.attributes).filter(function(attr) {
            if (typeof include.attributes == "undefined") return true
            else if ((Object.prototype.toString.call( include.attributes ) === '[object Array]') && (include.attributes.length > 0))
              return include.attributes.indexOf(attr) >= 0;
            else return true;
          }).map(function(attr) {
    ...
    insertQuery: function(tableName, attrValueHash, attributes) {
      ...
      Object.keys(attrValueHash).forEach(function(attr) {
        var value = attrValueHash[attr]
        if (value && typeof value === 'object' && !!attributes && !!attributes[attr] && attributes[attr].type && attributes[attr].type.toString() === 'JSON') {
          try {
            rowValues[rowValues.length] = this.escape(JSON.stringify(value))
          } catch (e) {
            console.trace(e);
          }
        } else {
    ...
    updateQuery: function(tableName, attrValueHash, where, options, attributes) {
      ...
      for (var key in attrValueHash) {
        ...
        if (value && typeof value === 'object' && !!attributes && !!attributes[key] && attributes[key].type && attributes[key].type.toString() === 'JSON') {

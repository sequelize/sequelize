...
const QueryGenerator = {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    if (options.comment && Utils._.isString(options.comment)) {
      comments += `; COMMENT ON TABLE ${quotedTableName} IS ${this.escape(options.comment)}`;
    }
    ...
    if (!!options.uniqueKeys) {
      Utils._.each(options.uniqueKeys, columns => {
        if (!columns.singleField) { // If it's a single field its handled in column def, not as an index
          joinedAttributes += `, UNIQUE (${columns.fields.map(f => this.quoteIdentifiers(f)).join(', ')})`;
        }
      });
    }
    ...
    if (pks.length > 0) {
      joinedAttributes += `, PRIMARY KEY (${pks})`;
    }

    return `CREATE TABLE ${databaseVersion === 0 || semver.gte(databaseVersion, '9.1.0') ? 'IF NOT EXISTS ' : ''}${quotedTableName} (${joinedAttributes})${comments};`;
  },
  ...
  changeColumnQuery(tableName, attributes) {
    ...
    for (const attributeName in attributes) {
      ...
      if (definition.indexOf('NOT NULL') > 0) {
        attrSql += `ALTER TABLE ${quotedTableName} ALTER COLUMN ${quotedAttributeName} SET NOT NULL;`;

        definition = definition.replace('NOT NULL', '').trim();
      } else if (!definition.match(/REFERENCES/)) {
        attrSql += `ALTER TABLE ${quotedTableName} ALTER COLUMN ${quotedAttributeName} DROP NOT NULL;`;
      }

      if (definition.indexOf('DEFAULT') > 0) {
        const defaultExpr = definition.match(/DEFAULT ([^;]+)/)[1];
        attrSql += `ALTER TABLE ${quotedTableName} ALTER COLUMN ${quotedAttributeName} SET DEFAULT ${defaultExpr};`;

        definition = definition.replace(/(DEFAULT[^;]+)/, '').trim();
      } else if (!definition.match(/REFERENCES/)) {
        attrSql += `ALTER TABLE ${quotedTableName} ALTER COLUMN ${quotedAttributeName} DROP DEFAULT;`;
      }
      ...
      if (attributes[attributeName].match(/^ENUM\(/)) {
        ...
        definition += ` USING (${quotedAttributeName}::${this.quoteIdentifier(definition)})`;
      }

      if (definition.match(/UNIQUE;*$/)) {
        definition = definition.replace(/UNIQUE;*$/, '');
        const quotedUniqIndexName = this.quoteIdentifier(attributeName + '_unique_idx');
        attrSql += `ALTER TABLE ${quotedTableName} ADD CONSTRAINT ${quotedUniqIndexName} UNIQUE (${quotedAttributeName});`;
      }

      if (definition.match(/REFERENCES/)) {
        definition = definition.replace(/.+?(?=REFERENCES)/,'');
        const quotedFkIndexName = this.quoteIdentifier(attributeName + '_foreign_idx');
        attrSql += `ALTER TABLE ${quotedTableName} ADD CONSTRAINT ${quotedFkIndexName} FOREIGN KEY (${quotedAttributeName}) ${definition};`;
      } else {
        attrSql += `ALTER TABLE ${quotedTableName} ALTER COLUMN ${quotedAttributeName} TYPE ${definition};`;
      }
  ...
  deleteQuery(tableName, where, options, model) {
    ...
    if (options.truncate === true) {
      query = `TRUNCATE ${quotedTableName}`;
    ...
    if (options.limit) {
      ...
      const primaryKeysSelection = _.map(_.values(model.primaryKeys), pk => this.quoteIdentifier((pk.field))).join(',');
      const wrappedPrimaryKeys = model.primaryKeyAttributes.length > 1 ? `(${primaryKeysSelection})` : primaryKeysSelection;

      query = `DELETE FROM ${quotedTableName} ` +
        `WHERE ${wrappedPrimaryKeys} IN (` +
          `SELECT ${primaryKeysSelection} ` +
          `FROM ${quotedTableName}${whereClause} ` +
          `LIMIT ${this.escape(options.limit)}` +
        `)`;
    } else {
      query = `DELETE FROM ${quotedTableName}${whereClause}`;
    }

  ...
  createTableQuery(tableName, attributes, options) {
    ...
    const values = {
      ...
      partition: options.partition
        ? ` PARTITION BY ${options.partition.type.toUpperCase()} (${this.quoteIdentifier(options.partition.attribute)})`
        : ''
    };
    ...
    return `CREATE TABLE ${databaseVersion === 0 || semver.gte(databaseVersion, '9.1.0') ? 'IF NOT EXISTS ' : ''}${values.table} (${values.attributes})${values.partition}${values.comments};`;
  },

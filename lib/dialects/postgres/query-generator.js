...
var QueryGenerator = {
  ...
  createSchema: function(schema) {
    var query = 'CREATE SCHEMA <%= data.schema %>;';
    return Utils.template(query, {schema: schema});
  },

  dropSchema: function(schema) {
    var query = 'DROP SCHEMA <%= data.schema %> CASCADE;';
    return Utils.template(query, {schema: schema});
  },
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= data.table %> (<%= data.attributes %>)<%= data.comments %>'
      , comments = ''
      , attrStr = []
      , j;

    if (options.comment && Utils._.isString(options.comment)) {
      comments += '; COMMENT ON TABLE <%= data.table %> IS ' + this.escape(options.comment);
    }

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      var attr = keys[i];
      j = attributes[attr].indexOf('COMMENT');
      if (j !== -1) {
        // Move comment to a seperate query
        comments += '; ' + attributes[attr].substring(j);
        attributes[attr] = attributes[attr].substring(0, j);
      }
    ...
    var values = {
      ...
      comments: Utils.template(comments, { table: this.quoteTable(tableName)})
    };
    ...
    return Utils.template(query, values).trim() + ';';
  },

  dropTableQuery: function(tableName, options) {
    options = options || {};
    var query = 'DROP TABLE IF EXISTS <%= data.table %><%= data.cascade %>;';
    return Utils.template(query, {
      table: this.quoteTable(tableName),
  ...
  describeTableQuery: function(tableName, schema) {
    ...
    var query = 'SELECT c.column_name as "Field", c.column_default as "Default", c.is_nullable as "Null", ' +
      ...
      'FROM information_schema.columns c WHERE table_name = <%= data.table %> AND table_schema = <%= data.schema %>';
  ...
  addColumnQuery: function(table, key, dataType) {
    var query = 'ALTER TABLE <%= data.table %> ADD COLUMN <%= data.attribute %>;'
    ...
    attribute = Utils.template('<%= data.key %> <%= data.definition %>', {
  ...
  removeColumnQuery: function(tableName, attributeName) {
    var query = 'ALTER TABLE <%= data.tableName %> DROP COLUMN <%= data.attributeName %>;';
    return Utils.template(query, {
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= data.tableName %> ALTER COLUMN <%= data.query %>;'
      , sql = []
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (definition.match(/UNIQUE;*$/)) {
        ...
        attrSql += Utils.template(query.replace('ALTER COLUMN', ''), {
          tableName: this.quoteTable(tableName),
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    var query = 'ALTER TABLE <%= data.tableName %> RENAME COLUMN <%= data.attributes %>;';
    var attrString = [];

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      attrString.push(Utils.template('<%= data.before %> TO <%= data.after %>', {
        before: this.quoteIdentifier(attrBefore),
        after: this.quoteIdentifier(keys[i])
      }));
    }
  ...
  fn: function(fnName, tableName, body, returns, language) {
    ...
    var query = 'CREATE OR REPLACE FUNCTION pg_temp.<%= data.fnName %>() RETURNS <%= data.returns %> AS $func$ BEGIN <%= data.body %> END; $func$ LANGUAGE <%= data.language %>; SELECT * FROM pg_temp.<%= data.fnName %>();';
  ...
  exceptionFn: function(fnName, tableName, main, then, when, returns, language) {
    ...
    var body = '<%= data.main %> EXCEPTION WHEN <%= data.when %> THEN <%= data.then %>;';
    body = Utils.template(body, {
      main: main,
      when: when,
      then: then
    });
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, modelAttributes) {
    ...
    var query = 'INSERT INTO <%= data.table %> (<%= data.attributes %>) VALUES <%= data.tuples %>'
      , tuples = []
      , serials = []
      , allAttributes = [];
    ...
    return Utils.template(query, replacements);
  },
  ...
  deleteQuery: function(tableName, where, options, model) {
    ...
    if (options.limit) {
      query = 'DELETE FROM <%= data.table %> WHERE <%= data.primaryKeys %> IN (SELECT <%= data.primaryKeysSelection %> FROM <%= data.table %><%= data.where %><%= data.limit %>)';
    } else {
      query = 'DELETE FROM <%= data.table %><%= data.where %>';
    }
  ...
  showIndexesQuery: function(tableName) {
    ...
    var query = 'SELECT i.relname AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, ' +
      ...
      "t.relkind = 'r' and t.relname = '<%= data.tableName %>' GROUP BY i.relname, ix.indexrelid, ix.indisprimary, ix.indisunique, ix.indkey ORDER BY i.relname;";

    return Utils.template(query, { tableName: tableName });
  },

  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    var sql = 'DROP INDEX IF EXISTS <%= data.indexName %>'
  ...
  attributeToSQL: function(attribute) {
    ...
    if (Utils.defaultValueSchemable(attribute.defaultValue)) {
      template += ' DEFAULT <%= data.defaultValue %>';
    ...
    if (attribute.references) {
      ...
      template += ' REFERENCES <%= data.referencesTable %> (<%= data.referencesKey %>)';
  ...
  attributesToSQL: function(attributes, options) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      result[attribute.field || keys[i]] = this.attributeToSQL(attribute, options);
    }
  ...
  findAutoIncrementField: function(factory) {
    ...
    var names = Object.keys(factory.attributes);
    for (var i = 0; i < names.length; i++) {
      var definition = factory.attributes[names[i]];
  ...
  createTrigger: function(tableName, triggerName, eventType, fireOnSpec, functionName, functionParams, optionsArray) {
    var sql = [
        'CREATE <%= data.constraintVal %>TRIGGER <%= data.triggerName %>'
        , '<%= data.eventType %> <%= data.eventSpec %>'
        , 'ON <%= data.tableName %>'
        , '<%= data.optionsSpec %>'
        , 'EXECUTE PROCEDURE <%= data.functionName %>(<%= data.paramList %>);'
      ].join('\n\t');
  ...
  dropTrigger: function(tableName, triggerName) {
    var sql = 'DROP TRIGGER <%= data.triggerName %> ON <%= data.tableName %> RESTRICT;';
    return Utils.template(sql, {
      triggerName: triggerName,
      tableName: tableName
    });
  ...
  renameTrigger: function(tableName, oldTriggerName, newTriggerName) {
    var sql = 'ALTER TRIGGER <%= data.oldTriggerName %> ON <%= data.tableName %> RENAME TO <%= data.newTriggerName %>;';
    return Utils.template(sql, {
      tableName: tableName,
  ...
  createFunction: function(functionName, params, returnType, language, body, options) {
    var sql = ['CREATE FUNCTION <%= data.functionName %>(<%= data.paramList %>)'
        , 'RETURNS <%= data.returnType %> AS $func$'
        , 'BEGIN'
        , '\t<%= data.body %>'
        , 'END;'
        , "$func$ language '<%= data.language %>'<%= data.options %>;"
    ].join('\n');
  ...
  dropFunction: function(functionName, params) {
    ...
    var sql = 'DROP FUNCTION <%= data.functionName %>(<%= data.paramList %>) RESTRICT;';
    return Utils.template(sql, {
      functionName: functionName,
      paramList: this.expandFunctionParamList(params)
    });
  ...
  renameFunction: function(oldFunctionName, params, newFunctionName) {
    var sql = 'ALTER FUNCTION <%= data.oldFunctionName %>(<%= data.paramList %>) RENAME TO <%= data.newFunctionName %>;';
    return Utils.template(sql, {
  ...
  databaseConnectionUri: function(config) {
    var template = '<%= data.protocol %>://<%= data.user %>:<%= data.password %>@<%= data.host %><% if (port) { %>:<%= data.port %><% } %>/<%= data.database %><% if (ssl) { %>?ssl=<%= data.ssl %><% } %>';
  ...
  */
  escape: function(value, field) {
    ...
    if (Utils._.isObject(value) && field && (field.type instanceof DataTypes.HSTORE || DataTypes.ARRAY.is(field.type, DataTypes.HSTORE))) {
      ...
      if (DataTypes.ARRAY.is(field.type, DataTypes.HSTORE)) {
        return 'ARRAY[' + Utils._.map(value, function(v) {
          return "'" + hstore.stringify(v) + "'::hstore";
        }).join(',') + ']::HSTORE[]';
      }
      ...
      if (DataTypes.ARRAY.is(field.type, DataTypes.RANGE)) { // escape array of ranges
        return 'ARRAY[' + Utils._.map(value, function(v){
          return "'" + range.stringify(v) + "'";

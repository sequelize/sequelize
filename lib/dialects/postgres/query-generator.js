...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  removeIndexQuery(tableNameOrSchema, indexNameOrAttributes) {
    let indexName = indexNameOrAttributes;

    const tableName = typeof tableNameOrSchema === 'string' ? { tableName: tableNameOrSchema } : tableNameOrSchema;

    if (typeof indexName !== 'string') {
      indexName = Utils.underscore(`${tableName.tableName}_${indexNameOrAttributes.join('_')}`);
    }
    indexName = this.quoteTable({ schema: tableName.schema, tableName: indexName });

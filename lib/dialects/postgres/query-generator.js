...
module.exports = (function() {
  var QueryGenerator = {
    ...
    describeTableQuery: function(tableName) {
      var query = 'SELECT c.column_name as "Field", c.column_default as "Default", c.is_nullable as "Null", (CASE WHEN c.data_type=\'USER-DEFINED\' THEN c.udt_name ELSE c.data_type END) as "Type", (SELECT array_agg(e.enumlabel) FROM pg_catalog.pg_type t JOIN pg_catalog.pg_enum e ON t.oid=e.enumtypid WHERE t.typname=c.udt_name) AS special FROM information_schema.columns c WHERE table_name = <%= table %>;'
      return Utils._.template(query)({

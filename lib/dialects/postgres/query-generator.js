...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  fn(fnName, tableName, parameters, body, returns, language) {
    ...
    return new Composition('CREATE OR REPLACE FUNCTION pg_temp.', fnName, '(', parameters, ') ', returns, ' AS $func$ BEGIN ', body, ' END; $func$ LANGUAGE ', language, '; SELECT * FROM pg_temp.', fnName, '();');
  }

  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    const primaryField = this.quoteIdentifier(model.primaryKeyField);

    const itemsToInsert = this.getItemsToInsert(insertValues, model.rawAttributes);
    const valuesToInsert = itemsToInsert.values;
    const fieldsToInsert = itemsToInsert.fields;
    const insertComposition = new Composition('INSERT INTO ',
      this.quoteTable(tableName), ' (', fieldsToInsert, ')',
      ' SELECT ', valuesToInsert, ' WHERE NOT EXISTS (SELECT primary_key FROM update_outcome LIMIT 1)');

    const update = this.updateProto(tableName, updateValues, where, options, model.rawAttributes);

    if (this._dialect.supports.returnValues.returning) {
      insertComposition.add(' RETURNING true AS created, ', primaryField, ' AS primary_key');
      update.return.set('RETURNING false AS created, ', primaryField, ' AS primary_key');
    }

    const composition = new Composition('WITH update_outcome AS (',
      update.toComposition(), '), insert_outcome AS (',
      insertComposition, ') SELECT * FROM update_outcome UNION ALL SELECT * FROM insert_outcome');
  ...
  deleteQuery(tableName, where, options = {}, model) {
    ...
    composition.add('DELETE FROM ', table);
    ...
    if (options.limit) {
      ...
      whereClause.set(' WHERE ', primaryKeys, ' IN (SELECT ', primaryKeysSelection,
        ' FROM ', table, whereClause, ' LIMIT ', new Slot(options.limit), ')');
    }
  ...
  addLimitAndOffset(options) {
    ...
    if (isLimit) composition.add('LIMIT ', new Slot(options.limit));
    if (options.offset !== null && options.offset !== undefined) {
      ...
      composition.add('OFFSET ', new Slot(options.offset));
    }

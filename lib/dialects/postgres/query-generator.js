      ...
      if (options.include) {
        ...
        for (var daoName in options.include) {
          if (options.include.hasOwnProperty(daoName)) {
            ...
            query += ' LEFT OUTER JOIN ' + addQuotes(rightTableName) + ' ON '
            query += addQuotes(leftTableName) + '.' + addQuotes(leftColumnName) + '='
            query += addQuotes(rightTableName) + '.' + addQuotes(rightColumnName)

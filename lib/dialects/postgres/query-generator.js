...
var QueryGenerator = {
  ...
  createSchema: function(schema) {
    var query = 'CREATE SCHEMA <%= d.schema %>;';
    return Utils.compileOrRunTemplate(query, {schema: schema});
  },

  dropSchema: function(schema) {
    var query = 'DROP SCHEMA <%= d.schema %> CASCADE;';
    return Utils.compileOrRunTemplate(query, {schema: schema});
  },
  ...
  createTableQuery: function(tableName, attributes, options) {
    ...
    var query = 'CREATE TABLE IF NOT EXISTS <%= d.table %> (<%= d.attributes %>)<%= d.comments %>'
      , comments = ''
      , attrStr = []
      , j;

    if (options.comment && Utils._.isString(options.comment)) {
      comments += '; COMMENT ON TABLE <%= d.table %> IS ' + this.escape(options.comment);
    }
    ...
    var values = {
      ...
      comments: Utils.compileOrRunTemplate(comments, { table: this.quoteTable(tableName)})
    };
    ...
    return Utils.compileOrRunTemplate(query, values).trim() + ';';
  },

  dropTableQuery: function(tableName, options) {
    options = options || {};
    var query = 'DROP TABLE IF EXISTS <%= d.table %><%= d.cascade %>;';
    return Utils.compileOrRunTemplate(query, {
      table: this.quoteTable(tableName),
  ...
  describeTableQuery: function(tableName, schema) {
    ...
    var query = 'SELECT c.column_name as "Field", c.column_default as "Default", c.is_nullable as "Null", ' +
      ...
      'FROM information_schema.columns c WHERE table_name = <%= d.table %> AND table_schema = <%= d.schema %>';
  ...
  addColumnQuery: function(table, key, dataType) {
    var query = 'ALTER TABLE <%= d.table %> ADD COLUMN <%= d.attribute %>;'
    ...
    attribute = Utils.compileOrRunTemplate('<%= d.key %> <%= d.definition %>', {
  ...
  removeColumnQuery: function(tableName, attributeName) {
    var query = 'ALTER TABLE <%= d.tableName %> DROP COLUMN <%= d.attributeName %>;';
    return Utils.compileOrRunTemplate(query, {
  ...
  changeColumnQuery: function(tableName, attributes) {
    var query = 'ALTER TABLE <%= d.tableName %> ALTER COLUMN <%= d.query %>;'
      , sql = []
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (definition.indexOf('NOT NULL') > 0) {
        attrSql += Utils.compileOrRunTemplate(query, {
          tableName: this.quoteTable(tableName),
      ...
      if (definition.match(/UNIQUE;*$/)) {
        ...
        attrSql += Utils.compileOrRunTemplate(query.replace('ALTER COLUMN', ''), {
          tableName: this.quoteTable(tableName),
  ...
  renameColumnQuery: function(tableName, attrBefore, attributes) {
    var query = 'ALTER TABLE <%= d.tableName %> RENAME COLUMN <%= d.attributes %>;';
    var attrString = [];

    var keys = Object.keys(attributes);
    for (var i = 0; i < keys.length; i++) {
      attrString.push(Utils.compileOrRunTemplate('<%= d.before %> TO <%= d.after %>', {
  ...
  fn: function(fnName, tableName, body, returns, language) {
    ...
    var query = 'CREATE OR REPLACE FUNCTION pg_temp.<%= d.fnName %>() RETURNS <%= d.returns %> AS $func$ BEGIN <%= d.body %> END; $func$ LANGUAGE <%= d.language %>; SELECT * FROM pg_temp.<%= d.fnName %>();';
  ...
  exceptionFn: function(fnName, tableName, main, then, when, returns, language) {
    ...
    var body = '<%= d.main %> EXCEPTION WHEN <%= d.when %> THEN <%= d.then %>;';
    body = Utils.compileOrRunTemplate(body, {
      main: main,
      when: when,
      then: then
    });
  ...
  bulkInsertQuery: function(tableName, attrValueHashes, options, modelAttributes) {
    ...
    var query = 'INSERT INTO <%= d.table %> (<%= d.attributes %>) VALUES <%= d.tuples %>'
      , tuples = []
      , serials = []
      , allAttributes = [];
    ...
    return Utils.compileOrRunTemplate(query, replacements);
  },
  ...
  deleteQuery: function(tableName, where, options, model) {
    ...
    if (options.limit) {
      query = 'DELETE FROM <%= d.table %> WHERE <%= d.primaryKeys %> IN (SELECT <%= d.primaryKeysSelection %> FROM <%= d.table %><%= d.where %><%= d.limit %>)';
    } else {
      query = 'DELETE FROM <%= d.table %><%= d.where %>';
    }
  ...
  showIndexesQuery: function(tableName) {
    ...
    var query = 'SELECT i.relname AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, ' +
      ...
      "t.relkind = 'r' and t.relname = '<%= d.tableName %>' GROUP BY i.relname, ix.indexrelid, ix.indisprimary, ix.indisunique, ix.indkey ORDER BY i.relname;";

    return Utils.compileOrRunTemplate(query, { tableName: tableName });
  },

  removeIndexQuery: function(tableName, indexNameOrAttributes) {
    var sql = 'DROP INDEX IF EXISTS <%= d.indexName %>'
    ...
    return Utils.compileOrRunTemplate(sql, {
  ...
  attributeToSQL: function(attribute) {
    ...
    if (Utils.defaultValueSchemable(attribute.defaultValue)) {
      template += ' DEFAULT <%= d.defaultValue %>';
    ...
    if (attribute.references) {
      ...
      template += ' REFERENCES <%= d.referencesTable %> (<%= d.referencesKey %>)';
      ...
      if (attribute.onDelete) {
        template += ' ON DELETE <%= d.onDeleteAction %>';
        replacements.onDeleteAction = attribute.onDelete.toUpperCase();
      }

      if (attribute.onUpdate) {
        template += ' ON UPDATE <%= d.onUpdateAction %>';
    ...
    return  Utils.compileOrRunTemplate(template, replacements);
  },
  ...
  createTrigger: function(tableName, triggerName, eventType, fireOnSpec, functionName, functionParams, optionsArray) {
    var sql = [
        'CREATE <%= d.constraintVal %>TRIGGER <%= d.triggerName %>'
        , '<%= d.eventType %> <%= d.eventSpec %>'
        , 'ON <%= d.tableName %>'
        , '<%= d.optionsSpec %>'
        , 'EXECUTE PROCEDURE <%= d.functionName %>(<%= d.paramList %>);'
      ].join('\n\t');
  ...
  dropTrigger: function(tableName, triggerName) {
    var sql = 'DROP TRIGGER <%= d.triggerName %> ON <%= d.tableName %> RESTRICT;';
    return Utils.compileOrRunTemplate(sql, {
      triggerName: triggerName,
      tableName: tableName
    });
  ...
  renameTrigger: function(tableName, oldTriggerName, newTriggerName) {
    var sql = 'ALTER TRIGGER <%= d.oldTriggerName %> ON <%= d.tableName %> RENAME TO <%= d.newTriggerName %>;';
    return Utils.compileOrRunTemplate(sql, {
      tableName: tableName,
  ...
  createFunction: function(functionName, params, returnType, language, body, options) {
    var sql = ['CREATE FUNCTION <%= d.functionName %>(<%= d.paramList %>)'
        , 'RETURNS <%= d.returnType %> AS $func$'
        , 'BEGIN'
        , '\t<%= d.body %>'
        , 'END;'
        , "$func$ language '<%= d.language %>'<%= d.options %>;"
    ].join('\n');
  ...
  dropFunction: function(functionName, params) {
    ...
    var sql = 'DROP FUNCTION <%= d.functionName %>(<%= d.paramList %>) RESTRICT;';
    return Utils.compileOrRunTemplate(sql, {
      functionName: functionName,
      paramList: this.expandFunctionParamList(params)
    });
  ...
  renameFunction: function(oldFunctionName, params, newFunctionName) {
    var sql = 'ALTER FUNCTION <%= d.oldFunctionName %>(<%= d.paramList %>) RENAME TO <%= d.newFunctionName %>;';
    return Utils.compileOrRunTemplate(sql, {
  ...
  databaseConnectionUri: function(config) {
    var template = '<%= d.protocol %>://<%= d.user %>:<%= d.password %>@<%= d.host %><% if (port) { %>:<%= d.port %><% } %>/<%= d.database %><% if (ssl) { %>?ssl=<%= d.ssl %><% } %>';

    return Utils.compileOrRunTemplate(template, {

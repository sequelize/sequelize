...
function pgEscape(s) {
  s = Utils.escape(s) // weird - this seems more like an unescape. It turns a backslash-escaped double quote into an unescaped one (?)
  if (typeof s == 'string') {
    ...
    updateQuery: function(tableName, attrValueHash, where) {
      ...
      for (var key in attrValueHash) {
        ...
        if (typeof(value) == 'object') {
          value = pgEscape(JSON.stringify(value))
        } else {
          value = pgEscape((value instanceof Date) ? pgSqlDate(value) : value)
        }
        values.push(addQuotes(key) + "=" + value)
      }

...
const QueryGenerator = {
  ...
  deleteQuery(tableName, where, options, model) {
    ...
    let whereConditions = this.getWhereConditions(where, null, model, options);
    if(whereConditions) {
    ...
    if (options.limit) {
      ...
      query = [
        ...
        `WHERE ${wrappedPrimaryKeys} IN (`,
          ...
          `FROM ${quotedTableName}${whereConditions}`,
          `LIMIT ${this.escape(options.limit)}`,
        `)`].join(' ');
    } else {
      query = `DELETE FROM ${quotedTableName}${whereConditions}`;
    }

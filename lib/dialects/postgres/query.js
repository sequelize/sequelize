...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    this.sql = sql;
    const isParam = parameters && parameters.length;

    let preQuery = Promise.resolve();
    if (!_.isEmpty(this.options.searchPath)) {
      const searchPathQuery = this.sequelize.getQueryInterface().QueryGenerator.setSearchPath(this.options.searchPath);
    ...
    debug('executing(%s) : %s with parameters %O', connection.uuid || 'default', this.sql, parameters);
    ...
    return query.catch(err => {
      ...
      .then(queryResult => {
        debug('executed(%s) : %s with parameters %O', connection.uuid || 'default', this.sql, parameters);

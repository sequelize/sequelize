...
Query.prototype.run = function(sql, parameters) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    if (self.isShowIndexesQuery()) {
      ...
      // This should really be thrown in the model instance, but requires significant refactoring to do properly.
      if (self.isUpdateQuery()) {
        if (result.rowCount < 1) {
          throw new sequelizeErrors.OptimisticLockError({
            model: self.instance.Model.name,
            sql: sql
          });
        }
      }

...
Query.prototype.run = function(sql) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    if (self.isShowIndexesQuery()) {
      ...
      return results;
    } else if (QueryTypes.BULKUPDATE === self.options.type || QueryTypes.BULKDELETE === self.options.type) {
    ...
    } else if (self.isInsertQuery() || self.isUpdateQuery() || QueryTypes.DELETE === self.options.type) {
      if(!result.rowCount) {
        return null;
      }

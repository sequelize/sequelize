...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    const { timeZone, dialect }=this.sequelize.options;
    if (!_.isEmpty(this.options.searchPath)) {
      sql = this.sequelize.getQueryInterface().QueryGenerator.setSearchPath(this.options.searchPath) + sql;
    }
    let logSql = sql;
    if (parameters) {
      sql = sql.replace(/\$(\$)/g, (match, key) => {
        return this.options.skipUnescape ? match : key;
      });
      logSql = logSql.replace(/(ST_GeomFromGeoJSON\()?\$(\$|\d+)/g, (match, isGeometry, key) => {
        if ('$' === key) {
          return this.options.skipUnescape ? match : key;
        }
        if (parameters) {
          if (Array.isArray(parameters[key-1]) && !parameters[key-1].length) {
            return "'{}'";
          }
          return SqlString.escape(isGeometry ? JSON.stringify(parameters[key-1]) : parameters[key-1], timeZone, dialect);
        }
        return match;
      });
    }
    ...
    const complete = this._logQuery(logSql, debug, parameters);

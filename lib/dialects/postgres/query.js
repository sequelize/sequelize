...
Query.prototype.run = function(sql) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    if (QueryTypes.DESCRIBE === self.options.type) {
      result = {};
    ...
    if (self.isShowOrDescribeQuery()) {
      return results;
    }

    if (QueryTypes.BULKUPDATE === self.options.type) {
    ...
    if (QueryTypes.BULKDELETE === self.options.type) {
      return +result.rowCount;
    }

    if (self.isUpsertQuery()) {
      return rows[0].sequelize_upsert;
    }

    if (self.isInsertQuery() || self.isUpdateQuery()) {
      if (self.instance && self.instance.dataValues) {
        ...
        if (rows[0] !== undefined) {
          ...
          for (var i = 0; i < keys.length; i++) {
            var record = rows[0][keys[i]];
            var attr = Utils._.find(self.model.rawAttributes, function (attribute) {
              return attribute.fieldName === keys[i] || attribute.field === keys[i];
            });
            self.instance.dataValues[attr && attr.fieldName || keys[i]] = record;
          }
        }
      }

...
class Query extends AbstractQuery {
  ...
  run(sql, parameters) {
    ...
    const isBinded = parameters && parameters.length;
    ...
    if (!_.isEmpty(this.options.searchPath)) {
      ...
      if (isBinded) {
        preQuery = new Promise((resolve, reject) => connection.query(searchPathQuery,
          (error, result) => error ? reject(error) : resolve(result)));
      } else {
        sql = searchPathQuery + sql;
      }
    }
    this.sql = sql;

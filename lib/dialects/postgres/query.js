...
export class Query extends BaseQuery {
  ...
  static formatBindParameters(sql, values, dialect) {
    ...
    if (Array.isArray(values)) {
      bindParam = values;
      sql = BaseQuery.formatBindParameters(sql, values, dialect, { skipValueReplace: true })[0];
    } else {
      let i = 0;
      const seen = {};
      const replacementFunc = (match, key, valuesToBind) => {
        if (seen[key] !== undefined) {
          return seen[key];
        }
        if (valuesToBind[key] !== undefined) {
          i = i + 1;
          bindParam.push(valuesToBind[key]);
          seen[key] = '$' + i;
          return '$' + i;
        }
        return undefined;
      };
      sql = BaseQuery.formatBindParameters(sql, values, dialect, replacementFunc)[0];
    }
  ...
  async run(sql, parameters) {
    ...
    const query =
      parameters && parameters.length
        ? new Promise((resolve, reject) =>
            this.client.query(this.sql, parameters, (error, result) => (error ? reject(error) : resolve(result)))
          )
        : new Promise((resolve, reject) =>
            this.client.query(this.sql, (error, result) => (error ? reject(error) : resolve(result)))
          );
    ...
    if (benchmark) {
      this.sequelize.log(
        'Executed (' + (this.client.uuid || 'default') + '): ' + this.sql,
        Date.now() - queryBegin,
        this.options
      );
    }
    ...
    if (this.isShowIndexesQuery()) {
      ...
      if (this.options.raw === false && this.sequelize.options.quoteIdentifiers === false) {
        const attrsMap = Object.keys(this.model.rawAttributes).reduce((m, k) => {
          m[k.toLowerCase()] = k;
          return m;
        }, {});
        result = rows.map((row) => {
      ...
      if (this.instance && this.instance.dataValues) {
        for (const key in rows[0]) {
          if (Object.hasOwn(rows[0], key)) {
            ...
            const attr = Object.values(this.model.rawAttributes).find(
  ...
  formatError(err) {
    ...
    switch (code) {
      // 23001 (restrict_violation) is what Postgres 18+ raises when a DELETE or
      // UPDATE trips a RESTRICT referential action; 17 and earlier reported
      // those as 23503 (foreign_key_violation) like any other FK violation.
      case '23001':
      case '23503':
        index = errMessage.match(/violates (?:RESTRICT setting of )?foreign key constraint "(.+?)"/);
      ...
      case '23505':
        ...
        if (errDetail && (match = errDetail.replace(/"/g, '').match(/Key \((.*?)\)=\((.*?)\)/))) {
          ...
          if (this.model && this.model.uniqueKeys) {
            ...
            for (const constraint of Object.values(this.model.uniqueKeys)) {
              if (
                constraint.fields.length === fieldKeys.length &&
                constraint.fields.every((f, i) => f === fieldKeys[i]) &&
                !!constraint.msg
              ) {
  ...
  isForeignKeysQuery() {
    return /SELECT conname as constraint_name, pg_catalog\.pg_get_constraintdef\(r\.oid, true\) as condef FROM pg_catalog\.pg_constraint r WHERE r\.conrelid = \(SELECT oid FROM pg_class WHERE relname = '.*' LIMIT 1\) AND r\.contype = 'f' ORDER BY 1;/.test(
      this.sql
    );
  }

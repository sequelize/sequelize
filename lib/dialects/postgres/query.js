    ...
    if (this.send('isSelectQuery')) {
      ...
      for (var key in rows[0]) {
        if (rows[0].hasOwnProperty(key)) {
          ...
          if (this.callee.daoFactory && this.callee.daoFactory.rawAttributes && this.callee.daoFactory.rawAttributes[key]) {
            var fct = this.callee.daoFactory.rawAttributes[key].get || (this.callee.daoFactory.rawAttributes[key].type && this.callee.daoFactory.rawAttributes[key].type.get) || undefined

            if (fct) {
              record = fct(record, this.callee)
            }
          }

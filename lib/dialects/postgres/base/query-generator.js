...
import * as Dot from '../../../utils/dot.js';
import Association from '../../../associations/base.js';
import BelongsTo from '../../../associations/belongs-to.js';
import BelongsToMany from '../../../associations/belongs-to-many.js';
import HasMany from '../../../associations/has-many.js';
import Op from '../../../operators.js';
import crypto from 'node:crypto';

// Cast types accepted in a JSON path (`where: { meta: { 'age::integer': 30 } }`).
//
// The cast is taken from an object key, which is frequently attacker-controlled -- the classic
// shape being `where: { metadata: req.body.filter }`. Unlike the JSON path *values*, which are
// escaped by `jsonPathExtractionQuery`, the cast is interpolated straight into `CAST(... AS x)`,
// so without an allow-list a key such as `'role::text) or 1=1--'` injects arbitrary SQL.
// See CVE-2026-30951; this list matches the upstream fix in sequelize 6.37.8.
//
// A few entries (nvarchar, datetime, signed, unsigned) are casts from other dialects that Postgres
// will reject anyway. They are kept so this stays a straight copy of the upstream allow-list.
...
const QueryGenerator = {
  ...
  /*
    Builds the RETURNING clause shared by the insert and update queries.

    `options.returning` is either an array of column names to restrict the clause to, or a boolean.
    The boolean form enumerates the model's own columns rather than using `RETURNING *`, so columns
    that exist on the table but are not declared on the model do not come back and end up in
    `dataValues`. Virtual attributes are skipped.

    Falls back to `*` when there is nothing to enumerate, which is the case for queries built without
    a model (e.g. `queryInterface.bulkInsert` called directly).

    The field list is returned alongside the fragment because `insertQuery`'s exception wrapper needs
    the columns on their own to build its SELECT.
   @private
  */
  generateReturnValues(modelAttributes, options) {
    ...
    if (Array.isArray(options.returning)) {
      returnFields = options.returning.map((field) => this.quoteIdentifier(field));
    } else if (modelAttributes) {
      for (const attribute of Object.values(modelAttributes)) {
    ...
    return { returnFields, returningFragment: ` RETURNING ${returnFields.join(',')}` };
  },
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    for (const key in valueHash) {
      if (Object.hasOwn(valueHash, key)) {
        ...
        // SERIALS can't be NULL in postgres, so use DEFAULT instead
    ...
    const onConflictDoNothing = options.ignoreDuplicates ? ' ON CONFLICT DO NOTHING' : '';
    const attributes = fields.join(',');

    let valueQuery = `INSERT INTO ${quotedTable} (${attributes}) VALUES (${values.join(',')})${onConflictDoNothing}`;
    let emptyQuery = `INSERT INTO ${quotedTable}${onConflictDoNothing} DEFAULT VALUES`;

    // The exception wrapper below selects out of the temp function rather than from the INSERT, so it
    // needs the field list on its own.
    let returnFields = ['*'];
    let returningFragment = '';

    if (options.returning) {
      const returnValues = this.generateReturnValues(modelAttributes, options);
      returnFields = returnValues.returnFields;
    ...
    if (options.exception) {
      ...
      // Use a UUID but prefix with 'func_' (numbers first not allowed)
      ...
      // `RETURNING ... INTO response` assigns positionally against the table's composite row type, so
      // the inner INSERT has to return every column in the table's physical order -- an enumerated
      // list in model-attribute order silently shuffles the values into the wrong fields whenever the
      // two orders differ (a table created by a migration rather than by sync()). The outer SELECT
      // still projects only the model's own columns, by name.
      valueQuery =
        `CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response ${quotedTable}, OUT sequelize_caught_exception text) RETURNS RECORD AS ` +
        delimiter +
        ' BEGIN ' +
        valueQuery +
        ' RETURNING * INTO response; EXCEPTION ' +
        options.exception +
        ' END ' +
        delimiter +
        ' LANGUAGE plpgsql; SELECT ' +
        returnFields.map((field) => `(testfunc.response).${field}`).join(', ') +
        ', testfunc.sequelize_caught_exception FROM pg_temp.testfunc(); DROP FUNCTION IF EXISTS pg_temp.testfunc()';
    } else {
      valueQuery += returningFragment;
      emptyQuery += returningFragment;
    }

    return (attributes.length ? valueQuery : emptyQuery) + ';';
  },
  ...
  */
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    for (const fieldValueHash of fieldValueHashes) {
      for (const key of Object.keys(fieldValueHash)) {
    ...
    const returning = options.returning
      ? this.generateReturnValues(fieldMappedAttributes, options).returningFragment
      : '';

    return `INSERT INTO ${quotedTable} (${attributes}) VALUES ${tuples.join(',')}${onConflictDoNothing}${returning};`;
  },
  ...
  */
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    if (options.returning) {
      ...
      returning = this.generateReturnValues(attributes, options).returningFragment;
    }
    ...
    for (const key in attrValueHash) {
      ...
      values.push(
        this.quoteIdentifier(key) +
          '=' +
          this.escape(value, (modelAttributeMap && modelAttributeMap[key]) || undefined, { context: 'UPDATE' })
      );
    }

    const quotedTable = this.quoteTable(tableName);
    const setFragment = values.join(',');
    // Called before the early return so that any option mutation it performs still happens, as it
    // did when this was built through a template.
    const whereFragment = this.whereQuery(where, options);

    if (values.length === 0) {
      return '';
    }

    // `whereQuery` returns '' when there is no condition, which leaves a trailing space -- hence
    // the trim, which the template form also relied on.
    return `UPDATE ${quotedTable} SET ${setFragment} ${whereFragment}${returning}`.trim();
  },
  ...
  */
  arithmeticQuery(operator, tableName, attrValueHash, where, options, attributes) {
    ...
    if (options.returning) {
      options.mapToModel = true;
      // `attributes` here is the set of extra columns to assign, not the model's attributes, so there
      // is nothing to enumerate. Increment/decrement keep returning every column unless the caller
      // asks for a specific list.
      returning = this.generateReturnValues(null, options).returningFragment;
    }
    ...
    // No trim here, unlike updateQuery: an empty where leaves a trailing space in the generated
    // SQL, and that is the existing behaviour.
    return `UPDATE ${this.quoteTable(tableName)} SET ${values.join(',')} ${this.whereQuery(where)}${returning}`;
  },
  ...
  */
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    // Postgres has no index type keyword -- the access method goes in USING instead.
    delete options.type;
    ...
    const ind = [
      ...
      options.where ? options.where : undefined
    ];

    return ind.filter(Boolean).join(' ');
  },
  ...
  getConstraintSnippet(tableName, options) {
    ...
    switch (options.type.toUpperCase()) {
      ...
      case 'CHECK':
        ...
        break;
      case 'DEFAULT':
        throw new Error('Default constraints are not supported by Postgres.');
      case 'PRIMARY KEY':
  ...
  */
  quote(collection, parent, connector) {
    ...
    if (typeof collection === 'string') {
        ...
        if (previousModel && previousModel.prototype instanceof Model) {
          ...
          if (model) {
            ...
            if (!(item instanceof Association)) {
              throw new Error(`Unable to find a valid association for model, '${model.name}'`);
            }
          }
        }
          ...
          if (index > 0 && orderIndex !== -1) {
            ...
            if (previousModel.associations !== undefined && previousModel.associations[item]) {
              ...
              if (
                ...
                previousModel.rawAttributes[itemSplit[0]].type instanceof DataTypes.JSON
              ) {
      ...
      throw new Error(
        `Unknown structure passed to order / group: ${collection instanceof Association ? collection.as : inspect(collection)}`
      );
    }
  },
  ...
  */
  quoteIdentifiers(identifiers) {
    if (identifiers.indexOf('.') !== -1) {
      ...
      return (
        this.quoteIdentifier(identifiers.slice(0, identifiers.length - 1).join('.')) +
        '.' +
        this.quoteIdentifier(identifiers[identifiers.length - 1])
      );
    } else {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        const baseQuery =
          ...
          ') AS sub'; // Every derived table must have its own alias
    ...
    if (options.lock) {
      ...
      if (lock === 'KEY SHARE' || lock === 'NO KEY UPDATE') {
        query += ' FOR ' + lock;
      } else if (lock === 'SHARE') {
        query += ' FOR SHARE';
      } else {
      ...
      if (options.lock.of && options.lock.of.prototype instanceof Model) {
      ...
      attributes.map((attr) => {
        ...
        if (Array.isArray(attr)) {
          ...
          if (attr[0] instanceof Utils.SequelizeMethod) {
            ...
            this.options.attributeBehavior === 'escape' ||
            (!attr[0].includes('(') && !attr[0].includes(')'))
          ) {
...
https://github.com/sequelize/sequelize/discussions/15694`);
          ...
          attr =
            attr.indexOf(Utils.TICK_CHAR) < 0 && attr.indexOf('"') < 0
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    const throughAttributes = through.attributes.map(
      (attr) =>
        this.quoteIdentifier(throughAs) +
        '.' +
        this.quoteIdentifier(Array.isArray(attr) ? attr[0] : attr) +
        ' AS ' +
    ...
    if (topInclude.through && Object(topInclude.through.model) === topInclude.through.model) {
            ...
            [Op.and]: [
              this.sequelize.asIs(
                [
                  ...
                  this.quoteIdentifier(topInclude.through.model.name) +
                    '.' +
                    this.quoteIdentifier(topAssociation.identifierField)
                ].join(' = ')
              ),
      ...
      query = this.selectQuery(
        topInclude.model.getTableName(),
        {
          ...
          where: {
            [Op.and]: [topInclude.where, { [Op.join]: this.sequelize.asIs(join) }]
          },
          limit: 1,
   ...
   *
   * A nested transaction generates no query by default, so its savepoint stays open until the root
   * transaction ends. That leaves server-side state to accumulate, but tolerates callers whose
   * nested transactions overlap, since nothing is ever released out of order.
   *
   * Opting in to `releaseSavepointsOnCommit` commits a nested transaction by releasing its
   * savepoint, which keeps the subtransaction stack from growing for the life of the root
   * transaction. Releasing discards every savepoint opened after it, so overlapping nested
   * transactions become an error rather than a leak.
   *
   * @param  {Transaction} transaction
   * @return {String}         The generated sql query.
   * @private
  ...
  commitTransactionQuery(transaction) {
    if (transaction.parent) {
      if (!this.options.releaseSavepointsOnCommit) {
        return;
      }

      // force quoting of savepoint identifiers for postgres
      return 'RELEASE SAVEPOINT ' + this.quoteIdentifier(transaction.name, true) + ';';
    }
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (Object.hasOwn(this.OperatorMap, smth.comparator)) {
    ...
    if (smth instanceof Utils.Where) {
      ...
      if (smth.attribute instanceof Utils.SequelizeMethod) {
        ...
        key =
          this.quoteTable(smth.attribute.Model.name) +
          '.' +
          this.quoteIdentifier(smth.attribute.field || smth.attribute.fieldName);
      }
  ...
  whereItemQuery(key, value, options) {
    ...
    if (key && typeof key === 'string' && key.indexOf('.') !== -1 && options.model) {
      ...
      if (
        ...
        Dot.set(tmp, keyParts.slice(1), value);
    ...
    if (!value) {
      return this._joinKeyValue(
        ...
        value === null ? this.OperatorMap[Op.is] : this.OperatorMap[Op.eq],
        options.prefix
      );
    }
    ...
    if (key === Op.placeholder) {
      return this._joinKeyValue(
        this.OperatorMap[key],
        this.escape(value, field),
        this.OperatorMap[Op.eq],
        options.prefix
      );
    }
  ...
  _replaceAliases(orig) {
    ...
    for (let [prop, item] of Object.entries(orig)) {
  ...
  _traverseJSON(items, baseKey, prop, item, path) {
    ...
    if (_.isPlainObject(item)) {
      ...
      for (const [itemProp, value] of Object.entries(item)) {
  ...
  */
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    throw new Error(
      `Unsupported where option value: ${inspect(smth)}. Please refer to the Sequelize documentation to learn more about which values are accepted as part of the where option.`
    );
  },

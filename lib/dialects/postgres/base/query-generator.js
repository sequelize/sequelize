...
const QueryGenerator = {
  ...
  */
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    const fields = [];
    const values = [];
    let valueQuery = 'INSERT INTO <%= table %> (<%= attributes %>) VALUES (<%= values %>)<%= onConflictDoNothing %>';
    let emptyQuery = 'INSERT INTO <%= table %><%= onConflictDoNothing %>';
    ...
    if (options.exception) {
      ...
      valueQuery =
        'CREATE OR REPLACE FUNCTION pg_temp.testfunc(OUT response <%= table %>, OUT sequelize_caught_exception text) RETURNS RECORD AS ' +
        delimiter +
        ' BEGIN ' +
        valueQuery +
    ...
    const replacements = {
      onConflictDoNothing: options.ignoreDuplicates ? ' ON CONFLICT DO NOTHING' : '',
  ...
  */
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    const query =
      'INSERT INTO <%= table %> (<%= attributes %>) VALUES <%= tuples %><%= onConflictDoNothing %><%= returning %>;';
    const tuples = [];
    const serials = {};
    const allAttributes = [];
    ...
    const replacements = {
      ...
      tuples: tuples.join(','),
      returning: options.returning ? ' RETURNING *' : '',
      onConflictDoNothing: options.ignoreDuplicates ? ' ON CONFLICT DO NOTHING' : ''
    };

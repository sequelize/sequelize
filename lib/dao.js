...
module.exports = (function() {
  var DAO = function(values, options, isNewRecord, reflection) {
    var self = this
    ...
    if (this.hasDefaultValues) {
      Utils._.each(this.defaultValues, function (value, name) {
        if (typeof self[name] === 'undefined') {
          ...
          // We check the dataType's get() function here to format the default values correctly (if applicable).
          // For example, {defaultValue: '"hello" => "world"'} for HSTORE needs
          // to be translated into an object (or if it's already an object, then just return that)
          if (reflection && reflection.rawAttributes && reflection.rawAttributes[name]) {
            var fct = reflection.rawAttributes[name].get || (reflection.rawAttributes[name].type && reflection.rawAttributes[name].type.get) || undefined
  ...
  DAO.prototype.save = function(fields) {
    ...
    for (var attrName in this.daoFactory.rawAttributes) {
      if (this.daoFactory.rawAttributes.hasOwnProperty(attrName)) {
        ...
        var setFct = definition.set || (definition.type && definition.type.set) || undefined
        if (setFct && hasValue) {
          values[attrName] = setFct(values[attrName], this)
        }
      }
    }
  ...
  var initAttributes = function(values, isNewRecord) {
    ...
    for (var key in values) {
      if (values.hasOwnProperty(key)) {
        if (this.__factory && this.__factory.rawAttributes && this.__factory.rawAttributes[key]) {
          var fct = this.__factory.rawAttributes[key].get || (this.__factory.rawAttributes[key].type && this.__factory.rawAttributes[key].type.get) || undefined

          if (fct) {
            values[key] = fct(values[key])
          }
        }

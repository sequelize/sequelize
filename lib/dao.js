...
module.exports = (function() {
  ...
  DAO.prototype.save = function(fields) {
    ...
    self.__factory.emit('preSave', self, self.__factory)
    ...
    query.on('success', function() {
      self.__factory.emit('postSave', self, self.__factory)
    });
  ...
  DAO.prototype.destroy = function() {
    ...
    if(this.__options.timestamps && this.__options.paranoid) {
      ...
      var identifier = this.__options.hasPrimaryKeys ? this.primaryKeyValues : this.id
      self.__factory.emit('preDestroy', self, self.__factory)
      var query = this.QueryInterface.delete(this, this.__factory.tableName, identifier)

      query.on('success', function() {
        self.__factory.emit('postDestroy', null, self.__factory)
      });
      return query
    }
  }

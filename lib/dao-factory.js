  ...
  */
  ...
  DAOFactory.prototype.bulkCreate = function(records, fieldsOrOptions, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      var done = function() {
        self.runHooks('afterBulkCreate', daos, options.fields, options, function(err, newRecords, newFields) {
          if (!!err) {
      ...
      self.runHooks('beforeBulkCreate', daos, options.fields, options, function(err, newRecords, newFields) {
        if (!!err) {
          return emitter.emit('error', err)
        }
        daos           = newRecords || daos
  ...
  DAOFactory.prototype.destroy = function(where, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      self.runHooks(self.options.hooks.beforeBulkDestroy, where, options, function(err, newWhere) {
        if (!!err) {
          return emitter.emit('error', err)
        }
        ...
        var runQuery = function(err, records) {
          ...
          .success(function(results) {
            var finished = function(err) {
              ...
              self.runHooks(self.options.hooks.afterBulkDestroy, where, options, function(err) {
                if (!!err) {
                  return emitter.emit('error', err)
                }
  ...
  DAOFactory.prototype.update = function(attrValueHash, where, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      var runSave = function() {
        self.runHooks(self.options.hooks.beforeBulkUpdate, attrValueHash, where, options, function(err, attributes, _where) {
          if (!!err) {
            return emitter.emit('error', err)
          }
          ...
          var runQuery = function(err, records) {
            ...
            .success(function(results) {
              var finished = function(err, records) {
                ...
                self.runHooks(self.options.hooks.afterBulkUpdate, attrValueHash, where, options, function(err) {
                  if (!!err) {
                    return emitter.emit('error', err)
                  }

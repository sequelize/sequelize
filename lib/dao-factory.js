...
module.exports = (function() {
  ...
  Utils.addEventEmitter(DAOFactory);
  ...
  DAOFactory.prototype.build = function(values, options) {
    options = options || {}
    options.isNewRecord = options.hasOwnProperty('isNewRecord') ? options.isNewRecord : true

    if (options.isNewRecord) {
      this.emit('preBuild', null, this)
    }
    ...
    var daoEmit = instance.emit
    instance.emit = function() { 
      var res = daoEmit.apply(instance, arguments)
      //Only bubble these events
      if (Utils._.contains(['preSave', 'postSave', 'preDestroy', 'postDestroy'], arguments[0])) {
    ...
    if (options.isNewRecord) {
      this.emit('postBuild', instance, this)
    }

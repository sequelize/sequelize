  ...
  */
  ...
  DAOFactory.prototype.bulkCreate = function(records, fieldsOrOptions, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      var done = function() {
        self.runHooks('afterBulkCreate', daos, options, function(err, newRecords, newFields) {
          if (!!err) {
      ...
      var next = function(err) {
        ...
        var iterate = function(i) {
          self.runHooks('beforeCreate', daos[i], options, function(err, newValues) {
            if (!!err) {
              return emitter.emit('error', err)
            }
            ...
            daos[i].save({ transaction: options.transaction }).error(function(err) {
              ...
              self.runHooks('afterCreate', daos[i], options, function(err, newValues) {
                if (!!err) {
                  return emitter.emit('error', err)
                }
      ...
      self.runHooks('beforeBulkCreate', daos, options, function(err, newRecords, newFields) {
        if (!!err) {
          return emitter.emit('error', err)
        }
        daos           = newRecords || daos
  ...
  DAOFactory.prototype.destroy = function(where, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      self.runHooks(self.options.hooks.beforeBulkDestroy, _.extend({}, options, { where: where }), function(err, newWhere) {
        if (!!err) {
          return emitter.emit('error', err)
        }
        ...
        var runQuery = function(err, records) {
          ...
          .success(function(results) {
            var finished = function(err) {
              ...
              self.runHooks(self.options.hooks.afterBulkDestroy, _.extend({}, options, { where: where }), function(err) {
                if (!!err) {
                  return emitter.emit('error', err)
                }
            ...
            if (options && options.hooks === true) {
              var tick = 0
              var next = function(i) {
                self.runHooks(self.options.hooks.afterDestroy, records[i], options, function(err, newValues) {
                  if (!!err) {
                    return finished(err)
                  }
        ...
        if (options && options.hooks === true) {
          ...
          .success(function(records) {
            var next = function(i) {
              self.runHooks(self.options.hooks.beforeDestroy, records[i], options, function(err, newValues) {
                if (!!err) {
                  return runQuery(err)
                }
  ...
  DAOFactory.prototype.update = function(attrValueHash, where, options) {
    ...
    return new Utils.CustomEventEmitter(function(emitter) {
      var runSave = function() {
        self.runHooks(self.options.hooks.beforeBulkUpdate, _.extend({}, options, { attrValueHash: attrValueHash, where: where }), function(err, attributes, _where) {
          if (!!err) {
            return emitter.emit('error', err)
          }
          ...
          var runQuery = function(err, records) {
            ...
            .success(function(results) {
              var finished = function(err, records) {
                ...
                self.runHooks(self.options.hooks.afterBulkUpdate, _.extend(options, { attrValueHash: attrValueHash, where: where }), function(err) {
                  if (!!err) {
                    return emitter.emit('error', err)
                  }
              ...
              if (options && options.hooks === true && !!records && records.length > 0) {
                var tick = 0
                var next = function(i) {
                  self.runHooks(self.options.hooks.afterUpdate, records[i], options, function(err, newValues) {
                    if (!!err) {
                      return finished(err)
                    }
          ...
          if (options.hooks === true) {
            ...
            .success(function(records) {
              ...
              var next = function(i) {
                self.runHooks(self.options.hooks.beforeUpdate, records[i], options, function(err, newValues) {
                  if (!!err) {
                    return runQuery(err)
                  }

...
module.exports = (function() {
  ...
  DAOFactory.prototype.init = function(daoFactoryManager) {
    ...
    if (this.options.setterMethods) {
      Utils._.each(this.options.setterMethods, function(fct, name) {
        if(!Utils._.has(self.DAO.prototype.rawAttributes, name)) {
          self.DAO.prototype.__defineSetter__(name, fct);
        } else {
          throw new Error('Cannot create setter "' + name + '" - property already exists.')
        }
      })
    }
    if (this.options.getterMethods) {
      Utils._.each(this.options.getterMethods, function(fct, name) {
        if(!Utils._.has(self.DAO.prototype.rawAttributes, name)) {
          self.DAO.prototype.__defineGetter__(name, fct);
        } else {
          throw new Error('Cannot create getter "' + name + '" - property already exists.')
        }
      })
    }

  ...
  */
  ...
  DAOFactory.prototype.count = function(options) {
    ...
    return new Utils.CustomEventEmitter(function (emitter) {
      ...
      var newInclude = []
      if (options.include) {
        Utils._.forEach(options.include, function(include) {
          // only use includes with where clauses as others will affect the count incorrectly
          if (include.options && include.options.whereCollection && include.options.whereCollection.length > 0) {
            newInclude.push(include)
          } else if (include.where && Utils._.keys(include.where).length > 0) {
            newInclude.push(include)
          }
        })
      }
      options.include = newInclude

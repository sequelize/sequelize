// Vendored from `@types/sequelize@4.28.20` with one substantive change: the
// `Promise = require("bluebird")` alias is replaced with the native `Promise`,
// reflecting this fork's migration off bluebird. Every `Promise<T>` signature in
// this file therefore resolves to the native Promise type.
//
...
declare namespace sequelize {
  ...
  interface QueryOptions extends SearchPathOptions, ReturningOptions {
    ...
    /**
     * If true, transforms objects with `.` separated property names into nested objects.
     * For example { 'user.username': 'john' } becomes
  ...
  interface Options {
     ...
     * format
     * +/-HH:MM. Will also accept IANA timezone names (e.g. 'America/Los_Angeles');

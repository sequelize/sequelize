...
class Query extends AbstractQuery {
    ...
    const query = new Promise((resolve, reject) => {
      ...
      if (sql.startsWith('BEGIN TRANSACTION')) {
        return connection.beginTransaction(
          ...
          connection.lib.ISOLATION_LEVEL[options.isolationLevel]
        );
      }
      if (sql.startsWith('COMMIT TRANSACTION')) {
        return connection.commitTransaction((error) =>
      ...
      if (sql.startsWith('SAVE TRANSACTION')) {
        return connection.saveTransaction(
          (error) => (error ? reject(error) : resolve([])),
          options.transaction.name
        );
      }
      ...
      if (parameters) {
        _.forOwn(parameters, (value, key) => {
          const paramType = this.getSQLTypeFromJsType(
            value,
  ...
  isShowOrDescribeQuery() {
    ...
    result =
      ...
      this.sql
        .toLowerCase()
        .startsWith('select tablename = t.name, name = ind.name,');
    result =
      result ||
      this.sql.toLowerCase().startsWith('exec sys.sp_helpindex @objname');
  ...
  handleInsertQuery(results, metaData) {
    if (this.instance) {
      ...
      if (
        ...
        autoIncrementAttributeAlias = this.model.rawAttributes[
          autoIncrementAttribute
        ].field;
      ...
      id =
        ...
        (autoIncrementAttributeAlias &&
          results &&
          results[0][autoIncrementAttributeAlias]);

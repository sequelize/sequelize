...
const { WhereSqlBuilder } = require('./where-sql-builder');
...
class QueryGenerator {
  whereSqlBuilder = new WhereSqlBuilder(this);
  ...
  */
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (smth instanceof Utils.Fn) {
      return this.handleFn(smth, tableName, factory, options, prepend);
    }
  ...
  handleFn(smth, tableName, factory, options, prepend) {
    return `${smth.fn}(${
      smth.args.map(arg => {
  ...
  whereItemsQuery(where, options) {
    return this.whereSqlBuilder.whereOptionsToSql(where, options);
  }
  ...
  whereItemQuery(key, value, options = {}) {
    if (typeof key === 'symbol') {
      return this.whereSqlBuilder.buildComparison(
        undefined,
        undefined,
        key,
        value,
  ...
  _getSafeKey(key, prefix) {
    if (key instanceof Utils.Col) {
      return this._prefixKey(this.handleSequelizeMethod(key), prefix);
    }

    if (key instanceof Utils.SequelizeMethod) {
      return this.handleSequelizeMethod(key);
    }
   ...
   * @deprecated
   */
  // TODO (@ephys): MIGRATE AND DELETE
  _whereParseSingleValueObject(key, field, prop, value, options) {
    return this.whereSqlBuilder.buildComparison(key, field, prop, value, options);
  }
  ...
  getWhereConditions(smth, tableName, factory, options = {}, prepend = true) {
    ...
    if (Array.isArray(smth)) {
      ...
      if (Utils.canTreatArrayAsAnd(smth)) {
        return this.getWhereConditions({ [Op.and]: smth }, tableName, factory, options, prepend);
      }

...
class QueryGenerator {
  ...
  renameTableQuery(before, after) {
    return `ALTER TABLE ${this.quoteTable(before)} RENAME TO ${this.quoteTable(
      after
    )};`;
  }
  ...
  bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
    ...
    if (this._dialect.supports.returnValues && options.returning) {
      const returnValues = this.generateReturnValues(
  ...
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    const query = `${tmpTable}UPDATE ${this.quoteTable(
      tableName
    )} SET ${values.join(',')}${outputFragment} ${this.whereQuery(
      where,
      whereOptions
    )}${suffix}`.trim();
    // Used by Postgres upsertQuery and calls to here with options.exception set to true
  ...
  arithmeticQuery(
    ...
    extraAttributesToBeUpdated = Utils.removeNullValuesFromHash(
      extraAttributesToBeUpdated,
      this.options.omitNull
    );
    ...
    for (const field in incrementAmountsByField) {
      ...
      updateSetSqlFragments.push(
        `${quotedField}=${quotedField}${operator} ${escapedAmount}`
      );
    }
  ...
  */
  addIndexQuery(tableName, attributes, options, rawTablename) {
    ...
    const fieldsSql = options.fields.map((field) => {
      ...
      if (!field.name) {
        throw new Error(
          `The following index field has no name: ${util.inspect(field)}`
        );
      }
    ...
    const concurrently =
      this._dialect.supports.index.concurrently && options.concurrently
        ? 'CONCURRENTLY'
        : undefined;
    let ind;
    ...
    ind = ind.concat(
      ...
      this._dialect.supports.index.using === 1 && options.using
        ? `USING ${options.using}`
        : '',
      !this._dialect.supports.indexViaAlter ? `ON ${tableName}` : undefined,
      this._dialect.supports.index.using === 2 && options.using
        ? `USING ${options.using}`
        : '',
      `(${fieldsSql.join(', ')})`,
      this._dialect.supports.index.parser && options.parser
        ? `WITH PARSER ${options.parser}`
        : undefined,
      this._dialect.supports.index.where && options.where
        ? options.where
        : undefined
    );
    ...
    switch (options.type.toUpperCase()) {
      case 'UNIQUE':
        constraintName = this.quoteIdentifier(
          options.name || `${tableName}_${fieldsSqlString}_uk`
        );
      ...
      case 'CHECK':
        ...
        constraintName = this.quoteIdentifier(
          options.name || `${tableName}_${fieldsSqlString}_ck`
        );
      ...
      case 'DEFAULT':
        if (options.defaultValue === undefined) {
          throw new Error(
            'Default value must be specifed for DEFAULT CONSTRAINT'
          );
        }
        ...
        constraintName = this.quoteIdentifier(
          options.name || `${tableName}_${fieldsSqlString}_df`
        );
        constraintSnippet = `CONSTRAINT ${constraintName} DEFAULT (${this.escape(
          options.defaultValue
        )}) FOR ${fieldsSql[0]}`;
        break;
      case 'PRIMARY KEY':
        constraintName = this.quoteIdentifier(
          options.name || `${tableName}_${fieldsSqlString}_pk`
        );
      ...
      case 'FOREIGN KEY':
        ...
        constraintName = this.quoteIdentifier(
          options.name ||
            `${tableName}_${fieldsSqlString}_${references.table}_fk`
        );
        const referencesSnippet = `${this.quoteTable(
          references.table
  ...
  quote(collection, parent, connector) {
    ...
    if (Array.isArray(collection)) {
      ...
      collection.forEach((item, index) => {
        ...
        if (previousModel && previousModel.prototype instanceof Model) {
          ...
          if (typeof item === 'function' && item.prototype instanceof Model) {
            ...
            item.model &&
            item.model.prototype instanceof Model
          ) {
            // set
            model = item.model;
            as = item.as;
          ...
          if (model) {
            ...
            if (!(item instanceof Association)) {
              throw new Error(
                util.format(
                  "Unable to find a valid association for model, '%s'",
                  model.name
                )
              );
            }
        ...
        if (typeof item === 'string') {
          ...
          if (index > 0 && orderIndex !== -1) {
            ...
            previousModel &&
            previousModel.prototype instanceof Model
          ) {
            // only go down this path if we have preivous model and check only once
            if (
              previousModel.associations !== undefined &&
              previousModel.associations[item]
            ) {
              ...
              if (
                previousModel.rawAttributes[itemSplit[0]].type instanceof
                DataTypes.JSON
              ) {
                // just quote identifiers for now
                const identifier = this.quoteIdentifiers(
                  `${previousModel.name}.${
                    previousModel.rawAttributes[itemSplit[0]].field
                  }`
                );
      ...
      for (i = 0; i < collectionLength - 1; i++) {
        ...
        if (
          typeof item === 'string' ||
          item._modelAttribute ||
          item instanceof Utils.SequelizeMethod
        ) {
          break;
    ...
    if (collection._modelAttribute) {
      return `${this.quoteTable(collection.Model.name)}.${this.quoteIdentifier(
    ...
    throw new Error(
      `Unknown structure passed to order / group: ${util.inspect(collection)}`
    );
  }
    ...
    if (value !== null && value !== undefined) {
      ...
      if (field && field.type) {
        ...
        if (field.type.stringify) {
          ...
          const simpleEscape = (escVal) =>
            SqlString.escape(escVal, this.options.timezone, this.dialect);
  ...
  */
  selectQuery(tableName, options, model) {
      ...
      : tableName
          .map((t) => {
            return Array.isArray(t)
              ? this.quoteTable(t[0], t[1])
              : this.quoteTable(t, true);
          })
          .join(', ');
      ...
      for (const keyAtt of mainTable.model.primaryKeyAttributes) {
        ...
        if (
          !attributes.main.some(
            (attr) =>
              keyAtt === attr || keyAtt === attr[0] || keyAtt === attr[1]
          )
        ) {
          attributes.main.push(
            mainTable.model.rawAttributes[keyAtt].field
              ? [keyAtt, mainTable.model.rawAttributes[keyAtt].field]
              : keyAtt
          );
        }
      }
    ...
    attributes.main = this.escapeAttributes(
      attributes.main,
      options,
      mainTable.as
    );
    attributes.main =
      attributes.main || (options.include ? [`${mainTable.as}.*`] : ['*']);
    ...
    if (options.include) {
      for (const include of options.include) {
        ...
        if (joinQueries.attributes.main.length > 0) {
          attributes.main = _.uniq(
            attributes.main.concat(joinQueries.attributes.main)
          );
        }
        if (joinQueries.attributes.subQuery.length > 0) {
          attributes.subQuery = _.uniq(
            attributes.subQuery.concat(joinQueries.attributes.subQuery)
          );
        }
      }
    }
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        if (options.groupedLimit.on instanceof BelongsToMany) {
          ...
          options.includeMap = Object.assign(
            groupedLimitOptions.includeMap,
            options.includeMap
          );
    ...
    if (
      ...
      options.where = this.getWhereConditions(
        options.where,
        mainTable.as || tableName,
        model,
        options
      );
      if (options.where) {
    ...
    if (options.group) {
      options.group = Array.isArray(options.group)
        ? options.group
            .map((t) => this.aliasGrouping(t, model, mainTable.as, options))
            .join(', ')
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'having')) {
      options.having = this.getWhereConditions(
        options.having,
        tableName,
        model,
        options,
    ...
    if (subQuery) {
      ...
      query = `SELECT ${attributes.main.join(', ')} FROM (${subQueryItems.join(
        ''
      )}) AS ${mainTable.as}${mainJoinQueries.join('')}${mainQueryItems.join(
        ''
      )}`;
    } else {
    ...
    if (options.lock && this._dialect.supports.lock) {
      ...
      if (
        this._dialect.supports.lockKey &&
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map((attr) => {
        ...
        if (Array.isArray(attr) && attr.length === 2) {
          ...
          attr = attr.map((attr) =>
            attr instanceof Utils.SequelizeMethod
              ? this.handleSequelizeMethod(attr)
              : attr
          );
    ...
    if (joinQuery.attributes.subQuery.length > 0) {
      attributes.subQuery = attributes.subQuery.concat(
        joinQuery.attributes.subQuery
      );
    }
    ...
    if (include.include) {
      for (const childInclude of include.include) {
        ...
        if (childJoinQueries.attributes.main.length > 0) {
          attributes.main = attributes.main.concat(
            childJoinQueries.attributes.main
          );
        }
        if (childJoinQueries.attributes.subQuery.length > 0) {
          attributes.subQuery = attributes.subQuery.concat(
            childJoinQueries.attributes.subQuery
          );
        }
      }
    }
  ...
  generateJoin(include, topLevelInfo) {
    ...
    const fieldRight =
      association instanceof BelongsTo
        ? right.rawAttributes[
            association.targetIdentifier || right.primaryKeyAttribute
          ].field
        : association.identifierField;
    let asRight = include.as;

    while (
      ($parent = ($parent && $parent.parent) || include.parent) &&
      $parent.association
    ) {
      if (asLeft) {
    ...
    let joinOn = `${this.quoteTable(asLeft)}.${this.quoteIdentifier(
      fieldLeft
    )}`;
      ...
      if (parentIsTop) {
        ...
        if (topLevelInfo.subQuery) {
          subqueryAttributes.push(
            `${tableName}.${this.quoteIdentifier(fieldLeft)}`
          );
        }
      } else {
    ...
    joinOn += ` = ${this.quoteIdentifier(asRight)}.${this.quoteIdentifier(
      fieldRight
    )}`;
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
    ...
    if (this._dialect.supports.joinTableDependent) {
      ...
      joinBody = `${this.quoteTable(
        throughTable,
        throughAs
      )} ON ${sourceJoinOn} ${joinType} ${this.quoteTable(
        include.model.getTableName(),
        includeAs.internalAs
      )}`;
  ...
  _generateSubQueryFilter(include, includeAs, topLevelInfo) {
    ...
    if (
      topInclude.through &&
      Object(topInclude.through.model) === topInclude.through.model
    ) {
          ...
          where: {
            [Op.and]: [
              this.sequelize.literal(
                [
                  `${this.quoteTable(
                    topParent.model.name
                  )}.${this.quoteIdentifier(topParent.model.primaryKeyField)}`,
                  `${this.quoteIdentifier(
                    topInclude.through.model.name
      ...
      const join = [
        `${this.quoteIdentifier(topInclude.as)}.${this.quoteIdentifier(
          targetField
        )}`,
        `${this.quoteTable(
          topParent.as || topParent.model.name
        )}.${this.quoteIdentifier(sourceField)}`,
      ].join(' = ');
    ...
    if (Array.isArray(options.order)) {
      for (let order of options.order) {
        ...
        if (
          ...
          !(
            typeof order[0] === 'function' &&
            order[0].prototype instanceof Model
          ) &&
          !(
        ...
        if (subQuery) {
          ...
          const subQueryAttribute = options.attributes.find(
            (a) => Array.isArray(a) && a[0] === order[0] && a[1]
          );
  ...
  _throwOnEmptyAttributes(attributes, extraInfo = {}) {
    ...
    const namePart =
      (extraInfo.modelName && `for model '${extraInfo.modelName}'`) || '';
  ...
  addLimitAndOffset(options) {
    ...
    if (options.offset != null && options.limit == null) {
      fragment +=
        ' LIMIT ' + this.escape(options.offset) + ', ' + 10000000000000;
  ...
  handleSequelizeMethod(smth, tableName, factory, options, prepend) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(this.OperatorMap, smth.comparator)
    ) {
    ...
    if (smth instanceof Utils.Cast) {
      if (smth.val instanceof Utils.SequelizeMethod) {
        result = this.handleSequelizeMethod(
          smth.val,
          tableName,
          factory,
          options,
      ...
      return `${smth.fn}(${smth.args
        .map((arg) => {
          if (arg instanceof Utils.SequelizeMethod) {
            return this.handleSequelizeMethod(
              arg,
              tableName,
              factory,
              options,
          ...
          return this.escape(
            typeof arg === 'string' ? arg.replace('$', '$$$') : arg
          );
        })
        .join(', ')})`;
    }
  ...
  whereItemsQuery(where, options, binding) {
    ...
    return (
      (items.length &&
        items.filter((item) => item && item.length).join(binding)) ||
      ''
    );
  }
  ...
  whereItemQuery(key, value, options = {}) {
    ...
    if (
      value instanceof Utils.SequelizeMethod &&
      !(key !== undefined && value instanceof Utils.Fn)
    ) {
      return this.handleSequelizeMethod(value);
    }
    ...
    if (value[Op.or]) {
      return this._whereBind(
        this.OperatorMap[Op.or],
        key,
        value[Op.or],
        options
      );
    ...
    if (
      isPlainObject &&
      fieldType instanceof DataTypes.JSON &&
      options.json !== false
    ) {
    ...
    if (isArray) {
      return this._whereParseSingleValueObject(
        key,
        field,
        Op.in,
        value,
  ...
  _findField(key, options) {
    ...
    if (
      ...
      options.model.rawAttributes[key]
    ) {
    ...
    if (
      options.model &&
      options.model.fieldRawAttributesMap &&
      options.model.fieldRawAttributesMap[key]
    ) {
  ...
  _whereGroupBind(key, value, options) {
    const binding =
      key === Op.or ? this.OperatorMap[Op.or] : this.OperatorMap[Op.and];
    const outerBinding = key === Op.not ? 'NOT ' : '';

    if (Array.isArray(value)) {
      value = value
        .map((item) => {
          let itemQuery = this.whereItemsQuery(
            item,
            options,
            this.OperatorMap[Op.and]
          );
  ...
  _traverseJSON(items, baseKey, prop, item, path) {
    ...
    items.push(
      this.whereItemQuery(this._castKey(pathKey, item, cast), { [Op.eq]: item })
    );
  }
  ...
  _castKey(key, value, cast, json) {
    ...
    if (cast) {
      return new Utils.Literal(
        this.handleSequelizeMethod(
          new Utils.Cast(new Utils.Literal(key), cast, json)
        )
      );
    }
  ...
  _whereParseSingleValueObject(key, field, prop, value, options) {
    ...
    if (_.isPlainObject(value)) {
      if (value[Op.col]) {
        return this._joinKeyValue(
          key,
          this.whereItemQuery(null, value),
          comparator,
          options.prefix
        );
      }
      ...
      (result, value, key) => {
        if (_.isObject(value)) {
          return result.concat(
            this.parseConditionObject(value, path.concat(key))
...
Object.assign(
  QueryGenerator.prototype,
  require('./query-generator/transaction')
);

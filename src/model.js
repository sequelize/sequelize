...
`.trim());
  ...
  where(checkVersion) {
    ...
    const primaryKeyAttribute = this.constructor.primaryKeyAttribute;
    const isAutoGenerated = this.constructor.rawAttributes[primaryKeyAttribute]._autoGenerated;

    if (primaryKeyAttribute && isAutoGenerated && this.isNewRecord === false) {
      throw new Error('You attempted to query over a model that does not have a primary key explicitly defined this is not allowed');
    }

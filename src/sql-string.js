...
export function escape(val, timeZone, dialect, format) {
  ...
  if (Array.isArray(val)) {
    ...
    if ((dialect === 'postgres' || dialect === 'yugabytedb') && !format) {
  ...
  if (['postgres', 'sqlite', 'mssql', 'snowflake', 'db2', 'ibmi', 'yugabytedb'].includes(dialect)) {
...
export function formatNamedParameters(sql, values, timeZone, dialect) {
  return sql.replace(/:+(?!\d)(\w+)/g, (value, key) => {
    if ((dialect === 'postgres' || dialect === 'yugabytedb') && value.slice(0, 2) === '::') {
      return value;
    }

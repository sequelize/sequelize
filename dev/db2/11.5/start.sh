...
if [ ! "$(sudo docker ps -q -f name=db2server)" ]; then
	...
	sudo docker run -h db2server --name db2server --restart=always --detach --privileged=true -p 50000:50000 --env-file .env_list -v /Docker:/database ibmcom/db2:11.5.1.0-CN1
	count=1
	while true
	do

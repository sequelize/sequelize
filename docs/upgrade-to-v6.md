...
`Sequelize.import` has been used by code generated by the sequelize CLI for example.
With the move to es6 classes and the `.init` static dependency injection of a sequelize instance and data types is no longer necessary.
This is incompatible with sequelize-cli@^5

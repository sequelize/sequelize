...
Sequelize allow setting `underscored` option for Model. When `true` this option will set
`field` option on all attributes to underscored version of its name. This also apply to
...
With underscored option attributes injected to model are still camel cased but `field` option is set to their underscored version.

...
new Sequelize.STRING(1234)                // VARCHAR(1234)
Sequelize.STRING.BINARY               // VARCHAR BINARY
Sequelize.TEXT                        // TEXT
new Sequelize.TEXT('tiny')                // TINYTEXT
Sequelize.CITEXT                      // CITEXT      PostgreSQL and SQLite only.

Sequelize.INTEGER                     // INTEGER
Sequelize.BIGINT                      // BIGINT
new Sequelize.BIGINT(11)                  // BIGINT(11)

Sequelize.FLOAT                       // FLOAT
new Sequelize.FLOAT(11)                   // FLOAT(11)
new Sequelize.FLOAT(11, 10)               // FLOAT(11,10)

Sequelize.REAL                        // REAL        PostgreSQL only.
new Sequelize.REAL(11)                    // REAL(11)    PostgreSQL only.
new Sequelize.REAL(11, 12)                // REAL(11,12) PostgreSQL only.

Sequelize.DOUBLE                      // DOUBLE
new Sequelize.DOUBLE(11)                  // DOUBLE(11)
new Sequelize.DOUBLE(11, 10)              // DOUBLE(11,10)

Sequelize.DECIMAL                     // DECIMAL
new Sequelize.DECIMAL(10, 2)              // DECIMAL(10,2)

Sequelize.DATE                        // DATETIME for mysql / sqlite, TIMESTAMP WITH TIME ZONE for postgres
new Sequelize.DATE(6)                     // DATETIME(6) for mysql 5.6.4+. Fractional seconds support with up to 6 digits of precision
Sequelize.DATEONLY                    // DATE without time.
Sequelize.BOOLEAN                     // TINYINT(1)

new Sequelize.ENUM('value 1', 'value 2')  // An ENUM with allowed values 'value 1' and 'value 2'
new Sequelize.ARRAY(Sequelize.TEXT)       // Defines an array. PostgreSQL only.
new Sequelize.ARRAY(Sequelize.ENUM)       // Defines an array of ENUM. PostgreSQL only.

Sequelize.JSON                        // JSON column. PostgreSQL, SQLite and MySQL only.
Sequelize.JSONB                       // JSONB column. PostgreSQL only.

Sequelize.BLOB                        // BLOB (bytea for PostgreSQL)
new Sequelize.BLOB('tiny')                // TINYBLOB (bytea for PostgreSQL. Other options are medium and long)
...
new Sequelize.RANGE(Sequelize.INTEGER)    // Defines int4range range. PostgreSQL only.
new Sequelize.RANGE(Sequelize.BIGINT)     // Defined int8range range. PostgreSQL only.
new Sequelize.RANGE(Sequelize.DATE)       // Defines tstzrange range. PostgreSQL only.
new Sequelize.RANGE(Sequelize.DATEONLY)   // Defines daterange range. PostgreSQL only.
new Sequelize.RANGE(Sequelize.DECIMAL)    // Defines numrange range. PostgreSQL only.

new Sequelize.ARRAY(Sequelize.RANGE(Sequelize.DATE)) // Defines array of tstzrange ranges. PostgreSQL only.

Sequelize.GEOMETRY                    // Spatial column.  PostgreSQL (with PostGIS) or MySQL only.
new Sequelize.GEOMETRY('POINT')           // Spatial column with geometry type. PostgreSQL (with PostGIS) or MySQL only.
new Sequelize.GEOMETRY('POINT', 4326)     // Spatial column with geometry type and SRID.  PostgreSQL (with PostGIS) or MySQL only.
```
...
new Sequelize.INTEGER(11).UNSIGNED          // INTEGER(11) UNSIGNED
new Sequelize.INTEGER(11).ZEROFILL          // INTEGER(11) ZEROFILL
new Sequelize.INTEGER(11).ZEROFILL.UNSIGNED // INTEGER(11) UNSIGNED ZEROFILL
new Sequelize.INTEGER(11).UNSIGNED.ZEROFILL // INTEGER(11) UNSIGNED ZEROFILL
```
...
The new range can be used in model definitions as `Sequelize.RANGE(Sequelize.NEWTYPE)` or `new DataTypes.RANGE(DataTypes.NEWTYPE)`.

...
export interface HasManyGetAndCountAssociationsMixinOptions extends FindOptions {
  /**
   * Apply a scope on the related model, or remove its default scope by passing false.
   */
  scope?: string | boolean;
...
/**
 * The getAndCountAssociations mixin applied to models with hasMany.
 * An example of usage is as follows:
 *
 * ```js
 *
 ...
 *  // hasRoles...
 *  // countRoles...
 *  getAndCountRoles: Sequelize.HasManyGetAndCountAssociationsMixin;
 * }
 * ```
 *
 ...
 * @see Instance
 */
export type HasManyGetAndCountAssociationsMixin = (options?: HasManyGetAndCountAssociationsMixinOptions) => Promise<{ rows: M[]; count: number }>;

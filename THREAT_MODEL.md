...
This document defines security boundaries and expected properties for Sequelize v7: the `@sequelize/core` package and the official `@sequelize/*` dialect and CLI packages, published from the `main` branch as v7 alpha releases. Sequelize v6 (the `sequelize` package) is out of scope; a separate model can cover it if requested. This is not a vulnerability list, audit report, or assertion that every requirement is currently satisfied. Verify behavior in the applicable code and tests before relying on it. Update this model when supported behavior, trust boundaries, attack methods, security guarantees, or protections change.
...
- Direct runtime dependencies where they process ORM-controlled or attacker-influenced data.
...
- Third-party dialects, hooks, model modules, validators, migrations, and log destinations.
- Sequelize v6 (the `sequelize` package) and its dialects and legacy CLI (`sequelize-cli`); a separate model can cover v6 if requested.
- The internal behavior of bundled libraries such as `validator.js`; a separate model can cover it. Sequelize remains responsible only for how it passes data to and consumes results from them.
...
| OUT-01  | Database administrator                                                                | Driver and network (TB-1) → result mapping and model creation                                                   | Crafted column aliases or nested result keys modify object prototypes, replace inherited methods, or create unsafe objects when `nest` or eager loading is used.                                                                                                 | Library: prevent special property names such as `__proto__`, `constructor`, and `prototype` from changing object prototypes or inherited behavior; use own-property checks and null-prototype objects where appropriate; test every position in a nested key.                                        |

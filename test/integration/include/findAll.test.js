...
describe(Support.getTestDialectTeaser('Include'), () => {
  describe('findAll', () => {
    ...
    it('should be possible to extend the on clause with a where option on a belongsTo include', async function () {
      ...
      await Promise.all([results.users[0].setGroup(results.groups[1]), results.users[1].setGroup(results.groups[0])]);
    ...
    it('should ignore include with attributes: [] and through: { attributes: [] } (used for aggregates)', async function () {
      ...
      const users = await User.findAll({
        attributes: [[this.sequelize.fn('COUNT', this.sequelize.col('projects.id')), 'projectsCount']],
        include: {

...
describe(Support.getTestDialectTeaser('Include'), () => {
  describe('findAll', () => {
        ...
        (function (singles) {
          ...
          singles.forEach((model) => {
            promise = (async () => {
              ...
              if (previousInstance) {
                await previousInstance[`set${_.upperFirst(model.name)}`](
                  instance
                );
    ...
    it('should include attributes from through models', async function () {
      ...
      await Promise.all([
        results.products[0].addTag(results.tags[0], {
          through: { priority: 1 },
        }),
        results.products[0].addTag(results.tags[1], {
          through: { priority: 2 },
        }),
        results.products[1].addTag(results.tags[1], {
          through: { priority: 1 },
        }),
        results.products[2].addTag(results.tags[0], {
          through: { priority: 3 },
        }),
        results.products[2].addTag(results.tags[1], {
          through: { priority: 1 },
        }),
        results.products[2].addTag(results.tags[2], {
          through: { priority: 2 },
        }),
      ]);
    ...
    it('should ignore include with attributes: [] (used for aggregates)', async function () {
      ...
      const posts = await Post.findAll({
        attributes: [
          [
            this.sequelize.fn('COUNT', this.sequelize.col('comments.id')),
            'commentCount',
          ],
        ],
    ...
    it('should ignore include with attributes: [] and through: { attributes: [] } (used for aggregates)', async function () {
      ...
      const users = await User.findAll({
        attributes: [
          [
            this.sequelize.fn('COUNT', this.sequelize.col('projects.id')),
            'projectsCount',
          ],
        ],
        include: {
    ...
    it('Should return posts with nested include with inner join with a m:n association', async function () {
      ...
      expect(posts[0].Entity.tags[0].EntityTag.entity_id).to.equal(
        posts[0].post_id
      );
    });

...
describe(Support.getTestDialectTeaser('Sequelize'), () => {
  describe('query', () => {
    ...
    it('uses properties `query` and `values` if query is tagged', async function() {
      let logSql;
      const result = await this.sequelize.query({ query: `select ? as foo, ? as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, values: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } });
    ...
    it('uses properties `query` and `bind` if query is tagged', async function() {
      const typeCast = dialect === 'postgres' || dialect === 'db2' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query({ query: `select $1${typeCast} as foo, $2${typeCast} as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, bind: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } });
      expect(result).to.deep.equal(expected);
    ...
    it('replaces named parameters with the passed object and ignore those which does not qualify', async function() {
      const expected = (dialect === 'db2' || dialect === 'ibmi') ? [{ FOO: 1, BAR: 2, BAZ: '00:00' }] : [{ foo: 1, bar: 2, baz: '00:00' }];
    ...
    it('replaces named parameters with the passed object using the same key twice', async function() {
      const expected = (dialect === 'db2' || dialect === 'ibmi') ? [{ FOO: 1, BAR: 2, BAZ: 1 }] : [{ foo: 1, bar: 2, baz: 1 }];
    ...
    it('replaces named parameters with the passed object having a null property', async function() {
      const expected = (dialect === 'db2' || dialect === 'ibmi') ? [{ FOO: 1, BAR: null }] : [{ foo: 1, bar: null }];
    ...
    it('binds token with the passed array', async function() {
      const typeCast = dialect === 'postgres' || dialect === 'db2' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $1${typeCast} as foo, $2${typeCast} as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { type: this.sequelize.QueryTypes.SELECT, bind: [1, 2], logging(s) { logSql = s;} });
      expect(result).to.deep.equal(expected);
      if (['postgres', 'sqlite'].includes(dialect)) {
        expect(logSql).to.include('$1');
      }
    ...
    it('binds named parameters with the passed object', async function() {
      const typeCast = dialect === 'postgres' || dialect === 'db2' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $one${typeCast} as foo, $two${typeCast} as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } });
    ...
    if (dialect !== 'db2') {
      it('binds named parameters with the passed object using the same key twice', async function() {
        ...
        let logSql;
        const result = await this.sequelize.query(`select $one${typeCast} as foo, $two${typeCast} as bar, $one${typeCast} as baz${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } });
    ...
    it('binds named parameters array handles escaped $$', async function() {
      const typeCast = dialect === 'postgres' || dialect === 'db2' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $1${typeCast} as foo, '$$ / $$1' as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: [1], logging(s) { logSql = s;} });

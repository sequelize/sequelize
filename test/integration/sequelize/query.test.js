...
describe(Support.getTestDialectTeaser('Sequelize'), () => {
  describe('query', () => {
    ...
    describe('logging', () => {
      ...
      describe('with logQueryParameters', () => {
        ...
        it('add parameters in log sql when use bind value', async function() {
          ...
          await this.sequelize.query(`select $1${typeCast} as foo, $2${typeCast} as bar${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { bind: ['foo', 'bar'], logging: s=>logSql = s });
    ...
    it.skip('properly adds and escapes replacement value', async function() {
      let logSql;
      const number  = 1,
        date = new Date(),
        string = 't\'e"st',
      ...
      const result = await this.sequelize.query({
        query: `select ? as number, ? as date,? as string,? as boolean,? as buffer${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`,
    ...
    it('it allows to pass custom class instances', async function() {
      ...
      class SQLStatement {
        ...
        get query() {
          return `select ? as ${qq('foo')}, ? as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`;
        }
      }
    ...
    it('uses properties `query` and `values` if query is tagged', async function() {
      let logSql;
      const result = await this.sequelize.query({ query: `select ? as ${qq('foo')}, ? as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, values: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } });
      expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
      expect(logSql).to.not.include('?');
    });
    ...
    it('uses properties `query` and `bind` if query is tagged', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query({ query: `select $1${typeCast} as ${qq('foo')}, $2${typeCast} as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, bind: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } });
      expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
    ...
    it('dot separated attributes when doing a raw query without nest', async function() {
      const tickChar = dialect === 'postgres' || dialect === 'mssql' || dialect === 'ibmi' ? '"' : '`',
        sql = `select 1 as ${Sequelize.Utils.addTicks('foo.bar.baz', tickChar)}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`;
    ...
    it('replaces token with the passed array', async function() {
      const result = await this.sequelize.query(`select ? as ${qq('foo')}, ? as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { type: this.sequelize.QueryTypes.SELECT, replacements: [1, 2] });
      expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
    });

    it('replaces named parameters with the passed object', async function() {
      await expect(this.sequelize.query(`select :one as ${qq('foo')}, :two as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, replacements: { one: 1, two: 2 } }).then(obj => obj[0]))
    ...
    it('replaces named parameters with the passed object and ignore those which does not qualify', async function() {
      await expect(this.sequelize.query(`select :one as ${qq('foo')}, :two as ${qq('bar')}, '00:00' as ${qq('baz')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, replacements: { one: 1, two: 2 } }).then(obj => obj[0]))
    ...
    it('replaces named parameters with the passed object using the same key twice', async function() {
      await expect(this.sequelize.query(`select :one as ${qq('foo')}, :two as ${qq('bar')}, :one as ${qq('baz')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, replacements: { one: 1, two: 2 } }).then(obj => obj[0]))
    ...
    it('replaces named parameters with the passed object having a null property', async function() {
      await expect(this.sequelize.query(`select :one as ${qq('foo')}, :two as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, replacements: { one: 1, two: null } }).then(obj => obj[0]))
    ...
    it('binds token with the passed array', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $1${typeCast} as ${qq('foo')}, $2${typeCast} as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { type: this.sequelize.QueryTypes.SELECT, bind: [1, 2], logging(s) { logSql = s;} });
      expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
      if (dialect === 'postgres' || dialect === 'sqlite') {
        expect(logSql).to.include('$1');
      }
    ...
    it('binds named parameters with the passed object', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $one${typeCast} as ${qq('foo')}, $two${typeCast} as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } });
    ...
    it('binds named parameters with the passed object using the same key twice', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $one${typeCast} as ${qq('foo')}, $two${typeCast} as ${qq('bar')}, $one${typeCast} as ${qq('baz')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } });
    ...
    it('binds named parameters with the passed object having a null property', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      const result = await this.sequelize.query(`select $one${typeCast} as ${qq('foo')}, $two${typeCast} as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1, two: null } });
    ...
    it('binds named parameters array handles escaped $$', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      const result = await this.sequelize.query(`select $1${typeCast} as ${qq('foo')}, '$$ / $$1' as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: [1], logging(s) { logSql = s;} });
    ...
    it('binds named parameters object handles escaped $$', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      const result = await this.sequelize.query(`select $one${typeCast} as ${qq('foo')}, '$$ / $$one' as ${qq('bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1 } });
      expect(result[0]).to.deep.equal([{ foo: 1, bar: '$ / $one' }]);
    });

    it('escape where has $ on the middle of characters', async function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      const result = await this.sequelize.query(`select $one${typeCast} as ${qq('foo$bar')}${dialect === 'ibmi' ? ' FROM SYSIBM.SYSDUMMY1' : ''}`, { raw: true, bind: { one: 1 } });

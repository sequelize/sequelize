  ...
  describe('query', () => {
      ...
      describe('with logQueryParameters', () => {
        ...
        if (dialect !== 'ibmi') {
          it('add parameters in log sql when use bind value', async function () {
            let logSql;
            let typeCast = dialect === 'postgres' || dialect === 'yugabytedb' ? '::text' : '';
    ...
    describe('rejections', () => {
      ...
      it('reject when binds passed with object and numeric $1 is also present', async function () {
        const typeCast = ['postgres', 'db2', 'yugabytedb'].includes(dialect) ? '::int' : '';
    ...
    it('dot separated attributes when doing a raw query without nest', async function () {
      const tickChar = ['postgres', 'mssql', 'db2', 'ibmi', 'yugabytedb'].includes(dialect) ? '"' : '`';
    ...
    if (dialect !== 'ibmi') {
      ...
      if (dialect !== 'db2') {
        it('binds named parameters with the passed object using the same key twice', async function () {
          const typeCast = (dialect === 'postgres' || dialect === 'yugabytedb') ? '::int' : '';
          let logSql;
    ...
    if (['postgres', 'sqlite', 'mssql', 'yugabytedb'].includes(dialect)) {

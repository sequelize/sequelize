    ...
    describe('many-to-many', () => {
      describe('where tables have the same prefix', () => {
        it('should create a table wp_table1wp_table2s', async function () {
          ...
          expect(this.sequelize.modelManager.getModel('wp_table1swp_table2s'))
            .to.exist;
        });
      });
      ...
      describe('when join table name is specified', () => {
        ...
        it('should not use only a specified name', function () {
          expect(this.sequelize.modelManager.getModel('ms_table1sms_table2s'))
            .not.to.exist;
          expect(this.sequelize.modelManager.getModel('table1_to_table2')).to
            .exist;
        });
      });
    });
    ...
    describe('HasMany', () => {
      beforeEach(async function () {
        ...
        this.User = this.sequelize.define(
          `User${Math.ceil(Math.random() * 10000000)}`,
          { name: DataTypes.STRING }
        );
        this.Task = this.sequelize.define(
          `Task${Math.ceil(Math.random() * 10000000)}`,
          { name: DataTypes.STRING }
        );
        this.users = null;
        this.tasks = null;
      ...
      describe('removeDAO', () => {
        ...
        it('should correctly remove associated objects', async function () {
          ...
          expect(await this.user.getTasks()).to.have.length(
            this.tasks.length - 1
          );

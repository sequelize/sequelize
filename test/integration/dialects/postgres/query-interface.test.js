  ...
  describe('[POSTGRES Specific] QueryInterface', () => {
    ...
    describe('createSchema', () => {
      beforeEach( function() {
        ...
        return this.queryInterface.dropSchema('testschema')
        .reflect();
      });
      ...
      it('creates a schema', function() {
        ...
        .then(() => {
          return this.sequelize.query('SELECT schema_name FROM information_schema.schemata WHERE schema_name = \'testschema\';', { type: this.sequelize.QueryTypes.SELECT });
        })
        .then(res => {
      ...
      it('errors if the schema exists', function() {
        return this.queryInterface.createSchema('testschema')
        .catch(err => {
          return expect(err.message).to.be.equal('schema "testschema" already exists');
        });
      });
    });
    ...
    describe('databaseVersion', () => {
      it('reports version', function() {
        ...
        .then(res => {
          ...
          return expect(res).to.match(/[0-9\.[0-9]\.[0-9]/);
        });
      });
    });
    ...
    describe('renameFunction', () => {
      beforeEach( function() {
        ...
        .reflect()
        .then(() => {
          return this.queryInterface.dropFunction('rftest2', []);
        })
        .reflect()
        .then(() => {
          return this.queryInterface.createFunction('rftest1', [], 'varchar', 'plpgsql', 'return \'testreturn\';', {});
        });
      });
      ...
      it('renames a function', function() {
        ...
        .then(() => {
          return this.sequelize.query('select rftest2();', { type: this.sequelize.QueryTypes.SELECT });
        })
        .then(res => {
          return expect(res[0].rftest2).to.be.eql('testreturn');
        });
      });
    });
    ...
    describe('indexes', () => {
      ...
      it('adds, reads and removes a named functional index to the table', function() {
        ...
        .then( () => {
          return this.queryInterface.showIndex('Group');
        })
        .then(indexes => {
          const indexColumns = _.uniq(indexes.map(index => {
            return index.name;
          }));
        ...
        .then( () => {
          return this.queryInterface.removeIndex('Group', 'group_username_lower');
        })
        .then( () =>{
          return this.queryInterface.showIndex('Group');
        })

  ...
  describe('[POSTGRES Specific] DAO', () => {
    ...
    describe('model', () => {
      ...
      it('should read hstore correctly from multiple rows', function () {
        return this.User.create({ username: 'user1', email: ['foo@bar.com'], settings: { test: '"value"' } })
          .then(() => {
      ...
      it('should read range correctly from multiple rows', function () {
        ...
        return User.create({ username: 'user1', email: ['foo@bar.com'], course_period: periods[0] })
          .then(() => {

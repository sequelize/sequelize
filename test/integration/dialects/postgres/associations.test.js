    ...
    describe('HasMany', () => {
      describe('addDAO / getModel', () => {
        ...
        it('should correctly add an association to the dao', async function() {
          ...
          const _tasks0 = await this.user.getTasks();
          expect(_tasks0).to.have.length(1);
        });
      });
      ...
      describe('removeDAO', () => {
        it('should correctly remove associated objects', async function() {
          ...
          const __tasks = await this.user.getTasks();
          expect(__tasks).to.have.length(0);
          await this.user.setTasks(this.tasks);
          const _tasks2 = await this.user.getTasks();
          expect(_tasks2).to.have.length(this.tasks.length);
          await this.user.removeTask(this.tasks[0]);
          const _tasks1 = await this.user.getTasks();
          expect(_tasks1).to.have.length(this.tasks.length - 1);
          await this.user.removeTasks([this.tasks[1], this.tasks[2]]);
          const _tasks0 = await this.user.getTasks();
          expect(_tasks0).to.have.length(this.tasks.length - 3);
        });
      });
    });
  });

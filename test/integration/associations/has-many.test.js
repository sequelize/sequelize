  ...
  describe('(1:N)', () => {
    ...
    describe('setAssociations', () => {
      if (current.dialect.supports.transactions) {
        it('supports transactions', async function() {
          ...
          ctx. t = t;
          await article.setLabels([label], { transaction: t });
          const labels0 = await ctx.Label.findAll({ where: { ArticleId: ctx.article.id }, transaction: undefined });
      ...
      it('supports passing the primary key instead of an object', async function() {
        ...
        await ctx.article.setLabels([ctx.label2.id]);
        const labels = await ctx.article.getLabels();
    ...
    describe('addAssociations', () => {
      if (current.dialect.supports.transactions) {
        it('supports transactions', async function() {
          ...
          const t = await ctx.sequelize.transaction();
          ctx.t = t;
          await ctx.article.addLabel(ctx.label, { transaction: ctx.t });
          const labels0 = await ctx.Label.findAll({ where: { ArticleId: ctx.article.id }, transaction: undefined });
          expect(labels0.length).to.equal(0);
    ...
    describe('addMultipleAssociations', () => {
      it('adds associations without removing the current ones', async function() {
        ...
        ctx.users = users0;
        await ctx.task.setUsers([users0[0]]);
    ...
    describe('createAssociations', () => {
      ...
      if (current.dialect.supports.transactions) {
        it('supports transactions', async function() {
          ...
          await sequelize.sync({ force: true });
          const article = await ctx.Article.create({ title: 'foo' });
          ctx.article = article;
          const t = await ctx.sequelize.transaction();
          ctx.t = t;
          await ctx.article.createLabel({ text: 'bar' }, { transaction: ctx.t });
          const labels1 = await ctx.Label.findAll();
          expect(labels1.length).to.equal(0);
          const labels0 = await ctx.Label.findAll({ where: { ArticleId: ctx.article.id } });
          expect(labels0.length).to.equal(0);
          const labels = await ctx.Label.findAll({ where: { ArticleId: ctx.article.id }, transaction: ctx.t });
          expect(labels.length).to.equal(1);
          return ctx.t.rollback();
    ...
    describe('getting assocations with options', () => {
      ...
      it('should treat the where object of associations as a first class citizen', async function() {
        ...
        ctx.article = article;
        await article.setLabels([label1, label2]);
        const labels = await ctx.article.getLabels({ where: { until: { [Op.gt]: moment('2014-01-02').toDate() } } });
  ...
  describe('foreign key constraints', () => {
    describe('1:m', () => {
      ...
      it('should be possible to remove all constraints', async function() {
        ...
        const task = await ctx.task.reload();

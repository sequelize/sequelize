...
describe(Support.getTestDialectTeaser('HasMany'), () => {
  ...
  describe('count', () => {
    it('should not fail due to ambiguous field', async function () {
      ...
      await Promise.all([
        user0
          .get('Tasks')[0]
          .createSubtask({ title: 'Make a startup', active: false }),
        user0
          .get('Tasks')[0]
          .createSubtask({ title: 'Engage rock stars', active: true }),
      ]);
      ...
      await expect(
        user.countTasks({
          ...
          group: this.sequelize.col(
            Task.name.concat('.', Task.primaryKeyField)
          ),
        })
        ...
        it('should fetch multiple layers of associations with limit and order with separate=true', async function () {
          ...
          await Promise.all([
            User.create(
              ...
              {
                include: [
                  { association: User.Tasks, include: [Task.SubTasks] },
                ],
              }
            ),
            User.create(
        ...
        it('should fetch associations for multiple instances with limit and order and a belongsTo relation', async function () {
          ...
          const users = await Promise.all([
            User.create(
              ...
              {
                include: [
                  { association: User.Tasks, include: [Task.Category] },
                ],
              }
            ),
            User.create(
  ...
  describe('(1:N)', () => {
    describe('hasAssociation', () => {
      ...
      it('should only generate one set of foreignKeys', function () {
        ...
        expect(Object.keys(this.Label.rawAttributes)).to.deep.equal([
          'id',
          'text',
          'ArticleId',
        ]);
    ...
    describe('createAssociations', () => {
      ...
      it('creates the object with the association directly', async function () {
        ...
        const label = await article.createLabel(
          { text: 'bar' },
          { logging: spy }
        );
    ...
    describe('1:m', () => {
      ...
      if (dialect !== 'mssql') {
        it('can cascade updates', async function () {
          ...
          const tableName = user.sequelize
            .getQueryInterface()
            .queryGenerator.addSchema(user.constructor);
  ...
  describe('Association options', () => {
    ...
    it('can specify data type for auto-generated relational keys', async function () {
      ...
      for (const dataType of dataTypes) {
        ...
        expect(Tasks[dataType].rawAttributes.userId.type).to.be.an.instanceof(
          dataType
        );
      }
    });
    ...
    it('infers the keyType if none provided', async function () {
      ...
      await this.sequelize.sync({ force: true });
      expect(Task.rawAttributes.UserId.type instanceof DataTypes.STRING).to.be
        .ok;
    });
    ...
    describe('allows the user to provide an attribute definition object as foreignKey', () => {
      it('works with a column that hasnt been defined before', function () {
        ...
        expect(Task.rawAttributes.uid.references.model).to.equal(
          User.getTableName()
        );
      ...
      it('works when taking a column directly from the object', function () {
        ...
        expect(Project.rawAttributes.user_id.references.model).to.equal(
          User.getTableName()
        );
  ...
  describe('sourceKey', () => {
    ...
    it('should create with nested associated models', async function () {
      ...
      expect(user0.tasks[0].title).to.be.equal(
        values.tasks[0].title,
      ...
      expect(tasks[0].title).to.be.equal(
        values.tasks[0].title,
        'task title is correct'
      );
    });
    ...
    it('should create nested associations with symmetric getters/setters on FK', async function () {
      ...
      expect(father.children[0].get('parent', { raw: true })).to.be.equal(
        'FORMAT-sJn369d8Em'
      );
    });
  });

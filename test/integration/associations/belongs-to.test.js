...
describe(Support.getTestDialectTeaser('BelongsTo'), () => {
  ...
  describe('getAssociation', () => {
    ...
    it('supports schemas', function () {
      const User = this.sequelize
          ...
          .schema('archive'),
        Task = this.sequelize
          .define('TaskXYZ', { title: Sequelize.STRING, status: Sequelize.STRING })
          .schema('archive');
  ...
  describe('setAssociation', () => {
    ...
    it('should throw a ForeignKeyConstraintError if the associated record does not exist', function () {
      ...
      return this.sequelize.sync({ force: true }).then(() => {
        return expect(Task.create({ title: 'task', UserXYZId: 5 }))
          ...
          return User.create({ username: 'foo' }).then(user => {
            return Task.create({ title: 'task' }).then(task => {
              return task.setUser(user).then(() => {
                ...
                return expect(user.sequelize.getQueryInterface().update(user, tableName, { id: 999 }, { id: user.id }))
                  .to.eventually.be.rejectedWith(Sequelize.ForeignKeyConstraintError)
                  .then(() => {

...
describe(Support.getTestDialectTeaser('HasOne'), () => {
  ...
  describe('getAssociation', () => {
    ...
    it('supports schemas', async function () {
      const User = this.sequelize
          .define('User', { username: Support.Sequelize.STRING })
          .schema('admin'),
        Group = this.sequelize
          .define('Group', { name: Support.Sequelize.STRING })
          .schema('admin');
  ...
  describe('setAssociation', () => {
    ...
    it('should throw a ForeignKeyConstraintError if the associated record does not exist', async function () {
      ...
      await Task.sync({ force: true });
      await expect(
        Task.create({ title: 'task', UserXYZId: 5 })
      ).to.be.rejectedWith(Sequelize.ForeignKeyConstraintError);
      const task = await Task.create({ title: 'task' });

      await expect(
        Task.update(
          { title: 'taskUpdate', UserXYZId: 5 },
          { where: { id: task.id } }
        )
      ).to.be.rejectedWith(Sequelize.ForeignKeyConstraintError);
    });
    ...
    it('supports setting same association twice', async function () {
      ...
      await user.setHome(home);
      await expect(user.getHome()).to.eventually.have.property(
        'id',
        home.get('id')
      );
    });
  ...
  describe('association column', () => {
    it('has correct type for non-id primary keys with non-integer type', async function () {
      ...
      await this.sequelize.sync({ force: true });
      expect(User.rawAttributes.GroupPKBTName.type).to.an.instanceof(
        Sequelize.STRING
      );
    });
  ...
  describe('Association options', () => {
    ...
    describe('allows the user to provide an attribute definition object as foreignKey', () => {
      it('works with a column that hasnt been defined before', function () {
        ...
        expect(Profile.rawAttributes.uid.references.model).to.equal(
          User.getTableName()
        );
      ...
      it('works when taking a column directly from the object', function () {
        ...
        expect(Profile.rawAttributes.user_id.references.model).to.equal(
          User.getTableName()
        );
      ...
      it('works when merging with an existing definition', function () {
        ...
        expect(Project.rawAttributes.userUid.references.model).to.equal(
          User.getTableName()
        );
  ...
  describe('Counter part', () => {
    describe('BelongsTo', () => {
      it('should only generate one foreign key', function () {
        const Orders = this.sequelize.define(
            'Orders',
            {},
            { timestamps: false }
          ),
          InternetOrders = this.sequelize.define(
            'InternetOrders',
            {},
            { timestamps: false }
          );

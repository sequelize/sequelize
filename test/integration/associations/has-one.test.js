...
describe(Support.getTestDialectTeaser('HasOne'), () => {
  ...
  describe('get', () => {
    describe('multiple', () => {
      it('should fetch associations for multiple instances', function () {
        const User = this.sequelize.define('User', {}), Player = this.sequelize.define('Player', {});
        ...
        )).then(players => Player.User.get(players).then(result => {
  ...
  describe('getAssocation', () => {
    ...
    it('should be able to handle a where object that\'s a first class citizen.', function() {
      const User = this.sequelize.define('UserXYZ', { username: Sequelize.STRING }), Task = this.sequelize.define('TaskXYZ', { title: Sequelize.STRING, status: Sequelize.STRING });
  ...
  describe('setAssociation', () => {
    ...
    it('clears the association if null is passed', function() {
      const User = this.sequelize.define('UserXYZ', { username: Sequelize.STRING }), Task = this.sequelize.define('TaskXYZ', { title: Sequelize.STRING });

      User.hasOne(Task);

      return User.sync({ force: true }).then(() => Task.sync({ force: true }).then(() => User.create({ username: 'foo' }).then(user => Task.create({ title: 'task' }).then(task => user.setTaskXYZ(task).then(() => user.getTaskXYZ().then(task => {
        expect(task).not.to.equal(null);

        return user.setTaskXYZ(null).then(() => user.getTaskXYZ().then(task => {
    ...
    it('supports passing the primary key instead of an object', function() {
      ...
      return this.sequelize.sync({ force: true }).then(() => User.create({}).then(user => Task.create({ id: 19, title: 'task it!' }).then(task => user.setTaskXYZ(task.id).then(() => user.getTaskXYZ().then(task => {
    ...
    it('supports updating with a primary key instead of an object', function() {
      ...
      .spread((user, task) => user.setTaskXYZ(task.id)
      ...
      .spread((user, task2) => user.setTaskXYZ(task2.id)
        .then(() => user.getTaskXYZ())
        .then((task) => {
          expect(task).not.to.be.null;
        }));
  ...
  describe('createAssociation', () => {
    it('creates an associated model instance', function() {
      const User = this.sequelize.define('User', { username: Sequelize.STRING }), Task = this.sequelize.define('Task', { title: Sequelize.STRING });

      User.hasOne(Task);

      return this.sequelize.sync({ force: true }).then(() => User.create({ username: 'bob' }).then(user => user.createTask({ title: 'task' }).then(() => user.getTask().then(task => {
        expect(task).not.to.be.null;
    ...
    if (current.dialect.supports.transactions) {
      it('supports transactions', function() {
        return Support.prepareTransactionTest(this.sequelize).then(sequelize => {
          const User = sequelize.define('User', { username: Sequelize.STRING }), Group = sequelize.define('Group', { name: Sequelize.STRING });
  ...
  describe('foreign key', () => {
    ...
    it('should support specifying the field of a foreign key', function() {
      ...
      return Task.sync({ force: true }).then(() => User.sync({ force: true })).then(() => Sequelize.Promise.all([
  ...
  describe('foreign key constraints', () => {
    ...
    it('works when cascading a delete with hooks but there is no associate (i.e. "has zero")', function() {
      ...
      return User.sync({ force: true }).then(() => Task.sync({ force: true }).then(() => User.create({ username: 'foo' }).then(user => user.destroy())));
    });
    ...
    if (Support.getTestDialect() !== 'mssql') {
      it('can cascade updates', function() {
        ...
        return User.sync({ force: true }).then(() => Task.sync({ force: true }).then(() => User.create({ username: 'foo' }).then(user => Task.create({ title: 'task' }).then(task => user.setTask(task).then(() => {
          ...
          return user.sequelize.getQueryInterface().update(user, tableName, {id: 999}, {id: user.id}).then(() => Task.findAll().then(tasks => {
  ...
  describe('Association options', () => {
    it('can specify data type for autogenerated relational keys', function() {
      const User = this.sequelize.define('UserXYZ', { username: Sequelize.STRING }), dataTypes = [Sequelize.INTEGER, Sequelize.BIGINT, Sequelize.STRING], self = this, Tasks = {};

      return Sequelize.Promise.map(dataTypes, dataType => {
  ...
  describe('Counter part', () => {
    describe('BelongsTo', () => {
      it('should only generate one foreign key', function() {
        const Orders = this.sequelize.define('Orders', {}, {timestamps: false}), InternetOrders = this.sequelize.define('InternetOrders', {}, {timestamps: false});

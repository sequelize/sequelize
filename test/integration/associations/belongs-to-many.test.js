...
describe(Support.getTestDialectTeaser('BelongsToMany'), () => {
  describe('getAssociations', () => {
    ...
    if (current.dialect.supports.transactions) {
      it('supports transactions', async function() {
        ...
        const articles0 = await ctx.Article.findAll({ transaction: ctx.t });
    ...
    it('should support schemas', async function() {
      ...
      await ctx.u.addProject(p, { through: { status: 'active', data: 42 } });
      const projects = await ctx.u.getProjects();
  ...
  describe('hasAssociations', () => {
    ...
    if (current.dialect.supports.transactions) {
      it('supports transactions', async function() {
        ...
        ctx.t = t;
        await ctx.article.setLabels([ctx.label], { transaction: t });
        const articles = await ctx.Article.findAll({ transaction: ctx.t });
  ...
  describe('setAssociations', () => {
    it('clears associations when passing null to the set-method', async function() {
      ...
      const _users0 = await ctx.task.getUsers();
      expect(_users0).to.have.length(1);

      await ctx.task.setUsers(null);
      const _users = await ctx.task.getUsers();
    ...
    it('should be able to set twice with custom primary keys', async function() {
      ...
      await ctx.task.setUsers([ctx.user1, ctx.user2]);
      const _users = await ctx.task.getUsers();
    ...
    it('supports passing the primary key instead of an object', async function() {
      ...
      await ctx.user.setTasks([ctx.task2.id]);
      const tasks = await ctx.user.getTasks();
      expect(tasks).to.have.length(1);
  ...
  describe('createAssociations', () => {
    ...
    if (current.dialect.supports.transactions) {
      it('supports transactions', async function() {
        ...
        const users0 = await ctx.task.getUsers();
  ...
  describe('addAssociations', () => {
    ...
    if (current.dialect.supports.transactions) {
      it('supports transactions', async function() {
        ...
        const hasUser0 = await ctx.task.hasUser(ctx.user);
      ...
      it('supports transactions when updating a through model', async function() {
        ...
        ctx.user = user;
        ctx.t = t;
        // Create without transaction, so the old value is accesible from outside the transaction
        await task.addUser(user, { through: { status: 'pending' } });
        // Add an already exisiting user in a transaction, updating a value in the join table
        await ctx.task.addUser(ctx.user, { transaction: ctx.t, through: { status: 'completed' } });
  ...
  describe('addMultipleAssociations', () => {
    ...
    it('adds associations without removing the current ones', async function() {
      ...
      await ctx.task.addUsers([ctx.users[1], ctx.users[2]]);
      const users0 = await ctx.task.getUsers();
      ...
      const users = await ctx.task.getUsers();
    ...
    describe('inserting in join table', () => {
      describe('add', () => {
        it('should insert data provided on the object into the join table', async function() {
          ...
          const up = await ctx.UserProjects.findOne({ where: { UserId: ctx.u.id, ProjectId: ctx.p.id } });
      ...
      describe('set', () => {
        it('should be able to combine properties on the associated objects, and default values', async function() {
          ...
          await user.setProjects([ctx.p1, ctx.p2], { through: { status: 'active' } });
  ...
  describe('Foreign key constraints', () => {
    ...
    if (current.dialect.supports.constraints.restrict) {
      ...
      it('can cascade and restrict deletes', async function() {
        ...
        const usertasks = await this.sequelize.model('tasksusers').findAll({ where: { taskId: ctx.task2.id } });

...
describe(Support.getTestDialectTeaser('Model'), () => {
  ...
  describe('findOrCreate', () => {
    ...
    if (current.dialect.supports.transactions) {
      it('should release transaction when meeting errors', function () {
        ...
        const test = function (times) {
          ...
          const findOrCreate = self.Student.findOrCreate({
            where: {
              no: 1
            }
          });
    ...
    describe('several concurrent calls', () => {
      ...
      it('should not fail silently with concurrency higher than pool, a unique constraint and a create hook resulting in mismatched values', function () {
        const User = this.sequelize.define('user', {
          username: {
            type: DataTypes.STRING,
            unique: true,
      ...
      it('should error correctly when defaults contain a unique key without a transaction', function () {
        const User = this.sequelize.define('user', {
          objectId: {
            type: DataTypes.STRING,
            unique: true
  ...
  describe('findCreateFind', () => {
    it('should work with multiple concurrent calls', function () {
      return Promise.all([
        this.User.findOrCreate({ where: { uniqueName: 'winner' } }),
        this.User.findOrCreate({ where: { uniqueName: 'winner' } }),
        this.User.findOrCreate({ where: { uniqueName: 'winner' } })
  ...
  describe('create', () => {
    ...
    it('should return an error for a unique constraint error', function () {
      ...
      return this.sequelize.sync({ force: true }).then(() => {
        return User.create({ email: 'hello@sequelize.com' }).then(() => {
          return User.create({ email: 'hello@sequelize.com' })
            .then(() => {
              assert(false);
            })
            .catch((err) => {
    ...
    it('allows the usage of options as attribute', function () {
      ...
      return User.sync({ force: true }).then(() => {
        return User.create({ name: 'John Doe', options }).then((user) => {
  ...
  it('should return autoIncrement primary key (create)', function () {
    ...
    return Maya.sync({ force: true })
      .then(() => Maya.create(M1, { returning: true }))
      .then((m) => {

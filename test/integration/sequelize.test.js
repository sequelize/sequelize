...
describe(Support.getTestDialectTeaser('Sequelize'), function() {
  ...
  describe('query', function() {
    ...
    it('uses properties `query` and `values` if query is tagged', function() {
      let logSql;
      return this.sequelize.query({ query: 'select ? as foo, ? as bar', values: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } }).then(function(result) {
        expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
        expect(logSql.indexOf('?')).to.equal(-1);
      });
    });
    ...
    it('uses properties `query` and `bind` if query is tagged', function() {
      ...
      let logSql;
      return this.sequelize.query({ query: 'select $1'+typeCast+' as foo, $2'+typeCast+' as bar', bind: [1, 2] }, { type: this.sequelize.QueryTypes.SELECT, logging(s) { logSql = s; } }).then(function(result) {
        expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
    ...
    it('binds token with the passed array', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $1'+typeCast+' as foo, $2'+typeCast+' as bar', { type: this.sequelize.QueryTypes.SELECT, bind: [1, 2], logging(s) { logSql = s; } }).then(function(result) {
        expect(result).to.deep.equal([{ foo: 1, bar: 2 }]);
        if (dialect === 'postgres' || dialect === 'sqlite') {
          expect(logSql.indexOf('$1')).to.be.above(-1);
        }
    ...
    it('binds named parameters with the passed object', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $one'+typeCast+' as foo, $two'+typeCast+' as bar', { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } }).then(function(result) {
    ...
    it('binds named parameters with the passed object using the same key twice', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $one'+typeCast+' as foo, $two'+typeCast+' as bar, $one'+typeCast+' as baz', { raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } }).then(function(result) {
    ...
    it('binds named parameters with the passed object having a null property', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $one'+typeCast+' as foo, $two'+typeCast+' as bar', { raw: true, bind: { one: 1, two: null }, logging(s) { logSql = s; } }).then(function(result) {
    ...
    it('binds named parameters array handles escaped $$', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $1'+typeCast+' as foo, \'$$ / $$1\' as bar', { raw: true, bind: [1 ], logging(s) { logSql = s; } }).then(function(result) {
    ...
    it('binds named parameters object handles escaped $$', function() {
      const typeCast = dialect === 'postgres' ? '::int' : '';
      let logSql;
      return this.sequelize.query('select $one'+typeCast+' as foo, \'$$ / $$one\' as bar', { raw: true, bind: { one: 1 }, logging(s) { logSql = s; } }).then(function(result) {
    ...
    if (dialect === 'postgres' || dialect === 'sqlite' || dialect === 'mssql') {
      it ('does not improperly escape arrays of strings bound to named parameters', function() {
        let logSql;
        return this.sequelize.query('select :stringArray as foo', { raw: true, replacements: { stringArray: [ '"string"' ] }, logging(s) { logSql = s; } }).then(function(result) {
    ...
    it('throw an exception when binds passed with object and numeric $1 is also present', function() {
      ...
      let logSql;
      expect(function() {
        self.sequelize.query('select $one'+typeCast+' as foo, $two'+typeCast+' as bar, \'$1\' as baz', {  raw: true, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } });
    ...
    it('throw an exception when binds passed as array and $alpha is also present', function() {
      ...
      let logSql;
      expect(function() {
        self.sequelize.query('select $1'+typeCast+' as foo, $2'+typeCast+' as bar, \'$foo\' as baz', { raw: true, bind: [1, 2], logging(s) { logSql = s; } });
      }).to.throw(Error, /Named bind parameter "\$\w+" has no value in the given object\./g);
    });
    ...
    if (Support.getTestDialect() === 'sqlite') {
      it('binds array parameters for upsert are replaced. $$ unescapes only once', function() {
        let logSql;
        return this.sequelize.query('select $1 as foo, $2 as bar, \'$$$$\' as baz', { type: this.sequelize.QueryTypes.UPSERT, bind: [1, 2], logging(s) { logSql = s; } }).then(function(result) {
      ...
      it('binds named parameters for upsert are replaced. $$ unescapes only once', function() {
        let logSql;
        return this.sequelize.query('select $one as foo, $two as bar, \'$$$$\' as baz', { type: this.sequelize.QueryTypes.UPSERT, bind: { one: 1, two: 2 }, logging(s) { logSql = s; } }).then(function(result) {
          // sqlite.exec does not return a result
          expect(logSql.indexOf('$one')).to.equal(-1);
          expect(logSql.indexOf('\'$$\'')).to.be.above(-1);
        });
  ...
  describe('import', function() {
    ...
    it('imports a dao definition from a file compiled with babel', function() {
      const es6project = babel.transformFileSync(__dirname + '/assets/es6project.es6', {
        presets: ['es2015']
      }).code;
    ...
    it('imports a dao definition from a function', function() {
      const Project = this.sequelize.import('Project', function(sequelize, DataTypes) {
        ...
        if (dialect === 'sqlite') {
          it('correctly scopes transaction from other connections', function() {
            ...
            const count = function(transaction) {
              ...
              return self.sequelizeWithTransaction.query(sql, { plain: true, transaction }).then(function(result) {
                return result.cnt;
              });
            };

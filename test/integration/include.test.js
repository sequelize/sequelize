  ...
  describe('find', () => {
    ...
    it('should support Sequelize.literal and renaming of attributes in included model attributes', async function() {
      ...
      const PostComment = this.sequelize.define('PostComment', {
        someProperty: Sequelize.VIRTUAL,
        // Since we specify the AS part as a part of the literal string, not with sequelize syntax, we have to tell sequelize about the field
    ...
    it('should support include when retrieving associated objects', async function() {
      ...
      await ctx.group.addMember(ctx.member);

      const groups = await ctx.owner.getGroups({
        include: [{
          model: User,
          as: 'Members'
        }]

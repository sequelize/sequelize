...
describe(Support.getTestDialectTeaser('Include'), () => {
  describe('find', () => {
    ...
    it('should support including a belongsTo association rather than a model/as pair', async function () {
      ...
      await this.sequelize.sync({ force: true });
      const [person0, company] = await Promise.all([
        Person.create(),
        Company.create(),
      ]);
    ...
    it('should support a simple nested belongsTo -> belongsTo include', async function () {
      ...
      await Promise.all([
        props0.task.setUser(props0.user),
        props0.user.setGroup(props0.group),
      ]);
      const props = props0;
    ...
    it('should support Sequelize.literal and renaming of attributes in included model attributes', async function () {
      ...
      if (dialect === 'mssql') {
        findAttributes = [
          ...
          [
            Sequelize.literal(
              'CAST(CASE WHEN EXISTS(SELECT 1) THEN 1 ELSE 0 END AS BIT)'
            ),
            'someProperty2',
          ],
        ];
    ...
    it('should support including date fields, with the correct timeszone', async function () {
      ...
      expect(user.groups[0].dateField.getTime()).to.equal(
        Date.UTC(2014, 1, 20)
      );
    });

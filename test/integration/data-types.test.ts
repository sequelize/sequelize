import type { CreationAttributes, InferAttributes, ModelStatic } from '@sequelize/core';
...
function beforeEach2<T extends Record<string, any>>(cb: () => Promise<T> | T): T {
  const out: T = {};
...
describe(getTestDialectTeaser('DataTypes'), () => {
  ...
  describe('DataTypes.TSVECTOR', () => {
    ...
    it('accepts ts_tsvector() functions', async () => {
      await testSimpleInOut(vars.User, 'tsvectorAttr', fn('to_tsvector', 'english', 'The Fat Rats'), `'fat':2 'rat':3`);
    });
  });
  ...
  describe('DataTypes.BOOLEAN', () => {
    const vars = beforeEach2(async () => {
      class User extends Model<InferAttributes<User>> {
        declare booleanAttr: boolean | string | number | bigint;
      }
...
DataTypes.RANGE(DataTypes.DECIMAL)    // numrange
  ...
  // TODO: CIDR ('10.1.2.3/32')
  // TODO: INET ('127.0.0.1')
  // TODO: MACADDR ('01:23:45:67:89:ab')
});

async function testSimpleInOut<M extends Model, Key extends keyof CreationAttributes<M>>(
  model: ModelStatic<M>,
  attributeName: Key,
  ...
  const fetchedUser = await model.findOne({ rejectOnEmpty: true, where: { id: createdUser.id } });

...
if (current.dialect.supports.transactions) {
  describe(Support.getTestDialectTeaser('Transaction'), () => {
    ...
    describe('autoCallback', () => {
      ...
      if (dialect === 'postgres') {
        it('do not rollback if already committed', async function () {
          ...
          await expect(
            Promise.all([transTest(80), transTest(80)])
    ...
    it('should not rollback if connection was not acquired', async function () {
      this.sinon
        .stub(this.sequelize.connectionManager, '_connect')
        .returns(new Promise(() => {}));
    ...
    if (dialect === 'sqlite') {
      ...
      it('fails with SQLITE_BUSY when retry.match is changed', async function () {
        ...
        const newTransactionFunc = async function () {
          ...
          await delay(1000);
          await User.create(
            { id: null, username: `test ${t.id}` },
            { transaction: t }
          );
          return t.commit();
        };
        await expect(
          Promise.all([newTransactionFunc(), newTransactionFunc()])
        ).to.be.rejectedWith('SQLITE_BUSY: database is locked');
      });
    }

...
describe('QueryGenerator#showIndexesQuery', () => {
  ...
  it('produces a SHOW INDEX query from a table', () => {
    // @ts-expect-error - showIndexesQuery is not yet typed
  ...
  it('produces a SHOW INDEX query from a table and schema in options', () => {
    ...
    expectsql(() => queryGeneratorSchema.showIndexesQuery('myTable'), {
      // TODO: Fix this so it uses the schema (see https://github.com/sequelize/sequelize/pull/14687)
      default: `SHOW INDEX FROM [myTable]`,
      postgres: `SELECT i.relname AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, `
        + `array_agg(a.attnum) as column_indexes, array_agg(a.attname) AS column_names, pg_get_indexdef(ix.indexrelid) `
        + `AS definition FROM pg_class t, pg_class i, pg_index ix, pg_attribute a `
  ...
  it('produces a SHOW INDEX query from a table and a database', () => {
    // @ts-expect-error - showIndexesQuery is not yet typed
    expectsql(() => queryGenerator.showIndexesQuery('myTable', { database: 'sequelize' }), {
      default: `SHOW INDEX FROM [myTable]`,
      postgres: `SELECT i.relname AS name, ix.indisprimary AS primary, ix.indisunique AS unique, ix.indkey AS indkey, `
        + `array_agg(a.attnum) as column_indexes, array_agg(a.attname) AS column_names, pg_get_indexdef(ix.indexrelid) `
        + `AS definition FROM pg_class t, pg_class i, pg_index ix, pg_attribute a `

...
describe(Support.getTestDialectTeaser('Model#changed()'), () => {
  beforeEach(function () {
  ...
  it('should return true for changed primitive', function () {
    ...
    user.set('name', 'b');
    user.set('meta', null);
    expect(user.changed('name')).to.equal(true, 'name has changed');
    expect(user.changed('meta')).to.equal(true, 'meta has changed');
  });

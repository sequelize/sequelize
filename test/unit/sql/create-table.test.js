    ...
    describe('with enums', () => {
      it('references enum in the right schema #3171', () => {
        expectsql(sql.createTableQuery(FooUser.getTableName(), sql.attributesToSQL(FooUser.rawAttributes), {}), {
          ...
          mssql: 'IF OBJECT_ID(\'[foo].[users]\', \'U\') IS NULL CREATE TABLE [foo].[users] ([id] INTEGER NOT NULL IDENTITY(1,1) , [mood] NVARCHAR(MAX) CHECK ([mood] IN(N\'happy\', N\'sad\')), PRIMARY KEY ([id]));',
          ibmi: `BEGIN

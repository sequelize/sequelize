    ...
    describe('with enums', () => {
      it('references enum in the right schema #3171', () => {
        expectsql(sql.createTableQuery(FooUser.getTableName(), sql.attributesToSQL(FooUser.rawAttributes), { }), {
          ...
          oracle: `BEGIN EXECUTE IMMEDIATE 'CREATE TABLE foo.users (id NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY, mood VARCHAR2(512),CONSTRAINT PKfoousersid PRIMARY KEY (id), CHECK (mood IN(''happy'', ''sad'')))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;`,
    ...
    describe('with references on primary key', () => {
      ...
      it('references on primary key #9461', () => {
        expectsql(sql.createTableQuery(Image.getTableName(), sql.attributesToSQL(Image.rawAttributes), { }), {
          ...
          oracle: `BEGIN EXECUTE IMMEDIATE 'CREATE TABLE images (id  NUMBER(*,0) GENERATED BY DEFAULT ON NULL AS IDENTITY ,CONSTRAINT PKimagesid PRIMARY KEY (id),CONSTRAINT FKimagesid FOREIGN KEY (id) REFERENCES files (id))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -955 THEN RAISE; END IF; END;`,

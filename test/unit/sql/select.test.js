...
describe(Support.getTestDialectTeaser('SQL'), () => {
  describe('select', () => {
    ...
    testsql(
      ...
      },
      {
        default:
          'SELECT [User].* FROM (' +
          [
            'SELECT * FROM (SELECT [email], [first_name] AS [firstName], [last_name] AS [lastName] FROM [User] WHERE [User].[companyId] = 1 ORDER BY [last_name] ASC' +
              sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] }) +
              ') AS sub',
            'SELECT * FROM (SELECT [email], [first_name] AS [firstName], [last_name] AS [lastName] FROM [User] WHERE [User].[companyId] = 5 ORDER BY [last_name] ASC' +
              sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] }) +
              ') AS sub'
    ...
    (function () {
      ...
      testsql(
        ...
        },
        {
          default:
            ...
            [
              'SELECT * FROM (SELECT [id_user] AS [id], [email], [first_name] AS [firstName], [last_name] AS [lastName] FROM [users] AS [user] WHERE [user].[companyId] = 1 ORDER BY [user].[last_name] ASC' +
                sql.addLimitAndOffset({ limit: 3, order: [['last_name', 'ASC']] }) +
                ') AS sub',
              'SELECT * FROM (SELECT [id_user] AS [id], [email], [first_name] AS [firstName], [last_name] AS [lastName] FROM [users] AS [user] WHERE [user].[companyId] = 5 ORDER BY [user].[last_name] ASC' +
                sql.addLimitAndOffset({ limit: 3, order: [['last_name', 'ASC']] }) +
                ') AS sub'
            ].join(current.dialect.supports['UNION ALL'] ? ' UNION ALL ' : ' UNION ') +
      ...
      testsql(
        ...
        {
          default:
            'SELECT [user].*, [POSTS].[id] AS [POSTS.id], [POSTS].[title] AS [POSTS.title], [POSTS->COMMENTS].[id] AS [POSTS.COMMENTS.id], [POSTS->COMMENTS].[title] AS [POSTS.COMMENTS.title] FROM (' +
            [
              'SELECT * FROM (SELECT [id_user] AS [id], [email], [first_name] AS [firstName], [last_name] AS [lastName] FROM [users] AS [user] WHERE [user].[companyId] = 1 ORDER BY [user].[last_name] ASC' +
                sql.addLimitAndOffset({ limit: 3, order: ['last_name', 'ASC'] }) +
                ') AS sub',
  ...
  describe('queryGenerator: selectQuery', () => {
    ...
    it('escapes attributes with parentheses if attributeBehavior is escape', () => {
      ...
      expectsql(
        escapeSequelize.queryInterface.QueryGenerator.selectQuery(
          Project.tableName,
          {
            model: Project,
            attributes: [['count(*)', 'count']]

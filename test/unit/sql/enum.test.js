    ...
    if (Support.getTestDialect() === 'postgres') {
      ...
      describe('pgEnumName', () => {
        it('does not add schema when options: { schema: false }', () => {
          expect(
            sql.pgEnumName(PublicUser.getTableName(), 'mood', { schema: false })
          ).to.equal('"enum_users_mood"');
          expect(
            sql.pgEnumName(FooUser.getTableName(), 'theirMood', {
              schema: false,
            })
        ...
        it('properly quotes both the schema and the enum name', () => {
          ...
          expect(
            sql.pgEnumName(
              ...
              'theirMood',
              FooUser.rawAttributes.mood.type
            )
      ...
      describe('pgEnum', () => {
        it('uses schema #3171', () => {
          expectsql(
            ...
            {
              postgres:
                'CREATE TYPE "foo"."enum_users_mood" AS ENUM(\'happy\', \'sad\');',
            }
          );
        });
        ...
        it('does add schema when public', () => {
          expectsql(
            ...
            {
              postgres:
                'CREATE TYPE "public"."enum_users_theirMood" AS ENUM(\'happy\', \'sad\');',
            }
          );
        });
      });
      ...
      describe('pgEnumAdd', () => {
        it('creates alter type with exists on 9.4', () => {
          ...
          expectsql(
            sql.pgEnumAdd(PublicUser.getTableName(), 'mood', 'neutral', {
              after: 'happy',
            }),
            {
              postgres:
        ...
        it('creates alter type without exists on 9.2 ', () => {
          ...
          expectsql(
            ...
            {
              postgres:
                'ALTER TYPE "public"."enum_users_mood" ADD VALUE \'neutral\' AFTER \'happy\'',
            }
          );
        });
      });
      ...
      describe('pgListEnums', () => {
        it('works with schema #3563', () => {
          expectsql(sql.pgListEnums(FooUser.getTableName(), 'mood'), {
            postgres:
              "SELECT t.typname enum_name, array_agg(e.enumlabel ORDER BY enumsortorder) enum_value FROM pg_type t JOIN pg_enum e ON t.oid = e.enumtypid JOIN pg_catalog.pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'foo' AND t.typname='enum_users_mood' GROUP BY 1",
          });
        });

...
describe(Support.getTestDialectTeaser('SQL'), () => {
  describe('order', () => {
    ...
    testsql({
      ...
      mysql: 'SELECT `Subtask`.`id`, `Subtask`.`name`, `Subtask`.`createdAt`, `Task`.`id` AS `Task.id`, `Task`.`name` AS `Task.name`, `Task`.`created_at` AS `Task.createdAt`, `Task->Project`.`id` AS `Task.Project.id`, `Task->Project`.`name` AS `Task.Project.name`, `Task->Project`.`created_at` AS `Task.Project.createdAt` FROM `sequelize_test`.`subtask` AS `Subtask` INNER JOIN `sequelize_test`.`task` AS `Task` ON `Subtask`.`task_id` = `Task`.`id` INNER JOIN `sequelize_test`.`project` AS `Task->Project` ON `Task`.`project_id` = `Task->Project`.`id` ORDER BY `Task->Project`.`created_at` ASC, `Task->Project`.`created_at`, `Task`.`created_at` ASC, `Task`.`created_at`, `Task->Project`.`created_at` ASC, `Task->Project`.`created_at`, `Task`.`created_at` ASC, `Task`.`created_at`, `Task->Project`.`created_at` ASC, `Task->Project`.`created_at`, `Task`.`created_at` ASC, `Task`.`created_at`, `Task->Project`.`created_at` ASC, `Task->Project`.`created_at`, `Task`.`created_at` ASC, `Task`.`created_at`, `Subtask`.`created_at` ASC, `Subtask`.`created_at`, `Subtask`.`created_at`;',

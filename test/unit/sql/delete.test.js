...
describe(Support.getTestDialectTeaser('SQL'), () => {
  describe('delete', () => {
    ...
    describe('truncate with cascade and restartIdentity', () => {
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        return expectsql(sql.truncateTableQuery(options.table, options), {
          ...
          mysql: 'TRUNCATE `public.test_users`',
          sqlite:
            "DELETE FROM `public.test_users`; DELETE FROM `sqlite_sequence` WHERE `name` = 'public.test_users';",
        });
      });
    });
    ...
    describe('delete without limit', () => {
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        return expectsql(
          sql.deleteQuery(options.table, options.where, options, User),
          {
            default: "DELETE FROM [public.test_users] WHERE `name` = 'foo'",
            postgres:
              'DELETE FROM "public"."test_users" WHERE "name" = \'foo\'',
            mariadb: "DELETE FROM `public`.`test_users` WHERE `name` = 'foo'",
            sqlite: "DELETE FROM `public.test_users` WHERE `name` = 'foo'",
            mssql:
              "DELETE FROM [public].[test_users] WHERE [name] = N'foo'; SELECT @@ROWCOUNT AS AFFECTEDROWS;",
          }
        );
      });
    });
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        return expectsql(
          sql.deleteQuery(options.table, options.where, options, User),
          {
            ...
            mariadb:
              "DELETE FROM `public`.`test_users` WHERE `name` = 'foo\\';DROP TABLE mySchema.myTable;' LIMIT 10",
            sqlite:

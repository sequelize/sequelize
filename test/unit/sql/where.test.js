...
describe(Support.getTestDialectTeaser('SQL'), () => {
  describe('whereQuery', () => {
    ...
    testsql(
      ...
      {
        default:
          "WHERE ([id] IN (1, 2, 3) OR [id] > 10) AND [name] = 'a project'",
        mssql:
          "WHERE ([id] IN (1, 2, 3) OR [id] > 10) AND [name] = N'a project'",
      }
    );
  ...
  describe('whereItemQuery', () => {
    ...
    describe('Op.in', () => {
      ...
      testsql(
        ...
        {
          default:
            '[equipment] IN (select order_id from product_orders where product_id = 3)',
        }
      );
    });
    ...
    describe('Op.notIn', () => {
      ...
      testsql(
        ...
        {
          default:
            '[equipment] NOT IN (select order_id from product_orders where product_id = 3)',
        }
      );
    });
    ...
    describe('Op.and/Op.or/Op.not', () => {
      describe('Op.or', () => {
        testsql(
          ...
          },
          {
            ...
            mssql:
              "([email] = N'maker@mhansen.io' OR [email] = N'janzeh@gmail.com')",
          }
        );
        ...
        testsql(
          Op.or,
          [{ email: 'maker@mhansen.io' }, { email: 'janzeh@gmail.com' }],
          {
            default:
              "([email] = 'maker@mhansen.io' OR [email] = 'janzeh@gmail.com')",
            mssql:
        ...
        testsql(
          ...
          ],
          {
            ...
            mssql:
              "([roleName] = N'NEW' OR ([roleName] = N'CLIENT' AND [type] = N'CLIENT'))",
          }
        );
        ...
        it('sequelize.or({group_id: 1}, {user_id: 2})', function () {
          expectsql(
            sql.whereItemQuery(
              undefined,
              this.sequelize.or({ group_id: 1 }, { user_id: 2 })
            ),
            {
        ...
        it("sequelize.or({group_id: 1}, {user_id: 2, role: 'admin'})", function () {
          expectsql(
            sql.whereItemQuery(
              undefined,
              this.sequelize.or({ group_id: 1 }, { user_id: 2, role: 'admin' })
            ),
            {
              default:
                "([group_id] = 1 OR ([user_id] = 2 AND [role] = 'admin'))",
              mssql:
      ...
      describe('Op.and', () => {
        ...
        testsql(
          ...
          {
            default:
              "([name] LIKE '%someValue1%' AND [name] LIKE '%someValue2%')",
            mssql:
              "([name] LIKE N'%someValue1%' AND [name] LIKE N'%someValue2%')",
          }
        );
      ...
      describe('ARRAY', () => {
        describe('Op.contains', () => {
          ...
          testsql(
            ...
            {
              postgres:
                "\"muscles\" @> ARRAY['stringValue1','stringValue2','stringValue3']",
            }
          );
          ...
          testsql(
            ...
            {
              postgres:
                "\"muscles\" @> ARRAY['stringValue1','stringValue2']::VARCHAR(255)[]",
            }
          );
        });
    ...
    if (current.dialect.supports.RANGE) {
      describe('RANGE', () => {
        testsql(
          ...
          {
            postgres:
              '"Timeline"."range" @> \'2000-02-01 00:00:00.000 +00:00\'::timestamptz',
          }
        );
      ...
      describe('JSON', () => {
        it('sequelize.json("profile.id"), sequelize.cast(2, \'text\')")', function () {
          expectsql(
            sql.whereItemQuery(
              ...
              this.sequelize.json(
                'profile.id',
                this.sequelize.cast('12346-78912', 'text')
              )
            ),
            {
              postgres: "(\"profile\"#>>'{id}') = CAST('12346-78912' AS TEXT)",
              sqlite:
                "json_extract(`profile`,'$.id') = CAST('12346-78912' AS TEXT)",
              mariadb:
                "json_unquote(json_extract(`profile`,'$.id')) = CAST('12346-78912' AS CHAR)",
              mysql:
                "json_unquote(json_extract(`profile`,'$.\\\"id\\\"')) = CAST('12346-78912' AS CHAR)",
            }
          );
        ...
        it('sequelize.json({profile: {id: "12346-78912", name: "test"}})', function () {
          expectsql(
            ...
            {
              postgres:
                "(\"profile\"#>>'{id}') = '12346-78912' AND (\"profile\"#>>'{name}') = 'test'",
              sqlite:
                "json_extract(`profile`,'$.id') = '12346-78912' AND json_extract(`profile`,'$.name') = 'test'",
              mariadb:
        ...
        testsql(
          ...
          {
            mariadb:
              "json_unquote(json_extract(`User`.`data`,'$.nested.attribute')) = 'value'",
            mysql:
              'json_unquote(json_extract(`User`.`data`,\'$.\\"nested\\".\\"attribute\\"\')) = \'value\'',
        ...
        testsql(
          ...
          {
            mariadb:
              "CAST(json_unquote(json_extract(`data`,'$.nested')) AS DECIMAL) IN (1, 2)",
            mysql:
              'CAST(json_unquote(json_extract(`data`,\'$.\\"nested\\"\')) AS DECIMAL) IN (1, 2)',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "CAST(json_unquote(json_extract(`data`,'$.nested')) AS DECIMAL) BETWEEN 1 AND 2",
            mysql:
              'CAST(json_unquote(json_extract(`data`,\'$.\\"nested\\"\')) AS DECIMAL) BETWEEN 1 AND 2',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "json_unquote(json_extract(`data`,'$.nested.attribute')) = 'value'",
            mysql:
              'json_unquote(json_extract(`data`,\'$.\\"nested\\".\\"attribute\\"\')) = \'value\'',
        ...
        testsql(
          ...
          {
            mariadb:
              "CAST(json_unquote(json_extract(`data`,'$.nested.attribute')) AS DECIMAL) = 4",
            mysql:
              'CAST(json_unquote(json_extract(`data`,\'$.\\"nested\\".\\"attribute\\"\')) AS DECIMAL) = 4',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "CAST(json_unquote(json_extract(`data`,'$.nested.attribute')) AS DECIMAL) IN (3, 7)",
            mysql:
              'CAST(json_unquote(json_extract(`data`,\'$.\\"nested\\".\\"attribute\\"\')) AS DECIMAL) IN (3, 7)',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "CAST(json_unquote(json_extract(`data`,'$.nested.attribute')) AS DECIMAL) > 2",
            mysql:
              'CAST(json_unquote(json_extract(`data`,\'$.\\"nested\\".\\"attribute\\"\')) AS DECIMAL) > 2',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb: `CAST(json_unquote(json_extract(\`data\`,'$.nested.attribute')) AS DATETIME) > ${sql.escape(
              dt
            )}`,
            mysql: `CAST(json_unquote(json_extract(\`data\`,'$.\\"nested\\".\\"attribute\\"')) AS DATETIME) > ${sql.escape(
              dt
            )}`,
            postgres: `CAST(("data"#>>'{nested,attribute}') AS TIMESTAMPTZ) > ${sql.escape(
              dt
            )}`,
            sqlite: `json_extract(\`data\`,'$.nested.attribute') > ${sql.escape(
              dt.toISOString()
            )}`,
          }
        );
        ...
        testsql(
          ...
          {
            mariadb:
              "json_unquote(json_extract(`data`,'$.nested.attribute')) = 'true'",
            mysql:
              'json_unquote(json_extract(`data`,\'$.\\"nested\\".\\"attribute\\"\')) = \'true\'',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "json_unquote(json_extract(`meta_data`,'$.nested.attribute')) = 'value'",
            mysql:
              'json_unquote(json_extract(`meta_data`,\'$.\\"nested\\".\\"attribute\\"\')) = \'value\'',
    ...
    describe('fn', () => {
      it("{name: this.sequelize.fn('LOWER', 'DERP')}", function () {
        expectsql(
          sql.whereQuery({ name: this.sequelize.fn('LOWER', 'DERP') }),
          {
  ...
  describe('getWhereConditions', () => {
    const testsql = function (value, expectation) {
      ...
      it(util.inspect(value, { depth: 10 }), () => {
        return expectsql(
          sql.getWhereConditions(value, User.tableName, User),
          expectation
        );
      });
    };
    ...
    testsql(
      current.where(current.fn('lower', current.col('name')), Op.ne, null),
      {
    ...
    testsql(
      current.where(current.fn('lower', current.col('name')), Op.not, null),
      {
        default: 'lower([name]) IS NOT NULL',
      }
    );

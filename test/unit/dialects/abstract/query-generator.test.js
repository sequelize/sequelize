...
const chai = require('chai'),
  ...
  getAbstractQueryGenerator = require('../../support')
...
describe('QueryGenerator', () => {
  describe('whereItemQuery', () => {
    ...
    it('should not parse any strings as aliases operators', function () {
      const QG = getAbstractQueryGenerator(this.sequelize);
      expect(() =>
        QG.whereItemQuery('$or', [{ test: 5 }, { test: 3 }])
      ).to.throw('Invalid value { test: 5 }');

      expect(() =>
        QG.whereItemQuery('$and', [{ test: 5 }, { test: 3 }])
      ).to.throw('Invalid value { test: 5 }');

      expect(() => QG.whereItemQuery('test', { $gt: 5 })).to.throw(
        "Invalid value { '$gt': 5 }"
      );

      expect(() => QG.whereItemQuery('test', { $between: [2, 5] })).to.throw(
        "Invalid value { '$between': [ 2, 5 ] }"
      );

      expect(() => QG.whereItemQuery('test', { $ne: 3 })).to.throw(
        "Invalid value { '$ne': 3 }"
      );

      expect(() => QG.whereItemQuery('test', { $not: 3 })).to.throw(
        "Invalid value { '$not': 3 }"
      );

      expect(() => QG.whereItemQuery('test', { $in: [4] })).to.throw(
        "Invalid value { '$in': [ 4 ] }"
      );
    });
    ...
    it('should correctly parse sequelize.where with .fn as logic', function () {
      ...
      QG.handleSequelizeMethod(
        this.sequelize.where(
          this.sequelize.col('foo'),
          'LIKE',
          this.sequelize.col('bar')
        )
      ).should.be.equal('foo LIKE bar');

      QG.handleSequelizeMethod(
        this.sequelize.where(this.sequelize.col('foo'), Op.ne, null)
      ).should.be.equal('foo IS NOT NULL');

      QG.handleSequelizeMethod(
        this.sequelize.where(this.sequelize.col('foo'), Op.not, null)
      ).should.be.equal('foo IS NOT NULL');
    });

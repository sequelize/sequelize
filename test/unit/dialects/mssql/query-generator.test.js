...
if (current.dialect.name === 'mssql') {
  describe('[MSSQL Specific] QueryGenerator', () => {
    ...
    it('createTableQuery', function () {
      expectsql(
        this.queryGenerator.createTableQuery('myTable', { int: 'INTEGER' }, {}),
        {
          mssql:
    ...
    it('getDefaultConstraintQuery', function () {
      expectsql(
        this.queryGenerator.getDefaultConstraintQuery(
    ...
    it('dropConstraintQuery', function () {
      expectsql(
        this.queryGenerator.dropConstraintQuery(
          { tableName: 'myTable', schema: 'mySchema' },
          'myConstraint'
        ),
        {
          mssql:
            'ALTER TABLE [mySchema].[myTable] DROP CONSTRAINT [myConstraint];',
        }
      );
    });
    ...
    it('getPrimaryKeyConstraintQuery', function () {
      expectsql(
        this.queryGenerator.getPrimaryKeyConstraintQuery(
          'myTable',
          'myColumnName'
        ),
        {
    ...
    it('renameTableQuery', function () {
      expectsql(
        this.queryGenerator.renameTableQuery('oldTableName', 'newTableName'),
        {
    ...
    it('dropTableQuery', function () {
      expectsql(this.queryGenerator.dropTableQuery('dirtyTable'), {
        mssql:
          "IF OBJECT_ID('[dirtyTable]', 'U') IS NOT NULL DROP TABLE [dirtyTable];",
      });
    });
    ...
    it('getForeignKeysQuery', function () {
      ...
      expectsql(
        this.queryGenerator.getForeignKeysQuery('myTable', 'myDatabase'),
        {
          mssql:
    ...
    it('dropForeignKeyQuery', function () {
      expectsql(
        this.queryGenerator.dropForeignKeyQuery('myTable', 'myColumnKey'),
        {
      ...
      [
        {
          ...
          expectation:
            "UPDATE [myTable] SET [foo]=[foo]+ N'bar' OUTPUT INSERTED.*",
        },
        {
          title: 'Should use the plus operator with where clause',
          arguments: ['+', 'myTable', { bar: 'biz' }, { foo: 'bar' }, {}, {}],
          expectation:
            "UPDATE [myTable] SET [foo]=[foo]+ N'bar' OUTPUT INSERTED.* WHERE [bar] = N'biz'",
        },
        {

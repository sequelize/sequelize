      ...
      getForeignKeyReferenceQuery: [
        {
          ...
          expectation: ` SELECT main.contypid::regclass    AS domain, schema.nspname             AS "tableSchema", (parse_ident(main.conrelid::regclass::text))[ array_length(parse_ident(main.conrelid::regclass::text),1) ]                          AS "tableName", JSON_AGG(tbl_att.attname)  AS "tableColumnNames", 'FOREIGN KEY'              AS "constraintType", schema.nspname             AS "constraintSchema", main.conname               AS "constraintName", pg_catalog.pg_get_constraintdef(main.constraint_oid, true) AS "constraintDefinition", CASE main.confupdtype WHEN 'a' THEN 'NO ACTION' WHEN 'r' THEN 'RESTRICT' WHEN 'c' THEN 'CASCADE' WHEN 'n' THEN 'SET NULL' WHEN 'd' THEN 'SET DEFAULT' END AS on_update, CASE main.confdeltype WHEN 'a' THEN 'NO ACTION' WHEN 'r' THEN 'RESTRICT' WHEN 'c' THEN 'CASCADE' WHEN 'n' THEN 'SET NULL' WHEN 'd' THEN 'SET DEFAULT' END AS on_delete, CASE main.confmatchtype WHEN 'f' THEN 'FULL' WHEN 'p' THEN 'PARTIAL' WHEN 's' THEN 'SIMPLE' END AS match, CASE WHEN array_length(parse_ident(main.confrelid::regclass::text),1) > 1 THEN (parse_ident(main.confrelid::regclass::text))[1] ELSE 'public' END                        AS "referencedTableSchema", (parse_ident(main.confrelid::regclass::text))[ array_length(parse_ident(main.confrelid::regclass::text),1) ]                          AS "referencedTableName", JSON_AGG(frn_att.attname) AS "referencedTableColumnNames", conindid::regclass         AS "referencedTableConstraintName" FROM ( select *, oid AS constraint_oid, unnest(conkey)  as ckey, unnest(confkey) as cfkey from pg_catalog.pg_constraint pgc ) AS main JOIN  pg_catalog.pg_attribute AS tbl_att ON (tbl_att.attrelid = conrelid  AND tbl_att.attnum = ckey) JOIN  pg_catalog.pg_attribute AS frn_att ON (frn_att.attrelid = confrelid AND frn_att.attnum = cfkey) JOIN  ( select  cl.oid, ns.nspname from    pg_class cl join    pg_namespace ns on (cl.relnamespace = ns.oid) where   relname = 'myTable' and nspname = 'public' ) AS schema ON(schema.oid = main.conrelid) WHERE main.contype = 'f' GROUP BY domain, "tableSchema", "tableName", "constraintType", "constraintSchema", "constraintName", "constraintDefinition", on_update, on_delete, match, "referencedTableSchema", "referencedTableName", "referencedTableConstraintName" HAVING "tableColumnNames" && {'myColumn'}::text[] ORDER BY "domain", "tableSchema", "tableName", "constraintName" ; `,
        },
        {
          ...
          expectation: ` SELECT main.contypid::regclass    AS domain, schema.nspname             AS "tableSchema", (parse_ident(main.conrelid::regclass::text))[ array_length(parse_ident(main.conrelid::regclass::text),1) ]                          AS "tableName", JSON_AGG(tbl_att.attname)  AS "tableColumnNames", 'FOREIGN KEY'              AS "constraintType", schema.nspname             AS "constraintSchema", main.conname               AS "constraintName", pg_catalog.pg_get_constraintdef(main.constraint_oid, true) AS "constraintDefinition", CASE main.confupdtype WHEN 'a' THEN 'NO ACTION' WHEN 'r' THEN 'RESTRICT' WHEN 'c' THEN 'CASCADE' WHEN 'n' THEN 'SET NULL' WHEN 'd' THEN 'SET DEFAULT' END AS on_update, CASE main.confdeltype WHEN 'a' THEN 'NO ACTION' WHEN 'r' THEN 'RESTRICT' WHEN 'c' THEN 'CASCADE' WHEN 'n' THEN 'SET NULL' WHEN 'd' THEN 'SET DEFAULT' END AS on_delete, CASE main.confmatchtype WHEN 'f' THEN 'FULL' WHEN 'p' THEN 'PARTIAL' WHEN 's' THEN 'SIMPLE' END AS match, CASE WHEN array_length(parse_ident(main.confrelid::regclass::text),1) > 1 THEN (parse_ident(main.confrelid::regclass::text))[1] ELSE 'public' END                        AS "referencedTableSchema", (parse_ident(main.confrelid::regclass::text))[ array_length(parse_ident(main.confrelid::regclass::text),1) ]                          AS "referencedTableName", JSON_AGG(frn_att.attname) AS "referencedTableColumnNames", conindid::regclass         AS "referencedTableConstraintName" FROM ( select *, oid AS constraint_oid, unnest(conkey)  as ckey, unnest(confkey) as cfkey from pg_catalog.pg_constraint pgc ) AS main JOIN  pg_catalog.pg_attribute AS tbl_att ON (tbl_att.attrelid = conrelid  AND tbl_att.attnum = ckey) JOIN  pg_catalog.pg_attribute AS frn_att ON (frn_att.attrelid = confrelid AND frn_att.attnum = cfkey) JOIN  ( select  cl.oid, ns.nspname from    pg_class cl join    pg_namespace ns on (cl.relnamespace = ns.oid) where   relname = 'myTable' and nspname = 'mySchema' ) AS schema ON(schema.oid = main.conrelid) WHERE main.contype = 'f' GROUP BY domain, "tableSchema", "tableName", "constraintType", "constraintSchema", "constraintName", "constraintDefinition", on_update, on_delete, match, "referencedTableSchema", "referencedTableName", "referencedTableConstraintName" HAVING "tableColumnNames" && {'myColumn'}::text[] ORDER BY "domain", "tableSchema", "tableName", "constraintName" ; `,
        },
      ],
    };

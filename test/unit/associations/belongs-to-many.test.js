...
describe(Support.getTestDialectTeaser('belongsToMany'), () => {
  it('throws when invalid model is passed', () => {
    ...
    expect(() => {
      ...
      "User.belongsToMany called with something that's not a subclass of Sequelize.Model"
    );
  });
  ...
  describe('proper syntax', () => {
    it('throws an AssociationError if the through option is undefined, true, or null', () => {
      ...
      const errorFunction1 = User.belongsToMany.bind(User, Task, {
        through: true,
      });
      const errorFunction2 = User.belongsToMany.bind(User, Task, {
        through: undefined,
      });
      const errorFunction3 = User.belongsToMany.bind(User, Task, {
        through: null,
      });
      for (const errorFunction of [
        errorFunction1,
        errorFunction2,
        errorFunction3,
      ]) {
        expect(errorFunction).to.throw(
          AssociationError,
          `${User.name}.belongsToMany(${Task.name}) requires through option, pass either a string or a model`
        );
      }
    });
    it('throws an AssociationError for a self-association defined without an alias', () => {
      ...
      const errorFunction = User.belongsToMany.bind(User, User, {
        through: 'jointable',
      });
      expect(errorFunction).to.throw(
        AssociationError,
  ...
  describe('timestamps', () => {
    it('follows the global timestamps true option', () => {
      ...
      expect(current.models.user_task1.rawAttributes).to.contain.all.keys([
        'createdAt',
        'updatedAt',
      ]);
    });
    ...
    it('allows me to override the global timestamps option', () => {
      ...
      expect(current.models.user_task2.rawAttributes).not.to.contain.all.keys([
        'createdAt',
        'updatedAt',
      ]);
    });
    ...
    it('follows the global timestamps false option', () => {
      ...
      expect(current.models.user_task3.rawAttributes).not.to.have.all.keys([
        'createdAt',
        'updatedAt',
      ]);
    });
  ...
  describe('pseudo associations', () => {
    it('should setup belongsTo relations to source and target from join model with defined foreign/other keys', function () {
      ...
      Product.Tags = Product.belongsToMany(Tag, {
        through: ProductTag,
        foreignKey: 'productId',
        otherKey: 'tagId',
      });
      Tag.Products = Tag.belongsToMany(Product, {
        through: ProductTag,
        foreignKey: 'tagId',
        otherKey: 'productId',
      });
      ...
      expect(Product.Tags.toSource.foreignKey).to.equal(
        Product.Tags.foreignKey
      );
      expect(Product.Tags.toTarget.foreignKey).to.equal(Product.Tags.otherKey);

      expect(Tag.Products.toSource.foreignKey).to.equal(
        Tag.Products.foreignKey
      );
      ...
      expect(Object.keys(ProductTag.rawAttributes)).to.deep.equal([
        'id',
        'priority',
        'productId',
        'tagId',
    ...
    it('should setup hasMany relations to source and target from join model with defined foreign/other keys', function () {
      ...
      expect(Product.Tags.manyFromSource.foreignKey).to.equal(
        Product.Tags.foreignKey
      );
      expect(Product.Tags.manyFromTarget.foreignKey).to.equal(
        Product.Tags.otherKey
      );

      expect(Tag.Products.manyFromSource.foreignKey).to.equal(
        Tag.Products.foreignKey
      );
      expect(Tag.Products.manyFromTarget.foreignKey).to.equal(
        Tag.Products.otherKey
      );
    ...
    it('should setup hasOne relations to source and target from join model with defined foreign/other keys', function () {
      ...
      expect(Product.Tags.oneFromSource.foreignKey).to.equal(
        Product.Tags.foreignKey
      );
      expect(Product.Tags.oneFromTarget.foreignKey).to.equal(
        Product.Tags.otherKey
      );

      expect(Tag.Products.oneFromSource.foreignKey).to.equal(
        Tag.Products.foreignKey
      );
      expect(Tag.Products.oneFromTarget.foreignKey).to.equal(
        Tag.Products.otherKey
      );
    ...
    it('should setup hasOne relations to source and target from join model with defined source keys', function () {
      ...
      expect(Tag.Products.oneFromSource.sourceKey).to.equal(
        Tag.Products.sourceKey
      );
      expect(Tag.Products.oneFromTarget.sourceKey).to.equal(
        Tag.Products.targetKey
      );

      expect(Product.Tags.oneFromSource.sourceKey).to.equal(
        Product.Tags.sourceKey
      );
      expect(Product.Tags.oneFromTarget.sourceKey).to.equal(
        Product.Tags.targetKey
      );
  ...
  describe('self-associations', () => {
    ...
    it('works with singular and plural name for self-associations', () => {
      ...
      expect(Service.prototype)
        .to.have.ownProperty('getSupplements')
        .to.be.a('function');

      expect(Service.prototype)
        .to.have.ownProperty('addSupplement')
        .to.be.a('function');
      expect(Service.prototype)
        .to.have.ownProperty('addSupplements')
        .to.be.a('function');

      expect(Service.prototype)
        .to.have.ownProperty('getSupplemented')
        .to.be.a('function');
      expect(Service.prototype)
        .not.to.have.ownProperty('getSupplementeds')
        .to.be.a('function');

      expect(Service.prototype)
        .to.have.ownProperty('addSupplemented')
        .to.be.a('function');
      expect(Service.prototype)
        .not.to.have.ownProperty('addSupplementeds')
        .to.be.a('function');
    });
  });
  ...
  describe('constraints', () => {
    it('work properly when through is a string', function () {
      ...
      expect(
        Group.associations.MyUsers.through.model.rawAttributes.UserId.onUpdate
      ).to.equal('RESTRICT');
      expect(
        Group.associations.MyUsers.through.model.rawAttributes.UserId.onDelete
      ).to.equal('SET NULL');
      expect(
        Group.associations.MyUsers.through.model.rawAttributes.GroupId.onUpdate
      ).to.equal('SET NULL');
      expect(
        Group.associations.MyUsers.through.model.rawAttributes.GroupId.onDelete
      ).to.equal('RESTRICT');
    });
    ...
    it('generate unique identifier with very long length', function () {
      const User = this.sequelize.define(
          'User',
          {},
          { tableName: 'table_user_with_very_long_name' }
        ),
        Group = this.sequelize.define(
          'Group',
          {},
          { tableName: 'table_group_with_very_long_name' }
        ),
        UserGroup = this.sequelize.define(
          'GroupUser',
          {
      ...
      expect(
        Group.associations.MyUsers.through.model.rawAttributes
      ...
      expect(
        ...
        'table_user_group_with_very_long_name_id_group_very_long_field_id_user_very_long_field_unique'
      );
      expect(
        Group.associations.MyUsers.through.model.rawAttributes
          .id_group_very_long_field.unique

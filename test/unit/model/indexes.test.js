...
describe(Support.getTestDialectTeaser("Model"), () => {
  describe("indexes", () => {
    it("should automatically set a gin index for JSONB indexes", () => {
      ...
      expect(Model.getAttributes().eventData.index).not.to.equal(true);
    ...
    it("should not set getAttributes() when indexes are defined via options", () => {
    ...
    it("should not set getAttributes() when composite unique indexes are defined via options", () => {
      const User = current.define(
        "User",
        {
      ...
      expect(User.getAttributes().name.unique).to.be.undefined;
      expect(User.getAttributes().address.unique).to.be.undefined;
    });
  });
});

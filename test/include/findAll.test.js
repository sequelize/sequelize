...
describe(Support.getTestDialectTeaser('Include'), function() {
	describe('findAll', function() {
		...
		it('should be able to order by attribute of nested 1:1 associated model', function(done) {
			var Model1 = this.sequelize.define('Model1', { name: Sequelize.STRING }, { timestamps: false });
			var Model2 = this.sequelize.define('Model2', { name: Sequelize.STRING }, { timestamps: false });
			var Model3 = this.sequelize.define('Model3', { name: Sequelize.STRING }, { timestamps: false });
			Model1.hasOne(Model2);
			Model2.hasOne(Model3);
			return this.sequelize.sync({ force: true }).then(function() {
				return Promise.join(
					Model1.create({ name: 'instance1OfModel1' }),
					Model1.create({ name: 'instance2OfModel1' })
				);
			}).spread(function(model1_1, model1_2) {
				return Promise.join(
					model1_1.createModel2({ name: 'instance1OfModel2' }),
					model1_2.createModel2({ name: 'instance2OfModel2' }),
				);
			...
			.then(function(foundModel1s) {
				expect(foundModel1s.length).to.equal(2);
				expect(foundModel1s[0].Model2.Model3.name).to.equal('a_instance2OfModel3');
				expect(foundModel1s[1].Model2.Model3.name).to.equal('z_instance1OfModel3');
				done();
			});
		});
	});

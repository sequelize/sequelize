...
A third Auto-generated Timestamp called `deletedAt` exists. It is used by [Paranoid Models](./paranoid) to mark a row as deleted.
...
Changing the *attribute name* is done by setting the `createdAt`, `updatedAt`, or [`deletedAt`](./paranoid) Model option to a String.
...
Read more on this in [Configuring Column & Table names](./sql-name-inference)

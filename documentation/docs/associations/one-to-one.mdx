...
In sequelize, One-To-One relationships are defined using [`HasOne`](pathname:///api/classes/Model.html#hasOne) & [`BelongsTo`](pathname:///api/classes/Model.html#belongsTo) relationships.
...
To create a relationship, the `hasOne` and `belongsTo` associations are used together:
...
Calling [`Bar.sync()`](pathname:///api/classes/Model.html#sync) after the above will yield the following SQL (on PostgreSQL, for example):

...
- When through table is auto-created for many-to-many join, the model name is now singular and table name plural (e.g. model name TaskPerson, table name TasksPeople). Accessor for the through table also changed to match model name in `Model#findAll()` queries where many-to-many join is eager-loaded.
